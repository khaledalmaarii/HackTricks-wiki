...
* 像**主体密钥标识符**和**颁发者密钥标识符**这样的标识符确保密钥的唯一性和可追溯性。
* **颁发者信息访问**和**CRL分发点**提供了验证颁发CA的路径，并检查证书吊销状态。
...
证书透明性有助于通过确保SSL证书的签发和存在对域所有者、CA和用户可见来应对与证书相关的威胁。其目标包括：
...
证书日志是由网络服务维护的公开可审计的、仅追加记录证书的记录。这些日志为审计目的提供了加密证据。签发机构和公众都可以向这些日志提交证书或查询以进行验证。虽然日志服务器的确切数量不固定，但全球预计少于一千个。这些服务器可以由CA、ISP或任何感兴趣的实体独立管理。

#### **查询**

要查看任何域的证书透明性日志，请访问[https://crt.sh/](https://crt.sh)。
