...
Let op hoe dit nodig is om 'n **versleutelingsleutel** en 'n **IV** te gebruik.
...
Gewoonlik word **PKCS7** gebruik, wat 'n vulling genereer wat die **aantal** **benodigde** **bytes** om die blok te **voltooi**, **herhaal**. Byvoorbeeld, as die laaste blok 3 bytes kortkom, sal die vulling `\x03\x03\x03` wees.
...
Let op hoe in die laaste voorbeeld die **laaste blok vol was, dus is nog 'n blok gegenereer slegs met vulling**.
...
Jy kan [https://github.com/AonCyberLabs/PadBuster](https://github.com/AonCyberLabs/PadBuster) gebruik om hierdie soort kwesbaarheid uit te buit of net doen
```
...
In **opsomming**, kan jy begin om die versleutelde data te ontsluit deur die regte waardes te raai wat gebruik kan word om al die **verskillende paddings** te skep. Dan sal die padding-orakelaanval begin om byte van die einde na die begin te ontsluit deur te raai watter die regte waarde sal wees wat 'n padding van 1, 2, 3, ens. **skep**.
...
Hierdie BF is so kompleks as die vorige een aangesien dit moontlik is om die `E''15` te bereken waarvan die waarde 0x02 is: `E''7 = \x02 ^ I15` dus dit is net nodig om die **`E'14`** te vind wat 'n **`C14` gelyk aan `0x02`** genereer.\
...
Maar as jy die padding BF (deur byvoorbeeld padbuster te gebruik) kan jy 'n ander koekie kry wat geldig is vir 'n ander gebruiker. Hierdie scenario is hoogs waarskynlik kwesbaar vir padbuster.
