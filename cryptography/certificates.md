...
Dans un schéma d'infrastructure à clé publique ([public-key infrastructure](https://en.wikipedia.org/wiki/Public-key_infrastructure) - PKI) typique, l'émetteur du certificat est une autorité de certification ([certificate authority](https://en.wikipedia.org/wiki/Certificate_authority) - CA), généralement une entreprise qui facture ses clients pour émettre des certificats pour eux. En revanche, dans un schéma de [web of trust](https://en.wikipedia.org/wiki/Web_of_trust), les individus signent directement les clés les uns des autres, dans un format qui remplit une fonction similaire à celle d'un certificat de clé publique.

Le format le plus courant pour les certificats de clé publique est défini par [X.509](https://en.wikipedia.org/wiki/X.509). Comme X.509 est très général, le format est davantage contraint par des profils définis pour certains cas d'utilisation, tels que [Public Key Infrastructure (X.509)](https://en.wikipedia.org/wiki/PKIX) tel que défini dans le RFC 5280.
...
* **Organisation (O) :** Nom de l'organisation
* **Unité Organisationnelle (OU) :** Division d'une organisation (comme "Ressources Humaines").
* **Pas Avant :** La date et l'heure les plus précoces auxquelles le certificat est valide. Généralement réglé à quelques heures ou jours avant le moment où le certificat a été émis, pour éviter les problèmes de [décalage horaire](https://en.wikipedia.org/wiki/Clock_skew#On_a_network).
...
* **extensions x509v3**
* **Usage de la Clé :** Les utilisations cryptographiques valides de la clé publique du certificat. Les valeurs courantes incluent la validation de la signature numérique, le chiffrement de clé et la signature de certificat.
...
* Dans un certificat Web, cela apparaîtra comme une _extension X509v3_ et aura la valeur `Authentification de Serveur Web TLS`
...
* **CT Precertificate SCTs** : Journaux de la transparence des certificats concernant le certificat
...
Depuis [ici](https://www.arubanetworks.com/techdocs/ArubaOS%206_3_1_Web_Help/Content/ArubaFrameStyles/CertRevocation/About_OCSP_and_CRL.htm).
...
**Remarque :** Le format PKCS#12 ou PFX est un format binaire pour stocker le certificat serveur, les certificats intermédiaires et la clé privée dans un seul fichier pouvant être chiffré. Les fichiers PFX ont généralement des extensions telles que .pfx et .p12. Les fichiers PFX sont généralement utilisés sur les machines Windows pour importer et exporter des certificats et des clés privées.
```
