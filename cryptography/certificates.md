...
* 在 Web 证书中，这将显示为 _X509v3 扩展_，其值为 `Digital Signature`
* **扩展密钥用途**：证书可用于的应用程序。常见值包括 TLS 服务器身份验证、电子邮件保护和代码签名。
* 在 Web 证书中，这将显示为 _X509v3 扩展_，其值为 `TLS Web Server Authentication`
* **主体备用名称**：允许用户为单个 SSL 证书指定其他主机**名称**。使用 SAN 扩展是 SSL 证书的标准做法，它正在取代常见**名称**的使用。
* **基本约束**：此扩展描述证书是 CA 证书还是终端实体证书。CA 证书是签署其他证书的证书，而终端实体证书是例如网页中使用的证书（链的最后一部分）。
* **主体密钥标识符**（SKI）：此扩展声明证书中公钥的唯一**标识符**。所有 CA 证书都需要此扩展。CA 将其自己的 SKI 传播到已颁发证书的颁发者**密钥标识符**（AKI）扩展中。它是主体公钥的哈希值。
...
4. During the conversion process, you will be prompted to set a password for the PFX file. Choose a strong password and remember it, as you will need it to access the private key and certificate stored in the PFX file.

5. Once the conversion is complete, you will have a PFX file that contains the private key, certificate, and any intermediate certificates. This file can be used for various purposes, such as importing the certificate into a web server or using it for client authentication.

By following these steps, you can easily convert a CER certificate and its private key to the PFX format using OpenSSL.
```
