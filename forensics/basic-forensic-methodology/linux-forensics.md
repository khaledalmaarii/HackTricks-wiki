...
Tout d'abord, il est recommandé d'avoir une **clé USB** avec **des binaires et des bibliothèques bien connus dessus** (vous pouvez simplement obtenir ubuntu et copier les dossiers _/bin_, _/sbin_, _/lib,_ et _/lib64_), puis monter la clé USB, et modifier les variables d'environnement pour utiliser ces binaires :
```bash
...
* Les **processus Root** s'exécutent généralement avec des PIDS bas, donc si vous trouvez un processus root avec un grand PID, vous pourriez suspecter
* Vérifiez les **connexions enregistrées** des utilisateurs sans shell dans `/etc/passwd`
* Vérifiez la présence de **hashes de mot de passe** dans `/etc/shadow` pour les utilisateurs sans shell
...
Il y a **2 manières** d'arrêter le système, un **arrêt normal** et un **arrêt "débrancher la prise"**. Le premier permettra aux **processus de se terminer comme d'habitude** et au **système de fichiers** d'être **synchronisé**, mais il permettra également au **malware éventuel** de **détruire des preuves**. L'approche "débrancher la prise" peut entraîner **une perte d'informations** (pas beaucoup d'infos seront perdues car nous avons déjà pris une image de la mémoire) et le **malware n'aura aucune opportunité** de faire quoi que ce soit à ce sujet. Par conséquent, si vous **soupçonnez** la présence d'un **malware**, exécutez simplement la **commande `sync`** sur le système et débranchez la prise.
...
ls /opt /usr/local
```
Une autre bonne idée est de **vérifier** les **dossiers communs** à l'intérieur du **$PATH** pour les **binaires non liés** aux **paquets installés :**
```bash
...
Il est extrêmement courant que les malwares s'incrustent en tant que nouveau service non autorisé. Linux utilise un certain nombre de scripts pour démarrer des services lors du démarrage de l'ordinateur. Le script de démarrage d'initialisation _**/etc/inittab**_ appelle d'autres scripts tels que rc.sysinit et divers scripts de démarrage sous le répertoire _**/etc/rc.d/**_, ou _**/etc/rc.boot/**_ dans certaines anciennes versions. Sur d'autres versions de Linux, telles que Debian, les scripts de démarrage sont stockés dans le répertoire _**/etc/init.d/**_. De plus, certains services courants sont activés dans _**/etc/inetd.conf**_ ou _**/etc/xinetd/**_ selon la version de Linux. Les enquêteurs numériques doivent inspecter chacun de ces scripts de démarrage pour détecter des entrées anormales.
...
* **/var/log/maillog** ou **var/log/mail.log :** concerne les journaux du serveur de messagerie, utile pour les informations sur les services liés à la messagerie tels que postfix, smtpd, ou les services de messagerie électronique fonctionnant sur votre serveur.
...
* **/var/log/faillog** : enregistre les informations sur les échecs de connexion. Par conséquent, utile pour examiner les violations de sécurité potentielles comme les piratages de données d'identification de connexion et les attaques par force brute.
...
* **SSH** : Les connexions aux systèmes effectuées à l'aide de SSH vers et depuis un système compromis entraînent des entrées dans les fichiers pour chaque compte utilisateur (_**∼/.ssh/authorized\_keys**_ et _**∼/.ssh/known\_keys**_). Ces entrées peuvent révéler le nom d'hôte ou l'adresse IP des hôtes distants.
...
* **VIM** : Les comptes utilisateurs peuvent avoir un fichier _**∼/.viminfo**_ qui contient des détails sur l'utilisation de VIM, y compris l'historique des chaînes de recherche et les chemins vers les fichiers qui ont été ouverts à l'aide de vim.
* **Open Office** : Fichiers récents.
* **MySQL** : Les comptes utilisateurs peuvent avoir un fichier _**∼/.mysql\_history**_ qui contient les requêtes exécutées à l'aide de MySQL.
...
Examinez les fichiers _**/etc/passwd**_, _**/etc/shadow**_ et les **journaux de sécurité** pour détecter des noms ou des comptes inhabituels créés et/ou utilisés à proximité d'événements non autorisés connus. Vérifiez également les possibles attaques par force brute avec sudo.\
...
Enfin, recherchez des comptes **sans mots de passe** ou avec des mots de passe **facilement devinables**.
...
* En utilisant **autopsy**, vous pouvez voir la chronologie des événements qui peut être utile pour découvrir une activité suspecte. Vous pouvez également utiliser la fonctionnalité `mactime` de **Sleuth Kit** directement.
* Vérifiez la présence de **scripts inattendus** dans le **$PATH** (peut-être des scripts sh ou php ?)
* Les fichiers dans `/dev` sont censés être des fichiers spéciaux, vous pourriez y trouver des fichiers non spéciaux liés au malware.
...
* Étant donné que les inodes sont alloués sur la base du premier disponible, les **fichiers malveillants placés sur le système en même temps peuvent se voir attribuer des inodes consécutifs**. Par conséquent, après qu'un composant de malware a été localisé, il peut être productif d'inspecter les inodes voisins.
...
## Comparer les fichiers de différentes versions de systèmes de fichiers
