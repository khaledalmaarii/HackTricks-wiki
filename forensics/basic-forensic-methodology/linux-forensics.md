...
\
Tumia [**Trickest**](https://trickest.com/?utm\_campaign=hacktrics\&utm\_medium=banner\&utm\_source=hacktricks) kujenga na **kutumia workflows** kwa urahisi zaidi kutumia zana za **jamii ya juu zaidi** duniani.\
Pata Ufikiaji Leo:
...
Ili kupata kumbukumbu ya mfumo unaoendeshwa, inashauriwa kutumia [**LiME**](https://github.com/504ensicsLabs/LiME).\
...
Katika hali nyingine, unahitaji kupakua [**LiME**](https://github.com/504ensicsLabs/LiME) kutoka github na kuichambua na vichwa sahihi vya kernel. Ili **kupata vichwa sahihi vya kernel** vya mashine ya mwathiriwa, unaweza tu **kucopy directory** `/lib/modules/<toleo la kernel>` kwenye mashine yako, kisha **kuichambua** LiME ukitumia vichwa hivyo:
```bash
...
Kuna **njia 2** za kuzima mfumo, **kuzima kawaida** na **kuzima kwa "kunyakua waya"**. Ya kwanza itaruhusu **mchakato kumalizika kama kawaida** na **mfumo wa faili** kusawazishwa, lakini pia itaruhusu **programu hasidi** kuharibu **usahihi**. Kufanya kwa "kunyakua waya" kunaweza kusababisha **upotevu wa taarifa fulani** (sio taarifa nyingi itapotea kwani tayari tumepiga picha ya kumbukumbu) na **programu hasidi haitakuwa na fursa** ya kufanya chochote kuhusu hilo. Kwa hivyo, ikiwa **una shaka** kwamba kunaweza kuwa na **programu hasidi**, tekeleza tu **amri ya `sync`** kwenye mfumo na kunyakua waya.

#### Kupiga picha ya diski

Ni muhimu kuzingatia kwamba **kabla ya kuunganisha kompyuta yako na chochote kinachohusiana na kesi**, lazima uhakikishe kuwa itakuwa **imeunganishwa kama soma tu** ili kuepuka kuhariri habari yoyote.
```bash
...
Kutafuta kwa ufanisi programu zilizosakinishwa kwenye mifumo ya Debian na RedHat, fikiria kutumia nyaraka za mfumo na matabazi pamoja na uchunguzi wa mwongozo kwenye saraka za kawaida.
...
Kugundua programu zilizosakinishwa kwa mikono au nje ya mameneja haya ya pakiti, chunguza saraka kama _**`/usr/local`**_, _**`/opt`**_, _**`/usr/sbin`**_, _**`/usr/bin`**_, _**`/bin`**_, na _**`/sbin`**_. Changanya orodha za saraka na amri za mfumo ili kutambua programu za kutekelezwa ambazo hazihusiani na pakiti zinazojulikana, kuimarisha utafutaji wako wa programu zote zilizosakinishwa.
```bash
...
\
Tumia [**Trickest**](https://trickest.com/?utm\_campaign=hacktrics\&utm\_medium=banner\&utm\_source=hacktricks) kujenga na **kutumia** mifumo ya kiotomatiki inayotumia zana za jamii **zilizoendelea zaidi** duniani.\
Pata Ufikiaji Leo:
...
Fikiria mchakato uliotekelezwa kutoka /tmp/exec na kufutwa. Inawezekana kuichimbua
```bash
...
- **/etc/systemd/system/multi-user.target.wants/**: Ina viungo kwa huduma zinazopaswa kuanza katika kiwango cha kukimbia cha watumiaji wengi.
...
- **/lib/systemd/system/**: Faili za kawaida za kifurushi zinazotolewa na programu zilizosakinishwa.
...
Linux hutumia faili mbalimbali kutekeleza programu moja kwa moja wakati wa kuingia kwa mtumiaji, ikibeba programu hasidi kwa uwezekano:

- **/etc/profile.d/**\*, **/etc/profile**, na **/etc/bash.bashrc**: Hutekelezwa kwa kuingia kwa mtumiaji yeyote.
- **\~/.bashrc**, **\~/.bash\_profile**, **\~/.profile**, na **\~/.config/autostart**: Faili za mtumiaji zinazotekelezwa wakati wa kuingia kwao.
...
Mifumo ya Linux hufuatilia shughuli za mtumiaji na matukio ya mfumo kupitia faili mbalimbali za kumbukumbu. Kumbukumbu hizi ni muhimu kwa kutambua ufikiaji usiohalali, maambukizi ya programu hasidi, na matukio mengine ya usalama. Kumbukumbu muhimu ni pamoja na:

- **/var/log/syslog** (Debian) au **/var/log/messages** (RedHat): Hukamata ujumbe na shughuli za mfumo kwa ujumla.
- **/var/log/auth.log** (Debian) au **/var/log/secure** (RedHat): Hurekodi jaribio la uwakilishi, kuingia kwa mafanikio na kushindwa.
...
- **/var/log/faillog**: Hurekodi jaribio la kuingia lililoshindwa, likisaidia katika uchunguzi wa uvunjaji wa usalama.
- **/var/log/cron**: Hurekodi utekelezaji wa kazi za cron.
- **/var/log/daemon.log**: Hufuatilia shughuli za huduma za nyuma.
- **/var/log/btmp**: Hati jaribio la kuingia lililoshindwa.
- **/var/log/httpd/**: Ina makosa ya Apache HTTPD na kumbukumbu za ufikiaji.
- **/var/log/mysqld.log** au **/var/log/mysql.log**: Hurekodi shughuli za database ya MySQL.
- **/var/log/xferlog**: Hurekodi uhamisho wa faili za FTP.
- **/var/log/**: Daima angalia kumbukumbu zisizotarajiwa hapa.

{% hint style="info" %}
Kumbukumbu za mfumo wa Linux na mifumo ya ukaguzi inaweza kulemazwa au kufutwa katika uvamizi au tukio la programu hasidi. Kwa kuwa kumbukumbu kwenye mifumo ya Linux kwa ujumla zina habari muhimu zaidi kuhusu shughuli za uovu, wavamizi mara kwa mara huifuta. Kwa hivyo, wakati wa kuangalia faili za kumbukumbu zilizopo, ni muhimu kutafuta mapengo au kuingizwa kwa kuingia ambayo inaweza kuwa ishara ya kufutwa au kuharibiwa.
{% endhint %}
...
- **Firefox/Chrome**: Angalia historia ya kivinjari na vipakuliwa katika _\~/.mozilla/firefox_ au _\~/.config/google-chrome_ kwa shughuli shuki.
- **VIM**: Pitia _\~/.viminfo_ kwa maelezo ya matumizi, kama njia za faili zilizotembelewa na historia ya utafutaji.
- **Open Office**: Angalia ufikiaji wa nyaraka za hivi karibuni ambao unaweza kuashiria faili zilizodhuriwa.
...
Ni muhimu **kujua USB zote zilizotumiwa** na itakuwa na manufaa zaidi ikiwa una orodha iliyoruhusiwa ya USB za kupata "matukio ya uvunjaji" (matumizi ya USB ambazo hazipo kwenye orodha hiyo). 
...
```
Mifano zaidi na habari zaidi ndani ya github: [https://github.com/snovvcrash/usbrip](https://github.com/snovvcrash/usbrip)
...
Chunguza _**/etc/passwd**_, _**/etc/shadow**_ na **logs za usalama** kwa majina yasiyo ya kawaida au akaunti zilizoundwa au kutumika karibu na matukio yasiyoruhusiwa yanayojulikana. Pia, angalia mashambulizi ya nguvu ya sudo yanayowezekana.\
...
### Kuchambua Miundo ya Mfumo wa Faili katika Uchunguzi wa Programu Hasidi
...
* **Fanya uchambuzi kamili wa muda** kutumia zana kama **Autopsy** kwa kuona mfululizo wa matukio au **Sleuth Kit's** `mactime` kwa data za muda za kina.
...
* **Tambua faili za setuid root** kwa kutumia amri: `find / -user root -perm -04000 -print` Hii inatafuta faili zenye ruhusa zilizoinuliwa, ambazo zinaweza kutumiwa vibaya na wachomaji.
...
Kulinganisha matoleo ya mfumo wa faili na kubaini mabadiliko, tunatumia amri za `git diff` zilizorahisishwa:
