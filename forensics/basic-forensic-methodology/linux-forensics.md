...
* Raw (кожен сегмент з'єднано разом)
...
Є **2 способи** вимкнення системи: **нормальне вимкнення** та **вимкнення "вийняти штепсель"**. Перше дозволить **процесам завершитися як зазвичай** і **файловій системі** бути **синхронізованою**, але також дозволить можливому **шкідливому ПЗ** **знищити докази**. Підхід "вийняти штепсель" може призвести до **втрати деякої інформації** (не багато інформації буде втрачено, оскільки ми вже зробили знімок пам'яті) і **шкідливе ПЗ не матиме можливості** нічого з цим зробити. Тому, якщо ви **підозрюєте**, що може бути **шкідливе ПЗ**, просто виконайте **команду** **`sync`** на системі та вийміть штепсель.
...
* Користувачі RedHat можуть запитувати базу даних RPM за допомогою `rpm -qa --root=/mntpath/var/lib/rpm`, щоб отримати список встановлених пакетів.
...
* **Gnome Desktop**: Перегляньте _\~/.recently-used.xbel_ на предмет нещодавно відкритих файлів через програми Gnome.
...
* **VIM**: Перегляньте _\~/.viminfo_ на предмет деталей використання, таких як шляхи до файлів та історія пошуку.
...
* **MySQL**: Досліджуйте _\~/.mysql\_history_ на предмет виконаних запитів MySQL, що можуть вказувати на несанкціоновану активність бази даних.
* **Less**: Аналізуйте _\~/.lesshst_ на предмет історії використання, включаючи переглянуті файли та виконані команди.
* **Git**: Перевірте _\~/.gitconfig_ та проект _.git/logs_ на предмет змін у репозиторіях.

### Журнали USB

[**usbrip**](https://github.com/snovvcrash/usbrip) - це невеликий програмний продукт, написаний на чистому Python 3, який аналізує журнали Linux (`/var/log/syslog*` або `/var/log/messages*` в залежності від дистрибутиву) для створення таблиць історії подій USB.
...
* **Книга: Посібник з комп'ютерної криміналістики для Linux-систем: Полеві посібники з цифрової криміналістики**
