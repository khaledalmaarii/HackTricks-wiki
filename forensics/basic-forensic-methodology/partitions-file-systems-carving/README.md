...
| 3 (0x03)   | 1 (0x01)  | Bits les moins significatifs du cylindre de départ        |
...
| 7 (0x07)   | 1 (0x01)  | Bits les moins significatifs du cylindre de fin           |
...
Après avoir monté l'image de forensics avec [**ArsenalImageMounter**](https://arsenalrecon.com/downloads/), vous pouvez inspecter le premier secteur en utilisant l'outil Windows [**Active Disk Editor**](https://www.disk-editor.org/index.html)**.** Dans l'image suivante, un **MBR** a été détecté sur le **secteur 0** et interprété :
...
Le système de fichiers **FAT (File Allocation Table)** est conçu autour de son composant principal, la table d'allocation des fichiers, positionnée au début du volume. Ce système protège les données en maintenant **deux copies** de la table, assurant l'intégrité des données même si l'une est corrompue. La table, ainsi que le dossier racine, doivent être à un **emplacement fixe**, crucial pour le processus de démarrage du système.
