...
如果您对**黑客职业**感兴趣，并且想要黑掉不可黑的系统 - **我们正在招聘！**（_需要流利的波兰语书写和口语_）。

{% embed url="https://www.stmcyber.com/careers" %}

## 从编译过的二进制文件到.pyc

从一个**ELF**编译过的二进制文件，您可以用以下方法**获取.pyc**：
```bash
...
```
为了修复这个问题，你需要在生成的文件开头**添加正确的魔数**。

**魔数随着python版本而变化**，要获取**python 3.8**的魔数，你需要**打开一个python 3.8终端**并执行：
```
>> import imp
>> imp.get_magic().hex()
'550d0d0a'
```
**魔数**在这个案例中对于python3.8是**`0x550d0d0a`**，然后，要修复这个错误，你需要在**.pyc文件**的**开头****添加**以下字节：`0x0d550a0d000000000000000000000000`
...
有几个YARA规则可用于确定可执行文件是否用python编写（此脚本还确认可执行文件是否由py2exe或pyinstaller创建）。

### ImportError：文件名：'unpacked/malware\_3.exe/**pycache**/archive.cpython-35.pyc' 不存在

目前，使用unpy2exe或pyinstxtractor时，我们得到的Python字节码文件可能不完整，进而**无法被uncompyle6识别以获取纯Python源代码**。这是由于缺少Python**字节码版本号**所致。因此，我们加入了一个prepend选项；这将在其中包含一个Python字节码版本号，并有助于简化反编译过程。当我们尝试使用uncompyle6来反编译.pyc文件时，它会返回一个错误。然而，**一旦我们使用了prepend选项，我们可以看到Python源代码已经成功被反编译**。
```
...
如果您按照前面的步骤未能提取Python的“原始”代码，那么您可以尝试**提取** **汇编**代码（但是**它不是很描述性**，所以**尝试**再次提取原始代码）。在[这里](https://bits.theorem.co/protecting-a-python-codebase/)我找到了一个非常简单的代码来**反汇编** _.pyc_ 二进制文件（祝您好运理解代码流程）。如果_.pyc_是来自python2的，使用python2：
```bash
>>> import dis
>>> import marshal
>>> import struct
...
2. 对于有效载荷（在此例中，我们将其命名为 hello.py），使用类似图 1 中的脚本。选项 “bundle\_files” 的值为 1 会将包括 Python 解释器在内的所有内容打包进一个 exe 文件。
