...
PDF格式部分是纯文本，类似于HTML，但内容中包含许多二进制的“对象”。Didier Stevens撰写了关于该格式的[良好入门材料](https://blog.didierstevens.com/2008/04/09/quickpost-about-the-physical-and-logical-structure-of-pdf-files/)。这些二进制对象可以是压缩或加密数据，并包括使用JavaScript或Flash等脚本语言的内容。要显示PDF的结构，可以使用文本编辑器浏览它，或者使用支持PDF文件格式的编辑器（如Origami）打开它。
