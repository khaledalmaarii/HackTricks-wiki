...
{% hint style="info" %}
ध्यान रखें कि आप **LiME या किसी अन्य चीज़ को** विक्टिम मशीन में इंस्टॉल नहीं कर सकते क्योंकि यह कई परिवर्तन कर देगा।
{% endhint %}
...
सिस्टम को बंद करने के **2 तरीके** हैं, एक **सामान्य बंदी** और एक **"प्लग निकालने वाली" बंदी**। पहला वाला **प्रक्रियाएँ सामान्य रूप से समाप्त होने** और **फ़ाइल सिस्टम** को **समकालीन** होने देगा, लेकिन यह भी संभावित **मैलवेयर** को **सबूत नष्ट** करने की अनुमति देगा। "प्लग निकालने वाली" दृष्टिकोण से **कुछ जानकारी का नुकसान** हो सकता है (ज्यादा जानकारी नष्ट नहीं होने वाली है क्योंकि हमने पहले ही मेमोरी का इमेज ले लिया है) और **मैलवेयर को कुछ करने का कोई मौका नहीं** होगा। इसलिए, यदि आपको लगता है कि वहाँ कोई **मैलवेयर** हो सकता है, तो सिस्टम पर **`sync`** **कमांड** को निष्पादित करें और प्लग निकालें।

#### डिस्क का इमेज लेना

यह महत्वपूर्ण है कि **केस से संबंधित किसी भी चीज़ को अपने कंप्यूटर से कनेक्ट करने से पहले**, आपको यह सुनिश्चित करना होगा कि यह **केवल पढ़ने के लिए माउंट** किया जाएगा ताकि कोई भी जानकारी में कोई परिवर्तन न हो।
```bash
...
\
[**Trickest**](https://trickest.com/?utm_campaign=hacktrics&utm_medium=banner&utm_source=hacktricks) का उपयोग करें और दुनिया के सबसे उन्नत समुदाय उपकरणों द्वारा संचालित **ऑटोमेट वर्कफ़्लो** आसानी से बनाएं।\
आज ही पहुंचें:
...
Linux सिस्टम घटकों की अखंडता सुनिश्चित करने के लिए उपकरण प्रदान करता है, जो संभावित समस्यात्मक फ़ाइलों को पहचानने में महत्वपूर्ण है।
...
Debian और RedHat सिस्टमों पर स्थापित कार्यक्रमों की प्रभावी खोज के लिए, सामान्य निरीक्षण के साथ सिस्टम लॉग और डेटाबेस का सहारा लेना ध्यान में रखें।
...
इन पैकेज प्रबंधकों के बाहर मैन्युअल या स्थापित किए गए सॉफ़्टवेयर को खोजने के लिए, _**`/usr/local`**_, _**`/opt`**_, _**`/usr/sbin`**_, _**`/usr/bin`**_, _**`/bin`**_, और _**`/sbin`**_ जैसे निर्दिष्ट निर्देशिकाओं का अन्वेषण करें। सिस्टम-विशेष आदेशों के साथ निर्देशिका सूचियों को मिलाकर पहचानें जिन्हें जाने गए पैकेजों से जोड़ा नहीं गया है, सभी स्थापित कार्यक्रमों की खोज को बढ़ावा देने के लिए।
```bash
...
## हटाए गए चल रहे बाइनरी को वापस पाएं
...
* **/etc/rc.d/** और **/etc/rc.boot/**: सेवा स्टार्टअप के लिए स्क्रिप्ट्स को रखता है, जिसमें पुराने Linux संस्करणों में दूसरा पाया जाता है।
* **/etc/init.d/**: Debian जैसे कुछ Linux संस्करणों में स्टार्टअप स्क्रिप्ट्स को स्टोर करने के लिए उपयोग किया जाता है।
* सेवाएं **/etc/inetd.conf** या **/etc/xinetd/** के माध्यम से भी सक्रिय की जा सकती है, Linux प्रकार पर निर्भर करता है।
* **/etc/systemd/system**: सिस्टम और सेवा प्रबंधक स्क्रिप्ट के लिए एक निर्देशिका।
* **/etc/systemd/system/multi-user.target.wants/**: एक मल्टी-यूजर रनलेवल में शुरू किए जाने चाहिए सेवाओं के लिंक्स को रखता है।
...
* **/etc/modprobe.d**: मॉड्यूल लोडिंग को नियंत्रित करने के लिए कॉन्फ़िगरेशन फ़ाइलें रखता है।
* **/etc/modprobe** और **/etc/modprobe.conf**: वैश्विक मॉड्यूल सेटिंग्स के लिए फ़ाइलें।

### अन्य ऑटोस्टार्ट स्थान

लिनक्स उपयोगकर्ता लॉगिन पर स्वचालित रूप से कार्यक्रमों को निष्पादित करने के लिए विभिन्न फ़ाइलें उपयोग करता है, जो संभावित रूप से मालवेयर को छुपाने के लिए हो सकती हैं:
...
* **/etc/rc.local**: सभी सिस्टम सेवाएं शुरू होने के बाद चलता है, जो मल्टीयूजर परिवेश की ओर संकेत करता है।
...
* **/var/log/syslog** (Debian) या **/var/log/messages** (RedHat): सिस्टम-व्यापी संदेश और गतिविधियों को कैप्चर करती है।
* **/var/log/auth.log** (Debian) या **/var/log/secure** (RedHat): प्रमाणीकरण प्रयासों, सफल और असफल लॉगिन को रिकॉर्ड करती है।
...
* **/var/log/boot.log**: सिस्टम स्टार्टअप संदेश रखती है।
* **/var/log/maillog** या **/var/log/mail.log**: ईमेल सर्वर गतिविधियों को लॉग करती है, जो ईमेल संबंधित सेवाओं का ट्रैकिंग करने के लिए उपयोगी है।
* **/var/log/kern.log**: कर्नेल संदेश, त्रुटियाँ और चेतावनियाँ रखती है।
* **/var/log/dmesg**: डिवाइस ड्राइवर संदेश रखती है।
* **/var/log/faillog**: सुरक्षा उल्लंघन जांच में मदद करने वाले असफल लॉगिन प्रयासों को रिकॉर्ड करती है।
* **/var/log/cron**: क्रॉन जॉब क्रियाएँ लॉग करती है।
* **/var/log/daemon.log**: पिछले सेवा गतिविधियों का ट्रैक करती है।
* **/var/log/btmp**: असफल लॉगिन प्रयासों को दस्तावेज़ करती है।
* **/var/log/httpd/**: Apache HTTPD त्रुटि और एक्सेस लॉग्स रखती है।
* **/var/log/mysqld.log** या **/var/log/mysql.log**: MySQL डेटाबेस गतिविधियों को लॉग करती है।
* **/var/log/xferlog**: FTP फ़ाइल स्थानांतरण को रिकॉर्ड करती है।
* **/var/log/**: यहाँ अप्रत्याशित लॉग के लिए हमेशा जांच करें।

{% hint style="info" %}
लिनक्स सिस्टम लॉग और ऑडिट सबसिस्टम को एक उत्पीड़न या मालवेयर घटना में अक्षम या हटा दिया जा सकता है। क्योंकि लिनक्स सिस्टम पर लॉग आम तौर पर दुर्भाग्यपूर्ण गतिविधियों के बारे में सबसे उपयोगी जानकारी शामिल करते हैं, अतः अवैध पहुंच या टैम्परिंग के संकेत के रूप में गैप्स या अव्यवस्थित प्रविष्टियों के लिए देखना महत्वपूर्ण है।
{% endhint %}

**हर उपयोगकर्ता के लिए लिनक्स एक कमांड हिस्ट्री बनाए रखता है**, जिसे इसमें स्टोर किया जाता है:
...
अतिरिक्त विशेषाधिकार प्रदान कर सकने वाली फ़ाइलें जांचें:

* अनपेक्षित उपयोगकर्ता विशेषाधिकारों के लिए `/etc/sudoers` की समीक्षा करें जो प्रदान किए गए हो सकते हैं।
* अनपेक्षित उपयोगकर्ता विशेषाधिकारों के लिए `/etc/sudoers.d/` की समीक्षा करें जो प्रदान किए गए हो सकते हैं।
...
* **SSH**: अनधिकृत रिमोट कनेक्शन के लिए _\~/.ssh/authorized\_keys_ और _\~/.ssh/known\_hosts_ की जांच करें।
* **Gnome डेस्कटॉप**: नवीनतम फ़ाइलों क
```bash
pip3 install usbrip
...
अनूठे नामों या खातों की जांच करें जो अनुमति दी गई घटनाओं के निकट बनाए गए हों या उनका उपयोग किया गया हो, जो अनधिकृत हो सकते हैं। इसके अलावा, संभावित सुडू ब्रूट-फोर्स हमलों की जांच करें।\
अंततः, उपयोगकर्ताओं को **कोई पासवर्ड नहीं** है या **आसानी से अनुमान लगाया जा सकने वाला** पासवर्ड है, उन्हें ढूंढें।

## फ़ाइल सिस्टम की जांच

### मैलवेयर जांच में फ़ाइल सिस्टम संरचनाओं का विश्लेषण

मैलवेयर घटनाओं की जांच करते समय, फ़ाइल सिस्टम की संरचना एक महत्वपूर्ण सूत्र होती है, जो घटनाओं की क्रमवार्ता और मैलवेयर की सामग्री दोनों प्रकट करती है। हालांकि, मैलवेयर लेखक इस विश्लेषण को बाधित करने के लिए तकनीक विकसित कर रहे हैं, जैसे कि फ़ाइल टाइमस्टैम्प को संशोधित करना या डेटा भंडारण के लिए फ़ाइल सिस्टम से बचना।

इन एंटी-फोरेंसिक विधियों का विरोध करने के लिए, यह महत्वपूर्ण है कि:

* **घटना समयरेखा विश्लेषण** को एक विस्तृत टाइमलाइन डेटा के लिए **Autopsy** जैसे उपकरणों का उपयोग करके किया जाए, या विस्तृत टाइमलाइन डेटा के लिए **Sleuth Kit's** `mactime` का उपयोग करें।
...
* **अटिपिकल फ़ाइलों** के लिए `/dev` की जांच करें, क्योंकि यह पारंपरिक रूप से विशेष फ़ाइलों को शामिल करता है, लेकिन मैलवेयर संबंधित फ़ाइलों को भी रख सकता है।
* **छिपी हुई फ़ाइलों या निर्देशिकाओं** की जांच करें जिनके नाम ".. " (डॉट डॉट स्पेस) या "..^G" (डॉट डॉट कंट्रोल-जी) जैसे हों, जो दुर्भाग्यपूर्ण सामग्री को छुपा सकते हैं।
* **सेटयूआइड रूट फ़ाइलों** की पहचान करें जिसके लिए निम्नलिखित कमांड का उपयोग करें: `find / -user root -perm -04000 -print` यह उच्च अनुमतियों वाली फ़ाइलों को खोजता है, जिन्हें हमलावरों द्वारा दुरुपयोग किया जा सकता है।
* इनोड तालिकाओं में **मिटाने की समय चिह्नांकन** की समीक्षा करें ताकि बड़ी संख्या में फ़ाइलों को मिटाया गया हो, जो रूटकिट या ट्रोजन की मौजूदगी की संकेत कर सकता है।
* एक के बाद एक आने वाले दुर्भाग्यपूर्ण फ़ाइलों की निकटता में **क्रमिक इनोड** की जांच करें, क्योंकि वे एक साथ रखे गए हो सकते हैं।
* हाल ही में संशोधित फ़ाइलों के लिए सामान्य बाइनरी निर्देशिकाओं (_/bin_, _/sbin_) की जांच करें, क्योंकि ये मैलवेयर द्वारा संशोधित किए जा सकते हैं।
````bash
...
````
{% hint style="info" %}
ध्यान दें कि एक **हमलावर** समय को **संशोधित** कर सकता है ताकि **फ़ाइलें वैध** लगे, लेकिन वह **inode** को **संशोधित** नहीं कर सकता। अगर आपको लगता है कि एक **फ़ाइल** इस संकेत को दिखाती है कि यह उसी फ़ोल्डर में बाकी फ़ाइलों के साथ **एक ही समय** पर बनाई और संशोधित की गई थी, लेकिन **inode** **अप्रत्याशित रूप से बड़ा** है, तो उस **फ़ाइल के टाइमस्टैम्प को संशोधित किया गया था**।
{% endhint %}
...
```
* **फ़िल्टर विकल्प** (`--diff-filter`) मदद करते हैं किसी विशेष परिवर्तन को निर्दिष्ट करने में जैसे कि जोड़ी गई (`A`), हटाई गई (`D`), या संशोधित (`M`) फ़ाइलें।
* `A`: जोड़ी गई फ़ाइलें
* `C`: कॉपी की गई फ़ाइलें
* `D`: हटाई गई फ़ाइलें
* `M`: संशोधित फ़ाइलें
* `R`: नाम बदली गई फ़ाइलें
* `T`: प्रकार के परिवर्तन (उदाहरण के लिए, फ़ाइल से सिंबलिंक में)
* `U`: अमर्जित फ़ाइलें
* `X`: अज्ञात फ़ाइलें
* `B`: टूटी हुई फ़ाइलें
