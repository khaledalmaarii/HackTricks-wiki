...
Kwanza kabisa, ni vyema kuwa na **USB** na **binari na maktaba bora zilizojulikana** (unaweza tu kupata ubuntu na kunakili folda _/bin_, _/sbin_, _/lib,_ na _/lib64_), kisha funga USB, na badilisha mazingira ya env kutumia hizo binari:
```bash
export PATH=/mnt/usb/bin:/mnt/usb/sbin
export LD_LIBRARY_PATH=/mnt/usb/lib:/mnt/usb/lib64
```
Baada ya kuiweka mfumo kutumia binaries nzuri na zinazojulikana unaweza kuanza **kutoa baadhi ya taarifa za msingi**:
```bash
...
- Angalia **kuingia kwa usajili** wa watumiaji bila shell ndani ya `/etc/passwd`
- Angalia **hashi za nywila** ndani ya `/etc/shadow` kwa watumiaji bila shell
...
Kui **kupanga**, unahitaji kutumia **kernel sawa** ambao mashine ya mwathiriwa inatumia.

{% hint style="info" %}
Kumbuka kuwa huwezi **kusanikisha LiME au kitu kingine chochote** kwenye mashine ya mwathiriwa kwani itafanya mabadiliko kadhaa
{% endhint %}

Hivyo, ikiwa una toleo linalofanana na Ubuntu unaweza kutumia `apt-get install lime-forensics-dkms`\
Katika hali nyingine, unahitaji kupakua [**LiME**](https://github.com/504ensicsLabs/LiME) kutoka github na kuipanga na vichwa sahihi vya kernel. Ili **kupata vichwa sahihi vya kernel** vya mashine ya mwathiriwa, unaweza tu **kuchapisha saraka** `/lib/modules/<toleo la kernel>` kwenye mashine yako, na kisha **kupanga** LiME ukitumia:
```bash
...
Kwanza kabisa, utahitaji **kuzima mfumo**. Hii sio chaguo kila wakati kwani mara nyingine mfumo utakuwa seva ya uzalishaji ambayo kampuni haiwezi kumudu kuizima.\
Kuna **njia 2** za kuzima mfumo, **kuzima kawaida** na **kuzima kwa kutelekeza umeme**. Ya kwanza itaruhusu **mchakato kumalizika kama kawaida** na **mfumo wa faili** kusawazishwa, lakini pia itaruhusu **programu hasidi** kuharibu **usahihi**. Kukaribia "kutelekeza umeme" kunaweza kusababisha **upotevu wa taarifa fulani** (sio taarifa nyingi itapotea kwani tayari tulichukua picha ya kumbukumbu) na **programu hasidi haitakuwa na fursa** ya kufanya chochote kuhusu hilo. Kwa hivyo, ikiwa **una shaka** kwamba kunaweza kuwa na **programu hasidi**, tekeleza tu **amri ya `sync`** kwenye mfumo na kutelekeza umeme.

#### Kuchukua picha ya diski

Ni muhimu kutambua kwamba **kabla ya kuunganisha kompyuta yako na chochote kinachohusiana na kesi**, lazima uhakikishe kuwa itakuwa **imeunganishwa kama soma tu** ili kuepuka kuhariri habari yoyote.
```bash
...
* Kwa Debian, angalia _**`/var/lib/dpkg/status`**_ na _**`/var/log/dpkg.log`**_ kupata maelezo kuhusu usakinishaji wa pakiti, kutumia `grep` kuchuja taarifa maalum.
...
\
Tumia [**Trickest**](https://trickest.com/?utm_campaign=hacktrics&utm_medium=banner&utm_source=hacktricks) kujenga na **kutumia** mchakato kwa urahisi kutumia zana za **jamii ya juu zaidi** duniani.\
Pata Ufikiaji Leo:
...
- **/lib/systemd/system/**: Faili za kawaida za mfumo wa kimataifa zinazotolewa na pakiti zilizosakinishwa.
...
- **/etc/modprobe** na **/etc/modprobe.conf**: Faili za mipangilio ya moduli ya ulimwengu.
...
{% hint style="info" %}
Kumbukumbu za mfumo wa Linux na mifumo ya ukaguzi zinaweza kulemazwa au kufutwa katika uvamizi au tukio la programu hasidi. Kwa sababu kumbukumbu kwenye mifumo ya Linux kwa ujumla zina habari muhimu zaidi kuhusu shughuli za uovu, wavamizi mara kwa mara huifuta. Kwa hivyo, wakati wa kuangalia faili za kumbukumbu zilizopo, ni muhimu kutafuta mapengo au kuingizwa kwa kuingia ambayo inaweza kuwa ishara ya kufutwa au kuharibiwa.
{% endhint %}
...
Zaidi ya hayo, amri `last -Faiwx` hutoa orodha ya kuingia kwa mtumiaji. Angalia kwa kuingia kwa kuingia kwa kuingia au usiotarajiwa.
...
- Angalia `/etc/groups` kutambua uanachama au ruhusa za kikundi zisizotarajiwa.
- Angalia `/etc/passwd` kutambua uanachama au ruhusa za kikundi zisizotarajiwa.
...
- **Gnome Desktop**: Tazama _\~/.recently-used.xbel_ kwa faili zilizoangaliwa hivi karibuni kupitia programu za Gnome.
- **Firefox/Chrome**: Angalia historia ya kivinjari na vipakuzi katika _\~/.mozilla/firefox_ au _\~/.config/google-chrome_ kwa shughuli za shaka.
- **VIM**: Pitia _\~/.viminfo_ kwa maelezo ya matumizi, kama njia za faili zilizoangaliwa na historia ya utafutaji.
...
- **MySQL**: Chunguza _\~/.mysql\_history_ kwa mizunguko iliyotekelezwa ya MySQL, ikifunua shughuli zisizoidhinishwa za database.
- **Less**: Anwani _\~/.lesshst_ kwa historia ya matumizi, ikiwa ni pamoja na faili zilizoangaliwa na amri zilizotekelezwa.
- **Git**: Angalia _\~/.gitconfig_ na mradi _.git/logs_ kwa mabadiliko kwenye hazina.

### Kumbukumbu za USB

[**usbrip**](https://github.com/snovvcrash/usbrip) ni programu ndogo iliyoandikwa kwa Python 3 safi ambayo huchambua faili za kumbukumbu za Linux (`/var/log/syslog*` au `/var/log/messages*` kulingana na distro) kwa kujenga meza za historia ya matukio ya USB.
...
* **Fanya uchambuzi kamili wa muda** kutumia zana kama **Autopsy** kwa kuona muda wa matukio au `mactime` ya **Sleuth Kit** kwa data ya muda ya kina.
* **Chunguza hati za kushtukiza** katika $PATH ya mfumo, ambayo inaweza kuwa na hati za shell au PHP zinazotumiwa na wachomaji.
...
* **Kagua inode za mfululizo** kwa faili za kawaida za kudhuru baada ya kutambua moja, kwani zinaweza kuwekwa pamoja.
...
Kulinganisha matoleo ya mfumo wa faili na kugundua mabadiliko, tunatumia amri za `git diff` zilizorahisishwa:
