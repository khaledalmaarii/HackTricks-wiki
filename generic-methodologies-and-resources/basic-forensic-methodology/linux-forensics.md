...
{% hint style="info" %}
Zapamtite da **ne možete instalirati LiME ili bilo šta drugo** na mašinu žrtve jer će to izazvati nekoliko promena na njoj
{% endhint %}
...
LiME takođe može biti korišćen za **slanje dumpa putem mreže** umesto čuvanja na sistemu korišćenjem nečega poput: `path=tcp:4444`
...
* **Sistemi zasnovani na Debian-u**: `dpkg --verify` za početnu verifikaciju, zatim `debsums | grep -v "OK$"` (nakon instaliranja `debsums` sa `apt-get install debsums`) da identifikujete eventualne probleme.
...
- **/etc/rc.local**: Pokreće se nakon što su svi sistemski servisi pokrenuti, označavajući kraj tranzicije ka više korisničkom okruženju.
...
- **/var/log/maillog** ili **/var/log/mail.log**: Beleže aktivnosti email servera, korisne za praćenje usluga povezanih sa emailom.
...
{% hint style="info" %}
Linux sistemi logove i podsisteme za reviziju mogu biti onemogućeni ili obrisani u slučaju upada ili incidenta sa zlonamernim softverom. Pošto logovi na Linux sistemima generalno sadrže neke od najkorisnijih informacija o zlonamernim aktivnostima, napadači ih rutinski brišu. Stoga, prilikom pregleda dostupnih log datoteka, važno je tražiti praznine ili nepravilne unose koji bi mogli ukazivati na brisanje ili manipulaciju.
{% endhint %}
...
- Ispitajte `/etc/groups` kako biste identifikovali neobična članstva u grupama ili dozvole.
- Ispitajte `/etc/passwd` kako biste identifikovali neobična članstva u grupama ili dozvole.
...
- **FTP/SFTP**: Pregledajte logove u _\~/.ftp\_history_ ili _\~/.sftp\_history_ za prenose fajlova koji mogu biti neovlašćeni.
...
Prilikom istraživanja incidenata sa malverom, struktura fajl sistema je ključni izvor informacija, otkrivajući kako se događaji odvijaju i sadržaj malvera. Međutim, autori malvera razvijaju tehnike koje otežavaju ovu analizu, poput modifikacije vremena fajlova ili izbegavanja fajl sistema za skladištenje podataka.
...
* **Ispitati `/dev` za atipične fajlove**, jer tradicionalno sadrži specijalne fajlove, ali može sadržati fajlove povezane sa malverom.
...
* **Ispitati uzastopne inode-e** za bliske zlonamerne fajlove nakon identifikovanja jednog, jer su možda postavljeni zajedno.
* **Proveriti zajedničke binarne direktorijume** (_/bin_, _/sbin_) za nedavno modifikovane fajlove, jer bi ih malver mogao promeniti.
````bash
