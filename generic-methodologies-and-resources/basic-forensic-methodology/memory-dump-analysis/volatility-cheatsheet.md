...
Accédez à la documentation officielle dans [référence de commande Volatility](https://github.com/volatilityfoundation/volatility/wiki/Command-Reference#kdbgscan)
...
```
Si vous souhaitez utiliser un **nouveau profil que vous avez téléchargé** (par exemple un profil linux), vous devez créer quelque part la structure de dossier suivante : _plugins/overlays/linux_ et y mettre le fichier zip contenant le profil. Ensuite, obtenez le numéro des profils en utilisant :
```bash
...
Le dump mémoire d'un processus va **extraire tout** de l'état actuel du processus. Le module **procdump** va seulement **extraire** le **code**.
```
...
Il est possible de **lire l'historique bash à partir de la mémoire.** Vous pourriez également extraire le fichier _.bash\_history_, mais il a été désactivé, vous serez heureux de pouvoir utiliser ce module de volatilité.
