...
**Logičko blok adresiranje** (**LBA**) je uobičajena šema korišćena za **specifikaciju lokacije blokova** podataka smeštenih na računarskim uređajima za skladištenje, obično sekundarnim skladišnim sistemima poput hard disk drajvova. LBA je posebno jednostavna linearna shema adresiranja; **blokovi se lociraju pomoću celobrojnog indeksa**, pri čemu je prvi blok LBA 0, drugi LBA 1, i tako dalje.
...
* **Ciklična redundancija provere (CRC)**: GPT koristi CRC za osiguranje integriteta podataka. Aktivno nadgleda korupciju podataka, i kada je detektovana, GPT pokušava da povrati oštećene podatke sa druge lokacije na disku.
...
U operativnim sistemima koji podržavaju **GPT bazirano podizanje putem BIOS** servisa umesto EFI, prvi sektor se može koristiti za skladištenje prvog koraka **koda za podizanje** bootloadera, ali **modifikovanog** da prepozna **GPT** **particije**. Bootloader u MBR-u ne sme pretpostaviti veličinu sektora od 512 bajtova.
...
| 32 (0x20) | 8 bajtova  | Rezervisano; mora biti nula                                                                                                                                                          |
...
Nakon montiranja forenzičke slike pomoću [**ArsenalImageMounter**](https://arsenalrecon.com/downloads/), možete pregledati prvi sektor koristeći Windows alatku [**Active Disk Editor**](https://www.disk-editor.org/index.html)**.** Na sledećoj slici detektovan je **MBR** na **sektoru 0** i interpretiran:

![](<../../../.gitbook/assets/image (351).png>)

Ako je to bio **GPT sto umesto MBR-a**, trebalo bi da se pojavi potpis _EFI PART_ u **sektoru 1** (koji je prazan na prethodnoj slici).
## Sistemi datoteka

### Lista Windows sistem datoteka
...
**FAT (File Allocation Table)** sistem datoteka je dizajniran oko svoje osnovne komponente, tabele alokacije datoteka, koja se nalazi na početku zapisa. Ovaj sistem štiti podatke održavanjem **dve kopije** tabele, obezbeđujući integritet podataka čak i ako je jedna oštećena. Tabela, zajedno sa korenskim folderom, mora biti na **fiksnom mestu**, ključnom za proces pokretanja sistema.
...
Značajno ograničenje kroz verzije FAT-a je **maksimalna veličina datoteke od 4GB**, nametnuta 32-bitnim poljem korišćenim za skladištenje veličine datoteke.
...
* **Adresa FAT tabele** (ukazujući na početni klaster datoteke)
* **Veličina datoteke**

### EXT

**Ext2** je najčešći sistem datoteka za **particije bez dnevnika** (**particije koje se ne menjaju mnogo**), poput particije za podizanje sistema. **Ext3/4** su **s dnevnikom** i obično se koriste za **ostale particije**.

## **Metapodaci**

Neke datoteke sadrže metapodatke. Ove informacije se odnose na sadržaj datoteke koji ponekad može biti zanimljiv analitičaru jer, u zavisnosti od tipa datoteke, može sadržati informacije poput:
...
### Prijavljene obrisane datoteke

Kao što je već viđeno, postoji nekoliko mesta gde se datoteka i dalje čuva nakon što je "obrisana". To je zato što brisanje datoteke iz sistema datoteka obično označava kao obrisano, ali podaci nisu dirnuti. Zatim je moguće pregledati registre datoteka (poput MFT-a) i pronaći obrisane datoteke.

Takođe, OS obično čuva mnogo informacija o promenama u sistemu datoteka i rezervnim kopijama, pa je moguće pokušati ih koristiti za obnovu datoteke ili što više informacija.
...
**Izvlačenje datoteka** je tehnika koja pokušava **pronaći datoteke u masi podataka**. Postoje 3 glavna načina rada alata poput ovog: **Na osnovu zaglavlja i podnožja tipova datoteka**, na osnovu **strukture tipova datoteka** i na osnovu **sadržaja** samog.

Imajte na umu da ova tehnika **ne funkcioniše za povrat fragmentiranih datoteka**. Ako datoteka **nije smeštena u susednim sektorima**, tada ova tehnika neće moći da je pronađe ili bar deo nje.

Postoje različiti alati koje možete koristiti za izvlačenje datoteka koji ukazuju na tipove datoteka koje želite pretražiti.
...
Izvlačenje podataka iz struja slično je izvlačenju datoteka, ali **umesto traženja kompletnih datoteka, traži zanimljive fragmente** informacija.\
Na primer, umesto traženja kompletne datoteke koja sadrži prijavljene URL-ove, ovom tehnikom će se tražiti URL-ovi.
...
Očigledno, postoje načini za **"sigurno" brisanje datoteka i dela zapisa o njima**. Na primer, moguće je **prepisati sadržaj** datoteke sa beskorisnim podacima nekoliko puta, a zatim **ukloniti** zapise iz **$MFT** i **$LOGFILE** o datoteci, i **ukloniti kopije senki zapisa**.\
Možda ćete primetiti da čak i nakon obavljanja te radnje, postoji **još delova gde je postojanje datoteke i dalje zabeleženo**, što je tačno, a deo posla profesionalnog forenzičara je da ih pronađe.
