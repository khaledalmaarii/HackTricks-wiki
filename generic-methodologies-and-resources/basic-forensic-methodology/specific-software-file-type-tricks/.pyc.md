...
Wenn es Ihnen nicht gelungen ist, den python "originalen" Code gemäß den vorherigen Schritten zu extrahieren, können Sie versuchen, die **assembly** zu **extrahieren** (aber **es ist nicht sehr beschreibend**, also **versuchen** Sie, den **originalen** Code **noch einmal** zu extrahieren). Hier [hier](https://bits.theorem.co/protecting-a-python-codebase/) habe ich einen sehr einfachen Code gefunden, um die _.pyc_ Binärdatei zu **disassemblieren** (viel Glück beim Verstehen des Codeflusses). Wenn die _.pyc_ von python2 ist, verwenden Sie python2:
```bash
>>> import dis
>>> import marshal
>>> import struct
...
2. Für den Payload (in diesem Fall nennen wir ihn hello.py) verwenden Sie ein Skript wie das in Abbildung 1. Die Option “bundle\_files” mit dem Wert 1 wird alles, einschließlich des Python-Interpreters, in eine exe bündeln.
...
**Bug bounty tip**: **Melden Sie sich an** für **Intigriti**, eine Premium-**Bug-Bounty-Plattform, die von Hackern für Hacker erstellt wurde**! Treten Sie uns bei [**https://go.intigriti.com/hacktricks**](https://go.intigriti.com/hacktricks) heute bei und beginnen Sie, Prämien von bis zu **100.000 $** zu verdienen!
