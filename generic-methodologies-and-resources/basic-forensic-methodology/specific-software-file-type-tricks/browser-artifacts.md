...
- **Cache**: Mantém dados em cache. Para inspeção, os usuários do Windows podem utilizar [ChromeCacheView](https://www.nirsoft.net/utils/chrome\_cache\_view.html).
...
Metadados de downloads são acessíveis via [ESEDatabaseView](https://www.nirsoft.net/utils/ese\_database\_view.html), com containers específicos contendo dados como URL, tipo de arquivo e local de download. Os arquivos físicos podem ser encontrados em `%userprofile%\Appdata\Roaming\Microsoft\Windows\IEDownloadHistory`.
