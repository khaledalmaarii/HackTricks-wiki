...
Kada pronađete CID, preporučljivo je **pretražiti fajlove koji sadrže ovaj ID**. Možda ćete moći pronaći fajlove sa imenom: _**\<CID>.ini**_ i _**\<CID>.dat**_ koji mogu sadržati zanimljive informacije poput imena fajlova sinhronizovanih sa OneDrive-om.
...
Rezultujući heks je konačni ključ koji se koristi za enkripciju baza podataka koje se mogu dešifrovati sa:
```bash
...
**`filecache.db`** baza podataka sadrži informacije o svim fajlovima i fasciklama sinhronizovanim sa Dropbox-om. Tabela `File_journal` je ona sa najkorisnijim informacijama:
...
* **block\_ref**: Povezan je heš ID tabele `block_cache` sa ID fajla u tabeli `file_journal`
