...
\
使用 [**Trickest**](https://trickest.com/?utm_source=hacktricks&utm_medium=text&utm_campaign=ppc&utm_content=office-file-analysis) 轻松构建和 **自动化工作流程**，由世界上 **最先进** 的社区工具提供支持。\
今天就获取访问权限：
...
微软创建了许多办公文档格式，其中两种主要类型是 **OLE 格式**（如 RTF、DOC、XLS、PPT）和 **Office Open XML (OOXML) 格式**（如 DOCX、XLSX、PPTX）。这些格式可以包含宏，使其成为网络钓鱼和恶意软件的目标。OOXML 文件结构为 zip 容器，允许通过解压缩进行检查，揭示文件和文件夹层次结构及 XML 文件内容。

要探索 OOXML 文件结构，给出了解压文档的命令和输出结构。隐藏数据的技术已被记录，表明在 CTF 挑战中数据隐蔽的持续创新。
