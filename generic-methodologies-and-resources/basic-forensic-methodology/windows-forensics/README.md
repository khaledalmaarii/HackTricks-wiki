...
Tydlyn is 'n Windows kenmerk wat 'n **chronologiese geskiedenis** van besoekte webblaaie, bewerkte dokumente, en uitgevoerde toepassings bied.
...
Lêers wat afgelaai is, kan die **ADS Zone.Identifier** bevat wat aandui **hoe** dit vanaf die intranet, internet, ens. afgelaai is. Sommige sagteware (soos webblaaie) sit gewoonlik selfs **meer** **inligting** soos die **URL** waarvandaan die lêer afgelaai is.
...
Hierdie agterkopieë is gewoonlik geleë in die `\System Volume Information` van die wortel van die lêersisteem en die naam is saamgestel uit **UIDs** soos in die volgende afbeelding:

![](<../../../.gitbook/assets/image (91).png>)

Deur die forensiese beeld te monteer met die **ArsenalImageMounter**, kan die hulpmiddel [**ShadowCopyView**](https://www.nirsoft.net/utils/shadow\_copy\_view.html) gebruik word om 'n skadukopie te ondersoek en selfs **lêers uit die skadukopie-agterkopieë te onttrek**.

![](<../../../.gitbook/assets/image (573).png>)

Die registerinskrywing `HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\BackupRestore` bevat die lêers en sleutels **wat nie agterkopieë moet maak nie**:
...
Ook is die **skeppingstyd van die skakel** lêer die eerste **keer** wat die oorspronklike lêer **eerste** **gebruik** is en die **gewysigde datum** van die skakel lêer is die **laaste** **keer** wat die oorspronklike lêer gebruik is.
...
Die **springlyste** wat outomaties geskep word, word gestoor in `C:\Users\{gebruikersnaam}\AppData\Roaming\Microsoft\Windows\Recent\AutomaticDestinations\`. Die springlyste word benoem volgens die formaat `{id}.autmaticDestinations-ms` waar die aanvanklike ID die ID van die toepassing is.
...
Jy kan die springlyste inspekteer met behulp van [**JumplistExplorer**](https://ericzimmerman.github.io/#!index.md).
...
Die geskeduleerde taak wat bekend staan as 'Inprop en Speel Skoonmaak' is hoofsaaklik ontwerp vir die verwydering van verouderde bestuurderweergawes. In teenstelling met sy gespesifiseerde doel om die nuutste bestuurderpakketweergawe te behou, suggereer aanlynbronne dat dit ook mik op bestuurders wat vir 30 dae onaktief was. Gevolglik kan bestuurders vir verwyderbare toestelle wat nie in die afgelope 30 dae aangesluit is nie, onderwerp wees aan uitwissing.
...
Hierdie konfigurasie verseker gereelde instandhouding en skoonmaak van bestuurders, met bepalings vir herpoging van die taak in geval van opeenvolgende mislukkings.
...
Hierdie toepassing stoor e-posse in HTML of teks. Jy kan die e-posse binne subvouers binne `\Users\<gebruikersnaam>\AppData\Local\Comms\Unistore\data\3\` vind. Die e-posse word gestoor met die `.dat` uitbreiding.
...
* **Windows XP en 8-8.1**: Toegang tot 'n gids met duimnaels genereer 'n `thumbs.db`-lêer wat beeldvoorbeelde stoor, selfs na verwydering.
...
Sommige gereedskap is nuttig vir die analise van die registerlêers:
...
Binne die register `NTUSER.DAT` in die pad `Software\Microsoft\Current Version\Search\RecentApps` kan jy subklasse met inligting oor die **uitgevoerde toepassing**, **laaste keer** wat dit uitgevoer is, en **aantal kere** wat dit afgeskop is, vind.

### BAM (Agtergrondaktiwiteitsmoderator)

Jy kan die `SYSTEM`-lêer oopmaak met 'n registerredigeerder en binne die pad `SYSTEM\CurrentControlSet\Services\bam\UserSettings\{SID}` kan jy die inligting oor die **toepassings wat deur elke gebruiker uitgevoer is** vind (let op die `{SID}` in die pad) en **wanneer** hulle uitgevoer is (die tyd is binne die Data-waarde van die register).

### Windows Prefetch

Prefetching is 'n tegniek wat 'n rekenaar stilweg toelaat om die nodige hulpbronne wat nodig is om inhoud te vertoon wat 'n gebruiker **moontlik binnekort kan toegang** om hulpbronne vinniger toeganklik te maak.

Windows prefetch bestaan uit die skep van **kasgeheues van die uitgevoerde programme** om hulle vinniger te kan laai. Hierdie kassas word geskep as `.pf`-lêers binne die pad: `C:\Windows\Prefetch`. Daar is 'n limiet van 128 lêers in XP/VISTA/WIN7 en 1024 lêers in Win8/Win10.
...
In hierdie databasisse kan jy die **naam** van die **program**, **aantal** **uitvoerings**, **geopen** **lêers**, **volume** **toegang**, **volledige** **pad**, **tydperke** en **tydstempels** vind.
...
Die **AppCompatCache**, ook bekend as **ShimCache**, vorm deel van die **Toepassingsverenigbaarheidsdatabasis** ontwikkel deur **Microsoft** om toepassingsverenigbaarheidsprobleme aan te spreek. Hierdie stelselkomponent neem verskeie stukke lêermetadate op, wat insluit:
...
Hierdie lêer is opmerklik vir die stoor van rekords van onlangs uitgevoerde prosesse, insluitend die paaie na die uitvoerbare lêers en hul SHA1-hashe. Hierdie inligting is van onschatbare waarde vir die opsporing van die aktiwiteit van toepassings op 'n stelsel.
...
```
Onder die gegenereerde CSV-lêers is die `Amcache_Unassociated file entries` veral opmerklik vanweë die ryk inligting wat dit bied oor ongeassosieerde lêerinskrywings.
...
Binne die Toepassingstabel van hierdie databasis is dit moontlik om die kolomme te vind: "Toepassings-ID", "Pakketnommer", en "Vertoonnaam". Hierdie kolomme bevat inligting oor vooraf geïnstalleerde en geïnstalleerde toepassings en dit kan gevind word as sommige toepassings ongeïnstalleer is omdat die ID's van geïnstalleerde toepassings opeenvolgend moet wees.

Dit is ook moontlik om **geïnstalleerde toepassings** te **vind** binne die registerpad: `Software\Microsoft\Windows\CurrentVersion\Appx\AppxAllUserStore\Applications\`\
...
* **Netwerk Skoon (8)**: Skoon wagwoord oordrag, dikwels vanaf IIS.
* **Nuwe Gelde (9)**: Gebruik van verskillende gelde vir toegang.
* **Verre Interaktief (10)**: Verre lessenaar of terminaaldiens aanmelding.
* **Kes Interaktief (11)**: Aanmelding met gekeste gelde sonder kontak met die domeinbeheerder.
* **Kes Verre Interaktief (12)**: Verre aanmelding met gekeste gelde.
* **Gekes Ontsluit (13)**: Ontsluiting met gekeste gelde.

#### Status en Sub-statuskodes vir Gebeurtenis-ID 4625:
...
* **0xC0000234**: Gebruikersrekening gesluit - Kan volg op 'n bruto-kragpoging wat lei tot verskeie mislukte aanmeldings.
...
* **0xC0000133**: Tydsinkronisasieprobleme - Groot tydsverskille tussen klient en bediener kan dui op meer gesofistikeerde aanvalle soos 'n pas-die-kaartjie.
...
* **0xC0000225**: Dui op 'n stelsel fout eerder as 'n sekuriteitsprobleem.
...
* **Logverwydering**: Sekuriteitslogs wat skoongevee word, wat dikwels 'n rooi vlag is vir die bedekking van onwettige aktiwiteite.

#### Gebeurtenis-ID's vir USB-toestelopsporing:

* **20001 / 20003 / 10000**: Eerste koppeling van USB-toestel.
* **10100**: USB-bestuursopdatering.
* **Gebeurtenis-ID 112**: Tyd van USB-toestelinskakeling.

Vir praktiese voorbeelde oor die simulasie van hierdie aanmeldingstipes en geleenthede vir gelde-onttrekking, verwys na [Altered Security se gedetailleerde gids](https://www.alteredsecurity.com/post/fantastic-windows-logon-types-and-where-to-find-credentials-in-them).
...
Om die kanse op die herwinning van uitgevee Windows-gebeure te verbeter, word dit aanbeveel om die verdagte rekenaar af te skakel deur dit direk uit te trek. **Bulk\_extractor**, 'n herstelwerktuig wat die `.evtx`-uitbreiding spesifiseer, word aanbeveel vir die poging om sulke gebeure te herwin.
...
Aangeteken deur Gebeurtenis-ID 4616, veranderinge aan die stelseltyd kan forensiese analise kompliseer.

#### USB-toestelopsporing

Nuttige Stelsel-Gebeurtenis-ID's vir USB-toestelopsporing sluit 20001/20003/10000 vir aanvanklike gebruik, 10100 vir bestuurdersopdaterings, en Gebeurtenis-ID 112 van DeviceSetupManager vir inskakelingstydpeile.
#### Stelselkraggebeure
