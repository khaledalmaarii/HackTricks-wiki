...
Ένα αριθμός αυτόνομου συστήματος (**ASN**) είναι ένας **μοναδικός αριθμός** που ανατίθεται σε ένα **αυτόνομο σύστημα** (AS) από την **Αρχή Ανατεθειμένων Αριθμών Διαδικτύου (IANA)**.\
...
```
Επιπλέον, η υπο-κατηγοριοποίηση του [**BBOT**](https://github.com/blacklanternsecurity/bbot) εξάγει αυτόματα και συνοψίζει τους ASNs στο τέλος της σάρωσης.
```bash
...
> Γνωρίζουμε όλες τις εταιρείες εντός του πεδίου εφαρμογής και τα περιουσιακά τους στοιχεία, είναι η ώρα να βρούμε τα domains εντός του πεδίου εφαρμογής.
...
Καταρχάς, θα πρέπει να αναζητήσετε το **κύριο domain**(s) κάθε εταιρείας. Για παράδειγμα, για την _Tesla Inc._ θα είναι το _tesla.com_.
...
Ελέγξτε αυτό το [**άρθρο για περισσότερες πληροφορίες**](https://swarm.ptsecurity.com/discovering-domains-via-a-time-correlation-attack/).
...
Φαίνεται ότι είναι συνηθισμένο για ανθρώπους να αναθέτουν υποτομές σε διευθύνσεις IP που ανήκουν σε παρόχους νέφους και σε κάποιο σημείο **χάνουν αυτήν τη διεύθυνση IP αλλά ξεχνούν να αφαιρέσουν τον εγγραφή DNS**. Επομένως, απλά **εκκινώντας ένα VM** σε ένα νέφος (όπως το Digital Ocean) θα **αναλάβετε πραγματικά κάποιες υποτομές**.
...
Καθώς γνωρίζετε ήδη το όνομα του οργανισμού που κατέχει τον χώρο IP. Μπορείτε να αναζητήσετε με βάση αυτά τα δεδομένα στο shodan χρησιμοποιώντας: `org:"Tesla, Inc."` Ελέγξτε τους εντοπισμένους οικοδεσπότες για νέους απροσδόκητους τομείς στο πιστοποιητικό TLS.
...
Ελέγξτε για κάποια [κατάληψη τομέα](../../pentesting-web/domain-subdomain-takeover.md#domain-takeover). Ίσως κάποια εταιρεία **να χρησιμοποιεί έναν τομέα** αλλά **να έχει χάσει την ιδιοκτησία**. Απλά το καταχωρείτε (αν είναι αρκετά φθηνό) και ενημερώνετε την εταιρεία.
...
{% hint style="success" %}
Σημειώστε ότι μερικά από τα εργαλεία και τις τεχνικές για την εύρεση τομέων μπορούν επίσης να βοηθήσουν στην εύρεση υποτομέων!
{% endhint %}

### **DNS**

Ας προσπαθήσουμε να πάρουμε **υποτομές** από τα **αρχεία DNS**. Θα πρέπει επίσης να δοκιμάσουμε τη **μεταφορά ζώνης** (Αν είναι ευάλωτη, θα πρέπει να το αναφέρετε).
```bash
...
```
* [**SubDomainizer**](https://github.com/nsonaniya2010/SubDomainizer) **&** [**subscraper**](https://github.com/Cillian-Collins/subscraper): Αυτά τα εργαλεία σκαλίζουν τον ιστό ψάχνοντας για αρχεία JS και εξάγουν υποτομείς από εκεί.
```bash
...
* [**massdns**](https://github.com/blechschmidt/massdns): Αυτό ήταν το πρώτο εργαλείο που πραγματοποίησε μια αποτελεσματική δύναμη DNS brute-force. Είναι πολύ γρήγορο, αλλά είναι επιρρεπές σε Ϩενερότητες.
```bash
...
```
* [**aiodnsbrute**](https://github.com/blark/aiodnsbrute) χρησιμοποιεί το asyncio για να εκτελέσει βίαιη επίθεση στα ονόματα των domain ασύγχρονα.
```
...
* Μπορείτε να λάβετε τις μεταθέσεις **λίστας λέξεων** του altdns [**εδώ**](https://github.com/infosec-au/altdns/blob/master/words.txt).
```
altdns -i subdomains.txt -w /tmp/words-permutations.txt -o /tmp/asd3
```
* [**dmut**](https://github.com/bp0lr/dmut): Ένα άλλο εργαλείο για να πραγματοποιεί περιστροφές, μεταλλάξεις και τροποποιήσεις υποτομέων. Αυτό το εργαλείο θα εκτελέσει βίαιη επίθεση για το αποτέλεσμα (δεν υποστηρίζει wild card dns).
...
```
* [**alterx**](https://github.com/projectdiscovery/alterx)**:** Βασισμένο σε ένα domain, **δημιουργεί νέα πιθανά ονόματα subdomains** βασισμένα σε καθορισμένα πρότυπα προκειμένου να δοκιμάσει να ανακαλύψει περισσότερα subdomains.
...
Αν βρήκατε ένα διεύθυνση IP που περιέχει **μία ή περισσότερες ιστοσελίδες** που ανήκουν σε υποτομείς, μπορείτε να προσπαθήσετε να **βρείτε άλλους υποτομείς με ιστοσελίδες σε αυτήν την IP** ψάχνοντας σε **πηγές OSINT** για domains σε μια IP ή με το **brute-forcing ονομάτων domain VHost** σε αυτήν την IP.

#### OSINT

Μπορείτε να βρείτε μερικούς **VHosts σε IPs χρησιμοποιώντας** [**HostHunter**](https://github.com/SpiderLabs/HostHunter) **ή άλλα APIs**.
...
Καθώς αναζητάτε **υποτομές**, πρέπει να παρακολουθείτε αν αυτές **δείχνουν** σε οποιοδήποτε είδος **κάδου**, και σε αυτήν την περίπτωση [**ελέγξτε τις άδειες πρόσβασης**](../../network-services-pentesting/pentesting-web/buckets/)**.**\
Επίσης, αφού σε αυτό το σημείο θα γνωρίζετε όλους τους τομείς εντός του πεδίου εφαρμογής, δοκιμάστε να [**επιτεθείτε βίαια σε πιθανά ονόματα κάδων και ελέγξτε τις άδειες πρόσβασης**](../../network-services-pentesting/pentesting-web/buckets/).
...
Αν η **υποτομή** δείχνει σε κάποιο **κάδο S3**, [**ελέγξτε τις άδειες πρόσβασης**](../../network-services-pentesting/pentesting-web/buckets/).
...
**Σαρώστε όλες τις IP που δεν ανήκουν σε CDN** (καθώς πιθανότατα δεν θα βρείτε κάτι ενδιαφέρον εκεί). Στις υπηρεσίες που ανακαλύφθηκαν μπορείτε να **βρείτε ευπαθείες**.
...
Επιπλέον, μπορείτε να χρησιμοποιήσετε το [**eyeballer**](https://github.com/BishopFox/eyeballer) για να εκτελέσετε όλα τα **στιγμιότυπα οθονών** και να σας πει **τι πιθανόν περιέχει ευπάθειες**, και τι όχι.
...
Να θυμάστε όταν ψάχνετε για Περιουσιακά Στοιχεία στο Cloud να **ψάξετε για περισσότερα από απλά buckets στο AWS**.

### **Αναζήτηση ευπαθειών**

Αν βρείτε πράγματα όπως **ανοιχτά buckets ή εκθέτες λειτουργίες cloud** θα πρέπει να **έχετε πρόσβαση** σε αυτά και να δοκιμάσετε να δείτε τι σας προσφέρουν και αν μπορείτε να τα καταχραστείτε.
...
Η **πλειοψηφία των ευπαθειών** που βρίσκουν οι κυνηγοί ευρετήριων βρίσκεται μέσα σε **εφαρμογές ιστού**, οπότε σε αυτό το σημείο θα ήθελα να μιλήσω για μια **μεθοδολογία δοκιμών εφαρμογών ιστού**, και μπορείτε [**να βρείτε αυτές τις πληροφορίες εδώ**](../../network-services-pentesting/pentesting-web/).

Θέλω επίσης να κάνω μια ειδική αναφορά στην ενότητα [**Εργαλεία Αυτόματων Σαρωτών Ιστού με Ανοιχτό Κώδικα**](../../network-services-pentesting/pentesting-web/#automatic-scanners), καθώς, αν και δεν πρέπει να αναμένετε να βρουν πολύ ευαίσθητες ευπαθείες, είναι χρήσιμα για να εφαρμόσετε τα σενάρια σας για να έχετε κάποιες αρχικές πληροφορίες ιστού.
...
Αν σας ενδιαφέρει η **καριέρα στο χάκινγκ** και να χακάρετε το αχάκαρτο - **σας προσλαμβάνουμε!** (_απαιτείται άριστη γραπτή και προφορική γνώση της πολωνικής γλώσσας_).
