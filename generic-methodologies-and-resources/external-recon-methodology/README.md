...
> Tamam, bu noktada kapsamdaki tüm şirketleri bilmelisiniz. Şimdi varlıklarını nasıl bulacağımızı anlayalım.
...
Ayrıca, bazı [**port taramaları**](../pentesting-network/#discovering-hosts-from-the-outside) **başlatabilir veya** shodan **gibi hizmetleri kullanarak** açık portları **bulabilir ve bulduklarınıza bağlı olarak bu kitapta çalışan çeşitli hizmetleri nasıl pentest edeceğinize** bakmalısınız.\
...
Bunu çalıştırmak için yöneticinin PTR'yi manuel olarak etkinleştirmesi gerekmektedir.\
...
**Yeni bir alan bulduğunuzda her seferinde daha fazla alan adı keşfetmek için bu tekniği kullanabileceğinizi unutmayın.**

### **İzleyiciler**

Aynı **izleyici**nin **aynı kimliğini** 2 farklı sayfada bulursanız, **her iki sayfanın** de **aynı ekibin yönettiğini** varsayabilirsiniz.\
...
Hedefimize ait ilgili alan adlarını ve alt alanları aynı favicon ikonu karmasını arayarak bulabileceğimizi biliyor muydunuz? İşte bunu yapan [favihash.py](https://github.com/m4ll0k/Bug-Bounty-Toolz/blob/master/favihash.py) aracı, [@m4ll0k2](https://twitter.com/m4ll0k2) tarafından yapılmıştır. Nasıl kullanılacağı:
```bash
...
Ayrıca, favicon karmasını kullanarak teknolojileri de arayabilirsiniz, [**bu blog yazısında**](https://medium.com/@Asm0d3us/weaponizing-favicon-ico-for-bugbounties-osint-and-what-not-ace3c214e139) açıklandığı gibi. Bu, yani eğer bir web teknolojisinin savunmasız bir sürümünün faviconunun **karmasını biliyorsanız**, shodan'da arayabilir ve **daha fazla savunmasız yeri bulabilirsiniz**:
```bash
...
Aynı kuruluşun farklı web siteleri arasında paylaşılabilecek dizeleri aramak. **Telif hakkı dizesi** iyi bir örnek olabilir. Ardından bu dizeyi **Google**, diğer **tarayıcılarda** veya hatta **shodan**'da arayın: `shodan search http.html:"Telif hakkı dizesi"`
...
Sunucudaki tüm alan sertifikalarını yenilemek için. Bu, bunun için kullanılan CA'nın Geçerlilik süresinde üretildiği zamanı ayarlamadığı durumda bile, **sertifika şeffaflık günlüklerinde aynı şirkete ait alanları bulmak mümkündür**.
...
Görünüşe göre insanlar alt alanları bulut sağlayıcılarına ait IP'lere atarlar ve bir noktada **o IP adresini kaybederler ancak DNS kaydını kaldırmayı unuturlar**. Bu nedenle, sadece bir VM oluşturarak (Digital Ocean gibi) aslında **bazı alt alanları ele geçireceksiniz**.
...
**Bu tekniği yeni bir alan bulduğunuz her seferinde daha fazla alan adı keşfetmek için kullanabileceğinizi unutmayın**.

**Shodan**

IP alanını sahiplenen kuruluşun adını zaten biliyorsunuz. Bu verilerle shodan'da şuna benzer bir arama yapabilirsiniz: `org:"Tesla, Inc."` TLS sertifikasında yeni beklenmeyen alanlar için bulunan ana bilgisayarlara bakın.
...
Bazı [alan ele geçirme](../../pentesting-web/domain-subdomain-takeover.md#domain-takeover) durumlarını kontrol edin. Belki bir şirket **bir alan kullanıyor** ancak **sahipliğini kaybetti**. Ucuzsa kaydedin ve şirkete bildirin.

Varlıklar keşfinde bulduğunuz IP'lerden farklı bir IP'ye sahip **alan bulursanız**, temel bir güvenlik açığı taraması yapmalısınız (Nessus veya OpenVAS kullanarak) ve **nmap/masscan/shodan** ile bazı [**port taraması**](../pentesting-network/#discovering-hosts-from-the-outside) yapmalısınız. Hangi hizmetlerin çalıştığına bağlı olarak, **bu kitapta onları "saldırmak" için bazı püf noktaları bulabilirsiniz**.\
_Bazen alan, müşteri tarafından kontrol edilmeyen bir IP içinde barındırıldığından, bu kapsamda değildir, dikkatli olun._
...
{% hint style="success" %}
Bazı araçların ve tekniklerin alanları bulmaya yardımcı olabileceği gibi alt alanları bulmaya da yardımcı olabileceğini unutmayın!
{% endhint %}
...
```
**Alt alan adlarını bulmada doğrudan uzmanlaşmamış olsa da** alt alan adlarını bulmada faydalı olabilecek **diğer ilginç araçlar/API'ler** bulunmaktadır, örneğin:
...
crt tesla.com
```
* [**gau**](https://github.com/lc/gau)**:** Belirli bir alan adından AlienVault'ın Açık Tehdit Değişiminden, Wayback Machine'den ve Common Crawl'dan bilinen URL'leri alır.
```bash
...
Ayrıca iyi DNS çözücülerin IP'lerine de ihtiyacınız olacak. Güvenilir DNS çözücülerin bir listesini oluşturmak için çözücüleri [https://public-dns.info/nameservers-all.txt](https://public-dns.info/nameservers-all.txt) adresinden indirebilir ve bunları filtrelemek için [**dnsvalidator**](https://github.com/vortexau/dnsvalidator) kullanabilirsiniz. Veya şunu kullanabilirsiniz: [https://raw.githubusercontent.com/trickest/resolvers/main/resolvers-trusted.txt](https://raw.githubusercontent.com/trickest/resolvers/main/resolvers-trusted.txt)
...
```
* [**shuffledns**](https://github.com/projectdiscovery/shuffledns), aktif bruteforce kullanarak geçerli alt alanları numaralandırmanıza ve joker işleme sahip alt alanları çözmenize olanak tanıyan, go dilinde yazılmış `massdns` etrafında bir sarmalayıcıdır ve kolay giriş-çıkış desteği sağlar.
```
...
```
* [**goaltdns**](https://github.com/subfinder/goaltdns): Alan adları ve alt alan adları verildiğinde permütasyonlar oluşturun.
...
```
* [**alterx**](https://github.com/projectdiscovery/alterx)**:** Bir alan adına dayanarak, belirtilen desenlere göre yeni potansiyel alt alan adı adları oluşturur ve daha fazla alt alan adı keşfetmeye çalışır.
...
Bir alan adının **yeni alt alan adlarının** oluşturulup oluşturulmadığını **izleyebilirsiniz** [**Sertifika Şeffaflığı** Günlükleri](https://github.com/yassineaboukir/sublert/blob/master/sublert.py) tarafından yapılır.
...
Varlık keşfinde bulunan IP'lerden farklı bir IP'ye sahip herhangi bir **alt alan adı** bulursanız, bir **temel zafiyet taraması** yapmalısınız (Nessus veya OpenVAS kullanarak) ve **nmap/masscan/shodan** ile bazı [**port taraması**](../pentesting-network/#discovering-hosts-from-the-outside) yapmalısınız. Hangi hizmetlerin çalıştığını bulabilir ve **bu kitapta onları "saldırmak" için bazı hileler bulabilirsiniz**.\
_Not: Bazen alt alan adı, müşteri tarafından kontrol edilmeyen bir IP içinde barındırılır, bu nedenle kapsamda değildir, dikkatli olun._
...
Lütfen, bu **web uygulamaları keşfi için yönlendirilecektir**, bu nedenle kapsam tarafından **izin verildiği takdirde** **zafiyet** ve **port taraması** da yapmalısınız.

**Web** sunucularıyla ilgili **açık portları keşfetmek için hızlı bir yöntem** [**burada masscan**](../pentesting-network/#http-port-discovery) bulunabilir.\
...
Ayrıca, daha sonra tüm **ekran görüntülerini** çalıştırmak için [**eyeballer**](https://github.com/BishopFox/eyeballer)'ı kullanabilirsiniz, size **olası olarak zafiyet içeren şeyleri** ve olmayanları söylemek için.
...
E-postalar daha sonra **web girişlerini ve kimlik doğrulama hizmetlerini kaba kuvvet uygulamak** (örneğin SSH) ve **phishing** için gereklidir. Ayrıca, bu API'lar size e-posta arkasındaki kişi hakkında daha fazla **bilgi verecektir**, bu da phishing kampanyası için faydalıdır.
...
Kimlik bilgisi sızıntıları, **duyarlı bilgilerin sızdırıldığı ve satıldığı** şirketlerin hack'lenmesiyle ilgilidir. Ancak, şirketler, o veritabanlarında olmayan başka sızıntılardan da etkilenebilir:

### Github Sızıntıları

Kimlik bilgileri ve API'lar, **şirketin** veya o şirketin github şirketi tarafından çalışan **kullanıcıların** **genel depolarında** sızdırılmış olabilir.\
[**Leakos**](https://github.com/carlospolop/Leakos) aracını kullanarak bir **kuruluşun** ve **geliştiricilerinin** tüm **genel depolarını indirip** [**gitleaks**](https://github.com/zricethezav/gitleaks) otomatik olarak çalıştırabilirsiniz.

**Leakos**, bazen **web sayfaları da sırlar içerebildiği için** onlara geçirilen **URL'leri çalıştırmak** için de kullanılabilir.

#### Github Dorks

Saldırganlar veya sadece çalışanlar bazen **şirket içeriğini bir yapıştırma sitesinde yayınlarlar**. Bu, **duyarlı bilgiler** içerebilir veya içermeyebilir, ancak aramak çok ilginçtir.\
...
Eski ama altın google dorks, **orada olmaması gereken açık bilgileri bulmak** için her zaman faydalıdır. Tek sorun, [**google-hacking-database**](https://www.exploit-db.com/google-hacking-database)'in çalıştıramayacağınız birkaç bin olası sorguyu içermesidir. Bu nedenle, favori 10 tanesini alabilir veya hepsini çalıştırmak için [**Gorks**](https://github.com/carlospolop/Gorks) gibi bir **araç kullanabilirsiniz**.

_Not: Google tarayıcısını kullanarak tüm veritabanını çalıştırmayı bekleyen araçlar çok uzun sürecek, çünkü Google sizi çok kısa sürede engelleyecektir._
...
Şirketin **açık kaynak koduna** sahip olduğunu tespit ettiyseniz, onu **analiz edebilir** ve üzerinde **zafiyetler** arayabilirsiniz.

**Dile bağlı olarak** kullanabileceğiniz farklı **araçlar** bulunmaktadır:

{% content-ref url="../../network-services-pentesting/pentesting-web/code-review-tools.md" %}
[code-review-tools.md](../../network-services-pentesting/pentesting-web/code-review-tools.md)
{% endcontent-ref %}

Ayrıca, şunlar gibi **genel depoları tarayabilmenize izin veren ücretsiz hizmetler** de bulunmaktadır:

* [**Snyk**](https://app.snyk.io/)
## [**Web Sızma Testi Metodolojisi**](../../network-services-pentesting/pentesting-web/)

**Bulunan zafiyetlerin çoğunluğu**, avcılar tarafından **web uygulamalarında** bulunur, bu yüzden bu noktada bir **web uygulaması testi metodolojisi** hakkında konuşmak istiyorum ve [**bu bilgiyi burada bulabilirsiniz**](../../network-services-pentesting/pentesting-web/).
