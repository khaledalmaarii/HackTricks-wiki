...
Op hierdie punt ken ons **alle bates binne die omvang**, so as jy toegelaat word, kan jy 'n paar **kwesbaarheidsskanderings** (Nessus, OpenVAS) oor al die gasheerservers uitvoer.\
...
_Geliewe op te let dat in die volgende voorgestelde tegnieke jy ook subdomeine kan vind en daardie inligting moet nie onderskat word nie._
...
Binne 'n **whois** kan jy baie interessante **inligting** vind soos **organisasienaam**, **adres**, **e-posse**, telefoonnommers... Maar wat selfs meer interessant is, is dat jy **meer bates wat verband hou met die maatskappy** kan vind as jy **omgekeerde whois-opsoeke uitvoer deur enige van daardie velde** (byvoorbeeld ander whois-registre waar dieselfde e-pos voorkom).\
...
Daar is 'n paar bladsye en gereedskap wat jou toelaat om te soek na hierdie opvolgers en meer:
...
Jy kan die **TLS-sertifikaat** van die hoofwebblad besoek, die **Organisasienaam** verkry en dan soek na daardie naam binne die **TLS-sertifikate** van al die webblaaie wat deur **Shodan** bekend is met die filter: `ssl:"Tesla Motors"` of gebruik 'n instrument soos [**sslsearch**](https://github.com/HarshVaragiya/sslsearch).
...
Kyk vir 'n [domeinoorneem](../../pentesting-web/domain-subdomain-takeover.md#domain-takeover). Dalk gebruik 'n maatskappy 'n domein, maar hulle het die eiendomsreg verloor. Registreer dit net (as dit goedkoop genoeg is) en laat die maatskappy weet.
...
_Merk op dat die domein soms gehuisves word binne 'n IP wat nie deur die klient beheer word nie, dus dit val buite die bestek, wees versigtig._
...
Laat ons probeer om nuwe **subdomeine** te vind deur DNS-bedieners te dwing met moontlike subdomeinname.
...
Die mees aanbevole gereedskappe vir DNS-brute force is:

* [**massdns**](https://github.com/blechschmidt/massdns): Dit was die eerste gereedskap wat 'n effektiewe DNS-brute force uitgevoer het. Dit is baie vinnig, maar vatbaar vir vals positiewe.
```bash
...
```
* [**shuffledns**](https://github.com/projectdiscovery/shuffledns) is 'n omhulsel rondom `massdns`, geskryf in go, wat jou in staat stel om geldige subdomeine te ontleed deur aktiewe bruteforce te gebruik, asook om subdomeine op te los met wildcard-hantering en maklike in- en uitvoer-ondersteuning.
```
...
```
* [**aiodnsbrute**](https://github.com/blark/aiodnsbrute) maak gebruik van asyncio om domeinname asinkroon te kragtig te ontsluit.
```
...
```
* [**gotator**](https://github.com/Josue87/gotator)**:** Gee die domeine en subdomeine genereer permutasies. As geen permutasie lêer aangedui word nie, sal gotator sy eie een gebruik.
```
...
Jy kan **moniter** of **nuwe subdomeine** van 'n domein geskep word deur die **Sertifikaat Transparantie** Logboeke te monitor [**sublert** ](https://github.com/yassineaboukir/sublert/blob/master/sublert.py)doen.
...
**Poortskandering al die IP's wat nie aan CDN's behoort nie** (aangesien jy waarskynlik niks interessants daar sal vind nie). In die lopende dienste wat ontdek is, kan jy **kwesbaarhede vind**.
...
Om potensiële wolkbates van 'n maatskappy te vind, moet jy **begin met 'n lys sleutelwoorde wat daardie maatskappy identifiseer**. Byvoorbeeld, vir 'n kriptomaatskappy kan jy woorde soos: `"kripto", "beursie", "dao", "<domein_naam>", <"subdomein_name">` gebruik.
