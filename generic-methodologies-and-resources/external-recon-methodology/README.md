...
Il est intéressant de savoir si l'**entreprise a attribué un ASN** pour trouver ses **plages d'IP**. Il sera intéressant d'effectuer un **test de vulnérabilité** contre tous les **hôtes** dans le **périmètre** et de rechercher des domaines à l'intérieur de ces IP.\
...
Apparemment, il est courant que les gens attribuent des sous-domaines à des adresses IP appartenant à des fournisseurs de cloud et à un moment donné **perdent cette adresse IP mais oublient de supprimer l'enregistrement DNS**. Par conséquent, simplement **lancer une machine virtuelle** dans un cloud (comme Digital Ocean) vous permettra en fait de **prendre le contrôle de certains sous-domaines**.

[**Cet article**](https://kmsec.uk/blog/passive-takeover/) explique une histoire à ce sujet et propose un script qui **démarre une machine virtuelle dans DigitalOcean**, **obtient** l'**IPv4** de la nouvelle machine, et **recherche dans Virustotal les enregistrements de sous-domaines** pointant vers elle.
...
* [**massdns**](https://github.com/blechschmidt/massdns): C'était le premier outil à effectuer une attaque par force brute DNS efficace. Il est très rapide mais sujet aux faux positifs.
```bash
...
Vous pouvez **surveiller** si de **nouveaux sous-domaines** d'un domaine sont créés en surveillant les **logs de transparence des certificats** avec [**sublert** ](https://github.com/yassineaboukir/sublert/blob/master/sublert.py).
...
Si vous trouvez des **identifiants ou jetons API valides** qui ont fuité, c'est une victoire très facile.
...
5. Trouvé tous les **IP** (des **CDN et non-CDN**) dans le périmètre.
