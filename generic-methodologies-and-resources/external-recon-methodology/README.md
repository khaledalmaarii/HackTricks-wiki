...
4. Użyć innych technik, takich jak filtry shodan `org` i `ssl`, aby szukać innych zasobów (szczegół można wykonać rekurencyjnie).

### **Przejęcia**

Po pierwsze, musimy dowiedzieć się, które **inne firmy są własnością głównej firmy**.\
...
```
Również, [**BBOT**](https://github.com/blacklanternsecurity/bbot)**'s** automatycznie agreguje i podsumowuje subdomeny ASN na końcu skanowania.
```bash
...
Wewnątrz **whois** można znaleźć wiele interesujących **informacji**, takich jak **nazwa organizacji**, **adres**, **adresy e-mail**, numery telefonów... Ale co jeszcze bardziej interesujące, to że można znaleźć **więcej zasobów związanych z firmą**, jeśli przeprowadzisz **odwrotne wyszukiwanie whois według tych pól** (na przykład inne rejestry whois, gdzie pojawi się ten sam adres e-mail).\
...
```
![favihash - odkrywaj domeny z takim samym haszem ikony favicon](https://www.infosecmatter.com/wp-content/uploads/2020/07/favihash.jpg)
...
Aby odnowić wszystkie certyfikaty domenowe na serwerze, nawet jeśli CA użyta do tego nie ustawia czasu generacji w Czasie ważności, można **znaleźć domeny należące do tej samej firmy w dziennikach transparentności certyfikatów**.\
...
Wygląda na to, że ludzie często przypisują subdomeny do adresów IP należących do dostawców chmury i w pewnym momencie **tracą ten adres IP, ale zapominają usunąć rekord DNS**. Dlatego po prostu **uruchamiając maszynę wirtualną** w chmurze (np. Digital Ocean) faktycznie **przejmiesz niektóre subdomeny**.
...
[**Assetfinder**](https://github.com/tomnomnom/assetfinder) to narzędzie, które szuka **powiązanych domen** z główną domeną i **ich subdomenami**, całkiem niesamowite.
...
Do tego działania będziesz potrzebował kilku **często używanych list słów dla subdomen, takich jak**:
...
```
* [**shuffledns**](https://github.com/projectdiscovery/shuffledns) to nakładka na `massdns`, napisana w go, która pozwala na wyliczenie prawidłowych subdomen za pomocą aktywnego bruteforce, a także rozwiązywanie subdomen z obsługą symboli wieloznacznych i łatwym wsparciem wejścia-wyjścia.
```
...
Sprawdź ten wpis na blogu, który napisałem o tym, jak **zautomatyzować odkrywanie subdomen** z domeny przy użyciu **przepływów Trickest**, aby nie musiał ręcznie uruchamiać wielu narzędzi na swoim komputerze:
...
Jeśli podejrzewasz, że jakaś subdomena może być ukryta na serwerze WWW, możesz spróbować ją siłowo odszukać:
```bash
...
Jeśli znajdziesz rzeczy takie jak **otwarte kubełki lub funkcje chmurowe wystawione**, powinieneś je **odwiedzić** i spróbować zobaczyć, co oferują i czy można je nadużyć.
...
Dzięki **domenom**, **poddomenom** i **emailom** możesz zacząć szukać wycieków poświadczeń, które miały miejsce w przeszłości i należą do tych adresów e-mail:
...
**Leakos** może być również używany do uruchomienia **gitleaks** na wszystkich **tekstach** przekazanych **URL-ów** do niego, ponieważ czasami **strony internetowe również zawierają sekrety**.

#### Dorki na Githubie

Sprawdź również tę **stronę** w poszukiwaniu potencjalnych **dorków na githubie**, których również możesz szukać w organizacji, którą atakujesz:
...
Jeśli interesuje Cię **kariera hakera** i hakowanie niemożliwych do zhakowania - **zatrudniamy!** (_wymagana biegła znajomość języka polskiego w piśmie i mowie_).
