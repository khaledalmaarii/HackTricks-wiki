...
```
![favihash - otkrijte domene sa istim hešom ikone favikona](https://www.infosecmatter.com/wp-content/uploads/2020/07/favihash.jpg)

Jednostavno rečeno, favihash će nam omogućiti da otkrijemo domene koje imaju isti heš ikone favikona kao naš cilj.

Osim toga, možete takođe pretraživati tehnologije koristeći heš favikona kako je objašnjeno u [**ovom blog postu**](https://medium.com/@Asm0d3us/weaponizing-favicon-ico-for-bugbounties-osint-and-what-not-ace3c214e139). To znači da ako znate **heš favikona ranjive verzije web tehnologije** možete pretražiti u shodanu i **pronaći više ranjivih mesta**:
```bash
...
Potrebno je obnoviti sve sertifikate domena na serveru. To znači da čak i ako CA koji se koristi za ovo ne postavlja vreme kada je generisan u vremenskom periodu Validnosti, moguće je **pronaći domene koje pripadaju istoj kompaniji u logovima transparentnosti sertifikata**.\
...
Pokušajmo da pronađemo nove **poddomene** grubo forsirajući DNS servere koristeći moguća imena poddomena.
...
```
### **Bruteforcing Bucketova**

Prilikom traženja **poddomena**, obratite pažnju da li on **ukazuje** na bilo kakav **bucket**, i u tom slučaju [**proverite dozvole**](../../network-services-pentesting/pentesting-web/buckets/)**.**\
Takođe, sada kada znate sve domene unutar opsega, pokušajte [**bruteforce-ovati moguća imena bucketova i proverite dozvole**](../../network-services-pentesting/pentesting-web/buckets/).

### **Monitorisanje**

Možete **pratiti** da li su **novi poddomeni** domena kreirani praćenjem **Certificate Transparency** logova [**sublert** ](https://github.com/yassineaboukir/sublert/blob/master/sublert.py) radi.

### **Traženje ranjivosti**

Proverite moguće [**preuzimanje poddomena**](../../pentesting-web/domain-subdomain-takeover.md#subdomain-takeover).\
Ako **poddomen** ukazuje na neki **S3 bucket**, [**proverite dozvole**](../../network-services-pentesting/pentesting-web/buckets/).
...
Takođe možete proveriti domene koje ukazuju na određenu IP adresu koristeći alat [**hakip2host**](https://github.com/hakluke/hakip2host)
...
U prethodnim koracima verovatno ste već izvršili neko **rekonnoitering IP adresa i otkrivenih domena**, tako da možda već imate **pronašli sve moguće veb servere**. Međutim, ako niste, sada ćemo videti neke **brze trikove za traženje veb servera** unutar opsega.

Molimo, imajte na umu da će ovo biti **usmereno na otkrivanje veb aplikacija**, pa biste trebali **izvršiti skeniranje ranjivosti** i **portova** takođe (**ako je dozvoljeno** u opsegu).

**Brz metod** za otkrivanje **otvorenih portova** vezanih za **veb** servere korišćenjem [**masscan** možete pronaći ovde](../pentesting-network/#http-port-discovery).\
...
Osim toga, zatim možete koristiti [**eyeballer**](https://github.com/BishopFox/eyeballer) da pregledate sve **snimke ekrana** i da vam kaže **šta je verovatno da sadrži ranjivosti**, a šta ne.
...
Zatim, sa tim rečima trebalo bi da generišete **permutacije** (proverite [**Druga runda DNS Brute-Force**](./#second-dns-bruteforce-round) za više informacija).
...
Ponekad napadači ili samo radnici će **objaviti sadržaj kompanije na sajtu za paste**. To može ili ne mora sadržati **poverljive informacije**, ali je veoma interesantno pretražiti to.\
Možete koristiti alat [**Pastos**](https://github.com/carlospolop/Pastos) da pretražujete u više od 80 sajtova za paste istovremeno.
