...
4. Χρησιμοποιήστε άλλες τεχνικές όπως τα shodan φίλτρα `org` και `ssl` για να αναζητήσετε άλλα περιουσιακά στοιχεία (η τεχνική με το `ssl` μπορεί να γίνει αναδρομικά).

### **Εξαγορές**

Καταρχάς, πρέπει να μάθουμε ποιες **άλλες εταιρείες ανήκουν στην κύρια εταιρεία**.\
Ένας τρόπος είναι να επισκεφθείτε το [https://www.crunchbase.com/](https://www.crunchbase.com), **αναζητήστε** τη **κύρια εταιρεία** και **κάντε κλικ** στο "**εξαγορές**". Εκεί θα δείτε άλλες εταιρείες που έχουν εξαγοραστεί από την κύρια.\
Άλλη επιλογή είναι να επισκεφθείτε τη σελίδα **Wikipedia** της κύριας εταιρείας και να αναζητήσετε **εξαγορές**. 
...
Σε αυτό το σημείο γνωρίζουμε **όλα τα assets εντός του scope**, οπότε αν σας επιτρέπεται μπορείτε να εκτελέσετε κάποιο **vulnerability scanner** (Nessus, OpenVAS) σε όλους τους hosts.\
Επίσης, μπορείτε να εκτελέσετε κάποια [**port scans**](../pentesting-network/#discovering-hosts-from-the-outside) **ή να χρησιμοποιήσετε υπηρεσίες όπως** shodan **για να βρείτε** ανοιχτές πόρτες **και ανάλογα με αυτό που βρείτε θα πρέπει** να εξετάσετε σε αυτό το βιβλίο πώς να κάνετε pentest σε πιθανές υπηρεσίες που τρέχουν.\
**Επίσης, αξίζει να σημειωθεί ότι μπορείτε επίσης να ετοιμάσετε κάποιες** λίστες προεπιλεγμένων ονομάτων χρήστη **και** κωδικών πρόσβασης **και να προσπαθήσετε να** κάνετε bruteforce σε υπηρεσίες με το [https://github.com/x90skysn3k/brutespray](https://github.com/x90skysn3k/brutespray).

## Domains

> Γνωρίζουμε όλες τις εταιρείες εντός του scope και τα assets τους, είναι ώρα να βρούμε τα domains εντός του scope.

_Παρακαλώ, σημειώστε ότι στις παρακάτω προτεινόμενες τεχνικές μπορείτε επίσης να βρείτε subdomains και αυτές οι πληροφορίες δεν πρέπει να υποτιμηθούν._

Καταρχάς, θα πρέπει να αναζητήσετε τα **κύρια domain**(s) κάθε εταιρείας. Για παράδειγμα, για την _Tesla Inc._ θα είναι το _tesla.com_.

### **Reverse DNS**

Αφού έχετε βρει όλες τις εύρεσης IP των domains μπορείτε να προσπαθήσετε να εκτελέσετε **αντίστροφες αναζητήσεις DNS** σε αυτά τα **IPs για να βρείτε περισσότερα domains εντός του scope**. Δοκιμάστε να χρησιμοποιήσετε κάποιον dns server του θύματος ή κάποιον γνωστό dns server (1.1.1.1, 8.8.8.8)
```bash
...
Μπορείτε επίσης να εκτελέσετε μερικές αυτόματες ανακαλύψεις αντίστροφης αναζήτησης whois με το [amass](https://github.com/OWASP/Amass): `amass intel -d tesla.com -whois`
...
```
![favihash - ανακαλύψτε domains με τον ίδιο κωδικό hash εικονίδιου favicon](https://www.infosecmatter.com/wp-content/uploads/2020/07/favihash.jpg)

Απλά ειπωμένο, το favihash θα μας επιτρέψει να ανακαλύψουμε domains που έχουν τον ίδιο κωδικό hash εικονίδιου favicon με τον στόχο μας.
...
Είναι συνηθισμένο να υπάρχει ένας προγραμματισμένος χρόνος εκτέλεσης όπως
```bash
# /etc/crontab
...
Ελέγξτε για κάποια [κατάληψη τομέα](../../pentesting-web/domain-subdomain-takeover.md#domain-takeover). Ίσως κάποια εταιρεία **να χρησιμοποιεί έναν τομέα** αλλά **έχασε την ιδιοκτησία**. Απλά το καταχωρείτε (αν είναι αρκετά φθηνό) και ενημερώνετε την εταιρεία.
...
**Συμβουλή για bug bounty**: **Εγγραφείτε** στο **Intigriti**, μια προηγμένη **πλατφόρμα ανταμοιβής ευπαθειών δημιουργημένη από χάκερς, για χάκερς**! Γίνετε μέλος μας στο [**https://go.intigriti.com/hacktricks**](https://go.intigriti.com/hacktricks) σήμερα, και αρχίστε να κερδίζετε ανταμοιβές μέχρι **$100,000**!
...
Ας προσπαθήσουμε να βρούμε νέα **subdomains** με βίαιη δύναμη σε διακομιστές DNS χρησιμοποιώντας πιθανά ονόματα subdomain.
...
* [**massdns**](https://github.com/blechschmidt/massdns): Αυτό ήταν το πρώτο εργαλείο που πραγματοποίησε μια αποτελεσματική βίαιη δύναμη DNS. Είναι πολύ γρήγορο, αλλά είναι επιρρεπές σε ψευδείς θετικά.
```bash
...
Αφού βρείτε υποτομείς χρησιμοποιώντας ανοιχτές πηγές και βίαιη δύναμη, μπορείτε να δημιουργήσετε τροποποιήσεις των υποτομέων που βρέθηκαν για να προσπαθήσετε να βρείτε ακόμη περισσότερους. Πολλά εργαλεία είναι χρήσιμα για αυτόν τον σκοπό:
...
* Μπορείτε να λάβετε τον κατάλογο με τις μεταθέσεις του altdns **wordlist** [**εδώ**](https://github.com/infosec-au/altdns/blob/master/words.txt).
```
altdns -i subdomains.txt -w /tmp/words-permutations.txt -o /tmp/asd3
```
* [**dmut**](https://github.com/bp0lr/dmut): Ένα άλλο εργαλείο για να πραγματοποιεί περιστροφές, μεταλλάξεις και αλλαγές στα subdomains. Αυτό το εργαλείο θα εκτελέσει βίαιη επίθεση στο αποτέλεσμα (δεν υποστηρίζει dns wild card).
* Μπορείτε να λάβετε τη λίστα λέξεων με τις περιστροφές του dmut [**εδώ**](https://raw.githubusercontent.com/bp0lr/dmut/main/words.txt).
```bash
...
Αν βρήκατε ένα διεύθυνση IP που περιέχει **μία ή περισσότερες ιστοσελίδες** που ανήκουν σε υποτομείς, μπορείτε να προσπαθήσετε να **βρείτε άλλους υποτομείς με ιστοσελίδες σε αυτή τη διεύθυνση IP** ψάχνοντας σε **πηγές OSINT** για domains σε μια IP ή με το **brute-forcing ονομάτων VHost σε αυτή τη διεύθυνση IP**.
...
Αν υποψιάζεστε ότι κάποιος υποτομέας μπορεί να είναι κρυμμένος σε ένα web server, μπορείτε να προσπαθήσετε να τον ανακαλύψετε με brute force:
```bash
...
Μερικές φορές θα βρείτε σελίδες που επιστρέφουν μόνο τον κεφαλίδα _**Access-Control-Allow-Origin**_ όταν ένα έγκυρο domain/subdomain έχει οριστεί στην κεφαλίδα _**Origin**_. Σε αυτά τα σενάρια, μπορείτε να εκμεταλλευτείτε αυτήν τη συμπεριφορά για να **ανακαλύψετε** νέα **υποτομείς**.
```bash
ffuf -w subdomains-top1million-5000.txt -u http://10.10.10.208 -H 'Origin: http://FUZZ.crossfit.htb' -mr "Access-Control-Allow-Origin" -ignore-body
```
### **Βίαιη Δύναμη Κάδων**

Κατά την αναζήτηση **υποτομέων**, πρέπει να παρατηρείτε αν αυτό **δείχνει** σε οποιοδήποτε είδος **κάδου**, και σε αυτήν την περίπτωση [**ελέγξτε τις άδειες**](../../network-services-pentesting/pentesting-web/buckets/)**.**\
Επίσης, αφού σε αυτό το σημείο θα γνωρίζετε όλα τα υποτομέα μέσα στο εύρος, δοκιμάστε [**βίαιη δύναμη πιθανών ονομάτων κάδων και ελέγξτε τις άδειες**](../../network-services-pentesting/pentesting-web/buckets/).

### **Παρακολούθηση**

Μπορείτε να **παρακολουθείτε** αν **νέοι υποτομείς** ενός τομέα δημιουργούνται με την παρακολούθηση των **Καταγραφών Διαφάνειας Πιστοποιητικού** [**sublert** ](https://github.com/yassineaboukir/sublert/blob/master/sublert.py)κάνει.

### **Αναζήτηση ευπαθειών**

Ελέγξτε για πιθανές [**καταλήψεις υποτομέων**](../../pentesting-web/domain-subdomain-takeover.md#subdomain-takeover).\
Αν ο **υποτομέας** δείχνει σε κάποιο **κάδο S3**, [**ελέγξτε τις άδειες**](../../network-services-pentesting/pentesting-web/buckets/).

Αν βρείτε κάποιο **υποτομέα με διαφορετική IP** από αυτές που έχετε ήδη βρει στην ανακάλυψη περιουσιακών στοιχείων, πρέπει να εκτελέσετε μια **βασική ανίχνευση ευπαθειών** (χρησιμοποιώντας το Nessus ή το OpenVAS) και μια [**σάρωση θυρών**](../pentesting-network/#discovering-hosts-from-the-outside) με **nmap/masscan/shodan**. Ανάλογα με τις υπηρεσίες που εκτελούνται, μπορείτε να βρείτε σε **αυτό το βιβλίο μερικά κόλπα για "επίθεση" σε αυτές**.\
_Σημειώστε ότι μερικές φορές ο υποτομέας φιλοξενείται μέσα σε μια IP που δεν ελέγχεται από τον πελάτη, οπότε δεν είναι στο εύρος, προσέξτε._

## IPs

Στα αρχικά βήματα μπορεί να έχετε **βρει μερικούς εύρους IP, τομείς και υποτομείς**.\
Ήρθε η ώρα να **συγκεντρώσετε όλες τις IP από αυτούς τους εύρους** και για τους **τομείς/υποτομείς (ερωτήσεις DNS).**

Χρησιμοποιώντας υπηρεσίες από τα ακόλουθα **δωρεάν APIs** μπορείτε επίσης να βρείτε **προηγούμενες IP που χρησιμοποιήθηκαν από τομείς και υποτομείς**. Αυτές οι IP ενδέχεται ακόμα να ανήκουν στον πελάτη (και ενδέχεται να σας επιτρέψουν να βρείτε [**παρακάμψεις CloudFlare**](../../network-services-pentesting/pentesting-web/uncovering-cloudflare.md))

* [**https://securitytrails.com/**](https://securitytrails.com/)

Μπορείτε επίσης να ελέγξετε τους τομείς που δείχνουν σε μια συγκεκριμένη διεύθυνση IP χρησιμοποιώντας το εργαλείο [**hakip2host**](https://github.com/hakluke/hakip2host)

### **Αναζήτηση ευπαθειών**

**Σαρώστε όλες τις IP που δεν ανήκουν σε CDN** (καθώς πιθανότατα δεν θα βρείτε κάτι ενδιαφέρον εκεί). Στις υπηρεσίες που ανακαλύφθηκαν μπορείτε **να βρείτε ευπαθειες**.

**Βρείτε ένα** [**οδηγό**](../pentesting-network/) **σχετικά με το πώς να σαρώσετε τους οικοδεσπότες.**

## Κυνήγι διακομιστών ιστού

> Έχουμε βρει όλες τις εταιρείες και τα περιουσιακά τους στοιχεία και γνωρίζουμε τους εύρους IP, τομείς και υποτομείς μέσα στο εύρος. Ήρθε η ώρα να αναζητήσουμε διακομιστές ιστού.

Στα προηγούμενα βήματα πιθανόν να έχετε ήδη εκτελέσει κάποια **αναγνώριση των IP και των τομέων που ανακαλύφθηκαν**, οπότε μπορεί να έχετε **ήδη βρει όλους τους πιθανούς διακομιστές ιστού**. Ωστόσο, αν δεν το έχετε κάνει, τώρα θα δούμε μερικά **γρήγορα κόλπα για την αναζήτηση διακομιστών ιστού** μέσα στο εύρος.

Παρακαλώ, σημειώστε ότι αυτό θα είναι **προσανατολισμένο για την ανακάλυψη εφαρμογών ιστού**, οπότε θα πρέπει να **εκτελέσετε την ευπαθεια** και τη **σάρωση θυρών** επίσης (**εάν επιτρέπεται** από το εύρος).

Ένα **γρήγορο μέθοδο** για την ανακάλυψη **ανοιχτών θυρών** που σχετίζονται με **διακομιστές web** χρησιμοποιώντας το [**masscan μπορεί να βρεθεί εδώ**](../pentesting-network/#http-port-discovery).\
...
Επιπλέον, μπορείτε να χρησιμοποιήσετε το [**eyeballer**](https://github.com/BishopFox/eyeballer) για να εκτελέσετε όλα τα **στιγμιότυπα οθόνης** και να σας πει τι είναι πιθανό να περιέχει ευπάθειες και τι όχι.
...
Αν βρείτε πράγματα όπως **ανοιχτά buckets ή εκθέτες λειτουργίες cloud** θα πρέπει **να τα επισκεφθείτε** και να δείτε τι σας προσφέρουν και αν μπορείτε να τα καταχραστείτε.
...
Το **Leakos** μπορεί επίσης να χρησιμοποιηθεί για να εκτελέσετε το **gitleaks** σε όλο το **κείμενο** που παρέχουν οι **URLs που περνάτε** σε αυτό, καθώς μερικές φορές **οι ιστοσελίδες περιέχουν επίσης μυστικά**.
...
### Διαρροές σε Ιστοσελίδες Επικόλλησης
...
## [**Μεθοδολογία Δοκιμών Διείσδυσης Ιστού**](../../network-services-pentesting/pentesting-web/)

Η **πλειοψηφία των ευπαθειών** που εντοπίζουν οι κυνηγοί ευρετήριων βρίσκεται μέσα σε **εφαρμογές ιστού**, οπότε σε αυτό το σημείο θα ήθελα να μιλήσω για μια **μεθοδολογία δοκιμών εφαρμογών ιστού**, και μπορείτε [**να βρείτε αυτές τις πληροφορίες εδώ**](../../network-services-pentesting/pentesting-web/).

Θέλω επίσης να κάνω μια ειδική αναφορά στην ενότητα [**Εργαλεία Αυτόματων Σαρωτών Ιστού με Ανοιχτό Κώδικα**](../../network-services-pentesting/pentesting-web/#automatic-scanners), καθώς, αν και δεν πρέπει να αναμένετε να βρουν πολύ ευαίσθητες ευπαθείες, είναι χρήσιμα για να εφαρμόσετε τα **workflows για να έχετε κάποιες αρχικές πληροφορίες ιστού.**
...
Αν ενδιαφέρεστε για μια **καριέρα στο χάκινγκ** και να χακάρετε το αχάκαρτο - **σας προσλαμβάνουμε!** (_απαιτείται άριστη γραπτή και προφορική γνώση της πολωνικής γλώσσας_).
