...
{% hint style="info" %}
Questo pacchetto si chiama `Reverse`. Tuttavia, è stato appositamente creato in modo che quando esci dalla reverse shell il resto dell'installazione fallirà, in modo che **non lasci alcun pacchetto Python extra installato sul server** quando te ne vai.
{% endhint %}
...
[**Di seguito c'è una funzione più grande**](./#recursive-search-of-builtins-globals) per trovare decine/**centinaia** di **luoghi** dove puoi trovare i **builtins**.
...
Per esempio, sapendo che con la libreria **`sys`** è possibile **importare librerie arbitrarie**, è possibile cercare tutti i **moduli caricati che hanno importato sys al loro interno**:
...
{% content-ref url="https://github.com/carlospolop/hacktricks/blob/it/generic-methodologies-and-resources/python/bypass-python-sandboxes/broken-reference/README.md" %}
[https://github.com/carlospolop/hacktricks/blob/it/generic-methodologies-and-resources/python/bypass-python-sandboxes/broken-reference/README.md](https://github.com/carlospolop/hacktricks/blob/it/generic-methodologies-and-resources/python/bypass-python-sandboxes/broken-reference/README.md)
{% endcontent-ref %}
...
In alcuni CTF potrebbe essere fornito il nome di una **funzione personalizzata dove si trova la flag** e potresti dover esaminare gli **interni** della **funzione** per estrarla.
...
Come nell'esempio seguente, **puoi accedere all'oggetto codice** di quella funzione, ma leggendo il disassemblaggio **non sai come calcolare il flag** (_immagina una funzione `calc_flag` più complessa_).
