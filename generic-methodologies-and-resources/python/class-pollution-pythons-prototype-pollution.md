...
<summary>Créer une valeur par défaut de propriété de classe pour exécuter du code à distance (subprocess)</summary>
```python
...
<summary>Exécution de sous-processus arbitraire</summary>
```python
...
**`__kwdefaults__`** est un attribut spécial de toutes les fonctions, selon la [documentation](https://docs.python.org/3/library/inspect.html) de Python, c'est une « cartographie de toutes les valeurs par défaut pour les paramètres **uniquement par mot-clé** ». Polluer cet attribut nous permet de contrôler les valeurs par défaut des paramètres uniquement par mot-clé d'une fonction, ce sont les paramètres de la fonction qui viennent après \* ou \*args.
```python
...
```
Dans ce scénario, vous avez besoin d'un gadget pour parcourir des fichiers afin d'atteindre le fichier principal pour **accéder à l'objet global `app.secret_key`** afin de modifier la clé secrète Flask et pouvoir [**escalader les privilèges** en connaissant cette clé](../../network-services-pentesting/pentesting-web/flask.md#flask-unsign).
...
Utilisez ce payload pour **changer `app.secret_key`** (le nom dans votre application pourrait être différent) afin de pouvoir signer de nouveaux cookies flask avec plus de privilèges.
