...
Observe que o shell definido na variável `SHELL` **deve** estar **listado dentro de** _**/etc/shells**_ ou `O valor da variável SHELL não foi encontrado no arquivo /etc/shells. Este incidente foi relatado`. Além disso, observe que os trechos a seguir só funcionam no bash. Se estiver no zsh, mude para o bash antes de obter o shell executando `bash`.
...
* (2b) Alvo Windows 10 (para versões anteriores, verifique [readme do projeto](https://github.com/Fahrj/reverse-ssh#features)):
