...
Ako naiđete na **RCE ranjivost** unutar web aplikacije zasnovane na Linuxu, može se desiti da postane teško **dobiti reverzni shell** zbog prisustva Iptables pravila ili drugih filtera. U takvim scenarijima, razmotrite kreiranje PTY shella unutar kompromitovanog sistema korišćenjem cijevi.
...
Zatim, možete jednostavno **slati komande** ili čak **koristiti `upgrade` komandu** da biste dobili potpuni PTY (imajte na umu da se cijevi čitaju i pišu sa oko 1.3s kašnjenja).
...
Pronađite ranjivosti koje su najvažnije kako biste ih mogli brže popraviti. Intruder prati vašu površinu napada, pokreće proaktivne skenove pretnji, pronalazi probleme širom vašeg celokupnog tehnološkog skupa, od API-ja do veb aplikacija i sistemima u oblaku. [**Isprobajte besplatno**](https://www.intruder.io/?utm\_source=referral\&utm\_campaign=hacktricks) danas.
...
Awk je moćan alat za obradu teksta koji se često koristi u shell skriptama. Može se koristiti za pretraživanje i obradu teksta, kao i za izvlačenje i prikazivanje određenih informacija iz datoteka. Awk koristi posebnu sintaksu za definisanje akcija koje treba izvršiti nad tekstom, što ga čini veoma fleksibilnim alatom za analizu podataka.
```bash
