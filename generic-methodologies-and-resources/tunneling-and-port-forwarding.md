...
स्थानीय पोर्ट --> कंप्रोमाइज़्ड होस्ट (SSH) --> तीसरा\_बॉक्स:पोर्ट
```bash
...
यह आपके होस्ट तक एक DMZ के माध्यम से आंतरिक होस्ट से रिवर्स शैल्स प्राप्त करने के लिए उपयोगी है:
```bash
...
```
ग्राहक साइड पर एक नया मार्ग निर्धारित करें
```
...
उदाहरण के लिए, 10.10.10.0/24 पर जा रहे सभी ट्रैफिक को फॉरवर्ड करना।
```bash
...
SOCKS (Socket Secure) प्रोटोकॉल एक नेटवर्क प्रोटोकॉल है जो नेटवर्क ट्राफिक को एनक्रिप्ट करने और उसे एक अन्य नेटवर्क के माध्यम से पारित करने की अनुमति देता है।
```bash
...
टीम सर्वर में एक पोर्ट खोलें जो सभी इंटरफेस में सुन रहा हो और जिसका उपयोग **बीकन के माध्यम से ट्रैफिक रूट करने के लिए** किया जा सकता है।
```bash
beacon> socks 1080
...
```
### साक्स के माध्यम से पोर्ट से पोर्ट टनलिंग
```bash
...
लोकल SSH पोर्ट (22) को हमले के होस्ट के 443 पोर्ट से कनेक्ट करें
```bash
...
1. [SocksOverRDP x64 Binaries](https://github.com/nccgroup/SocksOverRDP/releases) - यह टूल `Windows` की `Remote Desktop Service` फीचर से `Dynamic Virtual Channels` (`DVC`) का उपयोग करता है। `DVC` **RDP कनेक्शन के माध्यम से पैकेट्स को टनल करने** के लिए जिम्मेदार है।
...
```
अब हम **RDP** का उपयोग करके **`mstsc.exe`** के माध्यम से **विक्टिम** से **कनेक्ट** कर सकते हैं, और हमें एक **प्रॉम्प्ट** मिलना चाहिए जिसमें कहा जाएगा कि **SocksOverRDP प्लगइन सक्षम है**, और यह **127.0.0.1:1080** पर **सुनेगा**।
...
## Windows GUI Apps को प्रॉक्सीफ़ाई करें
...
**Profile -> Proxification Rules** में प्रोग्राम का नाम जो प्रॉक्सीफ़ाई करना है और आप जिन IPs को प्रॉक्सीफ़ाई करना चाहते हैं, उनके साथ कनेक्शन जोड़ें।
...
```
अब, यदि आप उदाहरण के रूप में पीड़ित में **SSH** सेवा को पोर्ट 443 में सुनने के लिए सेट करते हैं। आप इसे आक्रमक पोर्ट 2222 के माध्यम से कनेक्ट कर सकते हैं।\
आप एक **meterpreter** भी उपयोग कर सकते हैं जो localhost:443 से कनेक्ट करता है और हमलावर पोर्ट 2222 में सुन रहा है।

## YARP

माइक्रोसॉफ्ट द्वारा बनाया गया रिवर्स प्रॉक्सी। आप इसे यहाँ पा सकते हैं: [https://github.com/microsoft/reverse-proxy](https://github.com/microsoft/reverse-proxy)
...
```
यह टनल बहुत धीमा होगा। आप इस टनल के माध्यम से एक संकुचित SSH कनेक्शन बना सकते हैं:
```
...
DNS के माध्यम से एक C\&C चैनल स्थापित करता है। इसके लिए रूट विशेषाधिकार की आवश्यकता नहीं है।
```bash
...
आप [**dnscat2-powershell**](https://github.com/lukebaggett/dnscat2-powershell) का उपयोग कर सकते हैं ताकि पावरशेल में dnscat2 क्लाइंट चला सकें:
```
