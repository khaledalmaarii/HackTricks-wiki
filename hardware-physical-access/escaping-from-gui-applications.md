...
[**WhiteIntel**](https://whiteintel.io)은 **다크 웹**을 기반으로 한 검색 엔진으로, **무료** 기능을 제공하여 회사나 고객이 **스틸러 악성 소프트웨어**에 의해 **침해**되었는지 확인할 수 있습니다.
...
* **UNC 경로**: 공유 폴더에 연결하기 위한 경로. 로컬 머신의 C$에 연결을 시도해야 합니다 ("\\\127.0.0.1\c$\Windows\System32")
* **더 많은 UNC 경로:**
...
클릭하면 이미지 상단 왼쪽에 나타나는 툴바입니다. 탐색기에서 저장, 인쇄, 메일 보내기, "내 사진" 열기 등이 가능합니다. Kiosk는 인터넷 익스플로러를 사용해야 합니다.
...
* 네 손가락(또는 다섯 손가락)으로 위로 스와이프 / 홈 버튼 더블 탭: 멀티태스크 보기 및 앱 변경
* 네 손가락 또는 다섯 손가락으로 한쪽으로 스와이프: 다음/이전 앱으로 변경
* 다섯 손가락으로 화면을 집는다 / 홈 버튼을 터치한다 / 화면 하단에서 위쪽으로 한 손가락으로 빠르게 스와이프하여 위로 이동한다: 홈에 액세스
* 화면 하단에서 한 손가락으로 1-2인치만 아래로 스와이프(느리게): 독이 나타납니다.
* 화면 상단에서 한 손가락으로 아래로 스와이프: 알림 보기
* 화면의 오른쪽 상단에서 한 손가락으로 아래로 스와이프: iPad Pro의 제어 센터 보기
* 화면의 왼쪽에서 한 손가락으로 1-2인치만 빠르게 스와이프: 오늘 보기 보기
* 화면 중앙에서 빠르게 오른쪽이나 왼쪽으로 한 손가락으로 스와이프: 다음/이전 앱으로 변경
* iPad 오른쪽 상단의 전원/잠금/취침 버튼을 누르고 오른쪽으로 슬라이드하여 전원을 끕니다: 전원 끄기
* iPad 오른쪽 상단의 전원/잠금/취침 버튼을 누르고 홈 버튼을 몇 초간 누릅니다: 강제 종료
* iPad 오른쪽 상단의 전원/잠금/취침 버튼을 누르고 홈 버튼을 빠르게 누릅니다: 화면 하단 왼쪽에 팝업되는 스크린샷을 찍습니다. 두 버튼을 동시에 매우 짧게 누르면 몇 초 동안 누르는 것처럼 강제 종료가 수행됩니다.
...
| ⌘⇧4                                                | 스크린샷 찍고 편집기에서 열기                        |
| ⌘를 길게 누르기                                   | 앱에 대한 사용 가능한 바로 가기 목록 표시             |
