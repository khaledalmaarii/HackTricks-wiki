...
* [**Prikriveni upadi sa DDexec-ng & in-memory dlopen() - HackTricks Traka 2023**](https://www.youtube.com/watch?v=VM\_gjjiARaU)
...
Imajte na umu da sam spomenuo binarne fajlove, možete **izvršiti bilo koju skriptu** sve dok je interpretator unutar mašine, poput **shell skripte** ako je `sh` prisutan ili **python skripte** ako je `python` instaliran.

Međutim, to nije dovoljno da biste izvršili vaš binarni backdoor ili druge binarne alate koje možda treba pokrenuti.
...
Distroless kontejneri sadrže samo **apsolutno neophodne komponente potrebne za pokretanje određene aplikacije ili servisa**, poput biblioteka i zavisnosti za izvršavanje, ali isključuju veće komponente poput upravljača paketima, ljuske ili sistemskih alatki.
...
Ako ne postoje **zaštite `read-only/no-exec`** možete zloupotrebiti svoj reverse shell da **pišete u fajl sistem vaše binarne fajlove** i da ih **izvršite**.
