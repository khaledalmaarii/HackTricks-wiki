...
Inazidi kuwa kawaida kupata mashine za linux zilizomwekwa na **ulinzi wa mfumo wa faili wa soma-tu (ro)**, hasa kwenye kontena. Hii ni kwa sababu ya kuendesha kontena na mfumo wa faili wa soma-tu ni rahisi kama kuweka **`readOnlyRootFilesystem: true`** kwenye `securitycontext`:
...
Hata hivyo, hata kama mfumo wa faili umewekwa kama ro, **`/dev/shm`** bado itakuwa inaweza kuandikwa, hivyo ni uongo hatuwezi kuandika chochote kwenye diski. Hata hivyo, folda hii itakuwa **imefungwa na ulinzi wa hakuna-kutekeleza**, hivyo ikiwa unapakua binary hapa **hutaweza kuitekeleza**.
...
Tafadhali elewa kuwa nilitaja binaries, unaweza **kutekeleza skripti yoyote** ikiwa mkalimani yupo ndani ya mashine, kama **skripti ya shell** ikiwa `sh` iko au **skripti ya python** ikiwa `python` imefungwa.
...
Ikiwa una injini za skripti zenye nguvu ndani ya mashine, kama **Python**, **Perl**, au **Ruby** unaweza kupakua binary kutekeleza kutoka kumbukumbu, kuhifadhi kwenye maelezo ya faili ya kumbukumbu (`create_memfd` syscall), ambayo haitalindwa na ulinzi huo kisha itekeleze **kizuizi cha syscall** ikionyesha **fd kama faili ya kutekeleza**.

Kwa hili unaweza kutumia mradi [**fileless-elf-exec**](https://github.com/nnsee/fileless-elf-exec). Unaweza kumpitisha binary na itazalisha skripti kwa lugha iliyotajwa na **binary iliyosongwa na kubadilishwa kuwa msingi64** pamoja na maagizo ya **kudecode na kufuta** kwenye **fd** iliyoundwa kwa kuita `create_memfd` syscall na wito wa **kizuizi cha syscall** kuitekeleza.
...
Kwa lengo kama la DDexec, mbinu ya [**memdlopen**](https://github.com/arget13/memdlopen) inaruhusu njia **rahisi ya kupakia binaries** kwenye kumbukumbu kwa kuziendesha baadaye. Inaweza kuruhusu hata kupakia binaries na mahitaji.
...
Lengo la vyombo vya distroless ni **kupunguza eneo la shambulio la vyombo kwa kufuta vipengele visivyo vya lazima** na kupunguza idadi ya mapungufu yanayoweza kutumiwa.
...
{% hint style="success" %}
Hata hivyo, katika vyombo kama hivyo ulinzi huu kawaida utakuwepo, lakini unaweza kutumia **mbinu za awali za utekelezaji wa kumbukumbu kuzipuuza**.
{% endhint %}
