...
Informazioni interessanti, password o chiavi API nelle variabili di ambiente?
```bash
(env || set) 2>/dev/null
```
### Exploit del kernel
...
Grsecurity è una collezione di patch per il kernel Linux che fornisce funzionalità avanzate di sicurezza, inclusa la protezione da attacchi di escalation dei privilegi.
```bash
...
Execshield è una tecnica di protezione implementata nel kernel Linux per prevenire attacchi di esecuzione di codice arbitrario. Questa tecnica aiuta a proteggere il sistema da vulnerabilità come buffer overflow e stack smashing.
```bash
(grep "exec-shield" /etc/sysctl.conf || echo "Not found Execshield")
```
### SElinux

SELinux (Security-Enhanced Linux) è un meccanismo di controllo degli accessi obbligatorio implementato nel kernel Linux. Viene utilizzato per proteggere il sistema da attacchi di escalation dei privilegi limitando le azioni che gli utenti e i processi possono compiere.
```bash
(sestatus 2>/dev/null || echo "Not found sestatus")
```
### ASLR

Address Space Layout Randomization (ASLR) è una tecnica di protezione che mira a rendere più difficile per un attaccante prevedere la posizione della memoria all'interno di un processo. Questo rende più complesso sfruttare vulnerabilità di tipo buffer overflow per eseguire codice dannoso.
```bash
...
```
Inoltre, controlla se **è installato un compilatore**. Questo è utile se devi utilizzare qualche exploit del kernel poiché è consigliabile compilarlo nella macchina in cui lo utilizzerai (o in una simile)
```bash
...
{% hint style="info" %}
_Nota che questi comandi mostreranno molte informazioni che saranno per lo più inutili, pertanto è consigliabile utilizzare applicazioni come OpenVAS o simili che verificheranno se una qualsiasi versione del software installato è vulnerabile a exploit conosciuti_
{% endhint %}
...
top -n 1
```
Sempre controlla la presenza di [**debugger electron/cef/chromium** in esecuzione, potresti sfruttarlo per l'escalation dei privilegi](electron-cef-chromium-debugger-abuse.md). **Linpeas** li rileva controllando il parametro `--inspect` nella riga di comando del processo.\
...
* **kernel.yama.ptrace\_scope = 0**: tutti i processi possono essere debuggati, purché abbiano lo stesso uid. Questo è il modo classico di come funzionava il ptracing.
...
Per un dato ID processo, **maps mostra come la memoria è mappata all'interno dello spazio degli indirizzi virtuali di quel processo**; mostra anche le **autorizzazioni di ciascuna regione mappata**. Il file pseudo **mem espone la memoria dei processi stessi**. Dal file **maps sappiamo quali regioni di memoria sono leggibili** e i loro offset. Utilizziamo queste informazioni per **cercare nel file mem e scaricare tutte le regioni leggibili** in un file.
```bash
procdump()
(
...
* [**https://github.com/hajzer/bash-memory-dump**](https://github.com/hajzer/bash-memory-dump) (root) - \_È possibile rimuovere manualmente i requisiti di root e scaricare il processo di tua proprietà
...
Ad esempio, all'interno di _/etc/crontab_ è possibile trovare il PATH: _PATH=**/home/user**:/usr/local/sbin:/usr/local/bin:/sbin:/bin:/usr/sbin:/usr/bin_

(_Nota come l'utente "user" ha privilegi di scrittura su /home/user_)

Se all'interno di questa crontab l'utente root cerca di eseguire qualche comando o script senza impostare il percorso. Per esempio: _\* \* \* \* root overwrite.sh_\
Allora, è possibile ottenere una shell di root utilizzando:
```bash
...
Se uno script viene eseguito da root e contiene un "**\***" all'interno di un comando, potresti sfruttarlo per fare cose inaspettate (come l'elevazione dei privilegi). Esempio:
```bash
...
Puoi monitorare i processi per cercare processi che vengono eseguiti ogni 1, 2 o 5 minuti. Forse puoi approfittarne e ottenere privilegi elevati.
...
È possibile creare un lavoro cron **inserendo un ritorno a capo dopo un commento** (senza il carattere di nuova riga), e il lavoro cron funzionerà. Esempio (nota il carattere di ritorno a capo):
```bash
...
Verifica se puoi scrivere un file `.service`, se puoi, potresti modificarlo in modo che **esegua** il tuo **backdoor quando** il servizio viene **avviato**, **riavviato** o **arrestato** (potrebbe essere necessario attendere il riavvio della macchina).\
Ad esempio, crea il tuo backdoor all'interno del file .service con **`ExecStart=/tmp/script.sh`**

### Binari di servizio scrivibili

Tieni presente che se hai **permessi di scrittura sui binari eseguiti dai servizi**, puoi cambiarli in backdoor in modo che quando i servizi vengono ri-eseguiti, le backdoor verranno eseguite.
...
Se puoi modificare un timer, puoi farlo eseguire a delle entità esistenti di systemd.unit (come un `.service` o un `.target`)
```bash
...
* Trovare qualche unità systemd (come un `.service`) che stia **eseguendo un binario scrivibile**
...
* `ExecStartPre`, `ExecStartPost`: Prende una o più righe di comando, che vengono **eseguite prima** o **dopo** che i **socket**/FIFO in ascolto sono **creati** e vincolati, rispettivamente. Il primo token della riga di comando deve essere un nome di file assoluto, seguito dagli argomenti per il processo.
...
Controlla se puoi fare sniffing al traffico. Se riesci, potresti essere in grado di acquisire delle credenziali.
```
timeout 1 tcpdump
```
## Utenti
...
Se non ti dispiace fare molto rumore e i binari `su` e `timeout` sono presenti sul computer, puoi provare a forzare l'utente utilizzando [su-bruteforce](https://github.com/carlospolop/su-bruteforce).\
...
```
Nell'esempio seguente, l'utente `demo` può eseguire `vim` come `root`, ora è banale ottenere una shell aggiungendo una chiave ssh nella directory di root o chiamando `sh`.
```
sudo vim -c '!sh'
```
### SETENV
...
```
### Bypass dell'esecuzione di Sudo tramite percorsi
...
sudo less
```
Questa tecnica può essere utilizzata anche se un binario **suid** **esegue un altro comando senza specificarne il percorso (controllare sempre con** _**strings**_ **il contenuto di un binario SUID strano)**.
...
```
Infine, eseguire il binario SUID interessato dovrebbe attivare l'exploit, consentendo un potenziale compromissione del sistema.

## Hijacking dell'oggetto condiviso
```bash
...
Se puoi accedere a `sudo -l`, puoi utilizzare lo strumento [**FallOfSudo**](https://github.com/CyberOne-Security/FallofSudo) per verificare se trova come sfruttare una regola sudo.
...
**Se** riesci a **leggere** questo file potresti essere in grado di **ottenere informazioni interessanti**, e se riesci a **scrivere** su qualsiasi file sarai in grado di **elevare i privilegi**.
```bash
...
Se per qualche motivo **un utente ha permessi di scrittura** su uno dei percorsi indicati: `/etc/ld.so.conf`, `/etc/ld.so.conf.d/`, su qualsiasi file all'interno di `/etc/ld.so.conf.d/` o su qualsiasi cartella all'interno del file di configurazione dentro `/etc/ld.so.conf.d/*.conf`, potrebbe essere in grado di ottenere privilegi elevati.\
...
Nelle **versioni vecchie** potresti **dirottare** una sessione **shell** di un utente diverso (**root**).\
...
```
Quella configurazione indicherà che se provi a effettuare il login con la **chiave privata** dell'utente "**testusername**", ssh comparirà la chiave pubblica della tua chiave con quelle situate in `/home/testusername/.ssh/authorized_keys` e `/home/testusername/access`
...
Il file `/etc/sshd_config` può **consentire** o **negare** il forwarding dell'agente ssh con la parola chiave `AllowAgentForwarding` (per impostazione predefinita è consentito).

Se scopri che il Forward Agent è configurato in un ambiente, leggi la seguente pagina poiché **potresti essere in grado di sfruttarlo per ottenere privilegi elevati**:
...
Il file `/etc/profile` e i file in `/etc/profile.d/` sono **script che vengono eseguiti quando un utente avvia una nuova shell**. Pertanto, se riesci a **scrivere o modificare uno qualsiasi di essi, puoi ottenere privilegi elevati**.
```bash
...
A seconda del sistema operativo, i file `/etc/passwd` e `/etc/shadow` potrebbero avere nomi diversi o potrebbe esserci un backup. Pertanto, è consigliato **trovarli tutti** e **verificare se è possibile leggerli** per vedere **se contengono hash** all'interno dei file:
```bash
...
su - dummy
```
**NOTA:** Sulle piattaforme BSD, `/etc/passwd` si trova in `/etc/pwd.db` e `/etc/master.passwd`, mentre `/etc/shadow` è rinominato in `/etc/spwd.db`.
...
```
### Posizione/Files Posseduti Strani
```bash
...
Se riesci a leggere i log, potresti trovare **informazioni interessanti/confidenziali al loro interno**. Più strano è il log, più interessante sarà (probabilmente).\
Inoltre, alcuni **log di audit "cattivi"** configurati (backdoor?) potrebbero permetterti di **registrare password** all'interno dei log di audit come spiegato in questo post: [https://www.redsiege.com/blog/2019/05/logging-passwords-on-linux/](https://www.redsiege.com/blog/2019/05/logging-passwords-on-linux/).
```bash
...
Nel mio caso, l'attributo `NAME=` in questi script di rete non è gestito correttamente. Se hai **spazi bianchi nel nome, il sistema cerca di eseguire la parte dopo lo spazio bianco**. Ciò significa che **tutto ciò che segue il primo spazio bianco viene eseguito come root**.
...
La directory `/etc/init.d` ospita **script** per System V init (SysVinit), il **sistema classico di gestione dei servizi Linux**. Include script per `avviare`, `fermare`, `riavviare` e talvolta `ricaricare` servizi. Questi possono essere eseguiti direttamente o tramite collegamenti simbolici trovati in `/etc/rc?.d/`. Un percorso alternativo nei sistemi Redhat è `/etc/rc.d/init.d`.
