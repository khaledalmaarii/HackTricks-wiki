...
Το Docker engine μπορεί να προσπελαστεί είτε τοπικά μέσω ενός Unix socket είτε απομακρυσμένα χρησιμοποιώντας HTTP. Για την απομακρυσμένη πρόσβαση, είναι ουσιώδες να χρησιμοποιηθεί το HTTPS και το **TLS** για να διασφαλιστεί η εμπιστευτικότητα, η ακεραιότητα και η ταυτοποίηση.
...
```
Ωστόσο, δεν συνιστάται η εκθεση του Docker daemon μέσω HTTP λόγω ανησυχιών ασφαλείας. Συνιστάται να ασφαλίζονται οι συνδέσεις χρησιμοποιώντας HTTPS. Υπάρχουν δύο κύριες προσεγγίσεις για την ασφάλεια της σύνδεσης:
...
Οι εικόνες εμφυτευμάτων μπορούν να αποθηκευτούν είτε σε ιδιωτικούς είτε σε δημόσιους αποθετήριους. Το Docker προσφέρει αρκετές επιλογές αποθήκευσης για τις εικόνες εμφυτευμάτων:
...
* [**Docker Trusted Registry**](https://www.docker.com/docker-trusted-registry): Η εμπορική προσφορά του Docker, που περιλαμβάνει αυθεντικοποίηση χρηστών με βάση τον ρόλο και ολοκλήρωση με υπηρεσίες καταλόγου LDAP.

### Σάρωση Εικόνων

Οι εμφυτευμένες εικόνες μπορεί να έχουν **ευπάθειες ασφάλειας** είτε λόγω της βασικής εικόνας είτε λόγω του λογισμικού που είναι εγκατεστημένο πάνω στη βασική εικόνα. Το Docker εργάζεται σε ένα έργο που ονομάζεται **Nautilus** που κάνει σάρωση ασφάλειας των Εμφυτευμάτων και καταγράφει τις ευπάθειες. Το Nautilus λειτουργεί συγκρίνοντας κάθε επίπεδο εικόνας Εμφυτεύματος με το αποθετήριο ευπάθειών για την αναγνώριση των κενών ασφαλείας.

Για περισσότερες [**πληροφορίες διαβάστε αυτό**](https://docs.docker.com/engine/scan/).

* **`docker scan`**

Η εντολή **`docker scan`** σάς επιτρέπει να σαρώσετε υπάρχουσες εικόνες Docker χρησιμοποιώντας το όνομα ή το ID της εικόνας. Για παράδειγμα, εκτελέστε την ακόλουθη εντολή για να σαρώσετε την εικόνα hello-world:
```bash
...
```
Κατά τη μετάβαση σε νέους hosts του Docker, είναι απαραίτητο να μεταφερθούν τα root και τα κλειδιά αποθετηρίου για τη διατήρηση των λειτουργιών.
...
Σε περιβάλλοντα με εφαρμογές σε εμπορεύματα, η απομόνωση των έργων και των διεργασιών τους είναι ζωτικής σημασίας για την ασφάλεια και τη διαχείριση πόρων. Εδώ υπάρχει μια απλοποιημένη εξήγηση των βασικών έννοιων:
...
Αυτές είναι οι **υπόλοιπες δυνατότητες** μετά την απόρριψη των άλλων δυνατοτήτων από τη διεργασία:
...
Οι δυνατότητες επιτρέπουν **πιο λεπτό έλεγχο για τις δυνατότητες που μπορούν να επιτραπούν** για το ριζικό χρήστη. Το Docker χρησιμοποιεί το χαρακτηριστικό δυνατοτήτων του πυρήνα Linux για **περιορισμό των λειτουργιών που μπορούν να εκτελεστούν μέσα σε ένα Εμπορευματοκιβώτιο** ανεξαρτήτως του τύπου του χρήστη.

Όταν εκτελείται ένα δοχείο Docker, η **διαδικασία απορρίπτει ευαίσθητες δυνατότητες που θα μπορούσε να χρησιμοποιήσει η διαδικασία για να δραπετεύσει από την απομόνωση**. Αυτό προσπαθεί να διασφαλίσει ότι η διαδικασία δεν θα μπορεί να εκτελέσει ευαίσθητες ενέργειες και να δραπετεύσει:
...
Αυτό είναι ένα χαρακτηριστικό ασφαλείας που επιτρέπει στο Docker να **περιορίσει τις κλήσεις συστήματος** που μπορούν να χρησιμοποιηθούν μέσα στο δοχείο:
...
**AppArmor** είναι μια βελτίωση πυρήνα για τον περιορισμό των **δοχείων** σε ένα **περιορισμένο** σύνολο **πόρων** με **προφίλ ανά πρόγραμμα**.:
...
* **Ετικέτες Διεργασίας Δοχείων**: Όταν οι μηχανές δοχείων εκκινούν διεργασίες δοχείων, συνήθως τους ανατίθεται μια περιορισμένη ετικέτα SELinux, συνήθως `container_t`.
* **Ετικέτες Αρχείων εντός Δοχείων**: Τα αρχεία εντός του δοχείου συνήθως επισημαίνονται ως `container_file_t`.
...
Αυτός ο μηχανισμός εξασφαλίζει ότι ακόμη και αν μια διαδικασία μέσα σε ένα δοχείο είναι εκτεθειμένη, περιορίζεται στην αλληλεπίδραση μόνο με αντικείμενα που έχουν τις αντίστοιχες ετικέτες, περιορίζοντας σημαντικά την πιθανή ζημιά από τέτοιες εκθέσεις.
...
Αν δεν περιορίζετε σωστά τους πόρους που μπορεί να χρησιμοποιήσει ένα δοχείο, ένα διαρραγμένο δοχείο μπορεί να προκαλέσει DoS στον υπολογιστή όπου εκτελείται.
...
Εάν εκτελείτε ένα container όπου ένας εισβολέας καταφέρνει να αποκτήσει πρόσβαση ως χρήστης με χαμηλά προνόμια. Εάν έχετε ένα **κακώς διαμορφωμένο suid binary**, ο εισβολέας μπορεί να το καταχραστεί και **να αναβαθμίσει τα προνόμια μέσα** στο container. Αυτό, μπορεί να του επιτρέψει να δραπετεύσει από αυτό.
...
### Διαχείριση Μυστικών: Βέλτιστες Πρακτικές
...
* Ρυθμίστε τα προφίλ [seccomp](https://docs.docker.com/engine/security/seccomp), [AppArmor](https://docs.docker.com/engine/security/apparmor/) (ή SELinux) για να περιορίσετε τις ενέργειες και τις κλήσεις συστήματος που είναι διαθέσιμες για το container στο ελάχιστο απαιτούμενο.
* Χρησιμοποιήστε [επίσημες εικόνες docker](https://docs.docker.com/docker-hub/official_images/) και απαιτήστε υπογραφές ή δημιουργήστε τις δικές σας βασισμένες σε αυτές. Μην κληρονομείτε ή χρησιμοποιείτε εικόνες με παρασκευασμένες πόρτες. Αποθηκεύστε επίσης τα ριζικά κλειδιά, τον κωδικό πρόσβασης σε ασφαλές μέρος. Το Docker έχει σχέδια για τη διαχείριση των κλειδιών με το UCP.
* **Αναδημιουργήστε τακτικά** τις εικόνες σας για να **εφαρμόσετε πάτσιν ασφαλείας στον υπολογιστή και τις εικόνες.**
...
* **Μην τοποθετείτε ssh** μέσα στο container, το "docker exec" μπορεί να χρησιμοποιηθεί για να συνδεθείτε μέσω ssh στο Container.
* Έχετε **μικρότερες** εικόνες **containers**

## Διαρροή / Ανόρθωση Προνομίων Docker
...
* Το εργαλείο [**docker-bench-security**](https://github.com/docker/docker-bench-security) είναι ένα σενάριο που ελέγχει δεκάδες κοινές βέλτιστες πρακτικές γύρω από την ανάπτυξη containers Docker σε παραγωγή. Οι δοκιμές είναι όλες αυτοματοποιημένες και βασίζονται στο [CIS Docker Benchmark v1.3.1](https://www.cisecurity.org/benchmark/docker/).\
...
* [https://sreeninet.wordpress.com/2016/03/06/docker-security-part-2docker-engine/](https://sreeninet.wordpress.com/2016
<summary><strong>Μάθετε το χάκινγκ στο AWS από το μηδέν μέχρι τον ήρωα με το</strong> <a href="https://training.hacktricks.xyz/courses/arte"><strong>htARTE (HackTricks AWS Red Team Expert)</strong></a><strong>!</strong></summary>
