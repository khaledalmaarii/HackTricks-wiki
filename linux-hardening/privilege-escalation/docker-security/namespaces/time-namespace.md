...
L'espace de noms temporel permet des décalages spécifiques à chaque espace de noms pour les horloges monotones du système et celles du temps de démarrage. L'espace de noms temporel est adapté à l'utilisation de conteneurs Linux pour permettre de changer la date/l'heure à l'intérieur d'un conteneur et pour ajuster les horloges à l'intérieur d'un conteneur suite à la restauration à partir d'un point de contrôle/instantané.
...
```
### Trouver tous les espaces de noms Time
...
```
### Entrer dans un espace de noms Time
```bash
nsenter -T TARGET_PID --pid /bin/bash
```
Vous pouvez uniquement **entrer dans un autre espace de noms de processus si vous êtes root**. Et vous ne pouvez **pas entrer** dans un autre espace de noms **sans un descripteur** pointant vers celui-ci (comme `/proc/self/ns/net`).
