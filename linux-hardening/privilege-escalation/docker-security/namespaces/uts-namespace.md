...
2. 在UTS命名空间内，进程可以使用`sethostname()`和`setdomainname()`系统调用**更改主机名和NIS域名**。这些更改仅在该命名空间内有效，不会影响其他命名空间或主机系统。
...
```
通过使用参数`--mount-proc`挂载`/proc`文件系统的新实例，确保新的挂载命名空间具有**准确且隔离的进程信息视图，特定于该命名空间**。
