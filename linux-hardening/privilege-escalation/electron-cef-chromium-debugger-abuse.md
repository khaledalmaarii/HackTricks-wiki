...
```
DaH jImej inspected process lo'laH 'e' vItlhutlh:
```
Debugger ending on ws://127.0.0.1:9229/45ea962a-29dd-4cdd-be08-a6827840553d
For help, see: https://nodejs.org/en/docs/inspector
```
**CEF** (**Chromium Embedded Framework**) based processes require the use of the param: `--remote-debugging-port=9222` to open the **debugger** (the SSRF protections remain very similar). However, instead of granting a **NodeJS** **debug** session, they will communicate with the browser using the [**Chrome DevTools Protocol**](https://chromedevtools.github.io/devtools-protocol/), which is an interface to control the browser, but there is no direct RCE.
...
```
{% hint style="info" %}
QaStaHvIS containers vaj **process qutlh** 'ej **nIvbogh** 'e' vItlhutlhlaHbe'chugh **--inspect** vaj **ghItlh**. **container** vItlhutlhlaHbe'chugh **process** vItlhutlhlaH.
{% endhint %}

### Connect to inspector/debugger

**Chromium-based browser** vaj, `chrome://inspect` 'ej `edge://inspect` URLs vItlhutlhlaH. Configure button vIleghlaH, **target host 'ej port** qar'a'wI' vItlhutlhlaH. Remote Code Execution (RCE) jatlhlaH:

![](<../../.gitbook/assets/image (620) (1).png>)

**command line** vaj, debugger/inspector vItlhutlhlaH:
```bash
...
```
**[**https://github.com/taviso/cefdebug**](https://github.com/taviso/cefdebug)**, **ghItlh** **inspectors** **ghItlh** **inject code** **ghItlh** **local** **run** **find** **allow** **tool**.
```bash
...
```
{% hint style="info" %}
Qapla'! **NodeJS RCE exploits won't work** if connected to a browser via [**Chrome DevTools Protocol**](https://chromedevtools.github.io/devtools-protocol/) (you need to check the API to find interesting things to do with it).
{% endhint %}
...
```
Qapla'! calc.exe yIqej.

### yIvbeH

**yIghojmoH** **yIqawHa'moH** **ghItlh** **source code** **application** **malicious code** **yIghojmoH** **file** **download** **yIlo'** **folder** **ghItlh**.
```javascript
