...
* 您在**网络安全公司**工作吗？ 想要看到您的**公司在HackTricks中做广告**吗？ 或者您想要访问**PEASS的最新版本或下载HackTricks的PDF**吗？ 请查看[**SUBSCRIPTION PLANS**](https://github.com/sponsors/carlospolop)！
...
* **加入**[**💬**](https://emojipedia.org/speech-balloon/) **Discord群**或**电报群**或在**Twitter**上**🐦**[**@carlospolopm**](https://twitter.com/hacktricks_live)**上关注**我。
...
- **`euid`**：被称为**有效用户ID**，代表系统用于确定进程特权的用户身份。通常情况下，`euid`与`ruid`相同，除了像SetUID二进制执行这样的情况，其中`euid`会假定文件所有者的身份，从而授予特定的操作权限。
...
值得注意的是，虽然`setuid`可能是提升到root的特权的常见选择（因为它将所有ID都对齐到root），但在不同情况下理解和操作用户ID行为的区别是至关重要的。
...
- **命令执行**：通过`execl("/bin/sh", "sh", "-c", command, (char *) NULL);`在`sh`上执行命令。
...
这些机制在操作上各不相同，为执行和在程序之间转换提供了多样的选项，特定情况下管理和保留用户ID的方式也有特定的细微差别。
...
**目标**：了解`setuid`与`system`和`bash`作为`sh`结合使用的效果。
...
* **加入** [**💬**](https://emojipedia.org/speech-balloon/) [**Discord群**](https://discord.gg/hRep4RUj7f) 或者 [**电报群**](https://t.me/peass) 或者在**Twitter**上**🐦**关注我[**@carlospolopm**](https://twitter.com/hacktricks_live)**.**
