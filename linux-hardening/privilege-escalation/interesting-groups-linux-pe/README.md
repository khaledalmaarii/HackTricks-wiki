...
Questo perché di solito sono quei gruppi all'interno della **polkit policy**. Questa policy identifica fondamentalmente quali gruppi possono utilizzare `pkexec`. Controllalo con:
```bash
cat /etc/polkit-1/localauthority.conf.d/*
```
Nel file troverai quali gruppi sono autorizzati ad eseguire **pkexec** e **per impostazione predefinita** in alcune distribuzioni Linux compaiono i gruppi **sudo** e **admin**.
...
```
Questo significa che **qualsiasi utente che appartiene al gruppo wheel può eseguire qualsiasi comando come sudo**.
...
**staff**: Consente agli utenti di aggiungere modifiche locali al sistema (`/usr/local`) senza necessità di privilegi di root (nota che gli eseguibili in `/usr/local/bin` sono nel percorso di ricerca di qualsiasi utente e possono "sovrascrivere" gli eseguibili in `/bin` e `/usr/bin` con lo stesso nome). Confronta con il gruppo "adm", che è più legato al monitoraggio/sicurezza. [\[fonte\]](https://wiki.debian.org/SystemGroups)
...
Dirottare il programma `run-parts` è un modo facile per ottenere i permessi di root, poiché la maggior parte dei programmi eseguirà un comando simile a `run-parts` (come crontab, all'accesso via ssh).
```bash
...
Questa autorizzazione è quasi **equivalente all'accesso di root** poiché consente di accedere a tutti i dati all'interno della macchina.
...
Il gruppo **video** ha accesso per visualizzare l'output dello schermo. Fondamentalmente puoi osservare gli schermi. Per farlo è necessario **acquisire l'immagine corrente sullo schermo** in dati grezzi e ottenere la risoluzione che lo schermo sta utilizzando. I dati dello schermo possono essere salvati in `/dev/fb0` e potresti trovare la risoluzione di questo schermo su `/sys/class/graphics/fb0/virtual_size`
```bash
...
Sembra che per impostazione predefinita i **membri del gruppo root** potrebbero avere accesso per **modificare** alcuni **file di configurazione del servizio** o alcuni **file di librerie** o **altre cose interessanti** che potrebbero essere utilizzate per l'escalation dei privilegi...
...
Di solito i **membri** del gruppo **`adm`** hanno le autorizzazioni per **leggere i file di log** situati dentro _/var/log/_.\
