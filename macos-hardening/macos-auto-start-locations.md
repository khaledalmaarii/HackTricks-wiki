...
यह खंड ब्लॉग श्रृंखला [**Beyond the good ol' LaunchAgents**](https://theevilbit.github.io/beyond/) पर आधारित है, लक्ष्य है **अधिक Autostart स्थान** (यदि संभव हो) जोड़ना, इंद्रधनुष करना **कौन से तकनीक काम कर रहे हैं** आजकल के macOS के नवीनतम संस्करण (13.4) के साथ और **आवश्यक अनुमतियाँ** निर्दिष्ट करना।

## सैंडबॉक्स बाइपास

{% hint style="success" %}
यहाँ आपको सैंडबॉक्स बाइपास के लिए उपयोगी स्टार्ट स्थान मिलेंगे जो आपको केवल कुछ को **एक फ़ाइल में लिखकर** और बहुत **सामान्य क्रिया**, निर्धारित **समय की मात्रा** या एक **क्रिया** के लिए **प्रतीक्षा** करने की अनुमति देता है, जिसे आप साधारणत: रूट अनुमतियों की आवश्यकता नहीं होती।
{% endhint %}
...
{% hint style="success" %}
एक दिलचस्प तथ्य के रूप में, **`launchd`** में एक एम्बेडेड प्रॉपर्टी सूची है जो अन्य प्रसिद्ध सेवाएं शुरू करने वाले launchd में होनी चाहिए। इसके अलावा, इन सेवाओं में `RequireSuccess`, `RequireRun` और `RebootOnSuccess` शामिल हो सकते हैं जिसका अर्थ है कि वे चलाए जाने और सफलतापूर्वक पूरा किया जाना चाहिए।

यह तो स्वायत्त कोड साइनिंग के कारण संशोधित नहीं किया जा सकता।
{% endhint %}

#### विवरण और शोषण

**`launchd`** OX S कर्नेल द्वारा स्टार्टअप पर पहला **प्रक्रिया** है और बंद करने पर आखिरी होता है। इसे हमेशा **PID 1** होना चाहिए। यह प्रक्रिया **ASEP** **plists** में निर्दिष्ट विन्यासों को पढ़ेगा और चलाएगा:
...
**एजेंट और डेमन्स के मुख्य अंतर है कि एजेंट उपयोगकर्ता लॉग इन करते समय लोड होते हैं और डेमन्स सिस्टम स्टार्टअप पर लोड होते हैं** (क्योंकि एसएसएच जैसी सेवाएं हैं जो किसी भी उपयोगकर्ता को सिस्टम तक पहुंचने से पहले चलाने की आवश्यकता होती है)। इसके अलावा, एजेंट GUI का उपयोग कर सकते हैं जबकि डेमन्स को पिछले में चलाने की आवश्यकता होती है।
```xml
...
{% hint style="info" %}
नए डेमन या एजेंट कॉन्फ़िग फ़ाइलें **अगले बूट या** `launchctl load <target.plist>` **का उपयोग करके लोड की जाएंगी**। **इसके अलावा .plist फ़ाइलें भी बिना उस एक्सटेंशन के लोड किया जा सकता है** `launchctl -F <file>` (हालांकि वे plist फ़ाइलें बूट के बाद स्वचालित रूप से लोड नहीं होंगी)।\
इसे `launchctl unload <target.plist>` के साथ **अनलोड** करना भी संभव है (इसके द्वारा इशारा किया गया प्रक्रिया समाप्त हो जाएगी),

यह सुनिश्चित करने के लिए कि कोई **एजेंट** या **डेमन** **को चलाने से रोकने वाली कोई चीज़ नहीं है** (जैसे ओवरराइड), चलाएं: `sudo launchctl load -w /System/Library/LaunchDaemons/com.apple.smdb.plist`
{% endhint %}
...
```
{% hint style="warning" %}
अगर एक plist किसी उपयोगकर्ता के द्वारा स्वामित्व में है, तो यदि यह एक डेमन सिस्टम वाइड फोल्डर में है, **कार्य उपयोगकर्ता के रूप में** और न कि रूट के रूप में निष्पादित किया जाएगा। यह कुछ प्रिविलेज उन्नति हमलों को रोक सकता है।
{% endhint %}
...
* atd (`com.apple.atrun.plist`): `StartInterval` 30 मिनट का है
...
* &#x20;SockServiceName या तो एक पोर्ट होना चाहिए या `/etc/services` से कोई सेवा होनी चाहिए
...
* लेकिन आपको इन फ़ाइलों को लोड करने वाले शैल को निष्पादित करने वाले TCC छलन वाले एक ऐप ढूंढने की आवश्यकता है
...
* **`~/.bashrc`**
* **ट्रिगर**: बैश के साथ एक टर्मिनल खोलें
* `/etc/profile` (काम नहीं किया)
* `~/.profile` (काम नहीं किया)
* `~/.xinitrc`, `~/.xserverrc`, `/opt/X11/etc/X11/xinit/xinitrc.d/`
* **ट्रिगर**: xterm के साथ ट्रिगर किया जाना अपेक्षित है, लेकिन यह **स्थापित नहीं है** और स्थापित करने के बाद भी यह त्रुटि फेंकी जाती है: xterm: `DISPLAY is not set`
...
हम **`man zsh`** के साथ पढ़ सकते हैं।
```bash
...
{% hint style="danger" %}
इंडिकेटेड एक्सप्लोइटेशन कॉन्फ़िगर करना और लॉगइन और लॉगआउट या बूट करना मेरे लिए काम नहीं किया एप्लिकेशन को एक्सीक्यूट करने के लिए। (एप्लिकेशन एक्सीक्यूट नहीं हो रहा था, शायद यह चालू होना चाहिए जब ये कार्रवाई की जाती है)
{% endhint %}
...
* **ट्रिगर**: एप्लिकेशन फिर से खोलने को रीस्टार्ट करें

#### विवरण और एक्सप्लोइटेशन

सभी एप्लिकेशन फिर से खोलने के लिए प्लिस्ट `~/Library/Preferences/ByHost/com.apple.loginwindow.<UUID>.plist` में हैं।

इसलिए, अपनी एप्लिकेशन को सूची में जोड़ने के लिए बस **अपनी एप्लिकेशन जोड़ें**।
...
फिर से खोले जाने वाले एप्लिकेशन जांचने के लिए आप कर सकते हैं:
```bash
...
**`~/Library/Preferences`** में एप्लिकेशन में उपयोगकर्ता की प्राथमिकताएँ संग्रहीत हैं। इन प्राथमिकताओं में से कुछ प्राथमिकताएँ **अन्य एप्लिकेशन/स्क्रिप्ट को निष्पादित करने** के लिए एक विन्यास रख सकती हैं।
...
* उपयोगकर्ता को एफडीए अनुमतियाँ देने के लिए टर्मिनल का उपयोग करें
...
QuickLook प्लगइन्स को आप जब **फ़ाइल का पूर्वावलोकन ट्रिगर करते हैं** (फाइंडर में चयनित फ़ाइल के साथ स्पेस बार दबाएं) और एक **फ़ाइल प्रकार का समर्थन करने वाला प्लगइन** स्थापित होता है, तो उसे निष्पादित किया जा सकता है।

अपना खुद का QuickLook प्लगइन कंपाइल करना संभव है, इसे पिछले स्थानों में रखें और फिर समर्थित फ़ाइल पर जाएं और उसे ट्रिगर करने के लिए स्पेस दबाएं।
...
```
**रूट उपयोगकर्ता** का एक स्टोर **`/private/var/root/Library/Preferences/com.apple.loginwindow.plist`** में स्टोर किया जाता है।

## शर्ताधीन सैंडबॉक्स बाईपास

{% hint style="success" %}
यहाँ आपको **सैंडबॉक्स बाईपास** के लिए उपयोगी स्टार्ट स्थान मिलेंगे जो आपको बस किसी चीज को **एक फ़ाइल में लिखकर** उसे **चलाने की अपेक्षा नहीं करते** जैसे कि विशेष **प्रोग्राम स्थापित होना, "असामान्य" उपयोगकर्ता** क्रियाएँ या परिवेश।
{% endhint %}
...
```
वहाँ आप नियमित **cron** **जॉब्स**, **at** **जॉब्स** (जो बहुत कम प्रयोग होते हैं) और **periodic** **जॉब्स** (मुख्य रूप से अस्थायी फ़ाइलें साफ करने के लिए प्रयोग होते हैं) मिलेंगे। दैनिक periodic jobs को उदाहरण के लिए इस प्रकार से निष्पादित किया जा सकता है: `periodic daily`।
...
Launch Agents are used to run processes at **login**. They are set up per user and are started when the user logs in.

#### Launch Daemons

Launch Daemons are used to run processes at **boot**. They are system-wide and start even if no user logs in.

#### User Login Items

User Login Items are applications that open when a user logs in. They are managed in **System Preferences** > **Users & Groups** > **Login Items**.

#### Global Login Items

Global Login Items are applications that open when any user logs in. They are managed in **System Preferences** > **Users & Groups** > **Login Items**.
```bash
...
* **ट्रिगर**: जब xbar को निष्पादित किया जाता है

#### विवरण

यदि लोकप्रिय कार्यक्रम [**xbar**](https://github.com/matryer/xbar) स्थापित है, तो संभावना है कि एक शैल स्क्रिप्ट लिखा जा सकता है **`~/Library/Application\ Support/xbar/plugins/`** जिसे xbar शुरू होने पर निष्पादित किया जाए:
```bash
...
* **ट्रिगर**: एक बार हैमरस्पून को क्रियान्वित किया जाता है

#### विवरण

[**हैमरस्पून**](https://github.com/Hammerspoon/hammerspoon) **macOS** के लिए एक स्वचालन प्लेटफॉर्म के रूप में काम करता है, जिसमें इसके कार्यों के लिए **LUA स्क्रिप्टिंग भाषा** का उपयोग किया जाता है। विशेष रूप से, यह पूरे AppleScript कोड का एकीकरण समर्थित करता है और शैल स्क्रिप्टों का क्रियान्वयन करता है, जिससे इसकी स्क्रिप्टिंग क्षमताएँ काफी बढ़ जाती हैं।
...
* बैंडबॉक्स को दौड़ने के लिए उपयोगी: [✅](https://emojipedia.org/check-mark-button)
...
यह उपकरण कुछ शॉर्टकट दबाए जाने पर कुछ एप्लिकेशन या स्क्रिप्ट को संकेतित करने की अनुमति देता है। एक हमलावर अपने खुद के **शॉर्टकट और क्रिया को डेटाबेस में निष्पादित करने के लिए** इसे कॉन्फ़िगर कर सकता है ताकि यह अर्बिट्रे कोड निष्पादित करे (एक शॉर्टकट केवल एक कुंजी दबाने के लिए हो सकता है)।
...
यह शर्तों को पूरा करने पर कोड निष्पादित करने के लिए वर्कफ़्लो बनाने की अनुमति देता है। संभावना है कि एक हमलावर एक वर्कफ़्लो फ़ाइल बना सकता है और Alfred को इसे लोड करने के लिए बना सकता है (वर्कफ़्लो का उपयोग करने के लिए प्रीमियम संस्करण का उपयोग करना आवश्यक है)।
...
* सैंडबॉक्स को छलने के लिए उपयोगी: [✅](https://emojipedia.org/check-mark-button)
* लेकिन आपको `osascript` को तार देने की आवश्यकता है
...
* **ट्रिगर:** लॉगिन
* एक्सप्लॉइट पेलोड संग्रहित करने के लिए **`osascript`** को बुलाया गया है
...
एक और विकल्प हो सकता है कि उपयोगकर्ता होम में फ़ाइलें **`.bash_profile`** और **`.zshenv`** बनाएं ताकि अगर फ़ोल्डर LaunchAgents पहले से मौजूद है तो यह तकनीक फिर भी काम करेगी।
...
**डिफ़ॉल्ट** रूप से वे **डिसेबल** होते हैं लेकिन **रूट** उपयोगकर्ता उन्हें **एनेबल** कर सकता है:
```bash
...
* `019bdcd2` - हेक्स में समय। यह एपॉक के बाद बीते मिनटों को प्रतिनिधित करता है। `0x019bdcd2` दसमलवीय में `26991826` है। यदि हम इसे 60 से गुणा करें तो हमें `1619509560` मिलता है, जो `GMT: 2021. अप्रैल 27, मंगलवार 7:46:00` है।

अगर हम नौकरी फ़ाइल प्रिंट करें, तो हमें `at -c` का उपयोग करके प्राप्त की गई जानकारी मिलती है।
...
* इसमें डेस्कटॉप, दस्तावेज़ और डाउनलोड जैसी कुछ मूल TCC अनुमतियाँ हैं
...
2. फ़ोल्डर क्रियाएँ सेटअप में स्थिति मेनू के माध्यम से स्क्रिप्ट को मैन्युअल रूप से जोड़ना।
3. प्रोग्रामेटिक रूप से फ़ोल्डर क्रिया सेट करने के लिए `System Events.app` को Apple इवेंट संदेश भेजने के लिए OSAScript का उपयोग करना।
...
```
तो, `Folder Actions Setup` ऐप्लिकेशन खोलें, **फोल्डर का चयन करें जिसे आप देखना चाहें** और अपने मामले में **`folder.scpt`** का चयन करें (मेरे मामले में मैंने इसे output2.scp नाम दिया था):

<figure><img src="../.gitbook/assets/image (39).png" alt="" width="297"><figcaption></figcaption></figure>

अब, अगर आप **Finder** के साथ उस फोल्डर को खोलते हैं, तो आपका स्क्रिप्ट चलाया जाएगा।
...
1. **`~/Library/Preferences/com.apple.FolderActionsDispatcher.plist`** की बैकअप के लिए `/tmp` में **कॉपी करें**:
...
3. बैकअप फ़ाइल की कॉपी करें: `cp /tmp/com.apple.FolderActionsDispatcher.plist ~/Library/Preferences/`
...
व्राइटअप: [https://theevilbit.github.io/beyond/beyond\_0027/](https://theevilbit.github.io/beyond/beyond\_0027/)
...
कुछ **सामाजिक इंजीनियरिंग** का उपयोग करके आप **उदाहरण के रूप में Google Chrome का अनुकरण** डॉक के अंदर कर सकते हैं और वास्तव में अपनी स्क्रिप्ट को चला सकते हैं:
```bash
#!/bin/sh
...
**लेखन**: [https://theevilbit.github.io/beyond/beyond\_0026/](https://theevilbit.github.io/beyond/beyond\_0026/)\
**लेखन**: [https://objective-see.org/blog/blog\_0x11.html](https://objective-see.org/blog/blog\_0x11.html)
...
एप्लिकेशन में `फाइंडर सिंक एक्सटेंशन` हो सकता है। यह एक्सटेंशन एक एप्लिकेशन के अंदर जाएगा जो क्रियान्वित किया जाएगा। इसके अतिरिक्त, एक्सटेंशन को अपनी कोड को क्रियान्वित करने की क्षमता होने के लिए इसे **किसी वैध एप्पल डेवलपर प्रमाणपत्र के साथ साइन किया जाना चाहिए**, इसे **सैंडबॉक्स** किया जाना चाहिए (हालांकि ढीली छूटें जोड़ी जा सकती हैं) और ऐसा कुछ ऐसे के साथ पंजीकृत किया जाना चाहिए:
```bash
...
लेख: [https://posts.specterops.io/saving-your-access-d562bf5bf90b](https://posts.specterops.io/saving-your-access-d562bf5bf5b)
...
{% hint style="danger" %}
ध्यान दें कि क्योंकि इस कोड को लोड करने वाले बाइनरी की entitlements में (`/System/Library/Frameworks/ScreenSaver.framework/PlugIns/legacyScreenSaver.appex/Contents/MacOS/legacyScreenSaver`) आपको **`com.apple.security.app-sandbox`** मिल सकता है, आप **सामान्य एप्लिकेशन सैंडबॉक्स के अंदर होंगे**।
{% endhint %}
...
स्पॉटलाइट के अंतर्निहित मेकेनिज़्म में एक केंद्रीय प्रक्रिया होती है जिसका नाम 'mds' है, जो 'मेटाडेटा सर्वर' के लिए खड़ा है। इसके साथ, कई 'mdworker' डेमन होते हैं जो विभिन्न रखरखाव कार्य करते हैं, जैसे विभिन्न फ़ाइल प्रकारों को इंडेक्स करना (`ps -ef | grep mdworker`). ये कार्य स्पॉटलाइट इम्पोर्टर प्लगइन्स या **".mdimporter बंडल्स**" के माध्यम से संभव होते हैं, जो स्पॉटलाइट को विभिन्न फ़ाइल प्रारूपों के साथ सामग्री को समझने और इंडेक्स करने की सुविधा प्रदान करते हैं।

प्लगइन्स या **`.mdimporter`** बंडल पहले से उल्लिखित स्थानों में स्थित होते हैं और यदि एक नया बंडल दिखाई देता है तो इसे मिनटों के भीतर लोड किया जाता है (किसी भी सेवा को पुनः आरंभ करने की कोई आवश्यकता नहीं है)। इन बंडल्स को इंडिकेट करना आवश्यक है कि वे कौन से **फ़ाइल प्रकार और एक्सटेंशन को प्रबंधित कर सकते हैं**, इस तरह, स्पॉटलाइट उन्हें उपयोग करेगा जब एक नयी फ़ाइल उल्लिखित एक्सटेंशन के साथ बनाई जाती है।
...
अपना खुद का आयातकर्ता बनाने के लिए आप इस परियोजना के साथ शुरू कर सकते हैं: [https://github.com/megrimm/pd-spotlight-importer](https://github.com/megrimm/pd-spotlight-importer) और फिर नाम, **`CFBundleDocumentTypes`** बदलें और **`UTImportedTypeDeclarations`** जो आप समर्थन देना चाहेंगे उन विस्तारों को समर्थन देने के लिए और उन्हें **`schema.xml`** में पुनर्चित्रित करें।\
फिर **कोड** को बदलें फ़ंक्शन **`GetMetadataForFile`** का जब एक फ़ाइल बनाई जाती है जिसमें प्रसंस्कृत विस्तार है, तो आपका पेलोड निष्पादित करने के लिए।
...
{% hint style="success" %}
यहाँ आपको **सैंडबॉक्स छलावा** के लिए उपयोगी शुरू स्थान मिलेंगे जो आपको बस **किसी फ़ाइल में लिखकर कुछ निष्पादित करने** की अनुमति देते हैं जो **रूट** होने और/या अन्य **अजीब स्थितियों** की आवश्यकता होती है।
{% endhint %}
...
PAM अधिक **स्थायित्व** और मैलवेयर पर ध्यान केंद्रित है जो macOS के अंदर आसान निष्पादन पर है, इस ब्लॉग में इस तकनीक को बेहतर समझने के लिए विस्तृत व्याख्या नहीं दी जाएगी, **इस तकनीक को बेहतर समझने के लिए व्राइटअप्स पढ़ें**।
...
एक persistence/privilege escalation तकनीक PAM का दुरुपयोग करना /etc/pam.d/sudo मॉड्यूल को संशोधित करना इतना सरल है कि शुरु में इस लाइन को जोड़ना:
```bash
...
{% hint style="danger" %}
ध्यान दें कि यह निर्देशिका TCC द्वारा संरक्षित है, इसलिए उपयोगकर्ता को पहुंच के लिए पूछा जाएगा, यह संभावना बहुत अधिक है।
{% endhint %}
...
आप एक अधिकृति प्लगइन बना सकते हैं जो एक उपयोगकर्ता लॉग-इन करते समय निष्क्रिय किया जाएगा ताकि स्थिरता बनी रहे। इन प्लगइन्स में से एक बनाने के बारे में अधिक जानकारी के लिए पिछले लेखों की जाँच करें (और सावधान रहें, एक खराब लिखा हुआ प्लगइन आपको बाहर निकाल सकता है और आपको अपने मैक को पुनर्प्राप्ति मोड से साफ करने की आवश्यकता होगी)।
```objectivec
...
कॉन्फ़िग फ़ाइल **`/private/etc/man.conf`** इंडिकेट करती है कि मैन दस्तावेज़ फ़ाइलें खोलने के लिए कौन सा बाइनरी/स्क्रिप्ट उपयोग करना है। इसलिए एक्सीक्यूटेबल का पथ संशोधित किया जा सकता है ताकि हर बार जब उपयोगकर्ता किसी डॉक्स पढ़ने के लिए man का उपयोग करता है, एक बैकडोर निष्पादित हो।
...
जबकि auditd एक चेतावनी का पता लगाता है, तो स्क्रिप्ट **`/etc/security/audit_warn`** **निष्पादित** होती है। इसलिए आप अपने पेलोड को इस पर जोड़ सकते हैं।
```bash
...
{% tab title="superservicename" %}अत्यधिक सेवा नाम{% endtab %}
```bash
#!/bin/sh
. /etc/rc.common
...
{% hint style="danger" %}
मैं अपने macOS में इस कॉम्पोनेंट को नहीं ढूंढ पा रहा हूँ, इसके बारे में अधिक जानकारी के लिए राइटअप देखें
{% endhint %}

राइटअप: [https://theevilbit.github.io/beyond/beyond\_0023/](https://theevilbit.github.io/beyond/beyond\_0023/)

एप्पल द्वारा पेश किया गया, **emond** एक लॉगिंग मेकेनिज़्म है जो अधूरा या संभावित तौर पर छोड़ दिया गया लगता है, फिर भी यह पहुंचने में बना हुआ है। एक मैक व्यवस्थापक के लिए विशेष रूप से फायदेमंद नहीं होता होने के बावजूद, यह अप्रसिद्ध सेवा एक सूक्ष्म स्थिरता विधि के रूप में काम कर सकती है जिसे धमकी देने वाले कारकों के लिए, संभावित रूप से अधिकांश macOS व्यवस्थापकों द्वारा अनदेखा किया जा सकता है।
...
आप वर्तमान में लोड किए गए केक्स्ट फ़ाइलों की सूची देख सकते हैं:
```bash
...
जैसा कि प्रतीत होता है, `/System/Library/LaunchAgents/com.apple.amstoold.plist` से `plist` इस बाइनरी का उपयोग कर रहा था जबकि एक एक्सपीसी सेवा को उजागर कर रहा था... बात यह है कि बाइनरी मौजूद नहीं थी, इसलिए आप कुछ वहाँ रख सकते थे और जब एक्सपीसी सेवा को कॉल किया जाता है तो आपकी बाइनरी को कॉल किया जाएगा।
...
* रूट की आवश्यकता है
* **ट्रिगर**: जब सेवा चलाई जाती है (कम)

#### विवरण और शोषण

ऐसा लगता है कि इस स्क्रिप्ट को चलाना बहुत आम नहीं है और मैं अपने macOS में इसे नहीं ढूंढ पा रहा था, इसलिए अगर आप अधिक जानकारी चाहते हैं तो लिखावट देखें।
