...
{% hint style="danger" %}
मुझे इस एप्लिकेशन को चलाने के लिए निर्दिष्ट अनुच्छेद को कॉन्फ़िगर करने और लॉगआउट और लॉगइन या बूट करने का काम नहीं किया। (एप्लिकेशन को नहीं चलाया जा रहा था, शायद इन कार्रवाइयों को करने के दौरान यह चलाया जाना चाहिए)
{% endhint %}
...
तो, अपने एप्लिकेशन को सूची में जोड़ने के लिए फिर से खोलने वाले एप्लिकेशन को चलाएं, आपको बस **अपने एप्लिकेशन को जोड़ना होगा**।
...
उदाहरण के लिए, टर्मिनल स्टार्टअप में एक कमांड चला सकता है:
...
```
आप भी एक्सटेंशन **`.command`**, **`.tool`** का उपयोग कर सकते हैं, जिनमें नियमित शेल स्क्रिप्ट सामग्री होती है और वे टर्मिनल द्वारा भी खोले जाएंगे।
...
पिछले व्रिटअप के अनुसार, कुछ ऑडियो प्लगइन्स को **कंपाइल** किया जा सकता है और उन्हें लोड किया जा सकता है।
...
QuickLook प्लगइन्स को **फ़ाइल के पूर्वावलोकन** को ट्रिगर करने पर (फाइंडर में चयनित फ़ाइल के साथ स्पेस बार दबाएं) और एक **फ़ाइल प्रकार का समर्थन करने वाला प्लगइन** स्थापित होने पर निष्पादित किया जा सकता है।

आप अपना खुद का QuickLook प्लगइन कंपाइल कर सकते हैं, उसे पिछले स्थानों में रखें और फिर समर्थित फ़ाइल पर जाएं और उसे ट्रिगर करने के लिए स्पेस बार दबाएं।

### ~~लॉगिन/लॉगआउट हुक~~

{% hint style="danger" %}
यह मेरे लिए काम नहीं करा, न तो उपयोगकर्ता लॉगिन हुक और न ही रूट लॉगआउट हुक के साथ।
{% endhint %}
...
* आपको कुछ ऐसा करने की क्षमता होनी चाहिए जैसे `defaults write com.apple.loginwindow LoginHook /Users/$USER/hook.sh`
...
```
**`/private/var/root/Library/Preferences/com.apple.loginwindow.plist`** में रूट उपयोगकर्ता का संग्रहित होता है।

## शर्ताधीन सैंडबॉक्स बाईपास

{% hint style="success" %}
यहां आपको **सैंडबॉक्स बाईपास** के लिए उपयोगी स्टार्ट स्थान मिलेंगे जो आपको केवल किसी फ़ाइल में लिखकर कुछ को निष्पादित करने और विशेष **प्रोग्राम स्थापित, "असामान्य" उपयोगकर्ता** क्रियाएँ या पर्यावरण की जरूरत जैसी **आम नहीं होने** की उम्मीद करती है।
{% endhint %}
...
* सीधी लिखने की पहुंच के लिए रूट की आवश्यकता होती है। `crontab <file>` को निष्पादित कर सकते हैं तो रूट की आवश्यकता नहीं होती है
...
```
**`~/Library/Preferences/com.googlecode.iterm2.plist`** में स्थित iTerm2 प्राथमिकताएं, iTerm2 टर्मिनल खोलने पर **एक कमांड को निष्पादित करने की संकेत कर सकती हैं।**
...
यदि लोकप्रिय कार्यक्रम [**xbar**](https://github.com/matryer/xbar) स्थापित है, तो यह संभव है कि एक शैल स्क्रिप्ट लिखा जा सकता है **`~/Library/Application\ Support/xbar/plugins/`** में जो xbar की शुरुआत होते समय क्रियान्वित होगा:
```bash
...
```
फ़ाइल का नाम कतार, नौकरी नंबर और यह समय शामिल होता है जिसे चलाने की योजना बनाई गई है। उदाहरण के लिए, हम `a0001a019bdcd2` पर एक नज़र डालते हैं।

* `a` - यह कतार है
* `0001a` - हेक्स में नौकरी नंबर, `0x1a = 26`
* `019bdcd2` - हेक्स में समय। यह युग से बिताए गए मिनटों को प्रतिष्ठा करता है। `0x019bdcd2` दशमलव में `26991826` है। यदि हम इसे 60 से गुणा करें तो हमें `1619509560` मिलता है, जो `GMT: 2021 अप्रैल 27, मंगलवार 7:46:00` है।

यदि हम नौकरी फ़ाइल को प्रिंट करें, तो हमें `at -c` का उपयोग करके प्राप्त की गई जानकारी मिलती है।
...
एक फ़ोल्डर क्रिया स्क्रिप्ट उस फ़ोल्डर के साथ निष्पादित होता है जिसे इसके साथ जुड़ा होता है, जब उसमें आइटम जोड़े या हटाए जाते हैं, या जब इसकी विंडो खोली, बंद की जाती है, हटाई जाती है, या आकार बदला जाता है:
...
* UI के माध्यम से फ़ोल्डर से बाहर नेविगेट करें
...
2. एक फ़ोल्डर पर दायां क्लिक करें, `Folder Actions Setup...`, `Run Service` का चयन करें, और मैन्युअल रूप से एक स्क्रिप्ट जोड़ें।
3. OSAScript का उपयोग करके `System Events.app` को Apple Event संदेश भेजने के लिए प्रोग्रामेटिक रूप से एक नई `Folder Action` क्वेरी और पंजीकरण करें।
...
फिर निम्नलिखित स्क्रिप्ट को निष्पादित करें ताकि फ़ोल्डर एक्शन को सक्षम किया जा सके और पहले से कंपाइल किए गए स्क्रिप्ट को फ़ोल्डर **`/users/username/Desktop`** के साथ जोड़ें:
```javascript
...
स्पॉटलाइट मैकओएस का अंतर्निहित खोज सुविधा है, जिसका उद्देश्य उपयोगकर्ताओं को उनके कंप्यूटर पर डेटा का त्वरित और व्यापक उपयोग प्रदान करना है।\
...
प्लगइन या **`.mdimporter`** बंडल पहले से ही उल्लिखित स्थानों में स्थित होते हैं और यदि एक नया बंडल दिखाई देता है तो यह मिनटों के भीतर लोड हो जाता है (किसी भी सेवा को पुनः प्रारंभ करने की आवश्यकता नहीं होती है)। इन बंडलों को इंडिकेट करना आवश्यक होता है कि वे किस **फ़ाइल प्रकार और एक्सटेंशन को प्रबंधित कर सकते हैं**, इस तरह, स्पॉटलाइट उन्हें उपयोग करेगा जब एक नई फ़ाइल उल्लिखित एक्सटेंशन के साथ बनाई जाती है।

सभी लोड होने वाले `mdimporters` को खोजना संभव है इसको चलाकर:
```bash
mdimport -L
Paths: id(501) (
...
```
{% hint style="danger" %}
यदि आप अन्य `mdimporter` के Plist की जांच करते हैं तो आपको प्रविष्टि **`UTTypeConformsTo`** नहीं मिलेगी। इसलिए यह एक निर्मित _यूनिफॉर्म टाइप पहचानकर्ता_ ([UTI](https://en.wikipedia.org/wiki/Uniform\_Type\_Identifier)) है और इसे विस्तारों को निर्दिष्ट करने की आवश्यकता नहीं होती है।
...
यह छोटा-सा जाना-माना सेवा एक Mac व्यवस्थापक के लिए **बहुत उपयोगी नहीं हो सकती है**, लेकिन एक धमकी प्रदाता के लिए एक बहुत अच्छा कारण होगा कि वह इसे एक **स्थायित्व मेकेनिज़्म के रूप में उपयोग** करें, जिसके बारे में शायद अधिकांश macOS व्यवस्थापक जानते ही नहीं होंगे। emond के दुष्प्रभाव का पता लगाना कठिन नहीं होना चाहिए, क्योंकि सेवा के लिए सिस्टम लॉन्चडेमन केवल एक ही स्थान में चलाने के लिए स्क्रिप्ट ढूंढ़ता है:
```bash
...
यहां **स्टार्टअप पर निष्पादित होने वाले कमांड** भी रखना संभव है। आम rc.common स्क्रिप्ट का उदाहरण:
```bash
#
...
* **अपने हैकिंग ट्रिक्स को हाकट्रिक्स रेपो** (https://github.com/carlospolop/hacktricks) **और** [**हैकट्रिक्स-क्लाउड रेपो**](https://github.com/carlospolop/hacktricks-cloud) **में पीआर जमा करके अपने हैकिंग ट्रिक्स साझा करें।**
