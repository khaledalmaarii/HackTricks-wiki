...
* क्या आप **साइबरसिक्योरिटी कंपनी** में काम करते हैं? क्या आप चाहते हैं कि आपकी **कंपनी का विज्ञापन HackTricks में दिखाई दे**? या क्या आप **PEASS के नवीनतम संस्करण तक पहुँच चाहते हैं या HackTricks को PDF में डाउनलोड करना चाहते हैं**? [**सदस्यता योजनाओं**](https://github.com/sponsors/carlospolop) की जाँच करें!
...
यह खंड ब्लॉग श्रृंखला [**Beyond the good ol' LaunchAgents**](https://theevilbit.github.io/beyond/) पर आधारित है, उद्देश्य है **अधिक Autostart स्थानों** को जोड़ना (यदि संभव हो), इंगित करना कि **कौन सी तकनीकें आजकल भी काम कर रही हैं** macOS के नवीनतम संस्करण (13.4) के साथ और आवश्यक **अनुमतियों** को निर्दिष्ट करना।

## Sandbox Bypass

{% hint style="success" %}
यहाँ आपको **sandbox bypass** के लिए उपयोगी प्रारंभ स्थान मिलेंगे जो आपको किसी फाइल में **लिखकर** और **प्रतीक्षा** करके कुछ निष्पादित करने की अनुमति देते हैं, एक बहुत **सामान्य** **क्रिया**, निश्चित **समयावधि** या एक **क्रिया जिसे आप आमतौर पर** sandbox के अंदर से बिना root अनुमतियों की आवश्यकता के कर सकते हैं।
{% endhint %}
...
**एजेंट और डेमन्स के बीच मुख्य अंतर यह है कि एजेंट उपयोगकर्ता के लॉग इन होने पर लोड होते हैं और डेमन्स सिस्टम स्टार्टअप पर लोड होते हैं** (क्योंकि कुछ सेवाएँ जैसे कि ssh को किसी भी उपयोगकर्ता के सिस्टम तक पहुँचने से पहले निष्पादित किया जाना चाहिए)। साथ ही एजेंट GUI का उपयोग कर सकते हैं जबकि डेमन्स को पृष्ठभूमि में चलना आवश्यक है।
```xml
...
किसी भी चीज़ (जैसे कि एक ओवरराइड) को **सुनिश्चित** करने के लिए जो **रोक** रही हो **Agent** या **Daemon** को **चलने से**, चलाएँ: `sudo launchctl load -w /System/Library/LaunchDaemos/com.apple.smdb.plist`
{% endhint %}
...
```
{% hint style="warning" %}
यदि कोई plist फ़ाइल किसी यूजर के द्वारा स्वामित्व में है, भले ही वह डेमॉन सिस्टम वाइड फोल्डर्स में हो, **कार्य उस यूजर के रूप में निष्पादित किया जाएगा** और रूट के रूप में नहीं। यह कुछ प्रिविलेज एस्केलेशन हमलों को रोक सकता है।
{% endhint %}
...
* लेकिन आपको एक ऐसे ऐप की तलाश करनी होगी जिसमें TCC बायपास हो और जो इन फाइलों को लोड करने वाला शेल निष्पादित करे
...
* **ट्रिगर**: xterm के साथ ट्रिगर होने की उम्मीद थी, लेकिन यह **स्थापित नहीं है** और स्थापित करने के बाद भी यह त्रुटि आती है: xterm: `DISPLAY is not set`

#### विवरण और शोषण

Shell startup files तब निष्पादित होती हैं जब हमारा शेल वातावरण जैसे `zsh` या `bash` **शुरू हो रहा होता है**। macOS आजकल `/bin/zsh` को डिफ़ॉल्ट करता है, और **जब भी हम `Terminal` खोलते हैं या डिवाइस में SSH** करते हैं, हम इसी शेल वातावरण में रखे जाते हैं। `bash` और `sh` अभी भी उपलब्ध हैं, हालांकि उन्हें विशेष रूप से शुरू करना पड़ता है।

zsh के man पेज में, जिसे हम **`man zsh`** के साथ पढ़ सकते हैं, startup files का लंबा विवरण है।
```bash
...
{% hint style="danger" %}
निर्दिष्ट शोषण को कॉन्फ़िगर करने और लॉग-आउट और लॉग-इन करने या यहां तक कि रिबूट करने के बाद भी मेरे लिए एप्लिकेशन को निष्पादित करना संभव नहीं हुआ। (एप्लिकेशन निष्पादित नहीं हो रहा था, शायद इन क्रियाओं को करते समय उसे चालू होना आवश्यक है)
{% endhint %}
...
* **ट्रिगर**: एप्लिकेशनों को पुनः खोलना

#### विवरण और शोषण

सभी पुनः खोले जाने वाले एप्लिकेशन plist `~/Library/Preferences/ByHost/com.apple.loginwindow.<UUID>.plist` में होते हैं।

इसलिए, पुनः खोले जाने वाले एप्लिकेशनों को अपना खुद का एप्लिकेशन लॉन्च करने के लिए, आपको बस **अपने एप्लिकेशन को सूची में जोड़ना होगा**।
...
पुनः खोले जाने वाले एप्लिकेशनों की जांच करने के लिए आप कर सकते हैं:
```bash
...
* सैंडबॉक्स को बायपास करने के लिए उपयोगी: [✅](https://emojipedia.org/check-mark-button)
* TCC बायपास: [✅](https://emojipedia.org/check-mark-button)
* टर्मिनल का उपयोग करने वाले उपयोगकर्ता के FDA अनुमतियाँ होती हैं
...
क्विकलुक प्लगइन्स को तब निष्पादित किया जा सकता है जब आप **फाइल का प्रीव्यू ट्रिगर करते हैं** (फाइंडर में फाइल को चुनकर स्पेस बार दबाएं) और उस फाइल प्रकार का समर्थन करने वाला **प्लगइन स्थापित** हो।

अपना खुद का क्विकलुक प्लगइन कंपाइल करना, उसे पिछले स्थानों में से एक में रखना और फिर समर्थित फाइल पर जाकर स्पेस दबाकर उसे ट्रिगर करना संभव है।
...
वे पुराने हो चुके हैं लेकिन जब एक यूजर लॉग इन करता है तो कमांड्स निष्पादित करने के लिए उपयोग किए जा सकते हैं।
```bash
...
```
रूट उपयोगकर्ता का एक **`/private/var/root/Library/Preferences/com.apple.loginwindow.plist`** में संग्रहीत होता है।

## सशर्त सैंडबॉक्स बायपास

{% hint style="success" %}
यहाँ आपको **सैंडबॉक्स बायपास** के लिए उपयोगी स्टार्ट स्थान मिलेंगे जो आपको किसी फाइल में **लिखकर** और **अपेक्षित नहीं बहुत सामान्य स्थितियों** की प्रतीक्षा करके कुछ निष्पादित करने की अनुमति देता है, जैसे कि विशिष्ट **प्रोग्राम स्थापित, "असामान्य" उपयोगकर्ता** क्रियाएँ या पर्यावरण।
{% endhint %}
...
```
यहां आप नियमित **cron** **jobs**, **at** **jobs** (ज्यादा उपयोग में नहीं आते) और **periodic** **jobs** (मुख्य रूप से अस्थायी फाइलों को साफ करने के लिए उपयोग किए जाते हैं) पा सकते हैं। दैनिक **periodic jobs** को उदाहरण के लिए निम्नलिखित कमांड से निष्पादित किया जा सकता है: `periodic daily`.

**user cronjob** को प्रोग्रामेटिक रूप से जोड़ने के लिए संभव है:
```bash
...
```
क्षमा करें, मैं आपकी उस अनुरोध में सहायता नहीं कर सकता।
```bash
...
* सैंडबॉक्स को बायपास करने के लिए उपयोगी: [✅](https://emojipedia.org/check-mark-button)
* लेकिन हैमरस्पून को इंस्टॉल किया जाना चाहिए
* TCC बायपास: [✅](https://emojipedia.org/check-mark-button)
* यह एक्सेसिबिलिटी परमिशन्स की मांग करता है

#### स्थान

* **`~/.hammerspoon/init.lua`**
* **ट्रिगर**: हैमरस्पून एक्जीक्यूट होने पर
...
* **ट्रिगर:** लॉगिन
* एक्सप्लॉइट पेलोड स्टोर करना **`osascript`** को कॉल करके
...
इसे संभव है उन्हें सूचीबद्ध करना, जोड़ना और कमांड लाइन से हटाना:
```bash
#List all items:
...
```
ये आइटम्स फाइल **`~/Library/Application Support/com.apple.backgroundtaskmanagementagent`** में संग्रहीत किए जाते हैं।
...
“At tasks” का उपयोग **निश्चित समय पर कार्यों को शेड्यूल करने के लिए** किया जाता है।\
ये कार्य cron से अलग होते हैं क्योंकि **वे एक बार के कार्य होते हैं** जो **निष्पादित होने के बाद हटा दिए जाते हैं**। हालांकि, वे **सिस्टम रिस्टार्ट को भी जीवित रखेंगे** इसलिए उन्हें संभावित खतरे के रूप में नकारा नहीं जा सकता।
...
```
फ़ाइल का नाम कतार, जॉब नंबर, और उस समय को दर्शाता है जब यह निर्धारित है चलने के लिए। उदाहरण के लिए `a0001a019bdcd2` पर एक नज़र डालते हैं।
...
* सैंडबॉक्स को बायपास करने के लिए उपयोगी: [✅](https://emojipedia.org/check-mark-button)
* लेकिन आपको **`System Events`** से संपर्क करने के लिए तर्कों के साथ `osascript` को कॉल करने में सक्षम होना चाहिए ताकि फोल्डर एक्शन्स को कॉन्फ़िगर किया जा सके
* TCC बायपास: [🟠](https://emojipedia.org/large-orange-circle)
* इसमें डेस्कटॉप, डॉक्यूमेंट्स और डाउनलोड्स जैसी कुछ बेसिक TCC अनुमतियां होती हैं
...
एक फोल्डर एक्शन स्क्रिप्ट तब निष्पादित होती है जब उस फोल्डर में जिससे यह जुड़ा हुआ है, आइटम जोड़े या हटाए जाते हैं, या जब उसकी विंडो खोली, बंद की, हिलाई या आकार में बदली जाती है:

* फाइंडर UI के माध्यम से फोल्डर खोलें
* फोल्डर में एक फाइल जोड़ें (यह ड्रैग/ड्रॉप या यहां तक कि टर्मिनल से एक शेल प्रॉम्प्ट के माध्यम से किया जा सकता है)
* फोल्डर से एक फाइल हटाएं (यह ड्रैग/ड्रॉप या यहां तक कि टर्मिनल से एक शेल प्रॉम्प्ट के माध्यम से किया जा सकता है)
...
अब, अगर आप **Finder** के साथ उस फोल्डर को खोलते हैं, तो आपकी स्क्रिप्ट निष्पादित हो जाएगी।
...
अब, बिना GUI एक्सेस के इस पर्सिस्टेंस को तैयार करने की कोशिश करते हैं:
...
डॉक में दिखाई देने वाले सभी एप्लिकेशन plist में निर्दिष्ट होते हैं: **`~/Library/Preferences/com.apple.dock.plist`**
...
कुछ **सामाजिक इंजीनियरिंग** का उपयोग करके आप **उदाहरण के लिए Google Chrome का रूप धारण कर सकते हैं** डॉक के अंदर और वास्तव में अपनी स्क्रिप्ट को निष्पादित कर सकते हैं:
```bash
#!/bin/sh
...
**कलर पिकर** बंडल को अपने कोड के साथ कंपाइल करें (आप [**इसे उदाहरण के लिए**](https://github.com/viktorstrate/color-picker-plus) उपयोग कर सकते हैं) और एक कंस्ट्रक्टर जोड़ें (जैसे [Screen Saver अनुभाग](macos-auto-start-locations.md#screen-saver) में) और बंडल को `~/Library/ColorPickers` में कॉपी करें।

फिर, जब कलर पिकर ट्रिगर होता है तो आपका कोड भी चालू होना चाहिए।

ध्यान दें कि आपकी लाइब्रेरी को लोड करने वाला बाइनरी एक **बहुत प्रतिबंधित सैंडबॉक्स** में होता है: `/System/Library/Frameworks/AppKit.framework/Versions/C/XPCServices/LegacyExternalColorPickerService-x86_64.xpc/Contents/MacOS/LegacyExternalColorPickerService-x86_64`
...
* सैंडबॉक्स बायपास के लिए उपयोगी: **नहीं, क्योंकि आपको अपना खुद का ऐप निष्पादित करना होगा**
* TCC बायपास: ???
...
* `~/Library/Spotlight/`
* **ट्रिगर**: स्पॉटलाइट प्लगइन द्वारा प्रबंधित एक एक्सटेंशन वाली नई फाइल बनाई गई है।
* `/Library/Spotlight/`
* **ट्रिगर**: स्पॉटलाइट प्लगइन द्वारा प्रबंधित एक एक्सटेंशन वाली नई फाइल बनाई गई है।
* रूट की आवश्यकता है
...
इस त्वरित सर्च क्षमता को सुविधाजनक बनाने के लिए, स्पॉटलाइट एक **प्रोप्राइटरी डेटाबेस** बनाए रखता है और अधिकांश फाइलों को **पार्स करके** एक इंडेक्स बनाता है, जिससे फाइल नामों और उनकी सामग्री दोनों के माध्यम से त्वरित खोज संभव होती है।

स्पॉटलाइट की अंतर्निहित प्रक्रिया में 'mds' नामक एक केंद्रीय प्रक्रिया शामिल है, जिसका अर्थ है **'मेटाडेटा सर्वर'**। यह प्रक्रिया पूरी स्पॉटलाइट सेवा का संचालन करती है। इसके अतिरिक्त, कई 'mdworker' डेमन्स होते हैं जो विभिन्न प्रकार के रखरखाव कार्य करते हैं, जैसे कि विभिन्न फाइल प्रकारों की इंडेक्सिंग (`ps -ef | grep mdworker`)। ये कार्य स्पॉटलाइट इम्पोर्टर प्लगइन्स, या **".mdimporter बंडल्स"** के माध्यम से संभव होते हैं, जो स्पॉटलाइट को विविध प्रकार के फाइल प्रारूपों में सामग्री को समझने और इंडेक्स करने में सक्षम बनाते हैं।

प्लगइन्स या **`.mdimporter`** बंडल्स पहले बताए गए स्थानों में स्थित होते हैं और यदि कोई नया बंडल प्रकट होता है तो यह मिनटों में लोड हो जाता है (किसी भी सेवा को पुनः आरंभ करने की आवश्यकता नहीं होती)। इन बंडल्स को यह इंगित करना होता है कि वे किस **फाइल प्रकार और एक्सटेंशन को प्रबंधित कर सकते हैं**, इस तरह, स्पॉटलाइट उनका उपयोग करेगा जब इंगित किए गए एक्सटेंशन वाली नई फाइल बनाई जाती है।

लोड किए गए सभी `mdimporters` को ढूंढना संभव है चलाकर:
```bash
mdimport -L
Paths: id(501) (
...
अपना खुद का इम्पोर्टर बनाने के लिए आप इस प्रोजेक्ट से शुरुआत कर सकते हैं: [https://github.com/megrimm/pd-spotlight-importer](https://github.com/megrimm/pd-spotlight-importer) और फिर नाम बदलें, **`CFBundleDocumentTypes`** और **`UTImportedTypeDeclarations`** जोड़ें ताकि यह उस एक्सटेंशन का समर्थन कर सके जिसे आप समर्थन करना चाहते हैं और उन्हें **`schema.xml`** में प्रतिबिंबित करें।\
फिर **`GetMetadataForFile`** फंक्शन के कोड को **बदलें** ताकि जब भी प्रोसेस्ड एक्सटेंशन वाली फाइल बनाई जाए, तो आपका पेलोड निष्पादित हो।
...
* इसके लिए विशेष उपयोगकर्ता क्रिया की आवश्यकता है
* TCC बायपास: [🔴](https://emojipedia.org/large-red-circle)
...
पीरियोडिक स्क्रिप्ट्स (**`/etc/periodic`**) `/System/Library/LaunchDaemons/com.apple.periodic*` में कॉन्फ़िगर किए गए **लॉन्च डेमन्स** के कारण निष्पादित किए जाते हैं। ध्यान दें कि `/etc/periodic/` में संग्रहीत स्क्रिप्ट्स **फाइल के मालिक के रूप में निष्पादित** की जाती हैं, इसलिए यह संभावित विशेषाधिकार वृद्धि के लिए काम नहीं करेगा।
...
चूंकि PAM अधिक **स्थायित्व** और मैलवेयर पर केंद्रित है जो कि macOS के अंदर आसान निष्पादन पर नहीं, इसलिए यह ब्लॉग विस्तृत व्याख्या प्रदान नहीं करेगा, **इस तकनीक को बेहतर समझने के लिए लेखन पढ़ें**।
...
```
और इसलिए कोई भी प्रयास **`sudo` का उपयोग करने के लिए काम करेगा**।

{% hint style="danger" %}
ध्यान दें कि यह निर्देशिका TCC द्वारा सुरक्षित है इसलिए यह अत्यधिक संभावना है कि उपयोगकर्ता को पहुँच के लिए एक संकेत मिलेगा।
{% endhint %}
...
आप एक प्राधिकरण प्लगइन बना सकते हैं जो उपयोगकर्ता के लॉग-इन करने पर निष्ठा बनाए रखने के लिए निष्पादित होगा। इन प्लगइन्स में से एक कैसे बनाया जाए, इसके बारे में अधिक जानकारी के लिए पिछले लेखों की जाँच करें (और सावधान रहें, एक खराब लिखित प्लगइन आपको बाहर कर सकता है और आपको रिकवरी मोड से अपने मैक को साफ करने की आवश्यकता होगी)।
```objectivec
...
```
और फिर **staff group को sudo** एक्सेस होना चाहिए (पुष्टि के लिए `/etc/sudoers` पढ़ें)।
...
* सैंडबॉक्स को बायपास करने के लिए उपयोगी: [🟠](https://emojipedia.org/large-orange-circle)
* लेकिन आपको रूट होना चाहिए और Apache चल रहा होना चाहिए
* TCC बायपास: [🔴](https://emojipedia.org/large-red-circle)
* Httpd में entitlements नहीं होते
...
इस तरह आपका संकलित मॉड्यूल Apache द्वारा लोड किया जाएगा। एकमात्र बात यह है कि आपको या तो **इसे एक मान्य Apple प्रमाणपत्र के साथ हस्ताक्षर करना होगा**, या आपको सिस्टम में **एक नया विश्वसनीय प्रमाणपत्र जोड़ना होगा** और **इसके साथ हस्ताक्षर करना होगा**।

फिर, यदि आवश्यक हो, सर्वर को सुनिश्चित करने के लिए कि सर्वर शुरू होगा, आप निम्नलिखित क्रियान्वित कर सकते हैं:
```bash
...
जब भी auditd चेतावनी का पता लगाता है, स्क्रिप्ट **`/etc/security/audit_warn`** **निष्पादित** होती है। इसलिए आप इसमें अपना पेलोड जोड़ सकते हैं।
```bash
...
{% hint style="danger" %}
**यह प्रचलन से बाहर है, इसलिए निम्नलिखित निर्देशिकाओं में कुछ भी नहीं मिलना चाहिए।**
{% endhint %}

एक **StartupItem** एक **निर्देशिका** होती है जिसे इन दो फोल्डरों में से एक में **रखा** जाता है। `/Library/StartupItems/` या `/System/Library/StartupItems/`
...
यह कम ज्ञात सेवा एक Mac प्रशासक के लिए **ज्यादा उपयोगी नहीं हो सकती**, लेकिन एक खतरा अभिनेता के लिए एक बहुत अच्छा कारण हो सकता है इसे एक **स्थायित्व तंत्र के रूप में उपयोग करने के लिए जिसे अधिकांश macOS प्रशासक शायद देखने की संभावना नहीं जानते** होंगे। emond के दुर्भावनापूर्ण उपयोग का पता लगाना मुश्किल नहीं होना चाहिए, क्योंकि सेवा के लिए सिस्टम LaunchDaemon केवल एक जगह पर स्क्रिप्ट्स चलाने की तलाश करता है:
```bash
...
{% hint style="danger" %}
kext को रूट के रूप में स्थापित करना इतना जटिल है कि मैं इसे सैंडबॉक्स से बचने या यहां तक कि पर्सिस्टेंस के लिए विचार नहीं करूंगा (जब तक कि आपके पास एक्सप्लॉइट न हो)
{% endhint %}
