...
यह खंड ब्लॉग श्रृंखला [**Beyond the good ol' LaunchAgents**](https://theevilbit.github.io/beyond/) पर आधारित है, उद्देश्य है **अधिक Autostart स्थान** (यदि संभव हो) जोड़ना, **जिन तकनीकों काम कर रही हैं** आजकल के macOS के नवीनतम संस्करण (13.4) के साथ और **आवश्यक अनुमतियाँ** निर्दिष्ट करना।

## सैंडबॉक्स बाइपास

{% hint style="success" %}
यहाँ आपको सैंडबॉक्स बाइपास के लिए उपयोगी स्टार्ट स्थान मिलेंगे जो आपको **केवल किसी फ़ाइल में लिखकर** कुछ को **आसानी से निष्पादित करने** की अनुमति देते हैं, बहुत **सामान्य** **क्रिया**, निर्धारित **समय की मात्रा** या एक **क्रिया जो आप सामान्यत: सैंडबॉक्स के अंदर से बिना रूट अनुमतियों की आवश्यकता के** कर सकते हैं।
{% endhint %}
...
**`launchd`** ओएक्स एस कर्नेल द्वारा स्टार्टअप पर पहला **प्रक्रिया** है और बंद करने पर अंतिम होता है। इसे हमेशा **PID 1** होना चाहिए। यह प्रक्रिया **ASEP** **plists** में निर्दिष्ट विन्यासों को **पढ़ेगा और निष्पादित करेगा**:
...
जब एक उपयोगकर्ता लॉग इन करता है तो `/Users/$USER/Library/LaunchAgents` और `/Users/$USER/Library/LaunchDemons` में स्थित plists **लॉग इन करने वाले उपयोगकर्ता की अनुमतियों** के साथ शुरू होते हैं।
...
{% hint style="info" %}
नए डेमन या एजेंट कॉन्फ़िग फ़ाइलें **अगले बूट के बाद या** `launchctl load <target.plist>` **का उपयोग करके लोड की जाएंगी**। इसके साथ ही `.plist` एक्सटेंशन के बिना भी `launchctl -F <file>` के साथ .plist फ़ाइलें लोड करना संभव है (हालांकि वे plist फ़ाइलें बूट के बाद स्वचालित रूप से लोड नहीं होंगी)।\
इसे `launchctl unload <target.plist>` के साथ **अनलोड** करना भी संभव है (इसके द्वारा इस प्रक्रिया को समाप्त कर दिया जाएगा)।
...
* लेकिन आपको एक एप्लिकेशन ढूंढने की आवश्यकता है जिसमें TCC बायपास हो और जो इन फ़ाइलों को लोड करने वाला शैल निष्पादित करता है।
...
```
### फिर से खुलने वाले एप्लिकेशन

{% hint style="danger" %}
सूचित शोषण को कॉन्फ़िगर करना और लॉगआउट और लॉगइन या बूट करना मेरे लिए काम नहीं किया एप्लिकेशन को निष्पादित करने के लिए। (एप्लिकेशन नहीं चल रहा था, शायद यह चालू होना चाहिए जब ये कार्रवाई की जाती है)
{% endhint %}
...
* **ट्रिगर**: एप्लिकेशनों को फिर से खोलने के लिए पुनरारंभ करें

#### विवरण और शोषण

सभी एप्लिकेशन फिर से खोलने के लिए `~/Library/Preferences/ByHost/com.apple.loginwindow.<UUID>.plist` में हैं।

इसलिए, अपनी एप्लिकेशन को सूची में जोड़ने के लिए बस **अपना एप्लिकेशन जोड़ें**।
...
* उपयोगकर्ता की एफडीए अनुमतियों को टर्मिनल का उपयोग करने देना
...
**`~/Library/Preferences`** में एप्लिकेशन में उपयोगकर्ता की प्राथमिकताएँ संग्रहीत हैं। इनमें से कुछ प्राथमिकताएँ **अन्य एप्लिकेशन/स्क्रिप्ट को निष्पादित** करने के लिए एक विन्यास रख सकती हैं।
...
[...]
```
इसलिए, अगर सिस्टम में टर्मिनल की प्राथमिकताओं का प्लिस्ट ओवरराइट किया जा सकता है, तो **`open`** कार्यक्षमता का उपयोग करके **टर्मिनल खोली जा सकती है और उस कमांड को निष्पादित किया जाएगा**।
...
* TCC बायपास: [✅](https://emojipedia.org/check-mark-button)
* उपयोगकर्ता को उसका एफडीए अनुमतियाँ होती हैं जो टर्मिनल का उपयोग करता है
...
लिखित विवरण: [https://theevilbit.github.io/beyond/beyond\_0013/](https://theevilbit.github.io/beyond/beyond\_0013/)\
लिखित विवरण: [https://posts.specterops.io/audio-unit-plug-ins-896d3434a882](https://posts.specterops.io/audio-unit-plug-ins-896d3434a882)

* सैंडबॉक्स को छलने के लिए उपयोगी: [✅](https://emojipedia.org/check-mark-button)
* TCC छलने: [🟠](https://emojipedia.org/large-orange-circle)
...
पिछले लेखों के अनुसार **कुछ ऑडियो प्लगइन्स को कंपाइल** करना संभव है और उन्हें लोड करने के लिए।

### QuickLook Plugins

लिखित विवरण: [https://theevilbit.github.io/beyond/beyond\_0028/](https://theevilbit.github.io/beyond/beyond\_0028/)
...
QuickLook प्लगइन्स को आप जब एक फ़ाइल का पूर्वावलोकन ट्रिगर करते हैं (फाइंडर में चयनित फ़ाइल के साथ स्पेस बार दबाएं) और एक **फ़ाइल प्रकार का समर्थन करने वाला प्लगइन** स्थापित होता है, तो उसे निष्पादित किया जा सकता है।
...
**लिखित विवरण**: [https://theevilbit.github.io/beyond/beyond\_0022/](https://theevilbit.github.io/beyond/beyond\_0022/)

* सैंडबॉक्स को छलने के लिए उपयोगी: [✅](https://emojipedia.org/check-mark-button)
* TCC छलने: [🔴](https://emojipedia.org/large-red-circle)
...
वे पुराने हैं लेकिन उपयोगकर्ता लॉग इन करते समय कमांड निष्पादित करने के लिए उपयोग किए जा सकते हैं।
```bash
...
{% hint style="success" %}
यहाँ आपको **सैंडबॉक्स बाईपास** के लिए उपयोगी स्टार्ट स्थान मिलेगा जो आपको **केवल किसी फ़ाइल में कुछ लिखकर** और **अपेक्षित गैर सामान्य स्थितियों** जैसे कि विशेष **प्रोग्राम स्थापित, "असामान्य" उपयोगकर्ता** क्रियाएँ या परिवेश के बिना कुछ निष्पेष करने की अनुमति देता है।
{% endhint %}
...
crontab -l
```
आप **`/usr/lib/cron/tabs/`** और **`/var/at/tabs/`** में उपयोगकर्ताओं की सभी cron jobs भी देख सकते हैं (रूट की आवश्यकता है)।

मैकओएस में कई फोल्डर्स में **निश्चित अंतराल** के साथ स्क्रिप्ट्स को निष्पादित किया जा सकता है:
```bash
# The one with the cron jobs is /usr/lib/cron/tabs/
ls -lR /usr/lib/cron/tabs/ /private/var/at/jobs /etc/periodic/
```
वहाँ आपको नियमित **cron** **जॉब्स**, **at** **जॉब्स** (कम प्रयोग किया जाता है) और **periodic** **जॉब्स** (मुख्य रूप से अस्थायी फ़ाइलें साफ करने के लिए प्रयोग किया जाता है) मिल सकते हैं। दैनिक periodic jobs को उदाहरण के लिए इस तरह से निष्पादित किया जा सकता है: `periodic daily`.

**उपयोगकर्ता cronjob कार्य** को प्रोग्रामेटिक रूप से जोड़ने के लिए निम्नलिखित का उपयोग किया जा सकता है:
```bash
...
* बाल्यवाद को छलने के लिए उपयोगी: [✅](https://emojipedia.org/check-mark-button)
...
यदि लोकप्रिय कार्यक्रम [**xbar**](https://github.com/matryer/xbar) स्थापित है, तो संभावना है कि एक शैल स्क्रिप्ट लिखा जा सकता है **`~/Library/Application\ Support/xbar/plugins/`** जिसे xbar शुरू होने पर क्रियान्वित किया जाएगा:
```bash
...
* **ट्रिगर**: जब Hammerspoon क्रियान्वित होता है

#### विवरण

[**Hammerspoon**](https://github.com/Hammerspoon/hammerspoon) **macOS** के लिए एक स्वचालन प्लेटफ़ॉर्म के रूप में काम करता है, जिसमें इसके कार्यों के लिए **LUA स्क्रिप्टिंग भाषा** का उपयोग किया जाता है। विशेष रूप से, यह पूरे AppleScript कोड का एकीकरण समर्थित करता है और शैल स्क्रिप्टों का क्रियान्वयन करता है, जिससे इसकी स्क्रिप्टिंग क्षमताएँ काफी बढ़ जाती हैं।

ऐप एक ही फ़ाइल, `~/.hammerspoon/init.lua`, की खोज करता है, और जब शुरू किया जाता है, तो स्क्रिप्ट क्रियान्वित होगा।
```bash
...
* बाल्यावस्था को छलने के लिए उपयोगी: [✅](https://emojipedia.org/check-mark-button)
* लेकिन ssh को सक्षम करना और उपयोग करना आवश्यक है
* TCC छलना: [✅](https://emojipedia.org/check-mark-button)
* SSH का उपयोग FDA एक्सेस के लिए करना होता है
...
* सैंडबॉक्स को छलने के लिए उपयोगी: [✅](https://emojipedia.org/check-mark-button)
* लेकिन आपको `osascript` को तार्किक सहित निष्पादित करना होगा
...
सिस्टम प्राथमिकताएँ -> उपयोगकर्ता और समूह -> **लॉगिन आइटम** में आप **उपयोगकर्ता लॉग इन करते समय निष्पादित करने के लिए आइटम** पा सकते हैं।\
...
यदि आप एक **ZIP** फ़ाइल को एक **लॉगिन आइटम** के रूप में संग्रहित करते हैं तो **`Archive Utility`** इसे खोलेगा और यदि ज़िप उदाहरण के लिए **`~/Library`** में संग्रहीत था और फ़ोल्डर **`LaunchAgents/file.plist`** शामिल था जिसमें एक बैकडोर था, तो वह फ़ोल्डर बनाया जाएगा (यह डिफ़ॉल्ट नहीं है) और प्लिस्ट जोड़ दिया जाएगा ताकि अगली बार जब उपयोगकर्ता फिर से लॉग इन करें, **प्लिस्ट में निर्दिष्ट बैकडोर कार्यान्वित होगा**।
...
* सैंडबॉक्स को छलने के लिए उपयोगी: [✅](https://emojipedia.org/check-mark-button)
* लेकिन आपको **`at`** को **क्रियाशील** करने की आवश्यकता है और यह **सक्षम** होना चाहिए
* TCC छलना: [🔴](https://emojipedia.org/large-red-circle)

#### स्थान

* आपको **`at`** को **क्रियाशील** करने की आवश्यकता है

#### **विवरण**

`at` कार्यों को **निश्चित समय पर क्रियान्वित करने** के लिए डिज़ाइन किया गया है। क्रॉन जॉब्स की तरह, `at` कार्यों को स्वचालित रूप से पोस्ट-क्रियान्वित कर दिया जाता है। यह महत्वपूर्ण है कि ये कार्य सिस्टम रीबूट के बाद भी स्थायी होते हैं, जिसे कुछ परिस्थितियों में सुरक्षा संबंधित चिंताओं के रूप में चिह्नित किया जाता है।
...
```
फ़ाइल का नाम कतार, नौकरी संख्या, और यह समय शामिल है जिसे यह चालू होने के लिए निर्धारित किया गया है। उदाहरण के लिए, चलो `a0001a019bdcd2` पर एक नज़र डालते हैं।

* `a` - यह कतार है
* `0001a` - हेक्स में नौकरी संख्या, `0x1a = 26`
* `019bdcd2` - हेक्स में समय। यह इपॉक से बीते मिनटों को प्रतिनिधित करता है। `0x019bdcd2` दस्मलवी में `26991826` है। यदि हम इसे 60 से गुणा करें तो हमें `1619509560` मिलता है, जो `GMT: 2021. अप्रैल 27, मंगलवार 7:46:00` है।

यदि हम नौकरी फ़ाइल को प्रिंट करें, हमें `at -c` का उपयोग करके प्राप्त की गई जानकारी मिलती है।
...
2. फ़ोल्डर क्रियाएँ सेटअप में एक स्क्रिप्ट मैन्युअल रूप से जोड़ना जैसे किसी फ़ोल्डर के संदर्भ मेनू में।
3. `System Events.app` के लिए Apple इवेंट संदेश भेजने के लिए OSAScript का उपयोग करना ताकि फ़ोल्डर क्रिया को प्रोग्रामेटिक रूप से सेटअप कर सकें।
...
```
धारणा करें कि स्क्रिप्ट को कंपाइल कर लिया गया है, तो नीचे दिए गए स्क्रिप्ट को निष्पादित करके फोल्डर क्रियाएँ सेट करें। यह स्क्रिप्ट फोल्डर क्रियाएँ को वैश्विक रूप से सक्षम करेगा और पिछले संकलित स्क्रिप्ट को विशेष रूप से डेस्कटॉप फोल्डर से जोड़ेगा।
```javascript
...
```
उपयोग के साथ सेटअप स्क्रिप्ट चलाएं:
```bash
osascript -l JavaScript /Users/username/attach.scpt
```
* यह उस दिशा है जिससे आप GUI के माध्यम से इस स्थायित्व को कार्यान्वित कर सकते हैं:

यह स्क्रिप्ट होगा जिसे कार्यान्वित किया जाएगा:
...
यह विन्यास **plist** में संग्रहित था जो **`~/Library/Preferences/com.apple.FolderActionsDispatcher.plist`** में base64 प्रारूप में स्थित है।
...
2. आपने अभी सेट किए गए फ़ोल्डर एक्शन्स को **हटाएं**:

<figure><img src="../.gitbook/assets/image (3) (1) (1).png" alt=""><figcaption></figcaption></figure>

अब हमारे पास एक खाली वातावरण है

3. बैकअप फ़ाइल को कॉपी करें: `cp /tmp/com.apple.FolderActionsDispatcher.plist ~/Library/Preferences/`
...
**अपने कोड के साथ एक कलर पिकर** बंडल को कंपाइल करें (आप [**उदाहरण के रूप में इसे उपयोग कर सकते हैं**](https://github.com/viktorstrate/color-picker-plus)) और एक कंस्ट्रक्टर जोड़ें (जैसे [स्क्रीन सेवर खंड](macos-auto-start-locations.md#screen-saver) में) और बंडल को `~/Library/ColorPickers` में कॉपी करें।

फिर, जब कलर पिकर ट्रिगर होता है, तो आपका भी होना चाहिए।

ध्यान दें कि आपकी लाइब्रेरी को लोड करने वाला बाइनरी **बहुत प्रतिबंधक सैंडबॉक्स** है: `/System/Library/Frameworks/AppKit.framework/Versions/C/XPCServices/LegacyExternalColorPickerService-x86_64.xpc/Contents/MacOS/LegacyExternalColorPickerService-x86_64`
```bash
...
एप्लिकेशनों में `फाइंडर सिंक एक्सटेंशन्स` हो सकती हैं। यह एक्सटेंशन एक एप्लिकेशन के अंदर जाएगी जो क्रियान्वित किया जाएगा। इसके अतिरिक्त, एक्सटेंशन अपना कोड निष्क्रिय करने के लिए **किसी वैध एप्पल डेवलपर प्रमाणपत्र के साथ साइन किया जाना चाहिए**, इसे **सैंडबॉक्स** किया जाना चाहिए (हालांकि ढीली छूटें जोड़ी जा सकती हैं) और ऐसा कुछ ऐसे के साथ पंजीकृत किया जाना चाहिए:
```bash
...
* इस्तेमाल करने के लिए उपयुक्त: [🟠](https://emojipedia.org/large-orange-circle)
...
स्पॉटलाइट macOS की अंदर बनी खोज सुविधा है, जिसका उद्देश्य उपयोगकर्ताओं को उनके कंप्यूटर पर डेटा तक तेजी से और व्यापक रूप से पहुंचने की सुविधा प्रदान करना है।\
इस त्वरित खोज क्षमता को सुविधाजनक बनाने के लिए, स्पॉटलाइट एक **स्वाधिकारी डेटाबेस** बनाए रखता है और एक सूची बनाता है जिसमें **अधिकांश फ़ाइलों को पार्स करके** एक इंडेक्स बनाता है, जिससे फ़ाइलों के नाम और उनकी सामग्री दोनों के माध्यम से त्वरित खोज की जा सके।

स्पॉटलाइट के अंतर्निहित मेकेनिज़्म में एक केंद्रीय प्रक्रिया होती है जिसका नाम 'mds' है, जो **'मेटाडेटा सर्वर'** के लिए खड़ा है। इसके साथ, कई 'mdworker' डेमन होते हैं जो विभिन्न रखरखाव कार्य करते हैं, जैसे कि विभिन्न फ़ाइल प्रकारों को इंडेक्स करना (`ps -ef | grep mdworker`). ये कार्य संभव होते हैं स्पॉटलाइट इम्पोर्टर प्लगइन्स, या **".mdimporter बंडल्स**", जो स्पॉटलाइट को विभिन्न फ़ाइल प्रारूपों के साथ सामग्री को समझने और इंडेक्स करने में सहायक होते हैं।

प्लगइन्स या **`.mdimporter`** बंडल पहले से उल्लिखित स्थानों में स्थित होते हैं और यदि एक नया बंडल दिखाई देता है तो इसे मिनटों के भीतर लोड किया जाता है (किसी सेवा को पुनः आरंभ करने की कोई आवश्यकता नहीं है)। ये बंडल्स इसे दर्शाने की आवश्यकता है कि वे किस **फ़ाइल प्रकार और एक्सटेंशन को प्रबंधित कर सकते हैं**, इस प्रकार, स्पॉटलाइट उन्हें उपयोग करेगा जब एक नयी फ़ाइल उल्लिखित एक्सटेंशन के साथ बनाई जाती है।
...
```
{% hint style="danger" %}
यदि आप अन्य `mdimporter` की Plist की जाँच करते हैं तो आपको प्रविष्टि **`UTTypeConformsTo`** नहीं मिलेगा। इसका कारण यह है कि यह एक बिल्ट-इन _यूनिफॉर्म टाइप पहचानकर्ता_ ([UTI](https://en.wikipedia.org/wiki/Uniform\_Type\_Identifier)) है और यह विस्तारों को निर्दिष्ट करने की आवश्यकता नहीं है।

इसके अतिरिक्त, सिस्टम डिफ़ॉल्ट प्लगइन हमेशा प्राथमिकता देते हैं, इसलिए एक हमलावर केवल उन फ़ाइलों तक पहुँच सकता है जो अन्यथा Apple के अपने `mdimporters` द्वारा सूचीबद्ध नहीं हैं।
{% endhint %}

अपना इम्पोर्टर बनाने के लिए आप इस परियोजना से शुरू कर सकते हैं: [https://github.com/megrimm/pd-spotlight-importer](https://github.com/megrimm/pd-spotlight-importer) और फिर नाम, **`CFBundleDocumentTypes`** बदलें और **`UTImportedTypeDeclarations`** जो आपके समर्थन करना चाहते हैं उन विस्तारों को समर्थन करने के लिए और उन्हें **`schema.xml`** में पुनर्चित्रित करें।\
फिर **`GetMetadataForFile`** फ़ंक्शन का कोड **बदलें** ताकि जब एक फ़ाइल जिस विस्तारण के साथ प्रोसेस किया गया हो, तो आपका पेलोड निष्पादित हो।

अंत में अपना नया `.mdimporter` **बनाएं और कॉपी** करें एक में से तीन पिछले स्थानों में और आप जब भी यह लोड होता है **लॉगों की मॉनिटरिंग** या **`mdimport -L`** की जांच कर सकते हैं।
...
{% hint style="success" %}
यहाँ आपको **सैंडबॉक्स छलावा** के लिए उपयोगी शुरू स्थान मिलेंगे जो आपको बस **किसी फ़ाइल में लिखकर कुछ को सीधे निष्पादित करने** की अनुमति देते हैं जो **रूट** होने पर होता है और/या अन्य **अजीब शर्तों** की आवश्यकता होती है।
{% endhint %}
...
आवधिक स्क्रिप्ट (**`/etc/periodic`**) `/System/Library/LaunchDaemons/com.apple.periodic*` में विन्यस्त **लॉन्च डेमन्स** के कारण निष्पादित होते हैं। ध्यान दें कि `/etc/periodic/` में संग्रहीत स्क्रिप्ट फ़ाइल के मालिक के रूप में **निष्पादित** किए जाते हैं, इसलिए यह किसी संभावित प्रिविलेज उन्नति के लिए काम नहीं करेगा।
{% code overflow="wrap" %}
```bash
...
वहाँ अन्य आवधिक स्क्रिप्ट हैं जो निष्पादित किए जाएंगे जैसा कि **`/etc/defaults/periodic.conf`** में दिखाया गया है:
```bash
...
PAM अधिक **स्थायित्व** और मैलवेयर पर ध्यान केंद्रित है, इसलिए इस ब्लॉग में एक विस्तृत व्याख्या नहीं दी जाएगी, **इस तकनीक को बेहतर समझने के लिए राइटअप्स को पढ़ें**।
...
```
**नियम** जोड़ने के लिए इस प्लगइन को लोड करें:
```bash
...
कॉन्फ़िग फ़ाइल **`/private/etc/man.conf`** इसका संकेत करती है कि मैन दस्तावेज़ फ़ाइलें खोलने के लिए कौन सा बाइनरी/स्क्रिप्ट उपयोग करना है। इसलिए एक्सीक्यूटेबल का पथ संशोधित किया जा सकता है ताकि हर बार जब उपयोगकर्ता किसी डॉक्स पढ़ने के लिए man का उपयोग करता है तो एक बैकडोर निष्पादित हो।
...
* TCC बाईपास: [🔴](https://emojipedia.org/large-red-circle)
* Httpd के entitlements नहीं हैं
...
इस तरह आपके संकलित मॉड्यूल Apache द्वारा लोड किए जाएंगे। एकमात्र बात यह है कि आपको इसे **एक मान्य Apple प्रमाणपत्र के साथ साइन करना होगा**, या फिर आपको सिस्टम में एक नया विश्वसनीय प्रमाणपत्र **जोड़ना** होगा और उसके साथ **साइन करना** होगा।

फिर, यदि आवश्यक हो, सर्वर को सुनिश्चित करने के लिए आपको निम्नलिखित को निष्पादित करना हो सकता है:
```bash
...
जबकि auditd एक चेतावनी का पता लगाता है, तो स्क्रिप्ट **`/etc/security/audit_warn`** **क्रियान्वित** होती है। इसलिए आप अपने पेलोड को इस पर जोड़ सकते हैं।
```bash
...
**StartupItem** एक निर्दिष्ट डायरेक्टरी है जो या तो `/Library/StartupItems/` या `/System/Library/StartupItems/` के भीतर स्थित होना चाहिए। एक बार जब यह डायरेक्टरी स्थापित होती है, तो इसमें दो विशिष्ट फ़ाइलें होनी चाहिए:
...
सुनिश्चित करें कि एरसी स्क्रिप्ट और `StartupParameters.plist` फ़ाइल दोनों सही ढंग से **StartupItem** डायरेक्टरी के भीतर स्थापित हैं ताकि स्टार्टअप प्रक्रिया उन्हें पहचान और उपयोग कर सके।
...
उदाहरण के लिए, यदि आप एक नया सुपर सेवा जोड़ना चाहते हैं, तो आपको `/Library/LaunchDaemons` या `/Library/LaunchAgents` में एक नया `.plist` फ़ाइल बनानी होगी। इसके बाद, आपको इस `.plist` फ़ाइल में अपनी सेवा को कैसे चालू करना है, इसकी जानकारी डालनी होगी।  
{% endtab %}
```bash
#!/bin/sh
. /etc/rc.common
...
{% hint style="danger" %}
मैं अपने macOS में इस कॉम्पोनेंट को नहीं ढूंढ पा रहा हूँ, इसके लिए अधिक जानकारी के लिए राइटअप देखें
{% endhint %}

राइटअप: [https://theevilbit.github.io/beyond/beyond\_0023/](https://theevilbit.github.io/beyond/beyond\_0023/)

एप्पल द्वारा पेश किया गया, **ईमोंड** एक लॉगिंग मेकेनिज़्म है जो अविकसित या संभावित तौर पर छोड़ दिया गया लगता है, फिर भी यह उपलब्ध है। एक मैक व्यवस्थापक के लिए विशेष रूप से फायदेमंद नहीं होता, यह अप्रसिद्ध सेवा एक धीमी स्थिरता विधि के रूप में काम कर सकती है जो धमाकेदार कलाकारों द्वारा उपयुक्त हो सकती है, जिसे अधिकांश macOS व्यवस्थापक नहीं देखेंगे।

इसके अस्तित्व के जानकारों के लिए, **ईमोंड** का किसी भी दुरुपयोग की पहचान सीधी है। इस सेवा के लिए सिस्टम का लॉन्चडेमन स्क्रिप्ट को एक ही निर्देशिका में निष्पादित करने की तलाश में होता है। इसे जांचने के लिए, निम्नलिखित कमांड का उपयोग किया जा सकता है:
```bash
...
* `/Library/Extensions`
* तीसरे पक्ष के सॉफ़्टवेयर द्वारा इंस्टॉल की गई केक्स्ट फ़ाइलें

आप वर्तमान में लोड केक्स्ट फ़ाइलों की सूची देख सकते हैं:
```bash
...
```
अधिक जानकारी के लिए [**कर्नेल एक्सटेंशन्स की जांच करें इस खंड में**](macos-security-and-privilege-escalation/mac-os-architecture/#i-o-kit-drivers).
...
जैसा कि प्रतीत होता है, `/System/Library/LaunchAgents/com.apple.amstoold.plist` से `plist` इस बाइनरी का उपयोग कर रहा था जबकि एक एक्सपीसी सेवा को उजागर कर रहा था... बात यह है कि बाइनरी मौजूद नहीं था, इसलिए आप कुछ वहाँ रख सकते थे और जब एक्सपीसी सेवा को कॉल किया जाता है तो आपकी बाइनरी को कॉल किया जाएगा।

मुझे अब अपने macOS में इसे नहीं मिल सकता।
...
ऐसा लगता है कि इस स्क्रिप्ट को चलाना बहुत सामान्य नहीं है और मुझे अपने macOS में यह भी नहीं मिला, इसलिए अगर आप अधिक जानकारी चाहते हैं तो लेख की जांच करें।
