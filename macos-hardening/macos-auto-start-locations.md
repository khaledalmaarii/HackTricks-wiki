...
{% hint style="success" %}
यहाँ आपको सैंडबॉक्स बाइपास के लिए उपयोगी स्टार्ट स्थान मिलेंगे जो आपको केवल कुछ को **एक फ़ाइल में लिखकर** और बहुत **सामान्य क्रिया** के लिए **प्रतीक्षा** करके **कुछ को निष्पादित करने** की अनुमति देता है, एक निर्धारित **समय की मात्रा** या एक **क्रिया जो आप सामान्य रूप से सैंडबॉक्स के अंदर से बिना रूट अनुमतियों की आवश्यकता के कुछ कर सकते हैं**।
{% endhint %}
...
**`launchd`** ओएक्स एस कर्नेल द्वारा स्टार्टअप पर पहला **प्रक्रिया** है और शट डाउन पर अंतिम होने वाला है। इसे हमेशा **पीआईडी 1** होना चाहिए। यह प्रक्रिया **ASEP** **प्लिस्ट** में निर्दिष्ट की गई विन्यासों को **पढ़े और निष्पादित** करेगा:
...
जब एक उपयोगकर्ता लॉग इन करता है, तो `/Users/$USER/Library/LaunchAgents` और `/Users/$USER/Library/LaunchDemons` में प्लिस्ट मौजूद होते हैं और **लॉग इन करने वाले उपयोगकर्ताओं की अनुमतियों** के साथ शुरू होते हैं।
...
{% hint style="info" %}
नए डेमन या एजेंट कॉन्फ़िग फ़ाइलें **अगले रीबूट के बाद या** `launchctl load <target.plist>` **का उपयोग करके लोड की जाएंगी**। **इसके अलावा, .plist फ़ाइलें बिना उस एक्सटेंशन के भी लोड किया जा सकता है** `launchctl -F <file>` (हालांकि वे plist फ़ाइलें रीबूट के बाद स्वचालित रूप से लोड नहीं होंगी)।\
इसे `launchctl unload <target.plist>` के साथ **अनलोड** करना भी संभव है (इसके द्वारा इसे संकेत किया गया प्रक्रिया समाप्त हो जाएगी)।

सुनिश्चित करने के लिए कि कोई **भी चीज़** (जैसे ओवरराइड) **किसी एजेंट या डेमन को चलाने से रोक रही है**, यह सुनिश्चित करें: `sudo launchctl load -w /System/Library/LaunchDaemos/com.apple.smdb.plist`
{% endhint %}
...
* लेकिन आपको एक एप्लिकेशन ढूंढने की आवश्यकता है जिसमें TCC बायपास हो जो इन फ़ाइलों को लोड करता है
...
{% hint style="danger" %}
सूचित अनुशासन को कॉन्फ़िगर करना और लॉगआउट और लॉगइन या बूट करना मेरे लिए काम नहीं किया एप्लिकेशन को निष्पादित करने के लिए। (एप्लिकेशन नहीं चल रहा था, शायद यह चालू होना चाहिए जब ये कार्रवाई की जाती है)
{% endhint %}
...
* **ट्रिगर**: एप्लिकेशनों को फिर से खोलने की पुनरारंभ करें
...
[...]
```
इसलिए, अगर सिस्टम में टर्मिनल की प्राथमिकताओं की plist को अधिलेखित किया जा सकता है, तो **`open`** कार्यक्षमता का उपयोग करके **टर्मिनल खोली जा सकती है और उस कमांड को क्रियान्वित किया जाएगा**।
...
* TCC छलना: [✅](https://emojipedia.org/check-mark-button)
* उपयोगकर्ता को उसका उपयोग करने के लिए टर्मिनल को एफडीए अनुमतियाँ होती हैं
...
QuickLook प्लगइन्स को आप जब आप एक फ़ाइल का पूर्वावलोकन ट्रिगर करते हैं (फाइंडर में फ़ाइल का चयन करके स्पेस बार दबाएं) और एक **फ़ाइल प्रकार का समर्थन करने वाला प्लगइन** स्थापित होता है, तो उसे निष्पादित किया जा सकता है।
...
{% hint style="danger" %}
यह मेरे लिए काम नहीं किया, न तो उपयोगकर्ता लॉगइनहुक और न ही रूट लॉगआउटहुक
{% endhint %}
...
{% hint style="success" %}
यहाँ आपको **सैंडबॉक्स बाईपास** के लिए उपयोगी स्टार्ट स्थान मिलेंगे जो आपको बस किसी चीज को **एक फ़ाइल में लिखकर** एक्सीक्यूट करने की अनुमति देते हैं और **अपेक्षा नहीं करते हैं कि विशेष रूप से "असामान्य" प्रोग्राम** स्थापित हों, "असामान्य" उपयोगकर्ता क्रियाएँ या परिवेश।
{% endhint %}
...
* सीधी लेखन पहुंच के लिए रूट की आवश्यकता है। यदि आप `crontab <file>` को एक्सीक्यूट कर सकते हैं तो रूट की आवश्यकता नहीं है
...
crontab -l
```
आप **`/usr/lib/cron/tabs/`** और **`/var/at/tabs/`** में उपयोगकर्ताओं के सभी cron jobs देख सकते हैं (रूट की आवश्यकता है)।
...
**प्रोग्रामेटिक रूप से एक उपयोगकर्ता cronjob जोड़ने** के लिए निम्नलिखित का प्रयोग किया जा सकता है:
```bash
...
* बालूबक्स को छलने के लिए उपयोगी: [✅](https://emojipedia.org/check-mark-button)
* TCC बायपास: [✅](https://emojipedia.org/check-mark-button)
...
```
iTerm2 प्राथमिकताएं **`~/Library/Preferences/com.googlecode.iterm2.plist`** में स्थित होती हैं जो **इसका संकेत दे सकती हैं कि कौन सा कमांड निष्पादित करने के लिए** जब iTerm2 टर्मिनल खोला जाता है।
...
यदि लोकप्रिय कार्यक्रम [**xbar**](https://github.com/matryer/xbar) स्थापित है, तो संभावना है कि एक शैल स्क्रिप्ट लिखा जा सकता है **`~/Library/Application\ Support/xbar/plugins/`** जिसे xbar शुरू होने पर क्रियान्वित किया जाए:
```bash
...
* **ट्रिगर**: जब Hammerspoon को क्रियान्वित किया जाता है

#### विवरण

[**Hammerspoon**](https://github.com/Hammerspoon/hammerspoon) **macOS** के लिए एक स्वचालन प्लेटफ़ॉर्म के रूप में काम करता है, जिसमें इसके परिचालन के लिए **LUA स्क्रिप्टिंग भाषा** का उपयोग किया जाता है। विशेष रूप से, यह पूरे AppleScript कोड का एकीकरण समर्थन करता है और शैल स्क्रिप्टों का क्रियान्वयन करता है, जिससे इसकी स्क्रिप्टिंग क्षमताएँ काफी बढ़ जाती हैं।
...
* TCC बायपास: [✅](https://emojipedia.org/check-mark-button)
* यह Automation-Shortcuts और Accessibility अनुमतियां मांगता है

#### स्थान

* `~/Library/Application Support/BetterTouchTool/*`

यह उपकरण इसका संकेत देने की अनुमति देता है कि किसी ऐप्लिकेशन या स्क्रिप्ट को किसी शॉर्टकट को दबाने पर क्रियान्वित करने के लिए। एक हमलावर अपने खुद के **शॉर्टकट और क्रिया को डेटाबेस में क्रियान्वित करने के लिए** कॉन्फ़िगर कर सकता है ताकि यह विचारशील कोड को क्रियान्वित करे (एक शॉर्टकट केवल एक कुंजी दबाने के लिए हो सकता है)।
...
* TCC बायपास: [✅](https://emojipedia.org/check-mark-button)
* यह Automation, Accessibility और फुल-डिस्क एक्सेस अनुमतियां मांगता है

#### स्थान

* `???`

यह वर्कफ़्लो बनाने की अनुमति देता है जो कि निश्चित स्थितियों पर कोड को क्रियान्वित कर सकता है। संभावना है कि एक हमलावर एक वर्कफ़्लो फ़ाइल बना सकता है और Alfred को इसे लोड करने के लिए बना सकता है (वर्कफ़्लो का उपयोग करने के लिए प्रीमियम संस्करण का उपयोग करना आवश्यक है)।
...
डिफ़ॉल्ट रूप से, जब तक `/etc/ssh/sshd_config` में `PermitUserRC no` न हो, जब एक उपयोगकर्ता **SSH के माध्यम से लॉगिन** करता है, स्क्रिप्ट **`/etc/ssh/sshrc`** और **`~/.ssh/rc`** क्रियान्वित हो जाएंगे।
...
अगर आप एक **ZIP** फ़ाइल को एक **लॉगिन आइटम** के रूप में स्टोर करते हैं तो **`Archive Utility`** इसे खोलेगा और यदि ज़िप उदाहरण के लिए **`~/Library`** में स्टोर किया गया था और फ़ोल्डर **`LaunchAgents/file.plist`** शामिल था जिसमें एक बैकडोर था, तो वह फ़ोल्डर बनाया जाएगा (यह डिफ़ॉल्ट नहीं है) और प्लिस्ट जोड़ दिया जाएगा ताकि अगली बार जब उपयोगकर्ता फिर से लॉग इन करें, **प्लिस्ट में दिखाए गए बैकडोर को निष्पादित किया जाएगा**।
...
* लेकिन आपको **`at`** को **चालू** करना होगा और यह **सक्षम** होना चाहिए
* TCC बायपास: [🔴](https://emojipedia.org/large-red-circle)

#### स्थान

* आपको **`at`** को **चालू** करना होगा और यह **सक्षम** होना चाहिए

#### **विवरण**

`at` tasks को **निश्चित समय पर निष्पादित करने के लिए निर्धारित** किया गया है। क्रॉन जॉब्स की तरह, `at` tasks को स्वचालित रूप से पोस्ट-निष्पादित कर दिया जाता है। यह महत्वपूर्ण है कि ये tasks डिफ़ॉल्ट रूप से **अक्षम** होते हैं लेकिन **रूट** उपयोगकर्ता इन्हें **सक्षम** कर सकते हैं:
```bash
...
```
फ़ाइल का नाम कतार, नौकरी संख्या, और यह समय शामिल है जिसे यह निर्धारित किया गया है कि कब चलाया जाएगा। उदाहरण के लिए, हम `a0001a019bdcd2` पर एक नज़र डालते हैं।
...
3. `System Events.app` के लिए Apple इवेंट संदेश भेजने के लिए OSAScript का उपयोग करना।
...
```
धारणा करें कि स्क्रिप्ट को कंपाइल कर दिया गया है, नीचे दिए गए स्क्रिप्ट को निष्पादित करके फोल्डर क्रियाएँ सेट करें। यह स्क्रिप्ट फोल्डर क्रियाएँ को वैश्विक रूप से सक्षम करेगा और पिछले संकलित स्क्रिप्ट को विशेष रूप से डेस्कटॉप फोल्डर से जोड़ देगा।
```javascript
...
यह वह स्क्रिप्ट है जो कार्यान्वित किया जाएगा:
...
अब, यदि आप **Finder** के साथ उस फ़ोल्डर को खोलते हैं, तो आपका स्क्रिप्ट चलाया जाएगा।
...
1. **`~/Library/Preferences/com.apple.FolderActionsDispatcher.plist`** की बैकअप के लिए `/tmp` में **`cp ~/Library/Preferences/com.apple.FolderActionsDispatcher.plist /tmp`** कॉपी करें।
2. आपने अभी सेट किए गए फ़ोल्डर एक्शन को **हटाएं**:
...
डॉक में जो एप्लिकेशन दिखाई देते हैं, वे सभी plist में निर्दिष्ट हैं: **`~/Library/Preferences/com.apple.dock.plist`**
...
फाइंडर सिंक एक्सटेंशन के साथ एक एप्लिकेशन उदाहरण [**यहाँ पाया जा सकता है**](https://github.com/D00MFist/InSync).
...
{% hint style="danger" %}
ध्यान दें कि इस कोड को लोड करने वाले बाइनरी की entitlements में (`/System/Library/Frameworks/ScreenSaver.framework/PlugIns/legacyScreenSaver.appex/Contents/MacOS/legacyScreenSaver`) आपको **`com.apple.security.app-sandbox`** मिलेगा इसलिए आप **सामान्य एप्लिकेशन सैंडबॉक्स के अंदर होंगे**।
{% endhint %}
...
स्पॉटलाइट के अंतर्निहित मेकेनिज़्म में एक केंद्रीय प्रक्रिया है जिसका नाम 'mds' है, जो 'मेटाडेटा सर्वर' के लिए खड़ा है। यह प्रक्रिया स्पॉटलाइट सेवा को संचालित करती है। इसके साथ, कई 'mdworker' डेमन हैं जो विभिन्न रखरखाव कार्य करते हैं, जैसे कि विभिन्न फ़ाइल प्रकारों को इंडेक्स करना (`ps -ef | grep mdworker`). ये कार्य स्पॉटलाइट इम्पोर्टर प्लगइन्स या **".mdimporter बंडल्स**" के माध्यम से संभव होते हैं, जो स्पॉटलाइट को विभिन्न फ़ाइल प्रारूपों के साथ सामग्री को समझने और इंडेक्स करने की अनुमति देते हैं।

प्लगइन्स या **`.mdimporter`** बंडल पहले से उल्लिखित स्थानों में स्थित हैं और यदि एक नया बंडल दिखाई देता है तो इसे मिनटों के भीतर लोड किया जाता है (किसी भी सेवा को पुनः आरंभ करने की आवश्यकता नहीं है)। ये बंडल्स इंडिकेट करने की आवश्यकता होती है कि वे कौन से **फ़ाइल प्रकार और एक्सटेंशन को प्रबंधित कर सकते हैं**, इस प्रकार, स्पॉटलाइट उन्हें उपयोग करेगा जब एक नयी फ़ाइल उल्लिखित एक्सटेंशन के साथ बनाई जाती है।

सभी `mdimporters` को खोजना संभव है जोड़कर चल रहा है:
```bash
mdimport -L
Paths: id(501) (
...
```
{% hint style="danger" %}
यदि आप अन्य `mdimporter` की Plist की जाँच करते हैं तो आपको प्रविष्टि **`UTTypeConformsTo`** नहीं मिलेगा। इसका कारण यह है कि यह एक बिल्ट-इन _यूनिफॉर्म टाइप पहचानकर्ता_ ([UTI](https://en.wikipedia.org/wiki/Uniform\_Type\_Identifier)) है और इसे विस्तारित करने की आवश्यकता नहीं है।

इसके अतिरिक्त, सिस्टम डिफ़ॉल्ट प्लगइन हमेशा प्राधानता देते हैं, इसलिए एक हमलावर केवल उन फ़ाइलों तक पहुँच सकता है जिन्हें एप्पल के अपने `mdimporters` द्वारा अन्यथा सूचीबद्ध नहीं किया गया है।
{% endhint %}

अपना खुद का इम्पोर्टर बनाने के लिए आप इस परियोजना से शुरू कर सकते हैं: [https://github.com/megrimm/pd-spotlight-importer](https://github.com/megrimm/pd-spotlight-importer) और फिर नाम, **`CFBundleDocumentTypes`** बदलें और **`UTImportedTypeDeclarations`** जो आपके समर्थन करना चाहते हैं उन एक्सटेंशन को समर्थन करने के लिए जोड़ें और उन्हें **`schema.xml`** में पुनरावृत्त करें।\
फिर **`GetMetadataForFile`** फ़ंक्शन को बदलें ताकि जब प्रोसेस किए गए एक्सटेंशन वाली फ़ाइल बनाई जाए तो आपका पेलोड निष्पादित हो।
...
{% hint style="success" %}
यहाँ आपको **सैंडबॉक्स छलावा** के लिए उपयोगी शुरू स्थान मिलेंगे जो आपको बस **किसी फ़ाइल में लिखकर कुछ निष्पादित करने** की अनुमति देते हैं जो **रूट** होती है और/या अन्य **अजीब स्थितियों की आवश्यकता** होती है।
{% endhint %}
...
आवधिक स्क्रिप्ट (**`/etc/periodic`**) `/System/Library/LaunchDaemons/com.apple.periodic*` में कॉन्फ़िगर किए गए **लॉन्च डेमन्स** के कारण निष्पादित होते हैं। ध्यान दें कि `/etc/periodic/` में स्टोर किए गए स्क्रिप्ट्स **फ़ाइल के मालिक के रूप में निष्पादित** होते हैं, इसलिए यह किसी संभावित प्रिविलेज उन्नति के लिए काम नहीं करेगा।
{% code overflow="wrap" %}
```bash
...
अन्य नियमित स्क्रिप्ट हैं जो क्रियान्वित किए जाएंगे जो **`/etc/defaults/periodic.conf`** में निर्दिष्ट किए गए हैं:
```bash
...
PAM अधिक **स्थायित्व** और मैलवेयर पर ध्यान केंद्रित है जो macOS में आसान निष्पादन पर है, इस ब्लॉग में इस तकनीक को बेहतर समझने के लिए विस्तृत व्याख्या नहीं दी जाएगी, **लेखन को समझने के लिए लेखन पढ़ें**।
...
Launch Agents are used to run code at login or when a user logs in. They are stored in the following locations:

- **Global Launch Agents**: `/Library/LaunchAgents`
- **Per-User Launch Agents**: `~/Library/LaunchAgents`

#### Launch Daemons

Launch Daemons are used to run code at boot or when a system starts up. They are stored in the following locations:

- **Global Launch Daemons**: `/Library/LaunchDaemons`
- **Per-User Launch Daemons**: Not applicable

#### Startup Items

Startup Items are deprecated and no longer used in macOS.
```bash
auth       sufficient     pam_permit.so
```
इसका **रूप ऐसा होगा** कुछ इस प्रकार:
```bash
...
आप एक अधिकृति प्लगइन बना सकते हैं जो एक उपयोगकर्ता लॉग-इन करते समय निष्क्रिय किया जाएगा। इन प्लगइन्स में से एक बनाने के बारे में अधिक जानकारी के लिए पिछले लेखों की जाँच करें (और सावधान रहें, एक खराब लिखा हुआ प्लगइन आपको बाहर लॉक कर सकता है और आपको अपने मैक को पुनर्प्राप्ति मोड से साफ करने की आवश्यकता होगी)।
```objectivec
...
```
**`evaluate-mechanisms`** अधिकृति फ्रेमवर्क को बताएगा कि इसे **अधिकृति के लिए बाह्य तंत्र को कॉल करने की आवश्यकता होगी**। इसके अतिरिक्त, **`privileged`** इसे रूट द्वारा निष्पादित करने के लिए करेगा।
...
```
डायनामिक लोडेड बाइनरी (Dylib) के लिए कोड उदाहरण:
```objectivec
#include <stdio.h>
#include <syslog.h>
...
जबकि auditd एक चेतावनी का पता लगाता है, तो स्क्रिप्ट **`/etc/security/audit_warn`** **निष्पादित** होती है। इसलिए आप इस पर अपने पेलोड जोड़ सकते हैं।
```bash
...
**StartupItem** एक निर्दिष्ट डायरेक्टरी है जो या तो `/Library/StartupItems/` या `/System/Library/StartupItems/` के भीतर स्थित होना चाहिए। एक बार यह डायरेक्टरी स्थापित होती है, तो इसमें दो विशिष्ट फ़ाइलें होनी चाहिए:
...
{% tab title="superservicename" %}सुपर सेवा नाम{% endtab %}
```bash
#!/bin/sh
. /etc/rc.common
...
Apple द्वारा पेश किया गया, **emond** एक लॉगिंग मेकेनिज़्म है जो अविकसित या संभावित तौर पर छोड़ दिया गया लगता है, फिर भी यह उपलब्ध है। एक Mac प्रशासक के लिए विशेष रूप से फायदेमंद नहीं होता है, यह अप्रसिद्ध सेवा खतरे के कारकों के लिए एक सूक्ष्म स्थिरता विधि के रूप में काम कर सकती है, जिसे अधिकांश macOS प्रशासक शायद न देखें।

इसके अस्तित्व के जानकार लोगों के लिए **emond** का किसी भी दुरुपयोग की पहचान सरल है। इस सेवा के लिए सिस्टम का लॉन्चडेमन स्क्रिप्ट को एक ही निर्देशिका में निष्पादित करने की तलाश में होता है। इसे जांचने के लिए, निम्नलिखित कमांड का उपयोग किया जा सकता है:
```bash
...
{% hint style="danger" %}
केक्स्ट स्थापित करना इतना जटिल है कि मैं इसे सैंडबॉक्स से बाहर निकलने या स्थिरता के लिए नहीं मानता (जब तक आपके पास कोई शोषण नहीं है)
{% endhint %}

#### स्थान

केक्स्ट को स्टार्टअप आइटम के रूप में स्थापित करने के लिए, इसे निम्नलिखित स्थानों में स्थापित किया जाना चाहिए:

* `/System/Library/Extensions`
* ओएस एक्स ऑपरेटिंग सिस्टम में बिल्ट इन केक्स्ट फ़ाइलें।
* `/Library/Extensions`
* तृतीय पक्ष सॉफ़्टवेयर द्वारा स्थापित केक्स्ट फ़ाइलें

आप वर्तमान में लोड केक्स्ट फ़ाइलें की सूची बना सकते हैं:
```bash
...
ऐसा लगता है कि इस स्क्रिप्ट को चलाना बहुत आम नहीं है और मुझे अपने macOS में यह भी नहीं मिला, इसलिए अगर आप अधिक जानकारी चाहते हैं तो लेख की जाँच करें।
