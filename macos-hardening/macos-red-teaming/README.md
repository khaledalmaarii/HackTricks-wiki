...
如果你设法**获取管理员凭证**以访问管理平台，你可以通过在机器上分发你的恶意软件来**潜在地危害所有计算机**。

在 MacOS 环境中进行红队行动时，强烈建议了解 MDMs 的工作原理：
...
为了运行你自己的 MDM，你需要**由供应商签名的 CSR**，你可以尝试通过 [**https://mdmcert.download/**](https://mdmcert.download/) 获取。并且，为了运行你自己的苹果设备的 MDM，你可以使用 [**MicroMDM**](https://github.com/micromdm/micromdm)。

然而，要在已注册的设备上安装应用程序，你仍然需要它由开发者账户签名... 但是，在 MDM 注册时，**设备会将 MDM 的 SSL 证书添加为受信任的 CA**，所以你现在可以签名任何东西。

要在 MDM 中注册设备，你需要以 root 身份安装 **`mobileconfig`** 文件，可以通过 **pkg** 文件交付（你可以将其压缩成 zip，在从 safari 下载时它将被解压缩）。

**Mythic 代理 Orthrus** 使用了这种技术。

### 滥用 JAMF PRO

JAMF 可以运行**自定义脚本**（由系统管理员开发的脚本）、**原生负载**（创建本地账户、设置 EFI 密码、文件/进程监控...）和 **MDM**（设备配置、设备证书...）。

#### JAMF 自助注册

访问如 `https://<company-name>.jamfcloud.com/enroll/` 的页面，查看他们是否**启用了自助注册**。如果启用了，可能会**要求凭证以访问**。

你可以使用脚本 [**JamfSniper.py**](https://github.com/WithSecureLabs/Jamf-Attack-Toolkit/blob/master/JamfSniper.py) 来执行密码喷涂攻击。

此外，在找到合适的凭证后，你可以使用以下表单来暴力破解其他用户名：
...
**`jamf`** 二进制文件包含了打开钥匙链的秘密，当时的发现是**共享**给每个人的，那就是：**`jk23ucnq91jfu9aj`**。\
...
因此，攻击者可以放置一个恶意包（`pkg`），在安装时**覆盖这个文件**，将**URL设置为Mythic C2监听器，用于Typhon代理**，现在可以滥用JAMF作为C2。
...
* **JAMF 密钥链**位于：`/Library/Application\ Support/Jamf/JAMF.keychain`，其中包含设备证书
...
你也可以监控位置 `/Library/Application Support/Jamf/tmp/`，管理员可能想通过Jamf执行的**自定义脚本**会**放在这里，执行后删除**。这些脚本**可能包含凭证**。
...
* [**Bifrost**](https://github.com/its-a-feature/bifrost): Bifrost是一个旨在与macOS上的Heimdal krb5 API交互的Objective-C项目。该项目的目标是使用原生API在macOS设备上进行更好的Kerberos安全测试，无需在目标上安装任何其他框架或包。
...
例如，名为 _mark_ 的用户信息存储在 _/var/db/dslocal/nodes/Default/users/mark.plist_ 中，而组 _admin_ 的信息在 _/var/db/dslocal/nodes/Default/groups/admin.plist_ 中。
...
MacOS红队行动与常规Windows红队行动不同，通常**MacOS会直接与多个外部平台集成**。MacOS的常见配置是使用**OneLogin同步凭证访问计算机，并通过OneLogin访问多个外部服务**（如github, aws等）：
