...
* **Keychain systemowy** (`/Library/Keychains/System.keychain`), który przechowuje **dane uwierzytelniające na poziomie systemowym**, takie jak hasła do WiFi, certyfikaty root systemu, prywatne klucze systemowe oraz hasła do aplikacji systemowych.
...
* Jeśli określono **teamid**, to aby **uzyskać dostęp do wartości pozycji** bez **komunikatu**, używana aplikacja musi mieć **ten sam teamid**.
...
{% hint style="success" %}
**Wyliczenie i wyciek** sekretów z **keychaina**, które **nie generują monitu**, można wykonać za pomocą narzędzia [**LockSmith**](https://github.com/its-a-feature/LockSmith)
{% endhint %}

Lista i uzyskanie **informacji** o każdym wpisie w keychainie:

* API **`SecItemCopyMatching`** udziela informacji o każdym wpisie i istnieją pewne atrybuty, które można ustawić podczas jego użycia:
* **`kSecReturnData`**: Jeśli jest ustawione na true, spróbuje odszyfrować dane (ustaw na false, aby uniknąć potencjalnych okienek)
* **`kSecReturnRef`**: Uzyskaj również odniesienie do elementu keychaina (ustaw na true, jeśli później zobaczysz, że możesz odszyfrować bez okienka)
...
Uzyskaj **ACLs** dla każdego wpisu:

* Za pomocą API **`SecAccessCopyACLList`** można uzyskać **ACL dla elementu keychaina**, a zwróci listę ACL (takich jak `ACLAuhtorizationExportClear` i inne wcześniej wymienione), gdzie każda lista zawiera:
* Opis
...
* Potrzebne są odpowiednie **uprawnienia** (**`Nil`**, lub być **częścią** listy dozwolonych aplikacji w autoryzacji dostępu do informacji o sekrecie)
