...
Dans iOS, il se trouve dans **`/System/Library/Caches/com.apple.kernelcaches/kernelcache`** ; dans macOS, vous pouvez le trouver avec **`find / -name kernelcache 2>/dev/null`** ou **`mdfind kernelcache | grep kernelcache`**

Il est possible d'exécuter **`kextstat`** pour vérifier les extensions de noyau chargées.
...
Parfois, Apple publie le **kernelcache** avec les **symboles**. Vous pouvez télécharger certains firmwares avec des symboles en suivant les liens sur [https://theapplewiki.com](https://theapplewiki.com/).
