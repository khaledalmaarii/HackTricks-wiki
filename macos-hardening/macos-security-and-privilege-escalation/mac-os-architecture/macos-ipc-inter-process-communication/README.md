...
lsmp -p <pid>
```
**ghItlh** **iOS** **tool** **install** **jatlh** **[http://newosxbook.com/tools/binpack64-256.tar.gz ](http://newosxbook.com/tools/binpack64-256.tar.gz)**

### **Code** **example**

**Qap** **sender** **port** **allocate** **jatlh**, **send right** **name** `org.darlinghq.example` **create** **jatlh** **ghItlh** **bootstrap server** **ghItlh** **sender** **send right** **name** **ghItlh** **message** **send** **jatlh**.
...
int main(int argc, char** argv) {
    ...
    mach_msg_header_t* msg = (mach_msg_header_t*)malloc(sizeof(mach_msg_header_t) + strlen(message) + 1);
    ...
    memcpy((char*)msg + sizeof(mach_msg_header_t), message, strlen(message) + 1);
    ...
    // Deallocate the send right for the server port
    kr = mach_port_deallocate(mach_task_self(), server_port);
    if (kr != KERN_SUCCESS) {
        printf("Failed to deallocate send right for server port: %s\n", mach_error_string(kr));
        return 1;
    }
...
```
{% tab title="entitlements.plist" %}entitlements.plist{% endtab %}
```xml
...
**QaSvIp** vItlhutlh **thread** **process** Hijacking:
...
XPC, **XNU** (macOS **kernel**) **inter-Process Communication** **framework** **communication** **processes** **macOS** **iOS**. XPC **mechanism** **safe, asynchronous** **method calls** **processes** **system**. **Apple's security paradigm**, **creation** **privilege-separated applications** **component** **permissions** **needs** **job**, **limiting** **potential damage** **compromised process**.

**communication work** **vulnerable** **check**:
...
MIG **creation** **simplify** **Mach IPC** **code**. **generates** **needed code** **server** **client** **communicate** **definition**. **generated code** **ugly**, **developer** **import** **code** **simpler**.
