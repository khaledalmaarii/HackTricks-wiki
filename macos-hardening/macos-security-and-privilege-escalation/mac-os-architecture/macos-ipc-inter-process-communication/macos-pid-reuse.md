...
当 macOS 的 **XPC 服务**基于 **PID** 而不是 **审计令牌**来检查被调用的进程时，它就容易受到 PID 重用攻击。这种攻击基于一种 **竞争条件**，其中一个 **利用**将会 **滥用** 功能并在此之后执行 **`posix_spawn(NULL, target_binary, NULL, &attr, target_argv, environ)`** 的 **漏洞**，向 XPC 服务发送消息。

这个函数将使得 **允许的二进制文件拥有 PID**，但是 **恶意的 XPC 消息将在此之前被发送**。因此，如果 **XPC** 服务在执行 **`posix_spawn`** 之后使用 **PID** 来 **验证**发送者并在检查之后，它将认为它来自一个 **授权的** 进程。
...
* 一个**生成多个子进程**的部分
* **每个子进程**在发送消息后立即执行 **`posix_spawn`**，将负载发送到 XPC 服务。
