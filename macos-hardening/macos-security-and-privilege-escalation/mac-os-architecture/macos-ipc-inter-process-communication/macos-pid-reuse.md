...
当 macOS 的 **XPC 服务**基于 **PID** 而不是 **审计令牌** 来检查被调用的进程时，它容易受到 PID 重用攻击的影响。这种攻击基于一种 **竞争条件**，其中一个 **利用** 将会 **滥用** 功能并在此之后执行 **`posix_spawn(NULL, target_binary, NULL, &attr, target_argv, environ)`** 的 **漏洞** 将会 **向 XPC 服务发送消息**。

这个函数将使得 **允许的二进制文件拥有 PID**，但是 **恶意的 XPC 消息将在此之前被发送**。因此，如果 **XPC** 服务在执行 **`posix_spawn`** 之后使用 **PID** 来 **验证** 发送者并在验证之后检查它，它将认为它来自一个 **授权的** 进程。
...
* 一个 **生成多个 fork** 的部分
* **每个 fork** 在发送消息后立即执行 **`posix_spawn`** 将负载发送到 XPC 服务。
...
{% tabs %}
{% tab title="NSTasks" %}
第一种使用 **`NSTasks`** 和参数来启动子进程以利用 RC 的选项
```objectivec
...
```
{% tab title="fork" %}
这个例子使用原始的 **`fork`** 来启动**利用 PID 竞争条件的子进程**，然后通过**硬链接利用另一个竞争条件**：
```objectivec
...
* 你在一个**网络安全公司**工作吗？想要在HackTricks中**宣传你的公司**吗？或者你想要**获取最新版本的PEASS或下载PDF格式的HackTricks**吗？请查看[**订阅计划**](https://github.com/sponsors/carlospolop)！
