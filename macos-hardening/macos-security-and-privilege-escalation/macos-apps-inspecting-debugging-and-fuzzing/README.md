...
Die gereedskap kan gebruik word as 'n **vervanging** vir **codesign**, **otool**, en **objdump**, en bied 'n paar addisionele kenmerke. [**Laai dit hier af**](http://www.newosxbook.com/tools/jtool.html) of installeer dit met `brew`.
```bash
# Install
...
Hierdie hulpmiddel maak dit moontlik om Apple skyfbeelders (**.dmg**) lêers te **mount** om hulle te inspekteer voordat enige iets uitgevoer word:
```bash
...
{% hint style="danger" %}
Let daarop dat programme geskryf in Objective-C hul klaskondigings behou wanneer dit saamgestel word in [Mach-O binaêre lêers](../macos-files-folders-and-binaries/universal-binaries-and-mach-o-format.md). Sulke klaskondigings sluit die naam en tipe van in:
{% endhint %}
...
Wanneer 'n funksie in 'n binêre lêer geroep word wat Objective-C gebruik, sal die gekompileerde kode in plaas daarvan daardie funksie roep, **`objc_msgSend`** roep. Dit sal die finale funksie roep:
...
* Die tweede parameter, (**op**), is "die kieser van die metode wat die boodskap hanteer". Weereens, meer eenvoudig gestel, is dit net die **naam van die metode**.
...
Met Swift binêre lêers, aangesien daar Objective-C verenigbaarheid is, kan jy soms verklarings onttrek met behulp van [class-dump](https://github.com/nygard/class-dump/) maar nie altyd nie.

Met die **`jtool -l`** of **`otool -l`** opdragreëls is dit moontlik om verskeie afdelings te vind wat begin met die **`__swift5`** voorvoegsel:
```bash
...
{% hint style="warning" %}
Let daarop dat om bineêre lêers te ontleed, **SIP uitgeskakel moet word** (`csrutil disable` of `csrutil enable --without debug`) of om die bineêre lêers na 'n tydelike vouer te kopieer en die handtekening te **verwyder** met `codesign --remove-signature <binary-path>` of om die ontleed van die bineêre lêer toe te laat (jy kan hierdie skripsie gebruik](https://gist.github.com/carlospolop/a66b8d72bb8f43913c4b5ae45672578b))
{% endhint %}

{% hint style="warning" %}
Let daarop dat om **sisteem bineêre lêers** (soos `cloudconfigurationd`) op macOS te **instrumenteer**, **SIP uitgeskakel moet word** (net die handtekening verwyder sal nie werk nie).
{% endhint %}
...
In die linker paneel van hopper is dit moontlik om die simbole (**Etikette**) van die bineêre lêer, die lys van prosedures en funksies (**Proc**) en die strings (**Str**) te sien. Dit is nie al die strings nie, maar diegene wat gedefinieer is in verskeie dele van die Mac-O-lêer (soos _cstring of `objc_methname`).
...
Dit gee gebruikers toegang tot toepassings op 'n uiters **lae vlak** en bied 'n manier vir gebruikers om **programme te volg** en selfs hul uitvoervloei te verander. Dtrace gebruik **sondes** wat **deur die hele kernel geplaas is** en is op plekke soos die begin en einde van sisteemaanrope.

DTrace gebruik die **`dtrace_probe_create`**-funksie om 'n sonde vir elke sisteemaanroep te skep. Hierdie sonde kan in die **ingangs- en uitgangspunt van elke sisteemaanroep** afgevuur word. Die interaksie met DTrace vind plaas deur /dev/dtrace wat slegs beskikbaar is vir die rootgebruiker.

{% hint style="success" %}
Om Dtrace te aktiveer sonder om SIP-beskerming heeltemal uit te skakel, kan jy in herstelmodus uitvoer: `csrutil enable --without dtrace`
...
**Kyk na die volgende bladsy** om uit te vind hoe jy kan bepaal watter toepassing verantwoordelik is vir **die hantering van die gespesifiseerde skema of protokol:**
...
Dit werk eenvoudig met macOS GUI-hulpmiddels. Let op dat sommige macOS-toepassings spesifieke vereistes het soos unieke lêernaam, die regte uitbreiding, moet die lêers lees vanuit die sandboks (`~/Library/Containers/com.apple.Safari/Data`)...
