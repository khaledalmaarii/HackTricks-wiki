...
Αυτοί οι εγκαταστάτες έχουν σενάρια `preinstall` και `postinstall` bash που οι δημιουργοί κακόβουλου λογισμικού συνήθως καταχρώνται για να **διατηρήσουν** το **κακόβουλο λογισμικό**.
...
{% hint style="danger" %}
Σημειώστε ότι τα προγράμματα που γράφονται σε Objective-C **διατηρούν** τις δηλώσεις των κλάσεών τους **όταν** **μεταγλωττίζονται** σε [δυαδικά Mach-O](../macos-files-folders-and-binaries/universal-binaries-and-mach-o-format.md). Τέτοιες δηλώσεις κλάσεων περιλαμβάνουν το όνομα και τον τύπο των:
{% endhint %}
...
Όταν μια συνάρτηση καλείται σε ένα δυαδικό αρχείο που χρησιμοποιεί Objective-C, ο μεταγλωττισμένος κώδικας αντί να καλεί αυτήν τη συνάρτηση, θα καλέσει το **`objc_msgSend`**. Το οποίο θα καλεί την τελική συνάρτηση:

![](<../../../.gitbook/assets/image (305).png>)

Τα ορίσματα που αναμένει αυτή η συνάρτηση είναι:

* Το πρώτο όρισμα (**self**) είναι "ένας δείκτης που δείχνει στην **πρόσβαση της κλάσης που πρόκειται να λάβει το μήνυμα**". Ή απλά, είναι το αντικείμενο πάνω στο οποίο καλείται η μέθοδος. Εάν η μέθοδος είναι μια μέθοδος κλάσης, αυτό θα είναι ένα αντικείμενο του αντικειμένου της κλάσης (συνολικά), ενώ για μια μέθοδο παρουσίασης, το self θα δείχνει σε μια επιτεθείσα περίπτωση της κλάσης ως αντικείμενο.
...
Δείτε πώς μπορείτε **να λάβετε αυτές τις πληροφορίες εύκολα με το `lldb` σε ARM64** σε αυτήν τη σελίδα:
...
[**Dynadump**](https://github.com/DerekSelander/dynadump) είναι ένα εργαλείο για να λάβετε Objc-Classes από dylibs.
...
Με τις γραμμές εντολών **`jtool -l`** ή **`otool -l`** είναι δυνατό να βρείτε πολλές ενότητες που ξεκινούν με πρόθεμα **`__swift5`**:
```bash
...
[...]
```
Μπορείτε να βρείτε περισσότερες πληροφορίες σχετικά με τις [**πληροφορίες που αποθηκεύονται σε αυτήν την ενότητα σε αυτήν την ανάρτηση στο blog**](https://knight.sc/reverse%20engineering/2019/07/17/swift-metadata.html).
...
* Ο συμπικνωτής UPX για MacOS δημιουργεί μια ενότητα που ονομάζεται "\_\_XHDR"

## Δυναμική Ανάλυση

{% hint style="warning" %}
Σημείωση ότι για να αποσφραγίσετε δυαδικά αρχεία για αποσφράγιση, **πρέπει να απενεργοποιηθεί το SIP** (`csrutil disable` ή `csrutil enable --without debug`) ή να αντιγράψετε τα δυαδικά αρχεία σε ένα προσωρινό φάκελο και να **αφαιρέσετε την υπογραφή** με `codesign --remove-signature <διαδρομή-δυαδικού>` ή να επιτρέψετε την αποσφράγιση του δυαδικού (μπορείτε να χρησιμοποιήσετε [αυτό το σενάριο](https://gist.github.com/carlospolop/a66b8d72bb8f43913c4b5ae45672578b))
{% endhint %}

{% hint style="warning" %}
Σημείωση ότι για να **εργαλεύσετε δυαδικά συστήματος**, (όπως `cloudconfigurationd`) στο macOS, **πρέπει να απενεργοποιηθεί το SIP** (απλή αφαίρεση της υπογραφής δεν λειτουργεί).
{% endhint %}

### APIs

Το macOS εκθέτει μερικές ενδιαφέρουσες APIs που παρέχουν πληροφορίες σχετικά με τις διεργασίες:

* `proc_info`: Αυτό είναι το κύριο που δίνει πολλές πληροφορίες για κάθε διεργασία. Χρειάζεται να είστε ριζοχρήστης για να λάβετε πληροφορίες για άλλες διεργασίες, αλλά δεν χρειάζεστε ειδικά δικαιώματα ή mach ports.
...
Πρέπει να εκτελείται ως **ριζοχρήστης** και το daemon `/usr/libexec/sysdiagnosed` έχει πολύ ενδιαφέρουσες εγγυήσεις όπως `com.apple.system-task-ports` και `get-task-allow`.

Το plist του βρίσκεται στο `/System/Library/LaunchDaemons/com.apple.sysdiagnose.plist` το οποίο δηλώνει 3 MachServices:

* `com.apple.sysdiagnose.CacheDelete`: Διαγράφει παλιά αρχεία καταχωρήσεων στο /var/rmp
...
Επιπλέον, υπάρχουν κάποια αρχεία καταγραφής που θα περιέχουν την ετικέτα `<ιδιωτικό>` για να **αποκρύψουν** ορισμένες **προσωπικές** ή **αναγνωρίσιμες** **πληροφορίες χρήστη** ή **υπολογιστή**. Ωστόσο, είναι δυνατόν να **εγκαταστήσετε ένα πιστοποιητικό για να αποκαλύψετε αυτές τις πληροφορίες**. Ακολουθήστε τις εξηγήσεις από [**εδώ**](https://superuser.com/questions/1532031/how-to-show-private-data-in-macos-unified-log).

### Hopper

#### Αριστερό πλαίσιο

Στο αριστερό πλαίσιο του hopper είναι δυνατό να δείτε τα σύμβολα (**Ετικέτες**) του δυαδικού, τη λίστα των διαδικασιών και συναρτήσεων (**Διαδικασίες**) και τις συμβολοσειρές (**Συμβολοσειρές**). Δεν είναι όλες οι συμβολοσειρές αλλά αυτές που έχουν οριστεί σε διάφορα σημεία του αρχείου Mac-O (όπως _cstring ή_ `objc_methname`).
...
Κάνοντας δεξί κλικ σε ένα αντικείμενο κώδικα μπορείτε να δείτε **αναφορές προς/από αυτό το αντικείμενο** ή ακόμα και να αλλάξετε το όνομά του (αυτό δεν λειτουργεί στο αποδιασυναρμολογημένο ψευδοκώδικα):
...
```
Το όνομα της έρευνας αποτελείται από τέσσερα μέρη: ο πάροχος, το module, η λειτουργία και το όνομα (`fbt:mach_kernel:ptrace:entry`). Αν δεν καθορίσετε κάποιο μέρος του ονόματος, το Dtrace θα το εφαρμόσει ως μέρος μπαλακτέτ.
...
Για να αλληλεπιδράσετε με το `kdebug` χρησιμοποιείται το `sysctl` μέσω του namespace `kern.kdebug` και οι MIBs που πρέπει να χρησιμοποιηθούν βρίσκονται στο `sys/sysctl.h` με τις λειτουργίες υλοποιημένες στο `bsd/kern/kdebug.c`.

Για να αλληλεπιδράσετε με το kdebug με έναν προσαρμοσμένο πελάτη, συνήθως ακολουθούν τα παρακάτω βήματα:
...
Χρησιμοποιείται για προφίλινγκ σε επίπεδο πυρήνα και δημιουργείται χρησιμοποιώντας κλήσεις `Kdebug`.

Βασικά, ελέγχεται η καθολική μεταβλητή `kernel_debug_active` και αν είναι ενεργή, καλείται η `kperf_kdebug_handler` με τον κώδικα `Kdebug` και τη διεύθυνση του πυρήνα που καλείται. Αν ο κώδικας `Kdebug` ταιριάζει με έναν επιλεγμένο, λαμβάνει τις "ενέργειες" που έχουν διαμορφωθεί ως bitmap (ελέγξτε τις επιλογές στο `osfmk/kperf/action.h`).

Το Kperf έχει επίσης έναν πίνακα MIB sysctl: (ως ριζικός χρήστης) `sysctl kperf`. Αυτοί οι κώδικες μπορούν να βρεθούν στο `osfmk/kperf/kperfbsd.c`.
...
* Μπορείτε να ελέγξετε αν η συνάρτηση **`sysctl`** ή **`ptrace`** είναι **εισαγόμενη** (αλλά το malware θα μπορούσε να την εισάγει δυναμικά)
...
Λειτουργεί με εργαλεία GUI του macOS. Σημειώστε ότι μερικές εφαρμογές macOS έχουν συγκεκριμένες απαιτήσεις όπως μοναδικά ονόματα αρχείων, τη σωστή επέκταση, ανάγκη να διαβάζουν τα αρχεία από το sandbox (`~/Library/Containers/com.apple.Safari/Data`)...
