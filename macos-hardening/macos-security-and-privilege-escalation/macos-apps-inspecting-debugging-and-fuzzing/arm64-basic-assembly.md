...
* EL1 को EL0 से अधिक विशेषाधिकार प्राप्त होते हैं और यह सिस्टम संसाधनों तक पहुँच सकता है, लेकिन सिस्टम अखंडता सुनिश्चित करने के लिए कुछ प्रतिबंधों के साथ।
...
4. **`x16`** और **`x17`** - **इंट्रा-प्रोसीजरल कॉल रजिस्टर्स**। तत्काल मानों के लिए अस्थायी रजिस्टर्स। इनका उपयोग अप्रत्यक्ष फंक्शन कॉल्स और PLT (प्रोसीजर लिंकेज टेबल) स्टब्स के लिए भी किया जाता है।
...
* यह रजिस्टर **सामान्य-उद्देश्य रजिस्ट्री** के रूप में भी इस्तेमाल किया जा सकता है हालांकि यह आमतौर पर **स्थानीय चरों** क
```armasm
...
यह **इंटरवर्किंग शाखा निर्देशों** के दौरान सेट किया जाता है, लेकिन जब PC को गंतव्य रजिस्टर के रूप में सेट किया जाता है तो अन्य निर्देशों के साथ सीधे भी सेट किया जा सकता है। उदाहरण:
...
16 32-बिट रजिस्टर्स (r0-r15) होते हैं। **r0 से r14 तक** वे **किसी भी ऑपरेशन** के लिए इस्तेमाल किए जा सकते हैं, हालांकि कुछ आमतौर पर आरक्षित रहते हैं:
...
इसके अलावा, रजिस्टर्स **`बैंक्ड रजिस्ट्रीज`** में बैक अप होते हैं। ये वे स्थान हैं जो रजिस्टर्स के मूल्यों को संग्रहित करते हैं जिससे अपवाद संभालने और विशेषाधिकार प्राप्त ऑपरेशनों में **तेजी से संदर्भ स्विचिंग** करने की अनुमति मिलती है ताकि हर बार मैन्युअली रजिस्टर्स को सेव और रिस्टोर करने की आवश्यकता न हो।\
यह **`CPSR` से प्रोसेसर की स्थिति को `SPSR`** में सेव करके किया जाता है, जो प्रोसेसर मोड के लिए होता है जिसमें अपवाद लिया जाता है। अपवाद वापसी पर, **`CPSR`** **`SPSR`** से बहाल किया जाता है।
...
* **`Q`** ध्वज: जब भी **पूर्णांक संतृप्ति होती है** विशेष संतृप्ति अंकगणितीय निर्देश के निष्पादन के दौरान, इसे 1 पर सेट किया जाता है। एक बार जब यह **`1`** पर सेट हो जाता है, तो यह मूल्य को तब तक बनाए रखेगा जब तक कि इसे मैन्युअली 0 पर सेट नहीं किया जाता। इसके अलावा, कोई भी निर्देश इसके मूल्य की जांच स्वतः नहीं करता है, इसे मैन्युअली पढ़कर किया जाना चाहिए।
...
**`SEL`** निर्देश इन GE ध्वजों का उपयोग करके शर्तात्मक क्रियाएं करता है।
...
[**syscall_sw.c**](https://opensource.apple.com/source/xnu/xnu-3789.1.32/osfmk/kern/syscall_sw.c.auto.html) देखें। Mach traps में **x16 < 0** होगा, इसलिए आपको पिछली सूची से संख्याओं को **माइनस के साथ** कॉल करने की आवश्यकता है: **`_kernelrpc_mach_vm_allocate_trap`** है **`-10`**।

आप **`libsystem_kernel.dylib`** को एक डिससेंबलर में भी देख सकते हैं ताकि पता चल सके कि इन (और BSD) syscalls को कैसे कॉल किया जाए:
```bash
# macOS
...
```
{% hint style="success" %}
कभी-कभी **`libsystem_kernel.dylib`** से **डीकंपाइल** किया गया कोड देखना **सोर्स कोड** की जांच करने से आसान होता है क्योंकि कई सिसकॉल्स (BSD और Mach) के कोड स्क्रिप्ट्स के माध्यम से जनरेट किए जाते हैं (सोर्स कोड में टिप्पणियों की जांच करें) जबकि dylib में आप देख सकते हैं कि क्या कॉल किया जा रहा है।
{% endhint %}
...
लक्ष्य `execve("/bin/cat", ["/bin/cat", "/etc/passwd"], NULL)` को निष्पादित करना है, इसलिए दूसरा तर्क (x1) पैरामीटर्स का एक ऐरे है (जो मेमोरी में इसका मतलब पतों का एक स्टैक होता है)।
```armasm
...
```
#### sh के साथ कमांड इन्वोक करें एक फोर्क से ताकि मुख्य प्रक्रिया मारी न जाए
```armasm
.section __TEXT,__text     ; Begin a new section of type __TEXT and name __text
.global _main              ; Declare a global symbol _main
.align 2                   ; Align the beginning of the following code to a 4-byte boundary
