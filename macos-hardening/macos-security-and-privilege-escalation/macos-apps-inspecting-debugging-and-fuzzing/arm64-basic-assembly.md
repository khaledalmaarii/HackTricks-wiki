...
2. **`x8`** - Nel kernel Linux, `x8` è utilizzato come numero di chiamata di sistema per l'istruzione `svc`. **In macOS è il x16 quello utilizzato!**
3. **`x9`** a **`x15`** - Altri registri temporanei, spesso utilizzati per variabili locali.
4. **`x16`** e **`x17`** - **Registri di Chiamata Intra-procedurale**. Registri temporanei per valori immediati. Sono anche utilizzati per chiamate a funzioni indirette e stub PLT (Procedure Linkage Table).
...
6. **`x19`** a **`x28`** - Questi sono registri salvati dal chiamante. Una funzione deve preservare i valori di questi registri per il chiamante, quindi vengono memorizzati nello stack e recuperati prima di tornare al chiamante.
7. **`x29`** - **Puntatore al Frame** per tenere traccia del frame dello stack. Quando viene creato un nuovo frame dello stack perché viene chiamata una funzione, il registro **`x29`** viene **memorizzato nello stack** e l'indirizzo del **nuovo** frame (**indirizzo `sp`**) viene **memorizzato in questo registro**.
...
* il valore di **`sp`** dovrebbe sempre essere mantenuto almeno con un **allineamento a quadriword** o potrebbe verificarsi un'eccezione di allineamento.
...
**Ci sono centinaia di registri di sistema**, anche chiamati registri speciali (SPR), utilizzati per **monitorare** e **controllare** il **comportamento dei processori**.\
Possono essere letti o impostati solo utilizzando le istruzioni speciali dedicate **`mrs`** e **`msr`**.

I registri speciali **`TPIDR_EL0`** e **`TPIDDR_EL0`** sono comunemente trovati nel reverse engineering. Il suffisso `EL0` indica la **minima eccezione** dalla quale il registro può essere accessibile (in questo caso EL0 è il livello di eccezione regolare con cui i programmi regolari vengono eseguiti).\
Sono spesso utilizzati per memorizzare l'**indirizzo base della regione di memoria dello storage locale del thread**. Di solito il primo è leggibile e scrivibile per i programmi in esecuzione in EL0, ma il secondo può essere letto da EL0 e scritto da EL1 (come il kernel).

* `mrs x0, TPIDR_EL0 ; Leggi TPIDR_EL0 in x0`
* `msr TPIDR_EL0, X0 ; Scrivi x0 in TPIDR_EL0`
### **PSTATE**

**PSTATE** contiene diversi componenti del processo serializzati nel registro speciale **`SPSR_ELx`** visibile dal sistema operativo, essendo X il livello di **permesso dell'eccezione attivata** (questo permette di ripristinare lo stato del processo quando l'eccezione termina).\
...
* Nella sottrazione, quando un grande numero negativo viene sottratto da un numero positivo più piccolo (o viceversa), e il risultato non può essere rappresentato all'interno dell'intervallo della dimensione dei bit fornita.
* Ovviamente il processore non sa se l'operazione è con segno o meno, quindi controllerà C e V nelle operazioni e indicherà se si è verificato un riporto nel caso fosse con segno o senza.
...
* Il **livello di eccezione attuale** (**`EL`**): Un programma normale in esecuzione in EL0 avrà il valore 0
* Il flag di **singolo passo** (**`SS`**): Usato dai debugger per eseguire un passo alla volta impostando il flag SS su 1 all'interno di **`SPSR_ELx`** tramite un'eccezione. Il programma eseguirà un passo e emetterà un'eccezione di passo singolo.
* Il flag di stato di eccezione **illegale** (**`IL`**): Viene utilizzato per segnalare quando un software privilegiato esegue un trasferimento di livello di eccezione non valido, questo flag viene impostato su 1 e il processore attiva un'eccezione di stato illegale.
...
* Se **`A`** è 1 significa che verranno attivate le **interruzioni asincrone**. **`I`** configura la risposta alle **Richieste di Interruzione Hardware** esterne (IRQs) e F è relativo alle **Richieste di Interruzione Rapida** (FIRs).
* I flag di selezione del **puntatore dello stack (`SPS`)**: I programmi privilegiati in esecuzione in EL1 e superiori possono passare dall'utilizzo del proprio registro del puntatore dello stack a quello del modello utente (ad es. tra `SP_EL1` e `EL0`). Questo passaggio viene eseguito scrivendo nel registro speciale **`SPSel`**. Questo non può essere fatto da EL0.
...
* `str lr, [sp, #-4]!`, Memorizza il registro del link in sp e aggiorna il registro sp
...
* **Shift aritmetico a destra**: Come **`lsr`**, ma invece di aggiungere 0 se il bit più significativo è 1, vengono aggiunti 1 (divide n volte per 2 in firmato)
...
* **`bfm`**: **Spostamento di bit di campo**, queste operazioni **copiano i bit `0...n`** da un valore e li collocano nelle posizioni **`m..m+n`**. Il **`#s`** specifica la posizione del **bit più a sinistra** e il **`#r`** la **quantità di rotazione a destra**.
* Spostamento di bit: `BFM Xd, Xn, #r`
* Spostamento di bit firmato: `SBFM Xd, Xn, #r, #s`
* Spostamento di bit non firmato: `UBFM Xd, Xn, #r, #s`
* **Estrazione e inserimento di bitfield:** Copia un bitfield da un registro e lo copia in un altro registro.
...
* **Estendi segno a X:** Estende il segno (o aggiunge solo 0 nella versione non firmata) di un valore per poter eseguire operazioni con esso:
...
* **`b.ne`**: **Salta se non uguale**. Questa istruzione controlla i flag di condizione (che sono stati impostati da un'istruzione di confronto precedente) e se i valori confrontati non erano uguali, salta a un'etichetta o a un indirizzo.
* Esempio: Dopo un'istruzione `cmp x0, x1`, `b.ne label` — Se i valori in `x0` e `x1` non erano uguali, questo salta a `label`.
* **`cbz`**: **Confronta e salta se zero**. Questa istruzione confronta un registro con zero e, se sono uguali, salta a un'etichetta o a un indirizzo.
* Esempio: `cbz x0, label` — Se il valore in `x0` è zero, questo salta a `label`.
* **`cbnz`**: **Confronta e salta se non zero**. Questa istruzione confronta un registro con zero e, se non sono uguali, salta a un'etichetta o a un indirizzo.
* Esempio: `cbnz x0, label` — Se il valore in `x0` non è zero, salta a `label`.
* **`tbnz`**: Test del bit e salto se diverso da zero
* Esempio: `tbnz x0, #8, label`
* **`tbz`**: Test del bit e salto se zero
...
* **`ldrsw`**: **Carica** un valore firmato **di 32 bit** dalla memoria e lo **estende a 64 bit**.
...
* **`stur`**: **Memorizza un valore del registro in una posizione di memoria**, utilizzando un offset da un altro registro.
...
- `AIF`: Alcune eccezioni possono essere disabilitate utilizzando i bit `A`, `I`, `F`. Se `A` è 1 significa che verranno attivati **aborti asincroni**. Il `I` configura la risposta alle **richieste di interruzioni hardware esterne** (IRQ), e il F è relativo alle **richieste di interruzione veloce** (FIR).
```bash
# macOS
