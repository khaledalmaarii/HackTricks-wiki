...
* Οι εφαρμογές που τρέχουν στο EL0 είναι απομονωμένες μεταξύ τους και από το λογισμικό συστήματος, βελτιώνοντας την ασφάλεια και τη σταθερότητα.
...
* Αυτό είναι το πιο προνομιούχο επίπεδο και χρησιμοποιείται συχνά για ασφαλή εκκίνηση και περιβάλλοντα εκτέλεσης εμπιστευτικών εφαρμογών.
...
Η χρήση αυτών των επιπέδων επιτρέπει μια δομημένη και ασφαλή τρόπο διαχείρισης διαφορετικών πτυχών του συστήματος, από εφαρμογές χρηστών έως το πιο προνομιούχο λογισμικό συστήματος. Η προσέγγιση της ARMv8 στα επίπεδα προνομίων βοηθά στην απομόνωση διαφορετικών συστατικών του συστήματος, βελτιώνοντας έτσι την ασφάλεια και την ανθεκτικότητα του συστήματος.
...
8. **`x30`** ή **`lr`**- **Καταχωρητής Συνδέσμου**. Κρατά
### **PSTATE**

**PSTATE** περιέχει αρκετά στοιχεία διεργασίας που έχουν σειριοποιηθεί στο ειδικό μητρώο **`SPSR_ELx`** που είναι ορατό στο λειτουργικό σύστημα, με το Χ να είναι το **επίπεδο άδειας του πυροδοτηθέντος** εξαίρεσης (αυτό επιτρέπει την ανάκτηση της κατάστασης της διεργασίας όταν η εξαίρεση τελειώνει).\
Αυτά είναι τα προσβάσιμα πεδία:

<figure><img src="../../../.gitbook/assets/image (724).png" alt=""><figcaption></figcaption></figure>

* Τα σημαία **`N`**, **`Z`**, **`C`** και **`V`**:
...
* Στην αφαίρεση, όταν από ένα μικρότερο θετικό αριθμό αφαιρείται ένας μεγαλύτερος αρνητικός αριθμός (ή αντίστροφα), και το αποτέλεσμα δεν μπορεί να αναπαρασταθεί εντός του εύρους του δοθέντος μεγέθους bit.
* Φυσικά ο επεξεργαστής δεν γνωρίζει αν η λειτουργία είναι με πρόσημο ή όχι, οπότε θα ελέγξει τα C και V στις λειτουργίες και θα υποδείξει αν υπήρξε μεταφορά σε περίπτωση που ήταν με πρόσημο ή χωρίς.

{% hint style="warning" %}
Όχι όλες οι εντολές ενημερώνουν αυτές τις σημαίες. Κάποιες όπως **`CMP`** ή **`TST`** το κάνουν, και άλλες που έχουν κατάληξη s όπως **`ADDS`** το κάνουν επίσης.
{% endhint %}

* Η τρέχουσα σημαία **πλάτους (`nRW`)**: Αν η σημαία κρατά την τιμή 0, το πρόγραμμα θα εκτελείται στην κατάσταση εκτέλεσης AArch64 μόλις επαναφερθεί.
* Το τρέχον **Επίπεδο Εξαίρεσης** (**`EL`**): Ένα κανονικό πρόγραμμα που εκτελείται στο EL0 θα έχει την τιμή 0
* Η σημαία **μονής βήματος** (**`SS`**): Χρησιμοποιείται από debuggers για μονή βήματος με την ρύθμιση της σημαίας SS σε 1 μέσα στο **`SPSR_ELx`** μέσω μιας εξαίρεσης. Το πρόγραμμα θα εκτελέσει ένα βήμα και θα εκδώσει μια εξαίρεση μονής βήματος.
* Η σημαία κατάστασης παράνομης εξαίρεσης (**`IL`**): Χρησιμοποιείται για να επισημάνει όταν ένα προνομιούχο λογισμικό εκτελεί μια μη έγκυρη μεταφορά επιπέδου εξαίρεσης, αυτή η σημαία ορίζεται σε 1 και ο επεξεργαστής προκαλεί μια παράνομη κατάσταση εξαίρεσης.
...
* Οι σημαίες επιλογής δείκτη στοίβας (**`SPS`**): Τα προνομιούχα προγράμματα που εκτελούνται στο EL1 και πάνω μπορούν να αλλάζουν μεταξύ της χρήσης του δικού τους καταχωρητή δείκτη στοίβας και του καταχωρητή μοντέλου χρήστη (π.χ. μεταξύ `SP_EL1` και `EL0`). Αυτή η αλλαγή γίνεται με την εγγραφή στο ειδικό μητρώο **`SPSel`**. Αυτό δεν μπορεί να γίνει από το EL0.

## **Σύμβαση Κλήσης (ARM64v8)**

Η σύμβαση κλήσης ARM64 καθορίζει ότι οι **πρώτες οκτώ παράμετροι** μιας συνάρτησης περνιούνται στους καταχωρητές **`x0` έως `x7`**. **Επιπλέον** παράμετροι περνιούνται στη **στοίβα**. Η τιμή επιστροφής περνιέται πίσω στον καταχωρητή **`x0`**, ή επίσης στον **`x1`** εάν είναι μήκους 128 bits. Οι καταχωρητές **`x19`** έως **`x30`** και **`sp`** πρέπει να **διατηρηθούν** μεταξύ κλήσεων συναρτήσεων.

Όταν διαβάζετε μια συνάρτηση σε συναρμολόγηση, ψάξτε για το **προλόγιο και επίλογο** της συνάρτησης. Το **προλόγιο** συνήθως περιλαμβάνει την **αποθήκευση του δείκτη πλαισίου (`x29`)**, την **ρύθμιση** ενός **νέου δείκτη πλαισίου**, και την **κατανομή χώρου στοίβας**. Το **επίλογο** συνήθως περιλαμβάνει την **επαναφορά του αποθηκευμένου δείκτη πλαισίου** και την **επιστροφή** από τη συνάρτηση.
...
Οι εντολές ARM64 γενικά έχουν τη μορφή `opcode dst, src1, src2`, όπου το **`opcode`** είναι η **λειτουργία** που θα εκτελεστεί (όπως `add`, `sub`, `mov`, κλπ.), το **`dst`** είναι ο καταχωρητής **προορισμού** όπου θα αποθηκευτεί το αποτέλεσμα, και τα **`src1`** και **`src2`** είναι οι καταχωρητές **πηγή**. Μπορούν επίσης να χρησιμοποιηθούν τιμές άμεσα αντί για καταχωρητές πηγής.

* **`mov`**: **Μετακίνηση** μιας τιμής από έναν **καταχωρητή** σε έναν άλλο.
* Παράδειγμα: `mov x0, x1` — Αυτό μετακινεί την τιμή από το `x1` στο `x0`.
* **`ldr`**: **Φόρτωση** μιας τιμής από τη **μνήμη** σε έναν **καταχωρητή**.
* Παράδειγμα: `ldr x0, [x1]` — Αυτό φορτώνει μια τιμή από τη θέση μνήμης που δείχνει το `x1` στο `x0`.
* **Λειτουργία μετατόπισης**: Υπάρχει ένας μετατοπισμός που επηρεάζει τον δείκτη προέλευσης, για παράδειγμα:
...
* **Λειτουργία προ-ευρετηρίου**: Αυτό θα εφαρμόσει υπολογισμούς στην αρχική θέση, θα πάρει το αποτέλε
...
* **`div`**: **Διαίρεση** της τιμής ενός καταχωρητή με έναν άλλο και αποθήκευση του αποτελέσματος σε έναν καταχωρητή.
* Παράδειγμα: `div x0, x1, x2` — Αυτό διαιρεί την τιμή στο `x1` με το `x2` και αποθηκεύει το αποτέλεσμα στο `x0`.
* **`lsl`**, **`lsr`**, **`asr`**, **`ror`, `rrx`**:
* **Λογική μετατόπιση αριστερά**: Προσθέτει 0 από το τέλος μετακινώντας τα υπόλοιπα bits προς τα εμπρός (πολλαπλασιασμός κατά n φορές 2)
* **Λογική μετατόπιση δεξιά**: Προσθέτει 1 στην αρχή μετακινώντας τα υπόλοιπα bits προς τα πίσω (διαίρεση κατά n φορές 2 σε μη υπογεγραμμένο)
* **Αριθμητική μετατόπιση δεξιά**: Όπως το **`lsr`**, αλλά αντί να προσθέτει 0s αν το πιο σημαντικό bit είναι 1, προστίθενται 1s (διαίρεση κατά n φορές 2 σε υπογεγραμμένο)
* **Περιστροφή δεξιά**: Όπως το **`lsr`** αλλά ό,τι αφαιρείται από τα δεξιά προστίθεται στα αριστερά
* **Περιστροφή Δεξιά με Επέκταση**: Όπως το **`ror`**, αλλά με τη σημαία μεταφοράς ως το "πιο σημαντικό bit". Έτσι, η σημαία μεταφοράς μετακινείται στο bit 31 και το αφαιρούμενο bit στη σημαία μεταφοράς.
* **`bfm`**: **Μετακίνηση Πεδίου Μπιτ**, αυτές οι λειτουργίες **αντιγράφουν τα bits `0...n`** από μια τιμή και τα τοποθετούν σε θέσεις **`m..m+n`**. Το **`#s`** καθορίζει τη θέση του **αριστερότερου bit** και το **`#r`** την **ποσότητα περιστροφής δεξιά**.
* Μετακίνηση πεδίου μπιτ: `BFM Xd, Xn, #r`
* Μετακίνηση πεδίου μπιτ με υπογραφή: `SBFM Xd, Xn, #r, #s`
* Μετακίνηση πεδίου μπιτ χωρίς υπογραφή: `UBFM Xd, Xn, #r, #s`
* **Εξαγωγή και Εισαγωγή Πεδίου Μπιτ:** Αντιγράφει ένα πεδίο μπιτ από έναν καταχωρητή και το αντιγράφει σε έναν άλλο καταχωρητή.
...
* **`SBFIZ X1, X2, #3, #4`** Επέκταση με υπόγραμμα 4 bits από το X2 και εισαγωγή τους στο X1 ξεκινώντας από τη θέση bit 3 μηδενίζοντας τα δεξιά bits
* **`SBFX X1, X2, #3, #4`** Εξάγει 4 bits ξεκινώντας από το bit 3 από το X2, επεκτείνει το υπόγραμμα τους και τοποθετεί το αποτέλεσμα στο X1
* **`UBFIZ X1, X2, #3, #4`** Επέκταση μηδενικού με υπόγραμμα 4 bits από το X2 και εισαγωγή τους στο X1 ξεκινώντας από τη θέση bit 3 μηδενίζοντας τα δεξιά bits
* **`UBFX X1, X2, #3, #4`** Εξάγει 4 bits ξεκινώντας από το bit 3 από το X2 και τοποθετεί το αποτέλεσμα με επέκταση μηδενικού στο X1.
* **Επέκταση Υπογράμμισης Σε X:** Επεκτείνει το υπόγραμμα (ή προσθέτει μόνο 0s στη μη υπογεγραμμένη έκδοση) μιας τιμής για να είναι δυνατές οι λειτουργίες με αυτή:
* **`SXTB X1, W2`** Επεκτείνει το υπόγραμμα ενός byte **από W2 σε X1** (`W2` είναι το μισό του `X2`) για να γεμίσει τα 64 bits
* **`SXTH X1, W2`** Επεκτείνει το υπόγραμμα ενός 16bit αριθμού **από W2 σε X1** για να γεμίσει τα 64 bits
* **`SXTW X1, W2`** Επεκτείνει το υπόγραμμα ενός byte **από W2 σε X1** για να γεμίσει τα 64 bits
* **`UXTB X1, W2`** Προσθέτει 0s (μη υπογεγραμμένο) σε ένα byte **από W2 σε X1** για να γεμίσει τα 64 bits
...
* **`cmp`**: **Σύγκριση** δύο καταχωρητών και ρύθμιση σημαίας συνθηκών. Είναι ένας **ψευδώνυμο του `subs`** ρυθμίζοντας τον καταχωρητή προορισμού στο μηδενικό καταχωρητή. Χρήσιμο για να γνωρίζετε αν `m == n`.
...
* **`cmn`**: **Σύγκριση αρνητικού** όρου. Σε αυτήν την περίπτωση είναι ένας **ψευδώνυμο του `adds`** και υποστηρίζει την ίδια σύνταξη. Χρήσιμο για να γνωρίζετε αν `m == -n`.
* **`ccmp`**: Συνθήκες σύγκρισης, είναι μια σύγκριση που θα πραγματοποιηθεί μόνο αν μια προηγούμενη σύγκριση ήταν αληθής και θα ορίσει ειδικά τα bits nzcv.
* `cmp x1, x2; ccmp x3, x4, 0, NE; blt _func` -> αν x1 != x2 και x3 < x4, μετάβαση στη λειτουργία
* Αυτό είναι επειδή το **`ccmp`** θα εκτελεστεί μόνο αν το **προηγούμενο `cmp` ήταν ένα `NE`**, αν δεν ήταν, τα bits `nzcv` θα οριστούν σε 0 (που δεν θα ικανοποιήσει τη σύγκριση `blt`).
* Αυτό μπορεί επίσης να χρησιμοποιηθεί ως `ccmn` (ίδιο αλλά αρνητικό, όπως `cmp` έναντι `cmn`).
* **`tst`**: Ελέγχει αν οι τιμές της σύγκρισης είναι και οι δύο 1 (λειτουργεί σαν και ΑΝΔ χωρίς να αποθηκεύει το αποτέλεσμα οπουδήποτε). Είναι χρήσιμο για να ελέγξετε ένα καταχωρητή με μια τιμή και να ελέγξετε αν κάποια από τα bits του καταχωρητή που υποδεικνύονται στην τιμή είναι 1.
* Παράδειγμα: `tst X1, #7` Ελέγξτε αν κάποιο από τα τελευταία 3 bits του X1 είναι 1
* **`teq`**: Λειτουργία XOR απορρίπτοντας το αποτέλεσμα
* **`b`**: Ανυπόθετη Αλλαγή
* Παράδειγμα: `b myFunction`&#x20
* Παράδειγμα: `cbnz x0, label` — Αν η τιμή στο `x0` δεν είναι μηδέν, τότε αυτό μεταφέρει την εκτέλεση στην ετικέτα `label`.
* **`tbnz`**: Δοκιμάζει το bit και μεταφέρει στην επόμενη εντολή αν δεν είναι μηδέν
* Παράδειγμα: `tbnz x0, #8, label`
* **`tbz`**: Δοκιμάζει το bit και μεταφέρει στην επόμενη εντολή αν είναι μηδέν
* Παράδειγμα: `tbz x0, #8, label`
* **Λειτουργίες επιλογής με συνθήκη**: Αυτές είναι λειτουργίες οι οποίες συμπεριφέρονται διαφορετικά ανάλογα με τα συνθηκολογικά bits.
...
* Παράδειγμα: `ldrsw x0, [x1]` — Αυτό φορτώνει μια υπογραφόμενη τιμή 32 bits από τη θέση μνήμης στην οποία δείχνει το `x1`, την επεκτείνει σε 64 bits και την αποθηκεύει στο `x0`.
...
3. **Επιστροφή**: `ret` (επιστρέφει τον έλεγχο στον καλούντα χρησιμοποιώντας τη διεύθυνση στον μητρώο συνδέσμου)
...
Αυτό ορίζεται κατά τη διάρκεια των **εντολών αλλαγής κλάδου interworking**, αλλά μπορεί επίσης να οριστεί απευθείας με άλλες εντολές όταν το PC ορίζεται ως μητρώος προορισμού. Παράδειγμα:
...
Αυτό γίνεται με το **αποθήκευση της κατάστασης του επεξεργαστή από το `CPSR` στο `SPSR`** της λειτουργικής κατάστασης του επεξεργαστή στην οποία λαμβάνεται η εξαίρεση. Κατά την επιστροφή από την εξαίρεση, το **`CPSR`** αποκαθίσταται από το **`SPSR`**.
...
* **Λειτουργία και Μάσκα Εξαιρέσεων Καταχωρητών** (0-4): Καθορίζουν την τρέχουσα κατάσταση εκτέλεσης. Το **5ο** υποδεικνύει αν το πρόγραμμα τρέχει ως 32bit (ένα 1) ή 64bit (ένα 0). Τα άλλα 4 αντιπροσωπεύουν τη **λειτουργία εξαίρεσης που χρησιμοποιείται επί του παρόντος** (όταν συμβαίνει μια εξαίρεση και χειρίζεται). Ο αριθμός που ορίζεται **υποδεικνύει την τρέχουσα προτεραιότητα** σε περίπτωση που ενεργοποιηθεί μια άλλη εξαίρεση ενώ αυτή χειρίζεται.

<figure><img src="../../../.gitbook/assets/image (728).png" alt=""><figcaption></figcaption></figure>

* **`AIF`**: Ορισμένες εξαιρέσεις μπορούν να απενεργοποιηθούν χρησιμοποιώντας τα bits **`A`**, `I`, `F`. Αν το **`A`** είναι 1 σημαίνει ότι θα ενεργοποιηθούν **ασύγχρονες αποτυχίες**. Το **`I`** ρυθμίζει την ανταπόκριση σε εξωτερικά υλικά **Αιτήματα Διακοπών** (IRQs). και το F σχετίζεται με τα **Αιτήματα Γρήγορων Διακοπών** (FIRs).
...
```
{% hint style="success" %}
Μερικές φορές είναι πιο εύκολο να ελέγχετε τον **αποσυναρμολογημένο** κώδικα από το **`libsystem_kernel.dylib`** **από** τον έλεγχο του **πηγαίου κώδικα** επειδή ο κώδικας πολλών κλήσεων συστήματος (BSD και Mach) δημιουργείται μέσω σεναρίων (ελέγξτε τα σχόλια στον πηγαίο κώδικα), ενώ στο dylib μπορείτε να βρείτε τι καλείται.
{% endhint %}
