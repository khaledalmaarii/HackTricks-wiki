...
<summary><strong>htARTE (HackTricks AWS Red Team Expert)를 통해 제로부터 영웅이 되는 AWS 해킹 배우기</strong> <a href="https://training.hacktricks.xyz/courses/arte"><strong>htARTE (HackTricks AWS Red Team Expert)</strong></a><strong>!</strong></summary>
...
* EL0에서 실행되는 응용 프로그램은 서로 및 시스템 소프트웨어로부터 격리되어 보안 및 안정성을 향상시킵니다.
2. **EL1 - 운영 체제 커널 모드**:
...
ARM64에는 `x0`부터 `x30`까지 레이블이 지정된 **31개의 범용 레지스터**가 있습니다. 각각은 **64비트** (8바이트) 값을 저장할 수 있습니다. 32비트 값만 필요한 작업의 경우 동일한 레지스터를 `w0`부터 `w30`까지의 이름을 사용하여 32비트 모드로 액세스할 수 있습니다.
...
5. **`x18`** - **플랫폼 레지스터**. 일반적인 목적 레지스터로 사용될 수 있지만 일부 플랫폼에서는 이 레지스터가 플랫폼별 용도로 예약되어 있습니다: Windows의 현재 스레드 환경 블록을 가리키는 포인터 또는 Linux 커널에서 현재 **실행 중인 작업 구조체를 가리키는 포인터**.
6. **`x19`** ~ **`x28`** - 이들은 호출자 저장 레지스터입니다. 함수는 호출자를 위해 이러한 레지스터의 값을 보존해야 하므로 스택에 저장되고 호출자로 돌아가기 전에 복구됩니다.
7. **`x29`** - **프레임 포인터**는 스택 프레임을 추적하는 데 사용됩니다. 함수가 호출되어 새로운 스택 프레임이 생성될 때 **`x29`** 레지스터가 **스택에 저장**되고 **새로운** 프레임 포인터 주소 (**`sp`** 주소)가 **이 레지스터에 저장**됩니다.
...
10. **`pc`** - **프로그램 카운터**, 다음 명령을 가리킵니다. 이 레지스터는 예외 생성, 예외 반환 및 분기를 통해서만 업데이트될 수 있습니다. 이 레지스터를 읽을 수 있는 일반 명령은 분기와 링크 명령 (BL, BLR)으로만 **`pc`** 주소를 **`lr`** (링크 레지스터)에 저장하는 것입니다.
...
이러한 레지스터는 전용 특수 명령 **`mrs`** 및 **`msr`**을 사용하여만 읽거나 설정할 수 있습니다.

특수 레지스터 **`TPIDR_EL0`** 및 **`TPIDDR_EL0`**는 역공학에서 자주 발견됩니다. `EL0` 접미사는 레지스터에 액세스할 수 있는 **최소 예외**를 나타냅니다 (이 경우 EL0은 일반 예외 수준으로 일반 프로그램이 실행됩니다).\
이들은 일반적으로 메모리의 **스레드 로컬 저장소의 기본 주소**를 저장하는 데 사용됩니다. 일반적으로 첫 번째 레지스터는 EL0에서 실행되는 프로그램에 대해 읽기 및 쓰기가 가능하지만 두 번째 레지스터는 EL0에서 읽을 수 있고 EL1에서 쓸 수 있습니다 (커널과 같이).
### **PSTATE**

**PSTATE**에는 여러 프로세스 구성 요소가 직렬화되어 운영 체제에서 볼 수 있는 **`SPSR_ELx`** 특수 레지스터에 포함되어 있습니다. 여기에는 트리거된 예외의 **권한 수준인 X**가 포함됩니다(이는 예외가 종료될 때 프로세스 상태를 복구하는 데 사용됩니다).\
다음은 접근 가능한 필드입니다:
...
* 뺄셈의 경우, 큰 음수가 작은 양수에서 뺄셈되거나 그 반대인 경우, 그 결과가 주어진 비트 크기의 범위 내에 표현할 수 없는 경우.
* 당연히 프로세서는 작업이 부호 있는지 여부를 알 수 없으므로 작업에서 C 및 V를 확인하고 부호가 있는 경우 발생한 캐리를 표시합니다.
...
* 현재 **예외 레벨** (**`EL`**): EL0에서 실행 중인 일반 프로그램은 값 0을 갖습니다.
* **싱글 스텝** 플래그 (**`SS`**): 디버거가 **`SPSR_ELx`** 내부에서 SS 플래그를 1로 설정하여 단계별로 실행할 때 사용됩니다. 프로그램은 한 단계를 실행하고 단일 단계 예외를 발생시킵니다.
* **불법 예외** 상태 플래그 (**`IL`**): 특권 소프트웨어가 무효한 예외 레벨 전송을 수행할 때 사용되며, 이 플래그는 1로 설정되고 프로세서는 불법 상태 예외를 트리거합니다.
* **`DAIF`** 플래그: 이러한 플래그를 사용하면 특권 프로그램이 특정 외부 예외를 선택적으로 마스킹할 수 있습니다.
* **`A`**가 1이면 **비동기 중단**이 트리거됩니다. **`I`**는 외부 하드웨어 **인터럽트 요청** (IRQ)에 응답하도록 구성하고 F는 **빠른 인터럽트 요청** (FIR)와 관련이 있습니다.
* **스택 포인터 선택** 플래그 (**`SPS`**): EL1 이상에서 실행 중인 특권 프로그램은 자체 스택 포인터 레지스터와 사용자 모델 스택 포인터 사이를 전환할 수 있습니다(예: `SP_EL1`과 `EL0` 사이). 이 전환은 **`SPSel`** 특수 레지스터에 쓰기를 통해 수행됩니다. 이 작업은 EL0에서 수행할 수 없습니다.

## **호출 규약 (ARM64v8)**

ARM64 호출 규약은 함수에 전달되는 **첫 번째 여덟 매개변수**가 **`x0`부터 `x7`** 레지스터에 전달된다고 명시합니다. **추가** 매개변수는 **스택**에 전달됩니다. **반환** 값은 레지스터 **`x0`**에 다시 전달되거나 **128비트인 경우** **`x1`**에도 전달됩니다. **`x19`**부터 **`x30`** 및 **`sp`** 레지스터는 함수 호출 간에 **보존**되어야 합니다.
...
ARM64 명령어는 일반적으로 **`opcode dst, src1, src2`** 형식을 갖습니다. 여기서 **`opcode`**는 수행할 **작업**을 나타내고(`add`, `sub`, `mov` 등), **`dst`**는 결과가 저장될 **대상** 레지스터이며 **`src1`** 및 **`src2`**는 **소스** 레지스터입니다. 즉시 값은 소스 레지스터 대신 사용할 수도 있습니다.
...
* **오프셋 모드**: 오리지널 포인터에 영향을 주는 오프셋이 표시됩니다. 예를 들어:
* `ldr x2, [x1, #8]`, 이는 x1 + 8에서 x2에 값을 로드합니다.
* `ldr x2, [x0, x1, lsl #2]`, 이는 x0 배열에서 x1(인덱스) \* 4 위치의 객체를 x2에 로드합니다.
* **선행 인덱스 모드**: 이는 오리지널에 계산을 적용하고 결과를 얻은 후 새로운 오리지널을 오리지널에 저장합니다.
...
* `ldr x0, [x1], #8`, x1을 x0에 로드하고 x1을 `x1 + 8`로 업데이트합니다.
* **PC 상대 주소 지정**: 이 경우 로드할 주소가 현재 PC에 상대적으로 계산됩니다.
* `ldr x1, =_start`, 이는 `_start` 심볼이 시작하는 주소를 현재 PC에 상대적으로 x1에 로드합니다.
* **`str`**: **레지스터**에서 **메모리**로 **값을 저장**합니다.
* 예: `str x0, [x1]` — 이는 `x0`의 값을 `x1`이 가리키는 메모리 위치에 저장합니다.
* **`ldp`**: **레지스터 쌍 로드**. 이 명령은 **연속된 메모리** 위치에서 **두 레지스터를 로드**합니다. 메모리 주소는 일반적으로 다른 레지스터의 값에 오프셋을 추가하여 형성됩니다.
* 예: `ldp x0, x1, [x2]` — 이는 각각 `x2` 및 `x2 + 8` 위치의 메모리에서 `x0` 및 `x1`을 로드합니다.
* **`stp`**: **레지스터 쌍 저장**. 이 명령은 **두 레지스터를** **연속된 메모리** 위치로 **저장**합니다. 메모리 주소는 일반적으로 다른 레지스터의 값에 오프셋을 추가하여 형성됩니다.
...
* `add x5, x5, #1, lsl #12` — 이는 4096(1을 12번 시프트한 값)과 같습니다 -> 1 0000 0000 0000 0000
* **`adds`**는 `add`를 수행하고 플래그를 업데이트합니다.
* **`sub`**: 두 레지스터의 값을 빼고 결과를 레지스터에 저장합니다.
* **`add`** **구문**을 확인하세요.
* 예시: `sub x0, x1, x2` — 이 명령은 `x2`의 값을 `x1`에서 빼고 결과를 `x0`에 저장합니다.
* **`subs`** 이 명령은 sub과 유사하지만 플래그를 업데이트합니다.
* **`mul`**: 두 레지스터의 값을 곱하고 결과를 레지스터에 저장합니다.
* 예시: `mul x0, x1, x2` — 이 명령은 `x1`과 `x2`의 값을 곱하고 결과를 `x0`에 저장합니다.
* **`div`**: 한 레지스터의 값을 다른 값으로 나누고 결과를 레지스터에 저장합니다.
* 예시: `div x0, x1, x2` — 이 명령은 `x1`의 값을 `x2`로 나누고 결과를 `x0`에 저장합니다.
* **`lsl`**, **`lsr`**, **`asr`**, **`ror`, `rrx`**:
* **논리 왼쪽 시프트**: 끝에서 0을 추가하여 다른 비트를 앞쪽으로 이동시킵니다 (n번 2로 곱하기)
* **논리 오른쪽 시프트**: 시작 부분에 1을 추가하여 다른 비트를 뒤로 이동시킵니다 (부호 없는 경우 n번 2로 나누기)
* **산술 오른쪽 시프트**: **`lsr`**와 유사하지만 최상위 비트가 1이면 1을 추가합니다 (부호 있는 경우 n번 2로 나누기)
* **오른쪽으로 회전**: **`lsr`**와 유사하지만 오른쪽에서 제거된 것은 왼쪽에 추가됩니다.
* **확장된 오른쪽 회전**: **`ror`**와 유사하지만 캐리 플래그가 "가장 중요한 비트"로 사용됩니다. 따라서 캐리 플래그가 비트 31로 이동되고 제거된 비트가 캐리 플래그로 이동합니다.
* **`bfm`**: **비트 필드 이동**, 이러한 작업은 값을 복사하고 해당 값을 다른 레지스터에 복사합니다.
...
* **`BFI X1, X2, #3, #4`** X2에서 3번째 비트부터 4비트를 X1에 삽입합니다.
* **`BFXIL X1, X2, #3, #4`** X2의 3번째 비트부터 4비트를 추출하여 X1에 복사합니다.
* **`SBFIZ X1, X2, #3, #4`** X2에서 4비트를 부호 확장하고 3번째 비트 위치부터 X1에 삽입하여 오른쪽 비트를 0으로 설정합니다.
* **`SBFX X1, X2, #3, #4`** X2에서 3번째 비트부터 4비트를 추출하여 부호 확장하고 결과를 X1에 배치합니다.
* **`UBFIZ X1, X2, #3, #4`** X2에서 4비트를 0으로 확장하고 3번째 비트 위치부터 X1에 삽입합니다.
* **`UBFX X1, X2, #3, #4`** X2에서 3번째 비트부터 4비트를 추출하여 0으로 확장된 결과를 X1에 배치합니다.
* **X로 확장된 부호:** 값을 확장하여 해당 값으로 작업을 수행할 수 있습니다:
...
* **`UXTB X1, W2`** 0을 추가하여 W2의 바이트를 X1로 확장하여 64비트를 채웁니다.
* **`extr`:** 지정된 **레지스터 쌍에서 비트를 추출합니다**.
* 예시: `EXTR W3, W2, W1, #3` 이 명령은 **W1+W2**를 연결하고 **W2의 3번째 비트부터 W1의 3번째 비트까지** 가져와 W3에 저장합니다.
* **`cmp`**: 두 레지스터를 비교하고 조건 플래그를 설정합니다. 목적 레지스터를 제로 레지스터로 설정하는 **`subs`**의 별칭입니다. `m == n`인지 확인하는 데 유용합니다.
* **`subs`**와 동일한 구문을 지원합니다.
* 예시: `cmp x0, x1` — 이 명령은 `x0`와 `x1`의 값을 비교하고 조건 플래그를 설정합니다.
* **`cmn`**: **음수 비교** 피연산자. 이 경우 **`adds`**의 별칭이며 동일한 구문을 지원합니다. `m == -n`인지 확인하는 데 유용합니다.
* **`ccmp`**: 조건부 비교, 이전 비교가 참인 경우에만 수행되는 비교로 특정하게 `nzcv` 비트를 설정합니다.
* `cmp x1, x2; ccmp x3, x4, 0, NE; blt _func` -> x1 != x2이고 x3 < x4이면 func로 이동합니다.
* 이는 **이전 `cmp`가 `NE`인 경우에만 `ccmp`가 실행**되므로 비트 `nzcv`가 0으로 설정됩니다 (`blt` 비교를 충족하지 않음).
* 이것은 `ccmn`으로도 사용할 수 있습니다 (`cmp` 대 `cmn`과 유사).
* **`tst`**: 비교 값 중 어느 값이 1인지 확인합니다 (결과를 저장하지 않고 ANDS처럼 작동). 값이 1인 레지스터의 비트를 확인하는 데 유용합니다.
* 예시: `tst X1, #7` X1의 마지막 3비트 중 어느 비트가 1인지 확인합니다.
* **`teq`**: 결과를 버리는 XOR 연산
* **`b`**: 조건 없는 분기
* 예시: `b myFunction`&#x20;
* 이는 반환 주소를 링크 레지스터로 채우지 않습니다 (돌아와야 하는 서브루틴 호출에 적합하지 않음)
* **`bl`**: **링크가 있는 분기**, **서브루틴을 호출**하는 데 사용됩니다. **`x30`에 반환 주소를 저장**합니다.
* 예시: `bl myFunction` — 이 명령은 `myFunction` 함수를 호출하고 반환 주소를 `x30`에 저장합니다.
* **`blr`**: **레지스터로 링크가 있는 분기**, **레지스터에 지정된 대상**을 호출하는 데 사용됩니다. 반환 주소를 `x30`에 저장합니다. (이것은&#x20;
* 예시: `blr x1` — 이 명령은 `x1`에 포함된 주소의 함수를 호출하고 반환 주소를 `x30`에 저장합니다.
* **`ret`**: **서브루틴에서 반환**, 일반적으로 **`x30`**의 주소를 사용합니다.
* 예시: `ret` — 이 명령은 현재 서브루틴에서 `x30`에 있는 반환 주소를 사용하여 반환합니다.
* **`b.<cond>`**: 조건부 분기
* **`b.eq`**: **동일한 경우 분기**, 이전 `cmp` 명령을 기반으로 합니다.
* 예시: `b.eq label` — 이전 `cmp` 명령에서 두 값이 동일한 경우 `label`로 이동합니다.
* **`b.ne`**: **동일하지 않은 경우 분기**. 이 명령은 조건 플래그를 확인하고 (이전 비교 명령에 의해 설정된) 비교된 값이 동일하지 않으면 레이블이나 주소로 분기합니다.
* 예시: `cmp x0, x1` 명령 후, `b.ne label` — `x0`와 `x1`의 값이 동일하지 않으면 `label`로 이동합니다.
* **`cbz`**: **0에 대한 비교 및 분기**. 이 명령은 레지스터를 0과 비교하고 동일한 경우 레이블이나 주소로 분기합니다.
* 예시: `cbz x0, label` — `x0`의 값이 0이면 `label`로 이동합니다.
* **`cbnz`**: **0이 아닌 값에 대한 비교 및 분기**. 이 명령은 레지스터를 0과 비교하고 동일하지 않은 경우 레이블이나 주소로 분기합니다.
* 예시: `cbnz x0, label` — 만약 `x0`의 값이 0이 아니면, `label`로 이동합니다.
* **`tbnz`**: 비트를 테스트하고 0이 아닐 때 분기
* 예시: `tbnz x0, #8, label`
* **`tbz`**: 비트를 테스트하고 0일 때 분기
* 예시: `tbz x0, #8, label`
...
* 예시: `adrp x0, symbol` — `symbol`의 페이지 주소를 계산하여 `x0`에 저장합니다.
* **`ldrsw`**: 메모리에서 **부호 있는 32비트** 값을 **로드**하고 64비트로 **부호 확장**합니다.
* 예시: `ldrsw x0, [x1]` — `x1`이 가리키는 메모리 위치에서 부호 있는 32비트 값을 로드하고 64비트로 확장하여 `x0`에 저장합니다.
* **`stur`**: 레지스터 값을 메모리 위치에 **저장**하며, 다른 레지스터로부터 오프셋을 사용합니다.
* 예시: `stur x0, [x1, #4]` — `x1`에 현재 주소보다 4바이트 더 큰 메모리 주소에 `x0`의 값을 저장합니다.
* **`svc`** : **시스템 콜**을 수행합니다. "Supervisor Call"의 약자로, 프로세서가 이 명령을 실행하면 **사용자 모드에서 커널 모드로 전환**하고, **커널의 시스템 콜 처리** 코드가 위치한 메모리의 특정 위치로 이동합니다.
*   예시:

```armasm
mov x8, 93  ; 레지스터 x8에 종료 시스템 콜 번호인 93을 로드합니다.
mov x0, 0   ; 종료 상태 코드인 0을 레지스터 x0에 로드합니다.
svc 0       ; 시스템 콜을 수행합니다.
```
...
**특권을 가진** 64비트 프로그램은 **32비트 프로그램의 실행을 예약**할 수 있으며 이는 낮은 특권을 가진 32비트로 예외 수준 전환을 실행함으로써 이루어집니다.\
...
32비트 레지스터가 16개 있습니다 (r0-r15). **r0부터 r14까지**는 **어떤 작업에도** 사용할 수 있지만, 일부는 일반적으로 예약됩니다:
...
또한, 레지스터는 **`은행 레지스트리`**에 백업됩니다. 이는 레지스터 값을 저장하여 예외 처리 및 특권 작업에서 **빠른 컨텍스트 전환**을 수행할 수 있도록 하기 위한 곳입니다. 이는 예외가 발생한 프로세서 모드의 `CPSR`에서 프로세서 상태를 `SPSR`로 저장함으로써 수행됩니다. 예외가 반환되면 **`CPSR`**가 **`SPSR`**에서 복원됩니다.
...
- **`AIF`**: 특정 예외는 **`A`**, `I`, `F` 비트를 사용하여 비활성화할 수 있습니다. **`A`**가 1이면 **비동기 중단**이 트리거됩니다. **`I`**는 외부 하드웨어 **인터럽트 요청** (IRQs)에 응답하도록 구성하고, F는 **빠른 인터럽트 요청** (FIRs)과 관련이 있습니다.
...
[**syscall\_sw.c**](https://opensource.apple.com/source/xnu/xnu-3789.1.32/osfmk/kern/syscall\_sw.c.auto.html)를 확인하세요. Mach 트랩은 **x16 < 0**일 것이므로 이전 목록의 숫자를 **음수**로 호출해야 합니다: **`_kernelrpc_mach_vm_allocate_trap`**은 **`-10`**입니다.
...
목표는 `execve("/bin/cat", ["/bin/cat", "/etc/passwd"], NULL)`을 실행하는 것이므로 두 번째 인자 (x1)는 매개변수의 배열입니다 (메모리에서는 주소 스택을 의미함).
```armasm
...
[https://github.com/daem0nc0re/macOS_ARM64_Shellcode/blob/master/reverseshell.s](https://github.com/daem0nc0re/macOS_ARM64_Shellcode/blob/master/reverseshell.s)에서 **127.0.0.1:4444**로의 revshell
```armasm
.section __TEXT,__text
.global _main
.align 2
