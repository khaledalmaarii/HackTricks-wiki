...
* Der **`sp`**-Wert sollte immer mindestens eine **Quadword-Alignment** oder eine Ausnahmefehlerausnahme aufrechterhalten.
...
* Wenn **`A`** 1 ist, bedeutet dies, dass **asynchrone Abbrüche** ausgelöst werden. Das **`I`** konfiguriert die Reaktion auf externe Hardware-**Interruptanforderungen** (IRQs). und das F steht für **Fast Interrupt Requests** (FIRs).
...
* **`svc`** : Führt einen **Systemaufruf** durch. Es steht für "Supervisor Call". Wenn der Prozessor diese Anweisung ausführt, **wechselt er vom Benutzermodus in den Kernelmodus** und springt zu einer spezifischen Speicherstelle, an der sich der **Systemaufrufbehandlungscode des Kernels** befindet.
*   Beispiel:
...
Schauen Sie sich [**syscall\_sw.c**](https://opensource.apple.com/source/xnu/xnu-3789.1.32/osfmk/kern/syscall\_sw.c.auto.html) an. Mach-Fallen haben **x16 < 0**, daher müssen Sie die Nummern aus der vorherigen Liste mit einem **Minuszeichen** aufrufen: **`_kernelrpc_mach_vm_allocate_trap`** ist **`-10`**.
...
Parameter ([weitere Informationen in der Dokumentation](https://developer.apple.com/documentation/objectivec/1456712-objc\_msgsend)):

* x0: self -> Zeiger auf die Instanz
* x1: op -> Selektor der Methode
* x2... -> Rest der Argumente der aufgerufenen Methode

Daher können Sie, wenn Sie einen Breakpoint vor dem Sprung zu dieser Funktion setzen, leicht herausfinden, was in lldb aufgerufen wird (in diesem Beispiel ruft das Objekt ein Objekt von `NSConcreteTask` auf, das einen Befehl ausführt):
```
(lldb) po $x0
