...
* यह स्तर वर्चुअलाइजेशन के लिए इस्तेमाल होता है। EL2 पर चलने वाला एक हाइपरवाइजर एक ही भौतिक हार्डवेयर पर चलने वाले कई ऑपरेटिंग सिस्टम्स (प्रत्येक अपने EL1 में) का प्रबंधन कर सकता है।
...
* यह रजिस्टर **सामान्य-उद्देश्य रजिस्ट्री** के रूप में भी इस्तेमाल किया जा सकता है हालांकि यह आमतौर पर **स्थानीय चरों** के संदर्भ के
```armasm
...
**विशेषाधिकार प्राप्त** 64-बिट प्रोग्राम 32-बिट प्रोग्रामों के **निष्पादन को अनुसूचित** कर सकते हैं, एक अपवाद स्तर स्थानांतरण को निचले विशेषाधिकार प्राप्त 32-बिट में निष्पादित करके।\
...
**`इंटरवर्किंग`** CPSR के J और T बिट्स का उपयोग करके होती है। `J=0` और `T=0` का मतलब **`A32`** है और `J=0` और `T=1` का मतलब **T32** है। यह मूल रूप से निर्देश सेट T32 है इसे इंगित करने के लिए **निम्नतम बिट को 1 पर सेट करने** पर आधारित है।\
...
इसके अलावा, रजिस्टर्स **`बैंक्ड रजिस्ट्रीज`** में बैक अप होते हैं। जो रजिस्टर्स के मूल्यों को संग्रहित करते हैं जिससे अपवाद संभालने और विशेषाधिकार वाले ऑपरेशन्स में **तेजी से कॉन्टेक्स्ट स्विचिंग** करने की अनुमति मिलती है ताकि हर बार मैन्युअली रजिस्टर्स को सेव और रिस्टोर करने की जरूरत न पड़े।\
यह **प्रोसेसर की स्थिति को `CPSR` से `SPSR`** में सेव करके किया जाता है, जिस प्रोसेसर मोड में अपवाद लिया जाता है। अपवाद वापसी पर, **`CPSR`** को **`SPSR`** से बहाल किया जाता है।
...
फील्ड्स को कुछ समूहों में बांटा गया है:
...
* **`Q`** ध्वज: जब भी **पूर्णांक संतृप्ति होती है** विशेष संतृप्ति अंकगणितीय निर्देश के निष्पादन के दौरान, इसे 1 पर सेट किया जाता है। एक बार जब इसे **`1`** पर सेट किया जाता है, तो यह मूल्य को मैन्युअली 0 पर सेट किए जाने तक बनाए रखेगा। इसके अलावा, कोई भी निर्देश इसके मूल्य को अप्रत्यक्ष रूप से जांचता नहीं है, इसे मैन्युअली पढ़कर किया जाना चाहिए।
*   **`GE`** (बड़ा या बराबर) ध्वज: यह SIMD (एकल निर्देश, बहु डेटा) ऑपरेशन्स में इस्तेमाल होता है, जैसे "समानांतर जोड़" और "समानांतर घटाव"। ये ऑपरेशन्स एकल निर्देश में कई डेटा बिंदुओं को संसाधित करने की अनुमति देते हैं।

उदाहरण के लिए, **`UADD8`** निर्देश **चार जोड़े बाइट्स को जोड़ता है** (दो 32-बिट ऑपरेंड्स से) समानांतर में और परिणामों को 32-बिट रजिस्टर में संग्रहित करता है। फिर यह **`GE` ध्वज को `APSR` में सेट करता है** इन परिणामों के आधार पर। प्रत्येक GE ध्वज एक बाइट जोड़ के लिए मेल खाता है, यह दर्शाता है कि उस बाइट जोड़ी के लिए जोड़ **ओवरफ्लो** हुआ।
...
* **मोड और अपवाद मास्क बिट्स** (0-4): वे वर्तमान निष्पादन स्थिति को निर्धारित करते हैं। **पांचवां** यह दर्शाता है कि कार्यक्रम 32बिट (1) के रूप में चलता है या 64बिट (0) के रूप में। अन्य 4 **वर्तमान में इस्तेमाल किए जा रहे अपवाद मोड** को दर्शाते हैं (जब एक अपवाद होता है और इसे संभाला जा रहा होता है)। सेट किया गया नंबर **वर्तमान प्राथमिकता को दर्शाता है** अगर इसे संभालते समय एक और अपवाद ट्रिगर होता है।

<figure><img src="../../../.gitbook/assets/image (728).png" alt=""><figcaption></figcaption></figure>

* **`AIF`**: कुछ अपवादों को बिट्स **`A`**, `I`, `F` का उपयोग करके अक्षम किया जा सकता है। अगर **`A`** 1 है तो इसका मतलब है **असमकालिक रोक** ट्रिगर होगी। **`I`** बाहरी हार्डवेयर **इंटरप्ट अनुरोधों** (IRQs) का जवाब देने के लिए कॉन्फ़िगर किया गया है। और F **तेज़ इंटरप्ट अनुरोधों** (FIRs) से संबंधित है।

## macOS

### BSD syscalls

[**syscalls.master**](https://opensource.apple.com/source/xnu/xnu-1504.3.12/bsd/kern/syscalls.master) को देखें। BSD syscalls में **x16 > 0** होगा।
...
आप **`libsystem_kernel.dylib`** को एक डिससेंबलर में भी देख सकते हैं ताकि ये (और BSD) syscalls कैसे कॉल करें, यह पता लगा सकें:
```bash
# macOS
...
```
#### एक fork का उपयोग करके sh के साथ command इन्वोक करें ताकि मुख्य प्रक्रिया न मारी जाए
```armasm
.section __TEXT,__text     ; Begin a new section of type __TEXT and name __text
.global _main              ; Declare a global symbol _main
.align 2                   ; Align the beginning of the following code to a 4-byte boundary
