...
* Hii ni kiwango cha haki cha chini na hutumiwa kutekeleza nambari za maombi ya kawaida.
...
* Kiwango hiki hutumiwa kwa uvirtualization. Hypervisor inayotekelezwa kwenye EL2 inaweza kusimamia mifumo ya uendeshaji mingi (kila moja katika EL1 yake) inayotekelezwa kwenye vifaa vya kimwili sawa.
...
Matumizi ya viwango hivi hutoa njia iliyopangwa na salama ya kusimamia vipengele tofauti vya mfumo, kutoka kwa programu za watumiaji hadi programu ya mfumo yenye haki zaidi. Mbinu ya ARMv8 kwa viwango vya haki husaidia katika kujitenga kwa ufanisi sehemu tofauti za mfumo, hivyo kuboresha usalama na uthabiti wa mfumo.
...
5. **`x18`** - **Kudhibiti jukwaa**. Inaweza kutumika kama kudhibiti wa jumla, lakini kwenye majukwaa fulani, kudhibiti huu unahifadhiwa kwa matumizi maalum ya jukwaa: Kielekezi cha kizuizi cha mazingira ya watumiaji wa sasa katika Windows, au kuelekeza kwa muundo wa kazi inayoendeshwa kwa sasa katika kerneli ya Linux.
...
7. **`x29`** - **Kielekezi cha Fremu** cha kufuatilia fremu ya stak. Wakati fremu mpya ya stak inaundwa kwa sababu kazi inaitwa, kudhibiti cha **`x29`** kina **hifadhiwa kwenye stak** na anwani mpya ya fremu (**anwani ya `sp`**) inahifadhiwa kwenye kielekezi hiki.
...
8. **`x30`** au **`lr`**- **Kielekezi cha Kiungo**. Hifadhi anwani ya kurudi wakati maelekezo ya `BL` (Branch with Link) au `BLR` (Branch with Link to Register) yanatekelezwa kwa kuhifadhi thamani ya **`pc`** kwenye kielekezi hiki.

* Inaweza pia kutumika kama kudhibiti kingine chochote.
* Ikiwa kazi ya sasa itaita kazi mpya na hivyo kubadilisha `lr`, itahifadhi kwenye stak mwanzoni, hii ni epilogo (`stp x29, x30 , [sp, #-48]; mov x29, sp` -> Hifadhi `fp` na `lr`, unda nafasi na pata `fp` mpya) na kurejesha mwishoni, hii ni prologue (`ldp x29, x30, [sp], #48; ret` -> Rejesha `fp` na `lr` na rudisha).

9. **`sp`** - **Kielekezi cha Stak**, hutumiwa kufuatilia juu ya stak.

* thamani ya **`sp`** daima inapaswa kuhifadhiwa angalau kwa **urekebishaji wa neno la quad** au kosa la urekebishaji linaweza kutokea.
...
**Kuna mamia ya vipimo vya mfumo**, vinavyoitwa pia vipimo maalum vya kusudi (SPRs), hutumika kwa **kuangalia** na **kudhibiti** **tabia za processors**.\
Vinaweza kusomwa au kuwekwa tu kwa kutumia maagizo maalum yaliyotengwa **`mrs`** na **`msr`**.

Vipimo maalum **`TPIDR_EL0`** na **`TPIDDR_EL0`** mara nyingi hupatikana wakati wa kurekebisha. Kiambishi cha `EL0` kinaonyesha **kosa la chini** ambalo vipimo vinaweza kufikiwa (katika kesi hii EL0 ni kosa la kawaida (haki) ambalo programu za kawaida hufanya nalo).\
...
**PSTATE** ina vipengele vingi vya mchakato vilivyosanidishwa katika kipimo maalum cha **`SPSR_ELx`**, X ikiwa ni **kiwango cha ruhusa cha kosa** kilichosababisha (hii inaruhusu kupona kwa hali ya mchakato wakati kosa linamalizika).\
...
* **Kiwango cha sasa cha usajili (`nRW`)** hali: Ikiwa bendera inashikilia thamani 0, programu itaendesha katika hali ya utekelezaji wa AArch64 mara tu itakaporejeshwa.
...
* Bendera ya hali ya kosa **isio halali** (**`IL`**): Hutumiwa kuashiria wakati programu yenye ruhusa inafanya uhamisho wa kiwango cha kosa kisichofaa, bendera hii inawekwa kuwa 1 na processor huanzisha kosa la hali isiyofaa.
...
* Bendera za **uchaguzi wa kidole cha mstari** (**`SPS`**): Programu zenye ruhusa zinazoendesha katika EL1 na zaidi zinaweza kubadilisha kati ya kutumia usajili wao wa kidole cha mstari na wa mtumiaji (k.m. kati ya `SP_EL1` na `EL0`). Hii kubadilishana hufanywa kwa kuandika kwa kipimo maalum cha **`SPSel`**. Hii haiwezi kufanywa kutoka EL0.

## **Mkataba wa Kuita (ARM64v8)**

Mkataba wa kuita wa ARM64 unabainisha kwamba **parameta nane za kwanza** kwa kazi hutumwa katika usajili **`x0` hadi `x7`**. **Parameta zaidi** hutumwa kwenye **stakishi**. Thamani ya **kurudi** hutumwa tena kwenye usajili **`x0`**, au pia katika **`x1`** ikiwa ni **biti 128** ndefu. Usajili wa **`x19`** hadi **`x30`** na **`sp`** lazima uhifadhiwe kwa wito wa kazi.
...
* **`hamisha`**: **Hamisha** thamani kutoka kwa **usajili** mmoja hadi mwingine.
...
* **`subs`** Hii ni kama punguza lakini ikisasisha bendera
...
* **Geuza hesabu kulia**: Kama **`lsr`**, lakini badala ya kuongeza 0 ikiwa biti muhimu zaidi ni 1, \*\*1s zinaongezwa (\*\*gawanya mara n kwa 2 kwa ishara)
...
* Harakisha biti yenye ishara: `SBFM Xd, Xn, #r, #s`
* Harakisha biti isiyo na ishara: `UBFM Xd, Xn, #r, #s`
...
* **`SBFIZ X1, X2, #3, #4`** Isajili biti 4 kutoka X2 na uziweke kwa X1 kuanzia nafasi ya biti 3 ukiweka biti sahihi
* **`SBFX X1, X2, #3, #4`** Toa biti 4 kuanzia biti 3 kutoka X2, isajili biti sahihi, na weka matokeo kwa X1
* **`UBFIZ X1, X2, #3, #4`** Isajili biti 4 kutoka X2 na uziweke kwa X1 kuanzia nafasi ya biti 3 ukiweka biti sahihi
* **`UBFX X1, X2, #3, #4`** Toa biti 4 kuanzia biti 3 kutoka X2 na weka matokeo yaliyozidishwa na sifuri kwa X1.
* **Zidisha Ishara Kwenda X:** Ongeza ishara (au ongeza tu 0 kwa toleo lisilo na ishara) ya thamani ili uweze kufanya operesheni nayo:
* **`SXTB X1, W2`** Ongeza ishara ya baiti **kutoka W2 kwenda X1** (`W2` ni nusu ya `X2`) ili kujaza 64bits
* **`SXTH X1, W2`** Ongeza ishara ya nambari ya 16biti **kutoka W2 kwenda X1** ili kujaza 64bits
* **`SXTW X1, W2`** Ongeza ishara ya baiti **kutoka W2 kwenda X1** ili kujaza 64bits
* **`UXTB X1, W2`** Ongeza 0s (bila ishara) kwa baiti **kutoka W2 kwenda X1** ili kujaza 64bits
...
* **`cmp`**: **Hilinganisha** daftari mbili na weka bendera za hali. Ni **jina mbadala la `subs`** ikisawazisha daftari la marudio na sifuri. Inafaa kujua ikiwa `m == n`.
...
* **`cmn`**: **Hilinganisha hasi** ya operesheni. Katika kesi hii ni **jina mbadala la `adds`** na inaunga mkono sintaksia sawa. Inafaa kujua ikiwa `m == -n`.
* **`ccmp`**: Hilinganisha kwa sharti, ni hilinganishi litakalofanywa tu ikiwa hilinganishi la awali lilikuwa kweli na litaweka wazi hasa biti za nzcv.
* `cmp x1, x2; ccmp x3, x4, 0, NE; blt _func` -> ikiwa x1 != x2 na x3 < x4, ruka kwenye func
* Hii ni kwa sababu **`ccmp`** itatekelezwa tu ikiwa **`cmp` ya awali ilikuwa `NE`**, ikiwa haikuwa hivyo, biti za `nzcv` zitawekwa kama 0 (ambayo haitakidhi hilinganishi la `blt`).
* Hii inaweza pia kutumika kama `ccmn` (sawa lakini hasi, kama `cmp` vs `cmn`).
* **`tst`**: Inachunguza ikiwa thamani yoyote ya hilinganishi ni 1 (inafanya kazi kama na ANDS bila kuhifadhi matokeo mahali popote). Ni muhimu kuchunguza daftari na thamani na kuchunguza ikiwa biti yoyote ya daftari iliyotajwa katika thamani ni 1.
...
* **`bl`**: **Rukia** na kiungo, hutumiwa kwa **kuita** subrutine. Huhifadhi **anwani ya kurudi katika `x30`**.
* Mfano: `bl myFunction` — Hii inaita kazi `myFunction` na kuhifadhi anwani ya kurudi katika `x30`.
* Tafadhali elewa kuwa hii haitajaza daftari la kiungo na anwani ya kurudi (haifai kwa wito wa subrutine ambao unahitaji kurudi nyuma)
* **`blr`**: **Rukia** na Kiungo kwenda Daftari, hutumiwa kwa **kuita** subrutine ambapo lengo limetajwa katika daftari. Huhifadhi anwani ya kurudi katika `x30`. (Hii ni
...
* **`b.<hali>`**: Rukia za hali
* **`b.eq`**: **Rukia ikiwa sawa**, kulingana na maagizo ya awali ya `cmp`.
* Mfano: `b.eq label` — Ikiwa maagizo ya awali ya `cmp` yalipata thamani mbili sawa, hii inaruka kwenye `label`.
...
* **`adrp`**: Hesabu **anwani ya ukurasa wa ishara** na uhifadhi kwenye kisajili.
* Mfano: `adrp x0, symbol` — Hii inahesabu anwani ya ukurasa wa `symbol` na kuihifadhi kwenye `x0`.
...
3. **Kurudi**: `ret` (inarejesha udhibiti kwa mtu anayeita kwa kutumia anwani kwenye daftari la viungo)
...
Programu za biti 64 **zenye haki** zinaweza kupanga **utekelezaji wa programu za biti 32** kwa kutekeleza uhamisho wa kiwango cha kipekee kwenda kwa programu iliyopungukiwa ya biti 32.\
Tambua kwamba mpito kutoka biti 64 kwenda biti 32 hufanyika na kiwango cha kipekee cha chini (kwa mfano programu ya biti 64 katika EL1 inachochea programu katika EL0). Hii hufanywa kwa kuweka **biti 4 ya** **`SPSR_ELx`** daftari maalum **kuwa 1** wakati mchakato wa watekelezaji wa `AArch32` uko tayari kutekelezwa na sehemu iliyobaki ya `SPSR_ELx` inahifadhi mipango ya **`AArch32`** CPSR. Kisha, mchakato wenye haki huita maagizo ya **`ERET`** ili mchakato ubadilike kwenda **`AArch32`** ukiingia katika A32 au T32 kulingana na CPSR\*\*.\*\*
...
Angalia katika [**syscall\_sw.c**](https://opensource.apple.com/source/xnu/xnu-3789.1.32/osfmk/kern/syscall\_sw.c.auto.html) `mach_trap_table` na katika [**mach\_traps.h**](https://opensource.apple.com/source/xnu/xnu-3789.1.32/osfmk/mach/mach\_traps.h) mifano. Idadi ya juu ya Mach traps ni `MACH_TRAP_TABLE_COUNT` = 128. Mach traps zitakuwa na **x16 < 0**, hivyo unahitaji kuita nambari kutoka kwa orodha ya awali kwa kuongeza ishara ya **minus**: **`_kernelrpc_mach_vm_allocate_trap`** ni **`-10`**.
...
<div data-gb-custom-block data-tag="tab" data-title='pamoja na adr'>
