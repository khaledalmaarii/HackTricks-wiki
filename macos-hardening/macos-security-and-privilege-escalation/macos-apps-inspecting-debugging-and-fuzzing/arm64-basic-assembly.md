...
8. **`x30`** ή **`lr`**- **Καταχωρητής Συνδέσμου**. Κρατ
...
Μπορούν να διαβαστούν ή να οριστούν μόνο χρησιμοποιώντας την ειδική εντολή **`mrs`** και **`msr`**.
...
* Το **`C`** σημαίνει ότι η λειτουργία μετέφερε
...
* Στην αφαίρεση, όταν από έναν μικρότερο θετικό αριθμό αφαιρεθεί ένας μεγαλύτερος αρνητικός αριθμός (ή αντίστροφα), και το αποτέλεσμα δεν μπορεί να αναπαρασταθεί εντός του εύρους του δοθέντος μεγέθους bit.
* Φυσικά ο επεξεργαστής δεν γνωρίζει αν η λειτουργία είναι με πρόσημο ή όχι, οπότε θα ελέγξει τα C και V στις λειτουργίες και θα υποδείξει αν συνέβη μεταφορά σε περίπτωση που ήταν με πρόσημο ή χωρίς.
...
* Το τρέχον **σημαία πλάτους καταχωρητή (`nRW`)**: Αν η σημαία κρατά την τιμή 0, το πρόγραμμα θα τρέξει στην κατάσταση εκτέλεσης AArch64 μόλις επαναφερθεί.
* Το τρέχον **Επίπεδο Εξαίρεσης** (**`EL`**): Ένα κανονικό πρόγραμμα που τρέχει στο EL0 θα έχει την τιμή 0
* Η σημαία **μονής βήματος** (**`SS`**): Χρησιμοποιείται από debuggers για μονή βήματοποίηση με τοποθέτηση της σημαίας SS σε 1 μέσα στο **`SPSR_ELx`** μέσω μιας εξαίρεσης. Το πρόγραμμα θα εκτελέσει ένα βήμα και θα εκδώσει μια εξαίρεση μονής βήματος.
...
* Αν το **`A`** είναι 1 σημαίνει ότι θα ενεργοποιηθούν **ασύγχρονες αποτυχίες**. Το **`I`** ρυθμίζει την ανταπόκριση σε εξωτερικά υλικά **Αιτήσεις Διακοπών** (IRQs) και το F σχετίζεται με τις **Γρήγορες Αιτήσεις Διακοπών** (FIRs).
* Οι σημαίες επιλογής δείκτη στοίβας (**`SPS`**): Τα προνομιούχα προγράμματα που τρέχουν στο EL1 και πάνω μπορούν να αλλάζουν μεταξύ χρήσης του δικού τους καταχωρητή δείκτη στοίβας και του καταχωρητή δείκτη χρήστη (π.χ. μεταξύ `SP_EL1` και `EL0`). Αυτή η αλλαγή γίνεται με την εγγραφή στον ειδικό καταχωρητή **`SPSel`**. Αυτό δεν μπορεί να γίνει από το EL0.
...
Όταν διαβάζετε μια συνάρτηση σε συναρμολόγηση, ψάξτε για το **προλόγο και επίλογο** της συνάρτησης. Ο **πρόλογος** συνήθως περιλαμβάνει την **αποθήκευση του δείκτη πλαισίου (`x29`)**, την **ρύθμιση** ενός **νέου δείκτη πλαισίου** και την **κατανομή χώρου στοίβας**. Ο **επίλογος** συνήθως περιλαμβάνει την **επαναφορά του αποθηκευμένου δείκτη πλαισίου** και την **επιστροφή** από τη συνάρτηση.
...
* **`div`**: **Διαίρεση** της τιμής ενός καταχωρητή με μια άλλη και αποθήκευση του αποτελέσματος σε έναν καταχωρητή.
...
* **Περιστροφή Δεξιά με Επέκταση**: Όπως **`ror`**, αλλά με τη σημαία μεταφοράς ως "πιο σημαντικό ψηφίο". Έτσι, η σημαία μεταφοράς μεταφέρεται στο bit 31 και το αφαιρούμενο bit στη σημαία μεταφοράς.
...
* **Επέκταση Προσήμου Σε X:** Επεκτείνει το πρόσημο (ή προσθέτει απλώς 0 στη μη υπογεγραμμένη έκδοση) μιας τιμής για να είναι δυνατές οι λειτουργίες με αυτή:
...
* **`cmp`**: **Σύγκριση** δύο καταχωρητών και ρύθμιση σημαίων συνθηκών. Είναι ένας **ψευδώνυμο του `subs`** με την ρύθμιση του καταχωρητή προορισμού στο μηδενικό καταχωρητή. Χρήσιμο για να γνωρίζετε αν `m == n`.
...
* **`ccmp`**: Συνθηκολογική σύγκριση, είναι μια σύγκριση που θα πραγματοποιηθεί μόνο αν μια προηγούμενη σύγκριση ήταν αληθής και θα ρυθμίσει ειδικά τα bits nzcv.
* `cmp x1, x2; ccmp x3, x4, 0, NE; blt _func` -> αν x1 != x2 και x3 < x4, μετάβαση στο func
* Αυτό συμβαίνει επειδή το **`ccmp`** θα εκτελεστεί μόνο αν η **προηγούμενη `cmp` ήταν `NE`**, αν δεν ήταν, τα bits `nzcv` θα οριστούν σε 0 (που δεν θα ικανοποιήσει τη σύγκριση `blt`).
* Αυτό μπορεί ε
...
* **Λειτουργίες επιλογής με συνθήκες**: Αυτές είναι λειτουργίες οι οποίες συμπεριφέρονται διαφορετικά ανάλογα με τα συνθηκικά bits.
...
* **`ldrsw`**: **Φορτώνει** μια υπογραφόμενη **τιμή 32 bit** από τη μνήμη και **επεκτείνει το πρόσημό της σε 64** bit.
* Παράδειγμα: `ldrsw x0, [x1]` — Αυτό φορτώνει μια υπογραφόμενη τιμή 32 bit από τη θέση μνήμης στην οποία δείχνει το `x1`, επεκτείνει το πρόσημό της σε 64 bit και την αποθηκεύει στο `x0`.
...
3. **Διατείνεται χώρος στη στοίβα για τοπικές μεταβλητές** (εάν απαιτείται): `sub sp, sp, <size>` (όπου `<size>` είναι το πλήθος των bytes που απαιτούνται)
...
Τα **προνομιούχα** προγράμματα 64-bit μπορούν να προγραμματίσουν την **εκτέλεση προγραμμάτων 32-bit** εκτελώντας μια μεταφορά επιπέδου εξαίρεσης στο χαμηλότερου επιπέδου προνομιούχο 32-bit.\
...
Επιπλέον, οι καταχωρητές αντιγράφονται σε **`τράπεζες καταχωρητών`**. Αυτά είναι μέρη που αποθηκεύουν τις τιμές των καταχωρητών επιτρέποντας την εκτέλεση **γρήγορης αλλαγής πλαισίου** στην επεξεργασία εξαιρέσεων και σε προνομιούχες λειτουργίες για να αποφευχθεί η ανάγκη χειροκίνητης αποθήκευσης και επαναφοράς των καταχωρητών κάθε φορά.\
...
* **`AIF`**: Ορισμένες εξαιρέσεις μπορούν να απενεργοποιηθούν χρησιμοποιώντας τα bits **`A`**, `I`, `F`. Αν το **`A`** είναι 1 σημαίνει ότι θα ενεργοποιηθούν **ασύγχρονες αποτυχίες**. Το **`I`** ρυθμίζει την ανταπόκριση σε εξωτερικά υλικά **Αιτήσεις Διακοπών** (IRQs). και το F σχετίζεται με τις **Γρήγορες Αιτήσεις Διακοπών** (FIRs).
...
Ο στόχος είναι να εκτελεστεί η `execve("/bin/cat", ["/bin/cat", "/etc/passwd"], NULL)`, οπότε το δεύτερο όρισμα (x1) είναι ένας πίνακας παραμέτρων (που στη μνήμη αυτό σημαίνει ένα σωρό με τις διευθύνσεις).
```armasm
