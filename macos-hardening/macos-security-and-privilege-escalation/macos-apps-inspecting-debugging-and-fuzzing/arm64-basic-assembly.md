...
ARM64有**31个通用寄存器**，标记为`x0`至`x30`。每个可以存储一个**64位**（8字节）的值。对于只需要32位值的操作，可以使用w0至w30的名称以32位模式访问相同的寄存器。
...
6. **`x19`**至**`x28`** - 这些是被调用者保存的寄存器。函数必须为其调用者保留这些寄存器的值，因此它们被存储在栈中，并在返回调用者之前恢复。
7. **`x29`** - **帧指针**，用于跟踪栈帧。当因调用函数而创建新的栈帧时，**`x29`**寄存器会**存储在栈中**，并且新的帧指针地址（**`sp`**地址）会**存储在此寄存器中**。
...
ARM64调用约定规定，函数的**前八个参数**通过寄存器**`x0`至`x7`**传递。**额外**的参数通过**栈**传递。**返回**值通过寄存器**`x0`**返回，如果是128位的，也可以通过**`x1`**返回。**`x19`**至**`x30`**和**`sp`**寄存器必须在函数调用中保持不变。
...
请注意，从64位转到32位的转换伴随着异常级别的降低（例如，EL1中的64位程序触发EL0中的程序）。这是通过在`AArch32`进程线程准备执行时，将**`SPSR_ELx`**特殊寄存器的**第4位设置为1**，而`SPSR_ELx`的其余部分存储**`AArch32`**程序的CPSR。然后，特权进程调用**`ERET`**指令，处理器转换到**`AArch32`**，根据CPSR进入A32或T32**。**

**`交互操作`**是通过CPSR的J和T位进行的。`J=0` 和 `T=0` 表示 **`A32`**，`J=0` 和 `T=1` 表示 **T32**。这基本上是通过将**最低位设置为1**来指示指令集是T32。\
...
这是通过**将处理器状态从`CPSR`保存到发生异常的处理器模式的`SPSR`**来完成的。在异常返回时，**`CPSR`**从**`SPSR`**恢复。
...
* **`Q`**标志：在执行特殊的饱和算术指令期间，一旦发生**整数饱和**，就会将其设置为1。一旦设置为**`1`**，它将保持该值，直到手动设置为0。此外，没有任何指令会隐式检查其值，必须手动读取它。
*   **`GE`**（大于或等于）标志：在SIMD（单指令多数据）操作中使用，例如“并行加法”和“并行减法”。这些操作允许在单个指令中处理多个数据点。

例如，**`UADD8`**指令**并行添加两个32位操作数的四对字节**，并将结果存储在一个32位寄存器中。然后，它根据这些结果**在`APSR`中设置`GE`标志**。每个GE标志对应于一个字节加法，指示该字节对的加法是否**溢出**。
...
* **`E`**位：表示**字节序**。&#x20;
* **模式和异常屏蔽位**（0-4）：它们决定当前的执行状态。**第5位**指示程序是以32位（1）还是64位（0）运行。其他4位代表当前使用的**异常模式**（当发生异常并且正在处理时）。设置的数字**表示在处理此异常时触发另一个异常的当前优先级**。
