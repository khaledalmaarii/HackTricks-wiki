...
Pour créer une instance d'une classe, la méthode **`alloc`** est appelée, ce qui **alloue de la mémoire** pour chaque **propriété** et **met ces allocations à zéro**. Ensuite, **`init`** est appelé, ce qui **initialise les propriétés** aux **valeurs requises**.
```objectivec
// Something like this:
