...
Mac OS बाइनरीज आम तौर पर **यूनिवर्सल बाइनरीज** के रूप में कॉम्पाइल की जाती हैं। एक **यूनिवर्सल बाइनरी** में **एक ही फ़ाइल में कई आर्किटेक्चर का समर्थन** किया जा सकता है।
...
जैसा कि आप सोच रहे होंगे, आम तौर पर 2 आर्किटेक्चर के लिए एक यूनिवर्सल बाइनरी का आकार **एक के लिए किए गए के आकार को दोगुना** कर देता है।
...
**फ़ाइल का लेआउट मेमोरी में** यहाँ निर्दिष्ट किया गया है, **सिम्बल टेबल का स्थान** विस्तार से, कार्यान्वयन प्रारंभ पर मुख्य धागे का संदर्भ, और आवश्यक **साझा लाइब्रेरी**। निर्देश दिए गए हैं डायनामिक लोडर **(dyld)** को मेमोरी में बाइनरी के लोडिंग प्रक्रिया पर।

इसका उपयोग करता है **load\_command** संरचना, जो उल्लिखित **`loader.h`** में परिभाषित है:
```objectivec
struct load_command {
...
```
व्यवस्था को अलग-अलग ढंग से संभालने वाले लगभग **50 विभिन्न प्रकार के लोड कमांड** हैं। सबसे सामान्य वाले हैं: `LC_SEGMENT_64`, `LC_LOAD_DYLINKER`, `LC_MAIN`, `LC_LOAD_DYLIB`, और `LC_CODE_SIGNATURE`।

### **LC\_SEGMENT/LC\_SEGMENT\_64**

{% hint style="success" %}
मूल रूप से, इस प्रकार के लोड कमांड **कैसे \_\_TEXT** (कार्यात्मक कोड) **और \_\_DATA** (प्रक्रिया के लिए डेटा) **सेगमेंट** को **डेटा खंड में दिखाए गए ऑफसेट के अनुसार** बाइनरी को निष्पादित किया जाएगा।
{% endhint %}
...
इनमें **विभिन्न प्रकार** के सेगमेंट होते हैं, जैसे **\_\_TEXT** सेगमेंट, जो किसी प्रोग्राम का कार्यात्मक कोड रखता है, और **\_\_DATA** सेगमेंट, जो प्रक्रिया द्वारा उपयोग किया जाने वाला डेटा रखता है। ये **सेगमेंट** मैच-ओ फ़ाइल के डेटा सेक्शन में स्थित होते हैं।

**प्रत्येक सेगमेंट** को और भी अधिक **विभाजित** किया जा सकता है। **लोड कमांड संरचना** में **जानकारी होती है** **इन सेगमेंट के भीतर के खंडों** के बारे में।
...
otool -lv /bin/ls
```
इस cmd द्वारा लोड किए जाने वाले सामान्य सेगमेंट्स:

* **`__PAGEZERO`:** यह कर्नेल को निर्देशित करता है कि **पता शून्य** को **मैप** किया जाए ताकि इसे **पढ़ा न जा सके, लिखा न जा सके, या न किया जा सके**। संरचना में maxprot और minprot चर मान शून्य पर सेट किए जाते हैं ताकि इस पृष्ठ पर **कोई पढ़ने-लिखने-चलाने का अधिकार न हो**।
* यह आवंटन **NULL प्वाइंटर डिफरेंस वल्नरेबिलिटी** को **कम करने** के लिए महत्वपूर्ण है।
* **`__TEXT`**: **पाठ्यक्रियाशील** **कोड** को **read** और **execute** अनुमतियों के साथ रखता है (कोई लिखने की अनुमति नहीं)**.** इस सेगमेंट के सामान्य खंड:
...
* **`__DATA`**: डेटा को जो **पढ़ा जा सकता है** और **लिखा जा सकता है** (कोई क्रियान्वित नहीं)**.**
...
* **`__LINKEDIT`**: लिंकर (dyld) के लिए जानकारी जैसे, "सिम्बल, स्ट्रिंग, और स्थानांतरण सारणियों" को रखता है।
* **`__OBJC`**: ऑब्जेक्टिव-सी रनटाइम द्वारा उपयोग की जाने वाली जानकारी। हालांकि यह जानकारी \_\_DATA सेगमेंट में भी मिल सकती है, विभिन्न \_\_objc\_\* खंडों में।

### **`LC_MAIN`**

**entryoff विशेषता** में प्रवेश बिंदु शामिल है। लोड समय पर, **dyld** बस के (इन-मेमोरी) **बाइनरी के आधार में** इस मान को **जोड़ता** है, फिर इस निर्देशिका पर जाकर बाइनरी के कोड का निष्पादन शुरू करने के लिए **उछलता** है।

### **LC\_CODE\_SIGNATURE**

**Macho-O फ़ाइल के कोड हस्ताक्षर** के बारे में जानकारी शामिल है। यह केवल एक **ओफ़सेट** शामिल करता है जो **हस्ताक्षर ब्लॉब** की **ओर इशारा करता है**। यह सामान्यत: फ़ाइल के बहुत अंत में होता है।\
...
प्रक्रिया पता अंतरिक्ष में साझा लाइब्रेरी मैप करने वाले डायनामिक लिंकर कार्यक्षेत्र का **पथ शामिल** करता है। **मान हमेशा `/usr/lib/dyld` पर सेट किया जाता है**। यह महत्वपूर्ण है कि macOS में, dylib मैपिंग **उपयोगकर्ता मोड** में होता है, कर्नेल मोड में नहीं।
...
* यह लोड कमांड एक प्रकार की संरचना है **`dylib_command`** (जिसमें एक स्ट्रक्ट dylib होती है, वास्तविक निर्भर डायनामिक लाइब्रेरी का वर्णन करने वाली):
```objectivec
...
आप इस जानकारी को कमांड लाइन से भी प्राप्त कर सकते हैं:
```bash
otool -L /bin/ls
/bin/ls:
...
फ़ाइल के मूल में डेटा क्षेत्र होता है, जो लोड-कमांड्स क्षेत्र में परिभाषित कई सेगमेंटों से बना होता है। **प्रत्येक सेगमेंट में कई डेटा खंड हो सकते हैं**, प्रत्येक खंड में एक प्रकार के कोड या डेटा को धारित करता है।
