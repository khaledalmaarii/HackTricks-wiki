...
Mac OS बाइनरीज आम तौर पर **यूनिवर्सल बाइनरीज** के रूप में कॉम्पाइल की जाती हैं। एक **यूनिवर्सल बाइनरी** में **एक ही फ़ाइल में कई विश्वकर्माओं का समर्थन** किया जा सकता है।
...
हेडर में फ़ाइल के बारे में मूल जानकारी होती है, जैसे मैजिक बाइट्स ताकि इसे एक Mach-O फ़ाइल के रूप में पहचाना जा सके और लक्षित विश्वकर्म के बारे में जानकारी। आप इसे यहाँ पा सकते हैं: `mdfind loader.h | grep -i mach-o | grep -E "loader.h$"`
```c
...
**फ़ाइल का लेआउट मेमोरी में** यहाँ निर्दिष्ट किया गया है, **सिम्बल टेबल का स्थान** विस्तार से, कार्यान्वयन प्रारंभ पर मुख्य धागे का संदर्भ, और आवश्यक **साझा लाइब्रेरी**। निर्देश दिए गए हैं डायनामिक लोडर **(dyld)** को मेमोरी में बाइनरी को लोड करने की प्रक्रिया पर।
...
```
व्यवस्था विभिन्न प्रकार के लोड कमांड्स को अलग-अलग ढंग से संभालती है। सबसे सामान्य लोड कमांड्स हैं: `LC_SEGMENT_64`, `LC_LOAD_DYLINKER`, `LC_MAIN`, `LC_LOAD_DYLIB`, और `LC_CODE_SIGNATURE`।

### **LC\_SEGMENT/LC\_SEGMENT\_64**

{% hint style="success" %}
मूल रूप से, इस प्रकार के लोड कमांड व्यावसायिक कोड और प्रक्रिया के लिए डेटा सेगमेंट को भेजने के तरीके को परिभाषित करते हैं जब बाइनरी को निष्पादित किया जाता है में दिखाए गए ऑफसेट के अनुसार।
{% endhint %}
...
इनमें **विभिन्न प्रकार** के सेगमेंट होते हैं, जैसे **\_\_TEXT** सेगमेंट, जो किसी प्रोग्राम का व्यावसायिक कोड रखता है, और **\_\_DATA** सेगमेंट, जो प्रक्रिया द्वारा उपयोग किए जाने वाले डेटा को समेत रखता है। ये **सेगमेंट्स मैक-ओ फ़ाइल के डेटा सेक्शन में स्थित** होते हैं।

**प्रत्येक सेगमेंट** को और भी अधिक **खंडों में विभाजित** किया जा सकता है। **लोड कमांड संरचना** में **जानकारी** होती है **इन खंडों** के बारे में जो संबंधित सेगमेंट के भीतर होते हैं।
...
यह हेडर **उन खंडों की संख्या को परिभाषित करता है जिनके हेडर इसके बाद दिखाई देते हैं**:
```c
...
यदि आप **खंड ऑफसेट** (0x37DC) + **ऑफसेट** जो **आर्क शुरू होता है**, इस मामले में `0x18000` जोड़ते हैं --> `0x37DC + 0x18000 = 0x1B7DC`
...
**entryoff विशेषता में** प्रवेश बिंदु शामिल है। लोड समय पर, **dyld** बस (इन-मेमोरी) के **बेस में** इस मान को **जोड़ता** है, फिर इस निर्देशिका पर जाता है ताकि बाइनरी कोड का निष्पादन शुरू हो सके।

### **LC\_CODE\_SIGNATURE**

Macho-O फ़ाइल के **कोड हस्ताक्षर** के बारे में जानकारी शामिल है। यह केवल एक **ओफ़्सेट** शामिल करता है जो **हस्ताक्षर ब्लॉब** की ओर **इशारा करता है**। यह आम तौर पर फ़ाइल के अंत में होता है।\
...
डायनामिक लिंकर एक्जीक्यूटेबल के **पथ को शामिल** करता है जो शेयर्ड लाइब्रेरी को प्रक्रिया पता अंतरिक्ष में मैप करता है। **मान हमेशा `/usr/lib/dyld` पर सेट किया जाता है**। यह महत्वपूर्ण है कि macOS में, डायलिब मैपिंग **उपयोगकर्ता मोड** में होता है, कर्नेल मोड में नहीं।

### **`LC_LOAD_DYLIB`**

यह लोड कमांड एक **डायनामिक लाइब्रेरी** आवश्यकता का वर्णन करता है जो **लोडर** (dyld) को **उस लाइब्रेरी को लोड और लिंक करने के लिए निर्देशित** करता है। Mach-O बाइनरी के लिए प्रत्येक लाइब्रेरी के लिए एक LC\_LOAD\_DYLIB लोड कमांड होता है।

* यह लोड कमांड एक प्रकार की संरचना है **`dylib_command`** (जिसमें एक स्ट्रक्ट dylib होती है, जो वास्तविक निर्भर डायनामिक लाइब्रेरी का वर्णन करती है):
```objectivec
...
फ़ाइल के मूल में डेटा क्षेत्र होता है, जिसमें लोड-कमांड्स क्षेत्र में परिभाषित कई सेगमेंट होते हैं। **प्रत्येक सेगमेंट में कई डेटा खंड हो सकते हैं**, प्रत्येक खंड **कोड या डेटा** विशेष एक प्रकार के होते हैं।
