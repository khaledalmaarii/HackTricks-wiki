...
{% tabs %}
{% tab title="Імітація Chrome" %}
Деякі пропозиції:

* Перевірте в Dock, чи є там Chrome, і в такому випадку **видаліть** цей запис і **додайте** **фальшивий** **запис Chrome в такому ж місці** в масиві Dock.&#x20;
```bash
#!/bin/sh
...
{% tab title="Імітація Finder" %}
Деякі пропозиції:

* Ви **не можете видалити Finder з Dock**, тому якщо ви збираєтеся додати його до Dock, ви можете поставити підроблений Finder прямо поруч з реальним. Для цього вам потрібно **додати запис підробленого Finder на початку масиву Dock**.
...
* Попросіть Finder скопіювати новий **плагін авторизації** (Ви можете контролювати ім'я файлу, щоб запит на введення пароля вказував, що "Finder хоче скопіювати Finder.bundle")
```bash
#!/bin/sh
...
### CVE-2020-9771 - обхід TCC mount\_apfs та підвищення привілеїв
