...
可以滥用某些 Java 功能（如环境变量 **`_JAVA_OPTS`**）来使 Java 应用程序执行**任意代码/命令**。
...
可以通过**滥用 .Net 调试功能**（不受 macOS 保护，如运行时加固）来向 .Net 应用程序注入代码。
...
然而，大多数人使用 **Homebrew** 安装 Python，这会将 Python 安装在默认管理员用户可以写入的位置。您可以用类似以下方式劫持它：
```bash
