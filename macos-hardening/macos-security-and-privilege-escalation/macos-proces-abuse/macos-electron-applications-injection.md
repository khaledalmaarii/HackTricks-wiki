# macOS Elektronske Aplikacije Injekcija
...
Ove tehnike će biti razmatrane u nastavku, ali u poslednje vreme Electron je dodao nekoliko **sigurnosnih zastava kako bi ih sprečio**. To su [**Elektronski Fjuzovi**](https://www.electronjs.org/docs/latest/tutorial/fuses) i ovo su oni koji se koriste da **spreče** Electron aplikacije u macOS-u da **učitavaju proizvoljan kod**:

* **`RunAsNode`**: Ako je onemogućen, sprečava korišćenje env varijable **`ELECTRON_RUN_AS_NODE`** za ubacivanje koda.
...
* **`OnlyLoadAppFromAsar`**: Ako je ovo omogućeno, umesto traženja za učitavanjem u sledećem redosledu: **`app.asar`**, **`app`** i na kraju **`default_app.asar`**. Proveravaće i koristiće samo app.asar, čime se obezbeđuje da kada se **kombinuje** sa **`embeddedAsarIntegrityValidation`** fjuzom, bude **nemoguće** učitati nevalidiran kod.
* **`LoadBrowserProcessSpecificV8Snapshot`**: Ako je omogućeno, proces pretraživača koristi datoteku nazvanu `browser_v8_context_snapshot.bin` za svoj V8 snimak.

Još jedan interesantan fjuz koji neće sprečiti ubacivanje koda je:

* **EnableCookieEncryption**: Ako je omogućeno, cookie skladište na disku je enkriptovano korišćenjem kriptografskih ključeva na nivou OS-a.

### Provera Elektronskih Fjuzova
...
```
Možete učitati ovaj fajl na [https://hexed.it/](https://hexed.it/) i pretražiti prethodni string. Nakon ovog stringa možete videti u ASCII obliku broj "0" ili "1" koji pokazuje da li je svaka osigurač isključen ili uključen. Samo izmenite heksadecimalni kod (`0x30` je `0` i `0x31` je `1`) da **izmenite vrednosti osigurača**.

<figure><img src="../../../.gitbook/assets/image (34).png" alt=""><figcaption></figcaption></figure>

Imajte na umu da ako pokušate **prepisati** **binarni fajl Electron Framework-a** unutar aplikacije sa izmenjenim bajtovima, aplikacija neće raditi.
...
Ako ne postavite **`ELECTRON_RUN_AS_NODE`**, dobićete **grešku**: `Većina NODE_OPTION-a nije podržana u pakovanim aplikacijama. Pogledajte dokumentaciju za više detalja.`
{% endhint %}
...
{% hint style="danger" %}
Ako je osigurač **`EnableNodeCliInspectArguments`** onemogućen, aplikacija će **ignorisati node parametre** (kao što su `--inspect`) prilikom pokretanja, osim ako nije postavljena promenljiva okruženja **`ELECTRON_RUN_AS_NODE`**, koja će takođe biti **ignorisana** ako je osigurač **`RunAsNode`** onemogućen.
...
```
## Bypass TCC zloupotreba starijih verzija
