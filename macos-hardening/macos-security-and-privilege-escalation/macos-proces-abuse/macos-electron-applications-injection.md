...
## Adicionando código a Aplicações Electron

O código JS de um aplicativo Electron não é assinado, então um atacante poderia mover o aplicativo para um local gravável, injetar código JS malicioso e iniciar o aplicativo para abusar das permissões do TCC.
...
## Inspecionar Aplicação Electron
...
{% hint style="danger" %}
Observe que agora as aplicações Electron **fortificadas** com **RunAsNode** e **`EnableNodeCliInspectArguments`** desabilitados irão **ignorar os parâmetros do node** (como --inspect) ao serem iniciadas, a menos que a variável de ambiente **`ELECTRON_RUN_AS_NODE`** seja definida.

No entanto, ainda é possível usar o parâmetro `--remote-debugging-port=9229` do Electron, mas a carga útil anterior não funcionará para executar outros processos.

Você pode verificar essas flags em um aplicativo com:
```bash
...
{% hint style="warning" %}
Esta variável de ambiente só funcionará se a aplicação Electron não tiver sido devidamente protegida e permitir isso. Se estiver protegida, você também precisará usar a **variável de ambiente `ELECTRON_RUN_AS_NODE`**.
{% endhint %}
