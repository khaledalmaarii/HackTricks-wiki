...
- **`EnableEmbeddedAsarIntegrityValidation`**：如果启用，macOS 将验证加载的 **`asar`** **文件**。通过修改此文件的内容，以防止通过此方式注入代码。
...
正如[**在这里提出的**](https://www.trustedsec.com/blog/macos-injection-via-third-party-frameworks/)，您可以滥用这个环境变量在一个 plist 文件中来保持持久性：
```xml
...
使用参数 **`--remote-debugging-port=9222`** 可以从 Electron 应用程序中窃取一些信息，如**历史记录**（使用 GET 命令）或浏览器的**cookies**（因为它们在浏览器内部被**解密**，并且有一个**json端点**可以提供它们）。
