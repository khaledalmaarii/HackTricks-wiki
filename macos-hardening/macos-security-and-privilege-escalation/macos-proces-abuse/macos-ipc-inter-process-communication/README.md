...
एक प्रक्रिया एक विभिन्न कार्य को **कुछ अधिकारों के साथ एक अलग कार्य** को भेज सकती है और कर्नेल इसे **दूसरी कार्य की IPC तालिका में प्रविष्टि** कर देगा।

### पोर्ट अधिकार

पोर्ट अधिकार, जो यह परिभाषित करते हैं कि एक कार्य क्या कार्रवाई कर सकता है, इस संचार के लिए महत्वपूर्ण हैं। संभावित **पोर्ट अधिकार** हैं ([यहाँ से परिभाषाएँ](https://docs.darlinghq.org/internals/macos-specifics/mach-ports.html)):

* **प्राप्ति अधिकार**, जो पोर्ट को भेजे गए संदेश प्राप्त करने की अनुमति देता है। Mach पोर्ट्स MPSC (एकाधिक उत्पादक, एक उपभोक्ता) कतारें होती हैं, जिसका मतलब है कि पूरे सिस्टम में प्रत्येक पोर्ट के लिए केवल **एक प्राप्ति अधिकार** हो सकता है (जैसे कि पाइप्स में, जहां कई प्रक्रियाएं सभी एक पाइप के पढ़ने के अंत के लिए फ़ाइल डिस्क्रिप्टर्स को धारण कर सकती हैं)।
...
* अगर प्राप्ति अधिकार के मालिक **मर जाता है** या उसे मार देता है, तो **सेंड अधिकार अनर्थक (डेड नाम) हो जाता है**।
* **सेंड अधिकार**, जो पोर्ट को संदेश भेजने की अनुमति देता है।
* सेंड अधिकार को **क्लोन** किया जा सकता है ताकि सेंड अधिकार वाले एक कार्य अधिकार को क्लोन कर सके और इसे तीसरे कार्य को **प्रदान** कर सके।
...
* **पोर्ट सेट अधिकार**, जो एक _पोर्ट सेट_ को दर्शाता है बल्कि एक एकल पोर्ट नहीं। पोर्ट सेट से संदेश को डीक्यू करने पर यह उसमें शामिल पोर्टों में से एक से संदेश को डीक्यू करता है। पोर्ट सेट का उपयोग कई पोर्टों पर सुनने के लिए किया जा सकता है, बहुत सारे तरह से `select`/`poll`/`epoll`/`kqueue` की तरह।
* **डेड नाम**, जो वास्तव में एक पोर्ट अधिकार नहीं है, बल्कि केवल एक जगहधारी है। जब एक पोर्ट नष्ट होता है, तो पोर्ट के सभी मौजूदा पोर्ट अधिकार डेड नाम में बदल जाते हैं।

**कार्य SEND अधिकार को दूसरों को संदेश भेजने की अनुमति देने में सक्षम होते हैं**। **SEND अधिकार को क्लोन भी किया जा सकता है**, इसलिए एक कार्य एक संदेश भेजने की अनुमति को डुप्लिकेट कर सकता है और इसे तीसरे कार्य को दे सकता है। इसे **बूटस्ट्रैप सर्वर** के बीच एक मध्यस्थ प्रक्रिया के साथ मिलाकर, कार्यों के बीच प्रभावी संचार की अनुमति देता है।

### फाइल पोर्ट्स

फाइल पोर्ट्स को मैक पोर्ट्स में फाइल डिस्क्रिप्टर्स को एनकैप्सुलेट करने की अनुमति देते हैं (Mach पोर्ट अधिकार का उपयोग करके)। एक दिए गए FD से `fileport_makeport` का उपयोग करके `fileport` बनाना संभव है और `fileport_makefd` का उपयोग करके एक FD बनाना संभव है।

### संचार स्थापित करना

पहले से किसी अधिकार के बिना एक अधिकार भेजना संभव नहीं है, इसलिए, पहली संचार कैसे स्थापित की जाती है?

इसके लिए, **बूटस्ट्रैप सर्वर** (**मैक में launchd**) शामिल है, क्योंकि **हर कोई बूटस्ट्रैप सर्वर को SEND अधिकार प्राप्त कर सकता है**, इससे दूसरे प्रक्रिया को संदेश भेजने के लिए उससे अधिकार मांग सकता है:

1. कार्य **A** एक **नया पोर्ट बनाता है**, जिसे पर उसका **प्राप्ति अधिकार** होता है।
...
```
प्रक्रियाएँ जिनके पास _**प्राप्ति अधिकार**_ होते हैं, वे Mach पोर्ट पर संदेश प्राप्त कर सकती हैं। उल्टे, **भेजने वाले** को _**भेजने**_ या _**एक बार भेजने का अधिकार**_ प्रदान किया जाता है। एक बार भेजने का अधिकार केवल एक संदेश भेजने के लिए है, जिसके बाद यह अमान्य हो जाता है।
...
वाउचर, स्थानीय और दूरस्थ पोर्ट में निर्दिष्ट किए जा सकने वाले प्रकार हैं (स्रोत से [**mach/message.h**](https://opensource.apple.com/source/xnu/xnu-7195.81.3/osfmk/mach/message.h.auto.html)):
```c
...
{% hint style="danger" %}
ध्यान दें कि **मश संदेश** एक `मश पोर्ट` के माध्यम से भेजे जाते हैं, जो मश कर्नल में बनाया गया एक **एकल प्राप्तकर्ता**, **एकाधिक भेजने वाला** संचार चैनल है। **एकाधिक प्रक्रियाएँ** एक मश पोर्ट पर संदेश भेज सकती हैं, लेकिन किसी भी समय केवल **एक प्रक्रिया से पढ़ा जा सकता है**।
{% endhint %}
...
हालांकि, अन्य अधिक **जटिल** संदेश हैं, जैसे अतिरिक्त पोर्ट अधिकार या साझा मेमोरी पास करने वाले संदेश, जहां कर्नल को भी इन वस्तुओं को प्राप्तकर्ता को भेजने की आवश्यकता होती है। इस मामले में हेडर `msgh_bits` का सबसे महत्वपूर्ण बिट सेट किया जाता है।
...
- `mach_port_allocate` एक **पोर्ट सेट** भी बना सकता है: पोर्ट के समूह पर प्राप्ति अधिकार। जब भी एक संदेश प्राप्त होता है, तो इसमें संकेतित किया जाता है कि संदेश किस पोर्ट से आया था।
- `mach_port_allocate_name`: पोर्ट का नाम बदलें (डिफ़ॉल्ट रूप से 32बिट इंटीजर)
...
- `mach_port_allocate`: एक नया RECEIVE, PORT\_SET या DEAD\_NAME आवंटित करें
- `mach_port_insert_right`: उस पोर्ट में एक नया अधिकार बनाएं जिस पर आपके पास RECEIVE है
- `mach_port_...`
- **`mach_msg`** | **`mach_msg_overwrite`**: **मैक संदेश** भेजने और प्राप्त करने के लिए उपयोग किए जाने वाले फ़ंक्शन। ओवरराइट संस्करण संदेश प्राप्ति के लिए एक विभिन्न बफ़र निर्दिष्ट करने की अनुमति देता है (अन्य संस्करण इसे बस पुनः उपयोग करेगा)।
```c
...
[...]
```
**नाम** पोर्ट का डिफ़ॉल्ट नाम है (जांचें कि पहले 3 बाइट में कैसे **बढ़ रहा** है)। **`ipc-object`** पोर्ट की **विशिष्ट पहचानकर्ता** है जिसे **अस्पष्ट** बनाया गया है।
ध्यान दें कि केवल **`send`** अधिकार वाले पोर्ट्स कैसे इसके मालिक की पहचान कर रहे हैं (पोर्ट नाम + pid)।
यहां ध्यान दें कि **`+`** का उपयोग करके **दूसरे कार्यों को संबोधित** करने के लिए किया गया है।

यह भी संभव है [**procesxp**](https://www.newosxbook.com/tools/procexp.html) का उपयोग करके **पंजीकृत सेवा नामों** (SIP अक्षम होने के कारण `com.apple.system-task-port` की आवश्यकता होने पर) देखने के लिए:
```
procesp 1 ports
```
...
* **टास्क नाम पोर्ट:** _टास्क पोर्ट_ का एक अनुचित संस्करण। यह टास्क को संदर्भित करता है, लेकिन इसे नियंत्रित करने की अनुमति नहीं देता। इसके माध्यम से केवल `task_info()` उपलब्ध लगता है।
* **टास्क पोर्ट** (जिसे कर्नेल पोर्ट भी कहा जाता है)**:** इस पोर्ट पर भेजने की अनुमति होने पर टास्क को नियंत्रित करना संभव है (मेमोरी पढ़ना/लिखना, थ्रेड बनाना...।)
* कॉल करें `mach_task_self()` ताकि कॉलर टास्क के लिए इस पोर्ट का नाम प्राप्त कर सके। यह पोर्ट केवल **`exec()`** के अवसर पर **विरासत** में जाता है; `fork()` के साथ नया टास्क बनाया जाता है (एक विशेष मामले के रूप में, एक टास्क को `exec()` के बाद भी एक नया टास्क पोर्ट मिलता है।) एक टास्क को उत्पन्न करने और इसका पोर्ट प्राप्त करने का एकमात्र तरीका "पोर्ट स्वैप नृत्य" करके किया जा सकता है जब `fork()` किया जाता है।
* इस पोर्ट तक पहुंचने की प्रतिबंधाएँ (बाइनरी `AppleMobileFileIntegrity` से `macos_task_policy` से):
* यदि ऐप के पास **`com.apple.security.get-task-allow` entitlement** है तो **एक ही उपयोगकर्ता के प्रक्रियाएँ टास्क पोर्ट तक पहुंच सकती हैं** (डीबगिंग के लिए Xcode द्वारा सामान्य रूप से जोड़ा जाता है)। **नोटराइजेशन** प्रक्रिया इसे उत्पादन रिलीज़ में नहीं देगी।
...
* **रूट उस ऐप्लिकेशन के टास्क पोर्ट तक पहुंच सकता है** जिन्हें **हार्डन्ड** रनटाइम के साथ कंपाइल नहीं किया गया है (और न किसी भी Apple से)।

### शैलकोड इन्जेक्शन थ्रेड में टास्क पोर्ट के माध्यम से
...
यहाँ आपको एक उदाहरण दिया गया है कि कैसे आप entitlements.plist फ़ाइल का उपयोग करके IPC (इंटर-प्रोसेस कम्यूनिकेशन) के लिए अधिकार सेट कर सकते ह। इस फ़ाइल में अधिकारों की सूची होती है जो एक प्रक्रिया को अन्य प्रक्रियाओं के साथ संवाद करने की अनुमति देती है। इसे सुरक्षित रखना महत्वपूर्ण है ताकि अनधिकृत प्रक्रियाएँ इस्तेमाल न कर सकें।

आप entitlements.plist फ़ाइल में अधिकार जोड़ सकते ह। इसे उपयोग करने के लिए, आपको Xcode या टर्मिनल का उपयोग करके एप्लिकेशन को साइन करना होगा। इसके बाद, आप एप्लिकेशन को चलाने के लिए नए अधिकारों के साथ उसे सेट कर सकते ह।

यह एक प्रभावी तकनीक है जो IPC सुरक्षा को मजबूत करने में मदद कर सकती ह।
...
एक सरल शैलकोड को इंजेक्ट करना संभव था ताकि एक कमांड को निष्पादित किया जा सके क्योंकि इसे **posix के साथ काम करने की आवश्यकता नहीं थी**, केवल Mach के साथ। **अधिक जटिल इंजेक्शन** के लिए **थ्रेड** को भी **posix अनुरूप होना** चाहिए।

इसलिए, **थ्रेड** को सुधारने के लिए यह कहा जा सकता है कि यह **`pthread_create_from_mach_thread`** को कॉल करना चाहिए जो एक वैध pthread बनाएगा। फिर, इस नए pthread को **dlopen** को कॉल करने के लिए एक dylib लोड करने के लिए संभव होगा, इसलिए नए shellcode लिखने की बजाय अनुकूलित पुस्तकालयों को लोड करना संभव होगा।
...
MIG को **मैक IPC** कोड निर्माण की प्रक्रिया को सरल बनाने के लिए बनाया गया था। यह मौलिक रूप से एक विशिष्ट परिभाषा के साथ सर्वर और क्लाइंट के बीच संवाद के लिए आवश्यक कोड **उत्पन्न** करता है। यदी उत्पन्न कोड बेहद बदसूरत हो, तो एक डेवलपर को उसे आयात करने की आवश्यकता होगी और उसका कोड पहले की तुलना में बहुत सरल होगा।
