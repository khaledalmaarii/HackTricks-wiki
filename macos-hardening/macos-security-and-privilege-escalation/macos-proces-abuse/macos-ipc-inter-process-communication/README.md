...
Τα μηνύματα σχηματίζονται από την κεφαλίδα **`mach_msg_header_t`** ακολουθούμενη από το **σώμα** και από το **τρέιλερ** (εάν υπάρχει) και μπορεί να επιτρέψει την άδεια απάντησης σε αυτό. Σε αυτές τις περιπτώσεις, ο πυρήνας χρειάζεται απλώς να περάσει το μήνυμα από τη μία εργασία στην άλλη.
...
Οι δυνατοί περιγραφείς που περνούν ορίζονται στο [**`mach/message.h`**](https://opensource.apple.com/source/xnu/xnu-7195.81.3/osfmk/mach/message.h.auto.html):
```c
...
```
Στα 32bits, όλοι οι περιγραφείς είναι 12B και ο τύπος του περιγραφέα βρίσκεται στον 11ο. Στα 64 bits, τα μεγέθη ποικίλουν.

{% hint style="danger" %}
Το πυρήνας θα αντιγράψει τους περιγραφείς από μια εργασία σε μια άλλη, αλλά πρώτα **δημιουργεί ένα αντίγραφο στη μνήμη του πυρήνα**. Αυτή η τεχνική, γνωστή ως "Feng Shui", έχει καταχραστεί σε αρκετές εκμεταλλεύσεις για να κάνει τον **πυρήνα να αντιγράψει δεδομένα στη μνήμη του** κάνοντας ένα διεργασία να στείλει περιγραφείς στον εαυτό της. Στη συνέχεια η διεργασία μπορεί να λάβει τα μηνύματα (ο πυρήνας θα τα απελευθερώσει).
...
Σημειώστε ότι οι θύρες συσχετίζονται με το χώρο ονομάτων της εργασίας, έτσι για να δημιουργήσετε ή να αναζητήσετε μια θύρα, το χώρο ονομάτων της εργασίας επίσης ερευνάται (περισσότερα στο `mach/mach_port.h`):
...
* `mach_port_allocate`: Εκχώρηση νέου ΛΗΨΗΣ, ΣΥΝΟΛΟΥ_ΘΥΡΩΝ ή DEAD_NAME
...
{% tab title="sender.c" %}Αυτός ο κώδικας αποστολέας δημιουργεί έναν UNIX domain socket και στέλνει ένα μήνυμα στον παραλήπτη. Ο παραλήπτης πρέπει να εκτελείται πριν τον αποστολέα.{% endtab %}
```c
// Code from https://docs.darlinghq.org/internals/macos-specifics/mach-ports.html
...
{% tab title="entitlements.plist" %}Το αρχείο entitlements.plist περιέχει τις ειδικές άδειες που απαιτούνται από μια εφαρμογή για να εκτελέσει συγκεκριμένες λειτουργίες στο macOS. Αυτές οι άδειες μπορούν να περιορίσουν την πρόσβαση σε συγκεκριμένους πόρους του συστήματος, προστατεϋοντας έτσι την ασφάλεια του συστήματος από κακόβουλες επιθέσεις. Είναι σημαντικό να διαχειρίζεστε σωστά τις ειδικές άδειες στο macOS προκειμένου να αποτρέϋετε την κατάχρηση διεργασιών και την επέκταση προνομίων.{% endtab %}
```xml
...
**Μεταγλωττίστε** το προηγούμενο πρόγραμμα και προσθέστε τα **δικαιώματα** ώστε να μπορείτε να εισάγετε κώδικα με τον ίδιο χρήστη (αν όχι, θα χρειαστεί να χρησιμοποιήσετε **sudo**).
...
Ήταν δυνατό να **διεισδύσουμε ένα απλό shellcode** για να εκτελέσουμε έναν εντολή επειδή **δεν χρειαζόταν να λειτουργήσει με posix** συμμορφωμένα apis, μόνο με το Mach. **Πιο πολύπλοκες διεισδύσεις** θα χρειαζόντουσαν το **νήμα** να είναι επίσης **συμμορφωμένο με το posix**.
...
Για περισσότερες πληροφορίες σχετικά με το πως αυτή η **επικοινωνία λειτουργεί** και πώς **μπορεί να είναι ευάλωτη** ελέγξτε:
