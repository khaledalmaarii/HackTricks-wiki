...
* Se impostato su `true`, il proprietario della sessione (l'utente attualmente connesso) otterrebbe automaticamente questo diritto. Ciò potrebbe bypassare ulteriori autenticazioni se l'utente è già connesso.
4. **'shared': 'true'**
...
Se trovi la funzione: **`[HelperTool checkAuthorization:command:]`** probabilmente il processo sta utilizzando lo schema di autorizzazione menzionato in precedenza:
...
Controlla il **`/var/db/auth.db`** per vedere se è possibile ottenere autorizzazioni per chiamare azioni privilegiate senza interazione dell'utente.

### Comunicazione di Protocollo
