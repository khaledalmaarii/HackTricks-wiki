...
这个示例使用原始的 \*\*\`fork\`\*\* 来启动 \*\*利用PID竞争条件的子进程\*\*，然后通过硬链接利用 \*\*另一个竞争条件\*\*： \`\`\`objectivec // export OBJC\_DISABLE\_INITIALIZE\_FORK\_SAFETY=YES // gcc -framework Foundation expl.m -o expl
