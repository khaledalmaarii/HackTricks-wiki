# Ataque xpc\_connection\_get\_audit\_token macOS
...
As mensagens Mach são enviadas por uma _porta mach_, que é um canal de comunicação **único receptor, múltiplos remetentes** integrado ao kernel mach. **Múltiplos processos podem enviar mensagens** para uma porta mach, mas em qualquer momento **apenas um processo pode lê-la**. Assim como descritores de arquivo e soquetes, as portas mach são alocadas e gerenciadas pelo kernel e os processos veem apenas um número inteiro, que podem usar para indicar ao kernel qual de suas portas mach desejam usar.
