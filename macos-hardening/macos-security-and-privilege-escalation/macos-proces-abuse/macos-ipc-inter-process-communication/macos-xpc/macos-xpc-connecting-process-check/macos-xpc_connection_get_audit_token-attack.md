...
**这个技术是从**[**https://sector7.computest.nl/post/2023-10-xpc-audit-token-spoofing/**](https://sector7.computest.nl/post/2023-10-xpc-audit-token-spoofing/) **复制过来的**

## Mach消息基本信息

如果你不知道Mach消息是什么，请查看这个页面：
...
暂时记住以下内容：\
Mach消息通过一个_mach端口_发送，这是一个内置在mach内核中的**单接收器、多发送器通信**通道。**多个进程可以向一个mach端口发送消息**，但在任何时刻**只有一个进程可以从中读取**。就像文件描述符和套接字一样，mach端口由内核分配和管理，进程只看到一个整数，它们可以用来指示内核它们想要使用的mach端口。
...
* 审计令牌经常用于授权检查，以决定是否接受连接。由于这是使用对服务端口的消息进行的，所以**尚未建立连接**。在此端口上的更多消息只会被处理为额外的连接请求。因此，**在接受连接之前的任何检查都不会有漏洞**（这也意味着在`-listener:shouldAcceptNewConnection:`中，审计令牌是安全的）。因此，我们正在**寻找验证特定操作的XPC连接**。
...
* 服务**A**在**不在**`dispatch_async`的事件处理程序**内部**时调用**`xpc_connection_get_audit_token`**。
