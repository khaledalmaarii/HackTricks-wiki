...
**Pour plus d'informations, consultez l'article original :** [**https://sector7.computest.nl/post/2023-10-xpc-audit-token-spoofing/**](https://sector7.computest.nl/post/2023-10-xpc-audit-token-spoofing/). Ceci est un résumé :
...
Pour le moment, retenez que ([définition à partir d'ici](https://sector7.computest.nl/post/2023-10-xpc-audit-token-spoofing)) :\
...
Par conséquent, le service **B** est **`diagnosticd`** car il s'exécute en tant que **root** et peut être utilisé pour **surveiller** un processus, donc une fois la surveillance démarrée, il **enverra plusieurs messages par seconde**.
...
4. L'étape suivante consiste à demander à `diagnosticd` d'initier la surveillance d'un processus choisi (potentiellement celui de l'utilisateur). Simultanément, une vague de messages 1004 de routine est envoyée à `smd`. L'intention ici est d'installer un outil avec des privilèges élevés.
...
Cette distinction est cruciale car elle permet la possibilité de **parser les paquets de réponse de manière concurrente avec l'exécution d'un gestionnaire d'événements XPC**. Notamment, bien que `_xpc_connection_set_creds` implémente un verrouillage pour se protéger contre l'écrasement partiel du jeton d'audit, il n'étend pas cette protection à l'objet de connexion entier. Par conséquent, cela crée une vulnérabilité où le jeton d'audit peut être remplacé pendant l'intervalle entre l'analyse d'un paquet et l'exécution de son gestionnaire d'événements.
...
* **Réponse d'Apple** : Apple a traité le problème dans `smd` en remplaçant `xpc_connection_get_audit_token` par `xpc_dictionary_get_audit_token`.
* **Nature de la Correction** : La fonction `xpc_dictionary_get_audit_token` est considérée comme sécurisée car elle récupère le jeton d'audit directement du message mach lié au message XPC reçu. Cependant, elle ne fait pas partie de l'API publique, similaire à `xpc_connection_get_audit_token`.
* **Absence d'une Correction Plus Large** : Il n'est pas clair pourquoi Apple n'a pas mis en place une correction plus complète, comme le rejet des messages ne correspondant pas au jeton d'audit enregistré de la connexion. La possibilité de changements légitimes du jeton d'audit dans certains scénarios (par ex., l'utilisation de `setuid`) pourrait être un facteur.
