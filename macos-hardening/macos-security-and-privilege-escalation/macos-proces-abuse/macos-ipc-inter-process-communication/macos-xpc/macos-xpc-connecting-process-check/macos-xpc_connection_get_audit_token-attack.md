...
Na chwilę obecną pamiętaj, że ([definicja stąd](https://sector7.computest.nl/post/2023-10-xpc-audit-token-spoofing)):\
...
* Obsługa zdarzeń XPC jest obsługiwana synchronicznie. Oznacza to, że obsługa zdarzenia dla jednej wiadomości musi zostać zakończona przed jej wywołaniem dla następnej, nawet na równoległych kolejkach dyspozytorskich. Dlatego wewnątrz **obsługi zdarzeń XPC token audytu nie może zostać nadpisany** przez inne normalne (nie-odpowiedzi!) wiadomości.

Istnieją dwa różne sposoby, w jakie to może być wykorzystane:
...
To zdarzyło się z **`A`** jako `smd` i **`B`** jako `diagnosticd`. Funkcja [`SMJobBless`](https://developer.apple.com/documentation/servicemanagement/1431078-smjobbless?language=objc) z smb może być użyta do zainstalowania nowego narzędzia pomocniczego z uprawnieniami (jako **root**). Jeśli **proces działa jako root** i kontaktuje się z **smd**, nie będą wykonywane żadne inne kontrole.

Dlatego usługa **B** to **`diagnosticd`**, ponieważ działa jako **root** i może być użyta do **monitorowania** procesu, więc gdy monitorowanie zostanie uruchomione, będzie **wysyłać wiele wiadomości na sekundę.**
...
4. Następnym krokiem jest zlecanie `diagnosticd` rozpoczęcia monitorowania wybranego procesu (potencjalnie własnego użytkownika). Jednocześnie wysyłana jest fala rutynowych wiadomości 1004 do `smd`. Celem jest zainstalowanie narzędzia z podwyższonymi uprawnieniami.
5. Ta akcja wywołuje warunek wyścigowy wewnątrz funkcji `handle_bless`. Czas jest kluczowy: wywołanie funkcji `xpc_connection_get_pid` musi zwrócić PID procesu użytkownika (ponieważ narzędzie uprzywilejowane znajduje się w pakiecie aplikacji użytkownika). Jednak funkcja `xpc_connection_get_audit_token`, w szczególności w podprogramie `connection_is_authorized`, musi odnosić się do tokenu audytu należącego do `diagnosticd`.
...
2. Zamiast odpowiadać bezpośrednio na **`A`**, port odpowiedzi jest przejęty i użyty do wysłania wiadomości do usługi **`B`**.
...
* **Narzędzia Analizy**: Narzędzia takie jak IDA/Ghidra były używane do badania osiągalnych usług mach, ale proces był czasochłonny, skomplikowany przez wywołania związane z pamięcią podręczną dyld.
...
* **Charakter Naprawy**: Funkcja `xpc_dictionary_get_audit_token` jest uważana za bezpieczną, ponieważ pobiera token audytu bezpośrednio z wiadomości mach powiązanej z otrzymaną wiadomością XPC. Jednak nie jest to część publicznego interfejsu API, podobnie jak `xpc_connection_get_audit_token`.
* **Brak Szerokiej Naprawy**: Nie jest jasne, dlaczego Apple nie zaimplementowało bardziej kompleksowej naprawy, takiej jak odrzucanie wiadomości niezgodnych z zapisanym tokenem audytu połączenia. Może to być spowodowane możliwością legitimizacji zmiany tokenu audytu w określonych scenariuszach (np. użycie `setuid`).
