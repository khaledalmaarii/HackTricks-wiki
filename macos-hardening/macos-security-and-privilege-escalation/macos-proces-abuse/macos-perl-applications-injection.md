...
```
一些返回的文件夹甚至不存在，然而，**`/Library/Perl/5.30`** 确实**存在**，它**没有**被 **SIP** 保护，并且位于**被 SIP 保护的文件夹之前**。因此，有人可以滥用该文件夹，在其中添加脚本依赖项，以便高权限 Perl 脚本加载它。

{% hint style="warning" %}
但请注意，您**需要是 root 用户才能在该文件夹中写入**，而现在您将会收到这个**TCC提示**：
{% endhint %}
