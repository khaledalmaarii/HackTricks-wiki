...
Bu bir atlatma değil, sadece TCC'nin nasıl çalıştığıdır: **Yazmayı korumaz**. Eğer Terminal, bir kullanıcının Masaüstünü okuma erişimine sahip değilse, **yine de içine yazabilir**:
```shell-session
...
Ayrıca, meşru uygulamayı Dock'tan kaldırmak ve sahte olanı yerine koymak mümkündür, böylece kullanıcı sahte olanı (aynı simgeyi kullanabilir) tıkladığında, sahte olanın meşru olanı çağırabileceği, TCC izinlerini isteyebileceği ve kötü amaçlı yazılımı yürütebileceği inancına kapılabilir.
...
İşte bazı **zararlı yazılımların bu korumayı atlatmayı** nasıl başardığının örnekleri:
...
Dosyalara **belirli bir uygulamanın okuma izinlerini vermek** için **`com.apple.macl`** özniteliği verilir. Bu öznitelik, bir dosyayı bir uygulamanın üzerine sürükleyip bıraktığınızda veya bir kullanıcı dosyayı varsayılan uygulama ile açmak için çift tıkladığında ayarlanır.
...
**`com.apple.private.icloud-account-access`** ayrıcalığı ile **`com.apple.iCloudHelper`** XPC hizmeti ile iletişim kurulabilir ve bu hizmet **iCloud belgelerini sağlayabilir**.

**iMovie** ve **Garageband** bu ayrıcalığa sahipti ve diğerleri de vardı.
...
Notlar, TCC korumalı konumlara erişime sahipti ancak bir not oluşturulduğunda bu **korumalı olmayan bir konumda oluşturulur**. Bu nedenle, notlara korumalı bir dosyayı bir not içine (yani korumalı olmayan bir konuma) kopyalamalarını ve ardından dosyaya erişmelerini isteyebilirsiniz:
...
**`Müzik`** ilginç bir özelliğe sahiptir: Çalıştığında, kullanıcının "ortam kütüphanesine" **`~/Music/Music/Media.localized/Automatically Add to Music.localized`** bırakılan dosyaları **ithal eder**. Dahası, şuna benzer bir şeyi çağırır: **`rename(a, b);`** burada `a` ve `b` şunlardır:
...
Eğer **`SQLITE_SQLLOG_DIR="path/folder"`** ise, temelde **herhangi bir açık db o yola kopyalanır**. Bu CVE'de bu kontrol, **TCC veritabanına sahip bir işlem tarafından açılacak olan bir SQLite veritabanına iç yazmak** için kötüye kullanıldı ve ardından **`SQLITE_SQLLOG_DIR`**'yi **dosya adındaki sembolik bağlantı ile** kötüye kullanarak, bu veritabanı **açıldığında**, kullanıcı **TCC.db'si açılan ile üzerine yazılır**.
...
Eğer çevre değişkeni **`SQLITE_AUTO_TRACE`** ayarlanmışsa, **`libsqlite3.dylib`** kütüphanesi tüm SQL sorgularını **günlüğe kaydetmeye** başlayacaktır. Birçok uygulama bu kütüphaneyi kullandığından, tüm SQLite sorgularını günlüğe kaydetmek mümkündü.
...
Bu **çevre değişkeni, `Metal` çerçevesi tarafından kullanılır** ve çeşitli programların bağımlılığıdır, en önemlisi `Music` programıdır, FDA'ya sahiptir.

Aşağıdakini ayarlayarak: `MTL_DUMP_PIPELINES_TO_JSON_FILE="yol/isim"`. Eğer `yol` geçerli bir dizin ise, hata tetiklenecek ve programda neler olup bittiğini görmek için `fs_usage` kullanabiliriz:
...
* `path/.dat.nosyncXXXX.XXXXXX` `rename()` ile `path/isim` olarak yeniden adlandırılacak

Bu geçici bir dosya yazma işlemidir, ardından **güvenli olmayan bir şekilde** **`rename(eski, yeni)`** işlemi gerçekleşir.

Bu güvenli değildir çünkü **eski ve yeni yolları ayrı ayrı çözmesi** gerekir, bu da bir süre alabilir ve Yarış Koşulu'na karşı savunmasız olabilir. Daha fazla bilgi için `xnu` fonksiyonu olan `renameat_internal()`'ı kontrol edebilirsiniz.
...
Yeniden adlandırma, kontrol ettiğiniz bir klasöre erişirse ve kaynak dosyayı değiştirmişseniz veya bir FD'si varsa, hedef dosyayı (veya klasörü) bir sembolik bağa işaret etmek için değiştirebilirsiniz, böylece istediğiniz zaman yazabilirsiniz.
{% endhint %}

Bu, CVE'deki saldırıydı: Örneğin, kullanıcının `TCC.db` dosyasını üzerine yazmak için şunları yapabiliriz:

* `/Users/hacker/ourlink`'i `/Users/hacker/Library/Application Support/com.apple.TCC/`'ye yönlendiren bir sembolik bağ oluşturun
...
Root olarak bu hizmeti etkinleştirebilir ve **ARD ajanı tam disk erişimine sahip olacak** ve ardından bir kullanıcı tarafından kötüye kullanılabilir ve yeni bir **TCC kullanıcı veritabanı** kopyalamasını yapabilir.

## **NFSHomeDirectory** ile

TCC, kullanıcının EV Klasörü'ndeki bir veritabanını kontrol etmek için kullanır, bu da kullanıcının EV Klasörü'ne özgü kaynaklara erişimi kontrol eder **$HOME/Library/Application Support/com.apple.TCC/TCC.db**.\
Bu nedenle, kullanıcı, TCC'yi **farklı bir klasöre işaret eden bir $HOME çevre değişkeni ile yeniden başlatmayı başarırsa**, kullanıcı **/Library/Application Support/com.apple.TCC/TCC.db**'de yeni bir TCC veritabanı oluşturabilir ve TCC'yi herhangi bir uygulamaya herhangi bir TCC iznini vermesi için kandırabilir.

{% hint style="success" %}
Apple, **`NFSHomeDirectory`** özelliğinde saklanan ayarları kullanır ve **`$HOME`** değerini belirlemek için kullanıcının profilindeki ayarı kullanır, bu nedenle bu değeri değiştirmeye izin veren bir uygulamayı ele geçirirseniz (**`kTCCServiceSystemPolicySysAdminFiles`**), bu seçeneği bir TCC atlatma ile **silahlandırabilirsiniz**.
{% endhint %}
...
Eklentiler, genellikle kütüphane veya plist biçiminde ekstra kodlardır, ana uygulama tarafından **yüklenir** ve kendi bağlamı altında çalıştırılır. Bu nedenle, ana uygulamanın TCC kısıtlı dosyalara erişimi varsa (izin verilen izinler veya yetkilendirmeler aracılığıyla), **özel kodun da bunu** olacaktır.
...
Bu CVE'yi silahlandırmak için, **`NFSHomeDirectory`** değiştirilir (önceki yetkiyi kötüye kullanarak) ve TCC'yi atlatmak için kullanıcıların TCC veritabanını ele geçirebilmek için **değiştirilir**.
...
Core Media I/O aracılığıyla kamera akışını açan sistem uygulamaları (**`kTCCServiceCamera`'ya sahip uygulamalar**) **bu eklentileri** `/Library/CoreMediaIO/Plug-Ins/DAL` konumunda yükler (SIP kısıtlaması olmadan).

Oraya yaygın **kurucu** ile bir kütüphane depolamak, **kod enjekte etmek** için çalışacaktır.

Birçok Apple uygulaması buna karşı savunmasızdı.

### Firefox

Firefox uygulamasında `com.apple.security.cs.disable-library-validation` ve `com.apple.security.cs.allow-dyld-environment-variables` yetkileri vardı:
```xml
...
Telegram'ın **`com.apple.security.cs.allow-dyld-environment-variables`** ve **`com.apple.security.cs.disable-library-validation`** yetkileri vardı, bu nedenle kamerayla kayıt yapma gibi izinlerine **erişmek mümkündü**. [**Payload'ı yazıda bulabilirsiniz**](https://danrevah.github.io/2023/05/15/CVE-2023-26818-Bypass-TCC-with-Telegram/).

Özel bir plist dosyası oluşturularak bu kütüphaneyi enjekte etmek ve bunu başlatmak için **`launchctl`** kullanılarak bir ortam değişkenini yüklemek için nasıl kullanılacağına dikkat edin:
```xml
<?xml version="1.0" encoding="UTF-8"?>
...
Terminal **Tam Disk Erişimi (TDE)** vermek oldukça yaygındır, en azından teknik kişiler tarafından kullanılan bilgisayarlarda. Ve **`.terminal`** betiklerini bu erişimle çağırmak mümkündür.
...
**Herhangi bir kullanıcı** (hatta ayrıcalıklı olmayanlar bile) bir zaman makinesi anı görüntüsü oluşturabilir ve bağlayabilir ve o anı görüntüsündeki **TÜM dosyalara erişebilir**.\
...
### CVE-2021-1784 & CVE-2021-30808 - TCC dosyası üzerinden bağlama

TCC DB dosyası korunsa bile, yeni bir TCC.db dosyasını **dizin üzerine bağlamak** mümkündü:
```bash
# CVE-2021-1784
...
Aşağıdaki yöntemler, macOS TCC (İzinler ve Gizlilikler) mekanizmasını atlamak için kullanılabilir:  

- **TCC-DB Dosyası Değiştirme**: TCC veritabanı dosyasını değiştirerek uygulamaların erişim izinlerini değiştirebilirsiniz.  
- **TCC-Util Kullanımı**: TCC-Util aracını kullanarak uygulamaların erişim izinlerini değiştirebilirsiniz.  
- **Masaüstü Uygulamaları**: Masaüstü uygulamaları, TCC izinlerini atlayabilir ve kullanıcı izni olmadan sistem kaynaklarına erişebilir.  
- **Sistem Uygulamaları**: Sistem uygulamaları, TCC izinlerini atlayabilir ve kullanıcı izni olmadan sistem kaynaklarına erişebilir.  
- **Kötü Amaçlı Yazılım**: Kötü amaçlı yazılımlar, TCC izinlerini atlayabilir ve kullanıcı izni olmadan sistem kaynaklarına erişebilir.  

Bu yöntemler, macOS TCC mekanizmasını atlamak için kullanılabilecek çeşitli yöntemleri açıklamaktadır.
```python
