...
Dlatego użytkownik mógłby **zarejestrować złośliwą aplikację**, aby obsługiwała wszystkie rozszerzenia i wywołać usługi uruchamiania do **otwarcia** dowolnego pliku (więc złośliwy plik otrzyma dostęp do odczytu).
...
Jeśli aplikacja ma dostęp nad Finderem, może uruchomić skrypt podobny do tego:
...
Jeśli **`SQLITE_SQLLOG_DIR="ścieżka/folder"`**, oznacza to w zasadzie, że **każda otwarta baza danych jest kopiowana do tej ścieżki**. W tej CVE to sterowanie zostało wykorzystane do **zapisania** w **bazie danych SQLite**, która ma być **otwarta przez proces z bazą danych TCC**, a następnie wykorzystanie **`SQLITE_SQLLOG_DIR`** z **symlinkiem w nazwie pliku**, więc gdy ta baza danych jest **otwarta**, baza danych użytkownika **TCC.db jest nadpisywana** otwartą bazą danych.\
...
Jest to tymczasowa operacja zapisu pliku, po której następuje **`rename(stara, nowa)`** **która nie jest bezpieczna.**
...
Jeśli zmiana nazwy dotyczy folderu, który kontrolujesz, podczas gdy zmodyfikowałeś plik źródłowy lub masz do niego FD, zmieniasz plik docelowy (lub folder) na symlink, dzięki czemu możesz pisać kiedy chcesz.
{% endhint %}
...
* przechwycić `open()` `/Users/hacker/tmp/.dat.nosyncXXXX.XXXXXX` (X to losowa wartość)
* tutaj również otwieramy ten plik do zapisu i zatrzymujemy deskryptor pliku
* zamienić atomowo `/Users/hacker/tmp` z `/Users/hacker/ourlink` **w pętli**
* robimy to, aby zwiększyć szanse na sukces, ponieważ okno wyścigu jest dość wąskie, ale przegrana w wyścigu ma znikome skutki uboczne
* poczekaj chwilę
* sprawdź, czy mieliśmy szczęście
...
Binarny **`/usr/sbin/coreaudiod`** miał uprawnienia `com.apple.security.cs.disable-library-validation` oraz `com.apple.private.tcc.manager`. Pierwsze umożliwia **wstrzykiwanie kodu**, a drugie daje dostęp do **zarządzania TCC**.
...
Aplikacje systemowe, które otwierają strumień kamery za pomocą Core Media I/O (aplikacje z **`kTCCServiceCamera`**) wczytują **w procesie te wtyczki** znajdujące się w `/Library/CoreMediaIO/Plug-Ins/DAL` (nieobjęte SIP).
