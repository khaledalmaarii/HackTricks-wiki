...
Mshambuliaji anaweza **kuunda programu zenye jina lolote** (k.m. Finder, Google Chrome...) katika **`Info.plist`** na kufanya iombe upatikanaji wa eneo fulani lililolindwa na TCC. Mtumiaji atadhani kuwa programu halali ndiyo inayoomba upatikanaji huu.\
Zaidi ya hayo, inawezekana **kuondoa programu halali kutoka Dock na kuweka ile bandia**, hivyo mtumiaji akibofya ile bandia (ambayo inaweza kutumia alama ile ile) inaweza kuita ile halali, kuomba ruhusa za TCC na kutekeleza zisizo, hivyo kufanya mtumiaji aamini kuwa programu halali ndiyo iliyoomba upatikanaji.
...
Hapa unaweza kupata mifano ya jinsi baadhi ya **malware zimefanikiwa kukiuka ulinzi huu**:
...
Mkusanyiko wa muda mrefu **`com.apple.macl`** hutolewa kwa faili ili kumpa **programu fulani ruhusa za kuisoma.** Mkusanyiko huu unawekwa wakati wa **kuburuta\&kushusha** faili juu ya programu, au wakati mtumiaji **anabofya mara mbili** faili kuifungua na programu ya msingi.
...
Programu yenye ruhusa ya **`kTCCServiceAppleEvents`** itaweza **kudhibiti Programu Nyingine**. Hii inamaanisha kuwa inaweza **kutumia vibali vilivyotolewa kwa Programu Nyingine**.
...
Hii **`rename(a, b);`** inayotendeka ni dhaifu kwa **Hali ya Mashindano**, kwani inawezekana kuweka faili bandia ya **TCC.db** ndani ya folda ya `Ongeza Moja kwa Muziki.localized` na kisha wakati folda mpya (b) inapotengenezwa kuchukua faili, ifute, na ielekeze kwa **`~/Maktaba/Msaada wa Maombi/com.apple.TCC`**/.
...
**Maelezo zaidi** [**katika andiko**](https://gergelykalman.com/sqlol-CVE-2023-32422-a-macos-tcc-bypass.html) **na**[ **katika hotuba**](https://www.youtube.com/watch?v=f1HA5QhLQ7Y\&t=20548s).
...
* zindua kosa kwa kukimbia `Music` na hii variable ya mazingira
...
* badilisha `/Users/hacker/tmp` kwa `/Users/hacker/ourlink` **kwa mzunguko**
...
Kama mtumiaji wa mizizi unaweza kuwezesha huduma hii na **ARD agent atakuwa na ufikivu kamili wa diski** ambao kisha unaweza kutumia vibaya kufanya ifanye nakala ya **database mpya ya mtumiaji wa TCC**.
...
Kwa hivyo, ikiwa mtumiaji anafanikiwa kuanzisha upya TCC na $HOME env variable ikielekeza kwa **saraka tofauti**, mtumiaji anaweza kuunda database mpya ya TCC katika **/Library/Application Support/com.apple.TCC/TCC.db** na kudanganya TCC kutoa ruhusa ya TCC kwa programu yoyote.

{% hint style="success" %}
Tafadhali kumbuka kuwa Apple hutumia mipangilio iliyohifadhiwa ndani ya wasifu wa mtumiaji katika sifa ya **`NFSHomeDirectory`** kwa **thamani ya `$HOME`**, kwa hivyo ikiwa unahatarisha programu na ruhusa ya kurekebisha thamani hii (**`kTCCServiceSystemPolicySysAdminFiles`**), unaweza **kuifanya iwe silaha** na kufanya upitishaji wa TCC.
{% endhint %}
...
Zaidi ya hayo, njia ya kawaida zaidi ya kuingiza mchakato kwa kuzidi TCC iliyopatikana ni kupitia **plugins (kupakia maktaba)**.\
Vifaa vya ziada ni msimbo wa ziada kawaida katika fomu ya maktaba au plist, ambayo ita **pakuliwa na programu kuu** na kutekelezwa chini ya muktadha wake. Kwa hivyo, ikiwa programu kuu ilikuwa na ufikivu wa faili zilizozuiwa na TCC (kupitia ruhusa zilizotolewa au haki za kipekee), **msimbo wa desturi pia utakuwa nao**.

### CVE-2020-27937 - Directory Utility

Programu `/System/Library/CoreServices/Applications/Directory Utility.app` ilikuwa na ruhusa ya **`kTCCServiceSystemPolicySysAdminFiles`**, ilipakia plugins na kielezo cha **`.daplug`** na **haikuwa na** runtime iliyofanywa ngumu.

Ili kuifanya CVE hii kuwa silaha, **`NFSHomeDirectory`** inabadilishwa (kwa kutumia ruhusa ya awali) ili kuweza **kuchukua udhibiti wa database ya TCC ya watumiaji** ili kuzidi TCC.
...
Binaryi hii iliruhusu kupakia **programu-jalizi za mtu wa tatu** kutoka kwa folda `/Library/Audio/Plug-Ins/HAL`. Kwa hivyo, ilikuwa inawezekana **kupakia programu-jalizi na kutumia ruhusa za TCC** na hii PoC:
```objectivec
...
Programu za mfumo ambazo hufungua mtiririko wa kamera kupitia Core Media I/O (programu zinazotumia **`kTCCServiceCamera`**) hulipakia **katika mchakato huu wa programu** viendelezi hivi vilivyoko katika `/Library/CoreMediaIO/Plug-Ins/DAL` (sio kizuizi cha SIP).

Kuhifadhi tu huko maktaba na **mwanzilishi wa kawaida** kutafanya kazi kwa ajili ya **kuingiza msimbo**.
...
Ni kawaida kutoa **Upatikanaji Kamili wa Diski (FDA)** kwa terminali, angalau kwenye kompyuta zinazotumiwa na watu wa teknolojia. Na inawezekana kuita skripti za **`.terminal`** kutumia hilo.
...
### CVE-2020-9771 - kosa la TCC la mount\_apfs na upandishaji wa mamlaka

**Mtumiaji yeyote** (hata wale wasio na mamlaka) wanaweza kuunda na kufunga picha ya wakati wa mashine na **kupata FAIDA ya faili zote** za picha hiyo ya wakati.\
**Mamlaka pekee** inayohitajika ni kwa programu iliyotumiwa (kama vile `Terminal`) kuwa na **Ufikivu Kamili wa Diski** (FDA) (`kTCCServiceSystemPolicyAllfiles`) ambayo inahitaji kupewa na msimamizi.
...
Zana ya **`/usr/sbin/asr`** iliruhusu kunakili diski nzima na kuimount mahali pengine kwa kukiuka ulinzi wa TCC.
...
Folda ya **`/var/db/locationd/` haikuwa imekingwa kutokana na DMG mounting** hivyo ilikuwa inawezekana kuimount plist yetu.
...
Katika matukio kadhaa faili zitahifadhi taarifa nyeti kama barua pepe, namba za simu, ujumbe... katika maeneo yasiyolindwa (ambayo ni udhaifu katika Apple).
