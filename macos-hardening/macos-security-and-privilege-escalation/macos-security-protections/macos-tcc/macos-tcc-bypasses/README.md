...
<summary><strong>htARTE (HackTricks AWS Red Team Expert)를 통해 제로에서 영웅이 될 때까지 AWS 해킹을 배우세요</strong> <a href="https://training.hacktricks.xyz/courses/arte"><strong>htARTE (HackTricks AWS Red Team Expert)</strong></a><strong>!</strong></summary>
...
게다가, 정품 앱을 도크에서 제거하고 가짜 앱을 넣을 수도 있으므로 사용자가 가짜 앱(동일한 아이콘을 사용할 수 있음)을 클릭하면 가짜 앱이 실제 앱을 호출하여 TCC 권한을 요청하고 악성 코드를 실행시킬 수 있어 사용자는 정품 앱이 액세스를 요청한 것으로 생각할 수 있습니다.
...
기본적으로 **SSH를 통한 액세스는 "전체 디스크 액세스"를 가졌습니다**. 이를 비활성화하려면 목록에 나열되어 있지만 비활성화된 상태여야 합니다(목록에서 제거하면 해당 권한이 제거되지 않습니다):
...
따라서 사용자는 모든 확장자를 처리할 악성 앱을 **등록**하고 Launch Services를 호출하여 모든 파일을 **열 수 있도록** 할 수 있습니다(따라서 악성 파일이 액세스 권한을 부여받을 것입니다).
...
FDA가 없는 Terminal이 iTerm을 호출하고 사용하여 작업을 수행할 수 있습니다:
...
[이 스택 오버플로우 게시물](https://stackoverflow.com/questions/135688/setting-environment-variables-on-os-x/3756686#3756686)에 따르면, TCC 데몬은 현재 사용자 도메인 내에서 `launchd`를 통해 실행되므로 **모든 환경 변수를 제어**할 수 있습니다.\
...
노트는 TCC로 보호된 위치에 액세스할 수 있었지만 노트를 생성하면 **보호되지 않은 위치에 생성**됩니다. 따라서, 노트에 보호된 파일을 복사하도록 요청하고 그 파일에 액세스할 수 있습니다:

<figure><img src="../../../../../.gitbook/assets/image (6) (1) (3).png" alt=""><figcaption></figcaption></figure>

### CVE-2021-30782 - 이전

라이브러리 `libsecurity_translocate`를 사용하는 `/usr/libexec/lsd` 바이너리는 `com.apple.private.nullfs_allow` 허용권을 가지고 있어 **nullfs** 마운트를 생성하고 `com.apple.private.tcc.allow` 허용권을 가지고 있어 **`kTCCServiceSystemPolicyAllFiles`**로 모든 파일에 액세스할 수 있었습니다.

"Library"에 격리 속성을 추가하고 **`com.apple.security.translocation`** XPC 서비스를 호출하여 "Library"를 **`$TMPDIR/AppTranslocation/d/d/Library`**로 매핑하고 Library 내의 모든 문서에 **액세스**할 수 있었습니다.
...
이 **`rename(a, b);`** 동작은 **경쟁 조건**에 취약하며, `Automatically Add to Music.localized` 폴더에 가짜 **TCC.db** 파일을 넣고 새 폴더(b)가 생성될 때 파일을 복사하고 삭제하고 **`~/Library/Application Support/com.apple.TCC`**/로 지정할 수 있습니다.
...
* `open()`이 호출된 파일은 `경로/.dat.nosyncXXXX.XXXXXX` (X는 무작위)로 불릴 것입니다.
* 하나 이상의 `write()`가 파일에 내용을 쓸 것입니다 (우리는 이를 제어하지 않습니다).
* `경로/.dat.nosyncXXXX.XXXXXX`가 `rename()`되어 `경로/이름`으로 바뀔 것입니다.

이것은 임시 파일 쓰기이며, **보안이 되지 않은 `rename(old, new)`**를 따릅니다.

이것은 **이전과 새 경로를 별도로 해결**해야 하기 때문에 보안이 되지 않습니다. 이는 시간이 걸리고 Race Condition에 취약할 수 있습니다. 자세한 정보는 `xnu` 함수 `renameat_internal()`을 확인하십시오.

{% hint style="danger" %}
따라서, 특권 프로세스가 제어하는 폴더에서 이름을 바꾸면 RCE를 얻을 수 있으며, 특권 앱이 만든 파일을 열고 FD를 저장할 수 있습니다.

이름 바꾸기가 제어하는 폴더에 접근하면 소스 파일을 수정하거나 FD가 있는 동안 대상 파일(또는 폴더)을 심볼릭 링크로 지정하여 원하는 때마다 쓸 수 있습니다.
{% endhint %}
...
* `/Users/hacker/tmp/.dat.nosyncXXXX.XXXXXX` (X는 무작위)의 `open()`를 잡습니다.
...
루트로이 이 서비스를 활성화할 수 있으며 **ARD 에이전트는 전체 디스크 액세스**를 갖게 되어 사용자가 새 **TCC 사용자 데이터베이스를 복사**하도록 남용할 수 있습니다.

## **NFSHomeDirectory**에 의한
...
{% hint style="success" %}
Apple은 **`NFSHomeDirectory`** 속성 내에 저장된 설정을 사용하여 **`$HOME` 값**을 사용자 프로필 내에 저장하므로, 이 값을 수정할 권한이 있는 애플리케이션을 침해한다면 (**`kTCCServiceSystemPolicySysAdminFiles`**), TCC 우회를 위해 이 옵션을 **무기화**할 수 있습니다.
{% endhint %}
...
5. 수정된 디렉토리 서비스 항목을 [**dsimport**](https://www.unix.com/man-page/osx/1/dsimport/)를 사용하여 가져옵니다.
6. 사용자의 _tccd_를 중지하고 프로세스를 다시 시작합니다.

두 번째 POC는 **`/usr/libexec/configd`**를 사용하여 `com.apple.private.tcc.allow`가 `kTCCServiceSystemPolicySysAdminFiles` 값을 가지도록 한 것입니다.\
...
플러그인은 주로 라이브러리 또는 plist 형식의 추가 코드이며, **주 애플리케이션에 의해 로드**되어 해당 컨텍스트에서 실행됩니다. 따라서 주 애플리케이션이 TCC 제한 파일에 액세스 권한이 있었거나 엔트itlement를 부여했다면, **사용자 지정 코드도 해당 권한을 갖게 됩니다**.

### CVE-2020-27937 - Directory Utility

`/System/Library/CoreServices/Applications/Directory Utility.app` 애플리케이션은 **`kTCCServiceSystemPolicySysAdminFiles`** 엔트itlement를 가지고 있으며, **`.daplug`** 확장자를 가진 플러그인을 로드하고 **강화된** 런타임이 없었습니다.

이 CVE를 무기화하기 위해 **`NFSHomeDirectory`**가 변경되어 (이전 엔트itlement를 남용함) 사용자의 TCC 데이터베이스를 **인수**할 수 있도록 변경되었습니다.
...
바이너리 `/system/Library/Filesystems/acfs.fs/Contents/bin/xsanctl`에는 **`com.apple.private.tcc.allow`** 및 **`com.apple.security.get-task-allow`** 허용권한이 있었으며, 이를 통해 프로세스 내부에 코드를 삽입하고 TCC 권한을 사용할 수 있었습니다.

### CVE-2023-26818 - 텔레그램

텔레그램은 **`com.apple.security.cs.allow-dyld-environment-variables`** 및 **`com.apple.security.cs.disable-library-validation`** 허용권한을 가졌기 때문에, 이를 악용하여 카메라로 녹화하는 등의 권한에 **액세스**할 수 있었습니다. [**페이로드는 이 설명서에서 찾을 수 있습니다**](https://danrevah.github.io/2023/05/15/CVE-2023-26818-Bypass-TCC-with-Telegram/).
...
**`.terminal`** 스크립트는 다음과 같은 명령을 **`CommandString`** 키에서 실행하는 이 plist 파일입니다:
```xml
<?xml version="1.0" encoding="UTF-8"?>
...
폴더 **`/var/db/locationd/`는 DMG 장착을 방지하지 않았기 때문에** 우리만의 plist를 장착할 수 있었습니다.
...
다른 방법은 [**CoreGraphics 이벤트**](https://objectivebythesea.org/v2/talks/OBTS\_v2\_Wardle.pdf)를 사용하는 것입니다:
