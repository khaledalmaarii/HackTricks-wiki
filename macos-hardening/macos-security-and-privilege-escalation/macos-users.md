...
*   **Daemon** : Utilisateur réservé pour les daemons système. Les noms de compte daemon par défaut commencent généralement par un "_":
...
* **Utilisateur Standard :** L'utilisateur le plus basique. Cet utilisateur a besoin de permissions accordées par un utilisateur admin pour tenter d'installer des logiciels ou effectuer d'autres tâches avancées. Ils ne sont pas capables de le faire par eux-mêmes.
* **Utilisateur Admin :** Un utilisateur qui opère la plupart du temps comme un utilisateur standard mais qui est également autorisé à effectuer des actions root telles que l'installation de logiciels et d'autres tâches administratives. Tous les utilisateurs appartenant au groupe admin sont **donnés accès au root via le fichier sudoers**.
