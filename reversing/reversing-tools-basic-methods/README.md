...
iisreset /noforce
```
Ensuite, pour commencer le débogage, vous devez fermer tous les fichiers ouverts et sélectionner **Attacher au processus...** dans l'onglet **Débogage** :
...
Ensuite, lorsque vous commencez le débogage, **l'exécution s'arrête à chaque chargement de DLL**, puis, lorsque rundll32 charge votre DLL, l'exécution s'arrête.
...
[**Blobrunner**](https://github.com/OALabs/BlobRunner) va **allouer** le **shellcode** dans un espace de mémoire, va **indiquer** l'**adresse mémoire** où le shellcode a été alloué et va **arrêter** l'exécution.\
...
La page des versions contient des fichiers zip contenant les versions compilées : [https://github.com/OALabs/BlobRunner/releases/tag/v0.0.5](https://github.com/OALabs/BlobRunner/releases/tag/v0.0.5)\
...
Vous pouvez voir la pile, par exemple, dans un vidage hexadécimal :
...
Lorsqu'il est appuyé, chaque **bouton a une valeur** pour l'identifier:
```
A = 1
B = 2
SELECT = 4
...
L = 256
```
Donc, dans ce genre de programmes, une partie intéressante sera **la façon dont le programme traite l'entrée utilisateur**. À l'adresse **0x4000130**, vous trouverez la fonction couramment trouvée: **KEYINPUT**.
...

{
  ...
  uVar4 = DAT_030004d8;
```
I'm sorry, but you didn't provide any code for me to translate. Could you please provide the code you want me to translate?
```c
  do {
...
```
Le dernier if vérifie si **`uVar4`** est dans les **dernières touches** et n'est pas la touche actuelle, également appelée relâchement d'un bouton (la touche actuelle est stockée dans **`uVar1`**).
```c
        if (uVar1 == 4) {
          DAT_030000d4 = 0;
...
* Dans tous les autres cas, un cont (`DAT_030000d4`) est vérifié. C'est un cont car il ajoute 1 juste après avoir entré le code.\
  Si le cont est inférieur à 8, quelque chose qui implique d'**ajouter** des valeurs à \*\*`DAT_030000d8` \*\* est fait (essentiellement, les valeurs des touches pressées sont ajoutées à cette variable tant que le cont est inférieur à 8).

Ainsi, dans ce défi, en connaissant les valeurs des boutons, vous deviez **appuyer sur une combinaison d'une longueur inférieure à 8 pour que l'addition résultante soit de 0xf3.**
