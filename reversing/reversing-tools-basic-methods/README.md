...
यहाँ महत्व है कि यदि किसी खो गए स्रोत कोड की पुनर्स्थापना एक पुरानी असेम्बली से की जाती है, तो यह कार्रवाई समय बचा सकती है। इसके अतिरिक्त, dotPeek डिकंपाइल कोड के माध्यम से हाथ में हैंडी नेविगेशन प्रदान करता है, जिससे यह **Xamarin एल्गोरिथ्म विश्लेषण** के लिए सबसे उत्तम टूलों में से एक बन जाता है।

### [.NET Reflector](https://www.red-gate.com/products/reflector/)

एक व्यापक एड-इन मॉडल और एक एपीआई के साथ जो आपकी विशेष आवश्यकताओं को पूरा करने के लिए उपकरण को विस्तारित करता है, .NET रिफ्लेक्टर समय बचाता है और विकास को सरल बनाता है। चलिए इस उपकरण द्वारा प्रदान की जाने वाली पूरी पलटन की रिवर्स इंजीनियरिंग सेवाओं की ओर देखते हैं:
...
* अनदोक्यूमेंटेड और अनएक्सपोज़्ड फ़ंक्शनैलिटी का पता लगाता है ताकि एपीआई और प्रयुक्त प्रौद्योगिकियों से अधिक प्राप्त किया जा सके।
...
यदि आपको **डिकंपाइल**, **संशोधित** और **पुनः कंपाइल** करने की आवश्यकता है तो आप [**dnSpy**](https://github.com/dnSpy/dnSpy/releases) या इसके सक्रिय रूप से रखरखावित फोर्क, [**dnSpyEx**](https://github.com/dnSpyEx/dnSpy/releases) का उपयोग कर सकते हैं। (**राइट क्लिक -> मेथड मॉडिफाई** करने के लिए कुछ बदलाव करने के लिए फ़ंक्शन के अंदर।)
...
DNSpy का उपयोग करके कोड को डीबग करने के लिए आपको करना होगा:
...
```
## उल्टी विचार के लिए उपकरण, मूल विधियाँ

यहाँ हम कुछ मूल उपकरणों और विधियों का उल्लेख करेंगे जो आपको उल्टी विचार के लिए उपयोगी हो सकते हैं।
```
...
फिर नए फ़ाइल को _**फ़ाइल >> मॉड्यूल सहेजें...**_ के माध्यम से सहेजें:

![](<../../.gitbook/assets/image (279).png>)

यह आवश्यक है क्योंकि अगर आप ऐसा नहीं करते हैं, तो **एक्सक्यूशन** के समय कई **ऑप्टिमाइजेशन** को कोड पर लागू किया जा सकता है और यह संभावित है कि जब आप **डीबगिंग** कर रहे हों, तो **ब्रेक-पॉइंट कभी नहीं हिट** हो या कुछ **वेरिएबल्स मौजूद नहीं** हों।
...
* **rundll32 लोड करें** (64बिट C:\Windows\System32\rundll32.exe और 32 बिट C:\Windows\SysWOW64\rundll32.exe)
...
लेकिन, जो डीएलएल को लोड किया गया था, उसके कोड तक कैसे पहुंच सकते हैं? इस तरीके का उपयोग करके, मुझे नहीं पता।

### x64dbg/x32dbg का उपयोग

* **rundll32 लोड करें** (64बिट C:\Windows\System32\rundll32.exe और 32 बिट C:\Windows\SysWOW64\rundll32.exe)
* **Command Line बदलें** ( _File --> Change Command Line_ ) और dll का पथ और वह फ़ंक्शन सेट करें जिसे आप कॉल करना चाहते हैं, उदाहरण के लिए: "C:\Windows\SysWOW64\rundll32.exe" "Z:\shared\Cybercamp\rev2\\\14.ridii\_2.dll",DLLMain
...
फिर, जब आप डीएलएल को डीबग करने के लिए रुकते हैं, तो आप देख सकते हैं कि आप किस डीएलएल में रुके हैं।
```bash
...
**डंप बनाएं** विकल्प यदि किसी भी परिवर्तन किया जाता है तो अंतिम शेलकोड को डंप करेगा जो कि मेमोरी में शेलकोड को डाउनलोड करने के लिए उपयोगी है। **स्टार्ट ऑफसेट** एक विशिष्ट ऑफसेट पर शेलकोड को शुरू करने के लिए उपयोगी हो सकता है। **डीबग शेल** विकल्प शेलकोड को scDbg टर्मिनल का उपयोग करके डीबग करने के लिए उपयोगी है (हालांकि मैं इस विषय में पहले विवरणित किए गए किसी भी विकल्प को इसके लिए बेहतर मानता हूँ क्योंकि आप Ida या x64dbg का उपयोग कर सकेंगे)।

### CyberChef का उपयोग करके डिसएसेंबलिंग

अपनी शेलकोड फ़ाइल को इनपुट के रूप में अपलोड करें और इस रेसिपी का उपयोग करें उसे डिकॉम्पाइल करने के लिए: [https://gchq.github.io/CyberChef/#recipe=To\_Hex('Space',0)Disassemble\_x86('32','Full%20x86%20architecture',16,0,true,true)](https://gchq.github.io/CyberChef/#recipe=To\_Hex\('Space',0\)Disassemble\_x86\('32','Full%20x86%20architecture',16,0,true,true\))
...
यह प्लगइन बाइनरी को निष्पादित करेगा और डीबगिंग की शुरुआत में फ़ंक्शन नामों को गतिशील रूप से हल करेगा। डीबगिंग शुरू करने के बाद फिर से स्टार्ट बटन दबाएं (हरित या f9) और एक ब्रेकपॉइंट वास्तविक कोड की शुरुआत में हिट करेगा।
...
यदि आपको एक GBA गेम का **बाइनरी** मिलता है तो आप उसे **एम्युलेट** और **डीबग** करने के लिए विभिन्न उपकरणों का उपयोग कर सकते हैं:
...
L = 256
```
इस प्रकार के कार्यक्रम में, दिलचस्प हिस्सा **कार्यक्रम उपयोगकर्ता इनपुट को कैसे व्यवहार करता है** होगा। पता **0x4000130** पर आपको आमतौर पर पाए जाने वाला फ़ंक्शन मिलेगा: **KEYINPUT**।

![](<../../.gitbook/assets/image (579).png>)

पिछली छवि में आप देख सकते हैं कि फ़ंक्शन **FUN\_080015a8** से बुलाया जाता है (पते: _0x080015fa_ और _0x080017ac_)।

उस फ़ंक्शन में, कुछ प्रारंभिक ऑपरेशनों के बाद (किसी भी महत्व के बिना):
```c
...
```
आखिरी if **`uVar4`** की जांच कर रहा है कि वह **आखिरी कुंजी** में है और वर्तमान कुंजी में नहीं है, जिसे छोड़ दिया गया बटन कहा जाता है (वर्तमान कुंजी **`uVar1`** में संग्रहीत है)।
```c
if (uVar1 == 4) {
DAT_030000d4 = 0;
...
अगर 8 से कम कुछ ऐसा है जिसमें **मानों को जोड़ना** शामिल है जो \*\*`DAT_030000d8` \*\* में किया जाता है (मुख्य रूप से इस चर में दबाए गए कुंजियों के मानों को जोड़ रहा है जब तक cont 8 से कम है)।

तो, इस चैलेंज में, बटनों के मानों को जानकर आपको **8 से छोटी लंबाई वाली एक कॉम्बिनेशन दबानी थी जिससे परिणामी जोड़ 0xf3 हो।**
