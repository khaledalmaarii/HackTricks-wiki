...
* Sie können eine Liste von Argumenten über `args` und ein Wörterbuch von Umgebungsvariablen über `env` in `entry_state` und `full_init_state` übergeben. Die Werte in diesen Strukturen können Strings oder Bitvektoren sein und werden als Argumente und Umgebung in den Zustand der simulierten Ausführung serialisiert. Der Standardwert für `args` ist eine leere Liste, also sollten Sie immer `argv[0]` bereitstellen, wenn das Programm, das Sie analysieren, erwartet, es zu finden!
* Wenn Sie möchten, dass `argc` symbolisch ist, können Sie einen symbolischen Bitvektor als `argc` an die Konstruktoren von `entry_state` und `full_init_state` übergeben. Seien Sie jedoch vorsichtig: Wenn Sie dies tun, sollten Sie auch eine Einschränkung zum resultierenden Zustand hinzufügen, dass Ihr Wert für argc nicht größer sein kann als die Anzahl der Argumente, die Sie in `args` übergeben haben.
