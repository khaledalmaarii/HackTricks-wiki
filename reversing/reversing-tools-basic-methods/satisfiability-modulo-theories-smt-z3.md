...
매우 기본적으로, 이 도구는 우리가 몇 가지 조건을 만족해야 하는 변수의 값을 찾는 데 도움을 줄 것이며, 수작업으로 계산하는 것은 매우 귀찮을 것입니다. 따라서, Z3에 변수들이 만족해야 하는 조건을 지정하면, 가능한 경우 몇 가지 값을 찾아낼 것입니다.
...
**해석된 함수**는 산술과 같은 것으로, **함수 +**는 **고정된 표준 해석**을 가지고 있습니다(두 숫자를 더합니다). **해석되지 않은 함수**와 상수는 **최대 유연성**을 가지고 있으며, 이는 **함수** 또는 **상수**에 대한 **제약**과 일관된 **모든 해석**을 허용합니다.
