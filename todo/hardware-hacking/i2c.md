...
Per testare se un Bus Pirate funziona, collega +5V a VPU e 3.3V a ADC e accedi al Bus Pirate (utilizzando ad esempio Tera Term) e usa il comando `~`:
```bash
# Use command
HiZ>~
...
Found 0 errors.
```
Come puoi vedere nel comando precedente, è stato indicato che non sono stati trovati errori. Questo è molto utile per sapere che funziona dopo averlo acquistato o dopo aver flashato un firmware.

Per connettersi con il bus pirate, è possibile seguire la documentazione:
...
Per comunicare con il bus pirate, ho utilizzato Tera Term collegato alla porta COM del bus pirate con una configurazione --> Porta seriale --> Velocità di 115200.\
Nella seguente comunicazione, puoi trovare come preparare il bus pirate per comunicare tramite I2C e come scrivere e leggere dalla memoria (I commenti appaiono utilizzando "#", non aspettarti quella parte nella comunicazione):
```bash
...
In questo scenario stiamo per intercettare la comunicazione I2C tra l'arduino e l'EPROM precedente, è sufficiente comunicare entrambi i dispositivi e quindi collegare il bus pirate ai pin SCL, SDA e GND:
