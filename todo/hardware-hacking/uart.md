...
UART ha 4 porte: **TX**(Trasmetti), **RX**(Ricevi), **Vcc**(Tensione) e **GND**(Terra). Potresti essere in grado di trovare 4 porte con le lettere **`TX`** e **`RX`** **scritte** nel PCB. Ma se non c'è alcuna indicazione, potresti dover provare a trovarle tu stesso utilizzando un **multimetro** o un **analizzatore logico**.
...
Ci sono molti modi per farlo e la sezione SPI copre i metodi per estrarre il firmware direttamente dall'EEPROM con vari dispositivi. Anche se, è consigliato provare prima a fare il dumping del firmware con UART poiché il dumping del firmware con dispositivi fisici e interazioni esterne può essere rischioso.

Il dumping del firmware dalla console UART richiede prima di ottenere accesso ai bootloader. Molti fornitori popolari utilizzano uboot (Universal Bootloader) come loro bootloader per caricare Linux. Pertanto, ottenere accesso a uboot è necessario.
