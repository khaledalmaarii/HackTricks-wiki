...
Gewoonlik word die lyn hoog gehou (op 'n logiese 1-waarde) terwyl UART in die idle-toestand is. Dan, om die begin van 'n datatransmissie aan te dui, stuur die oordraer 'n beginbit na die ontvanger, tydens watter die sein laag gehou word (op 'n logiese 0-waarde). Vervolgens stuur die oordraer vyf tot agt databits wat die werklike boodskap bevat, gevolg deur 'n opsionele pariteitsbit en een of twee stopbietjies (met 'n logiese 1-waarde), afhangende van die konfigurasie. Die pariteitsbit, wat vir foutkontrole gebruik word, word selde in die praktyk gesien. Die stopbit (of bietjies) dui die einde van die transmissie aan.
...
UART het 4 poorte: **TX**(Oordra), **RX**(Ontvang), **Vcc**(Spanning), en **GND**(Grond). Jy mag dalk 4 poorte met die **`TX`** en **`RX`**-letters **geskryf** op die PCB vind. Maar as daar geen aanduiding is nie, mag jy dit dalk self moet probeer vind met 'n **multimeter** of 'n **logika-analiseerder**.

Met 'n **multimeter** en die toestel afgeskakel:

* Om die **GND**-pen te identifiseer, gebruik die **Continuïteitstoets**-modus, plaas die agterleier in die grond en toets met die rooi een totdat jy 'n geluid van die multimeter hoor. Verskeie GND-penne kan op die PCB gevind word, sodat jy dalk die een wat aan UART behoort, gevind het of nie.
...
Indien die adapter nie opgespoor word nie, maak seker dat die CP210X-bestuurders in die gasstelsel geïnstalleer is. Sodra die adapter opgespoor en aangesluit is, kan gereedskap soos picocom, minicom of skerm gebruik word.
...
Na konfigurasie, gebruik die `minicom` bevel om die UART Konsol te begin.

## UART Via Arduino UNO R3 (Verwyderbare Atmel 328p Chip Borde)

Indien UART Seriële na USB-adapters nie beskikbaar is nie, kan 'n Arduino UNO R3 met 'n vinnige hak gebruik word. Aangesien 'n Arduino UNO R3 gewoonlik oral beskikbaar is, kan dit baie tyd bespaar.

Arduino UNO R3 het 'n USB na Seriële adapter wat op die bord self gebou is. Om 'n UART-verbinding te kry, trek net die Atmel 328p mikrokontroller-skyfie uit die bord. Hierdie hak werk op Arduino UNO R3-variant met die Atmel 328p wat nie op die bord gesoldeer is nie (SMD-weergawe word daarin gebruik). Verbind die RX-pen van Arduino (Digitale Pen 0) met die TX-pen van die UART-koppelvlak en die TX-pen van die Arduino (Digitale Pen 1) met die RX-pen van die UART-koppelvlak.

Laastens word dit aanbeveel om die Arduino IDE te gebruik om die Seriële Konsol te kry. In die `tools` afdeling in die kieslys, kies die `Serial Console` opsie en stel die baudkoers in volgens die UART-koppelvlak.
...
UART-konsole bied 'n goeie manier om met die onderliggende firmware in die hardloop-omgewing te werk. Maar wanneer die toegang tot die UART-konsole slegs leesbaar is, kan dit baie beperkings inbring. In baie ingebedde toestelle word die firmware gestoor in EEPROM's en uitgevoer in verwerkers met vlugtige geheue. Daarom word die firmware as leesbaar gehou aangesien die oorspronklike firmware tydens vervaardiging binne die EEPROM self is en enige nuwe lêers verlore sou gaan as gevolg van vlugtige geheue. Daarom is dit waardevol om firmware te dump terwyl jy met ingebedde firmwares werk.
...
Firmware dump vanaf die UART-konsole vereis dat jy eerste toegang tot die opstartsprogramme kry. Baie gewilde verskaffers maak gebruik van <b>uboot</b> (Universal Bootloader) as hul opstartsprogram om Linux te laai. Daarom is dit noodsaaklik om toegang tot <b>uboot</b> te kry.

Om toegang tot die <b>boot</b> opstartsprogram te kry, verbind die UART-poort met die rekenaar en gebruik enige van die Seriële Konsole-hulpmiddels en hou die kragtoevoer na die toestel afgeskakel. Wanneer die opstelling gereed is, druk die Enter-sleutel in en hou dit vas. Verbind uiteindelik die kragtoevoer na die toestel en laat dit opstart.

Deur dit te doen, sal dit <b>uboot</b> onderbreek van laai en 'n kieslys voorsien. Dit word aanbeveel om <b>uboot</b> bevele te verstaan en die hulplys te gebruik om hulle te lys. Dit kan die `help` bevel wees. Aangesien verskillende verskaffers verskillende konfigurasies gebruik, is dit noodsaaklik om elkeen van hulle afsonderlik te verstaan.
...
Alhoewel, dit is noodsaaklik om daarop te let dat dit nie altyd die geval is dat die <b>uboot</b> oopgemaak is selfs al word dit gebruik nie. As die Enter-sleutel niks doen nie, kyk vir verskillende sleutels soos die Spasiesleutel, ens. As die laai-program gesluit is en nie onderbreek word nie, sal hierdie metode nie werk nie. Om te kyk of <b>uboot</b> die laai-program vir die toestel is, kyk na die uitset op die UART-konsole terwyl die toestel aan die laai is. Dit kan <b>uboot</b> noem terwyl dit aan die laai is.
