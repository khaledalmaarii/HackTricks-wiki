...
Novos tipos de cartões NFC serão adicionados à lista de cartões suportados. O Flipper Zero suporta os seguintes **cartões NFC tipo A** (ISO 14443A):

* ﻿**Cartões bancários (EMV)** — apenas lê UID, SAK e ATQA sem salvar.
* ﻿**Cartões desconhecidos** — lê (UID, SAK, ATQA) e emula um UID.

Para os **cartões NFC tipo B, tipo F e tipo V**, o Flipper Zero é capaz de ler um UID sem salvá-lo.
...
Para os **cartões NFC tipos B, F e V**, o Flipper Zero só pode **ler e exibir um UID** sem salvá-lo.
...
No entanto, **não é possível ler o CVV dessa forma** (os 3 dígitos no verso do cartão). Além disso, **os cartões bancários estão protegidos contra ataques de replay**, então copiá-lo com o Flipper e depois tentar emulá-lo para pagar algo não funcionará.
## Referências
