...
새로운 종류의 NFC 카드가 지원되는 카드 목록에 추가될 것입니다. Flipper Zero는 다음 **NFC 카드 유형 A** (ISO 14443A)을 지원합니다:
...
**NFC 카드 유형 B, F 및 V**의 경우, Flipper Zero는 UID를 저장하지 않고 읽기만 할 수 있습니다.
...
Flipper Zero는 **NFC 카드를 읽을 수 있지만**, ISO 14443을 기반으로 하는 **모든 프로토콜을 이해하지는 못합니다**. 그러나 **UID는 저수준 속성**이므로 **UID가 이미 읽혔지만 고수준 데이터 전송 프로토콜이 아직 알려지지 않은 상황**에 처할 수 있습니다. Flipper를 사용하여 UID를 읽고, 에뮬레이트하고, 원시 리더에 UID를 수동으로 입력할 수 있습니다.
...
* **저수준 읽기** — UID, SAK 및 ATQA만 읽습니다. Flipper는 이 카드에서 읽은 데이터를 기반으로 고수준 프로토콜을 추측하려고 합니다. 이는 특정 요소를 기반으로 한 가정일 뿐이므로 100% 확신할 수 없습니다.
