...
在分析时，Flipper Zero会扫描频率配置中所有可用频率的信号强度（RSSI）。Flipper Zero会显示具有最高RSSI值的频率，信号强度高于-90 [dBm](https://en.wikipedia.org/wiki/DBm)。
...
**读取**选项会**监听配置频率**上的指定调制：默认为433.92 AM。如果在读取时**发现了什么**，屏幕上会提供信息。此信息可用于将来复制信号。
...
您可以设置**您感兴趣的任何频率**，但是，如果您**不确定遥控器使用的是哪个频率**，请将Hopping设置为ON（默认为Off），然后按下按钮多次，直到Flipper捕获并提供您需要设置频率的信息。
...
**读取原始数据**选项会记录在监听频率上发送的信号。这可以用于**窃取**信号并**重放**。
