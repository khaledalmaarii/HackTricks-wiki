...
首先，你需要拥有一个[**Proxmark3**](https://proxmark.com)并[**安装软件及其依赖项**](https://github.com/Proxmark/proxmark3/wiki/Kali-Linux)[**。**](https://github.com/Proxmark/proxmark3/wiki/Kali-Linux)

### 攻击MIFARE Classic 1KB

它有**16个扇区**，每个扇区有**4个块**，每个块包含**16字节**。UID位于扇区0块0中（不可更改）。\
要访问每个扇区，你需要**2个密钥**（**A**和**B**），它们存储在**每个扇区的块3中**（扇区尾部）。扇区尾部还存储了**访问位**，它们使用这2个密钥给予对**每个块的读写权限**。\
如果你知道第一个密钥，则可以使用2个密钥来授予读取权限，如果你知道第二个密钥，则可以授予写入权限（例如）。
