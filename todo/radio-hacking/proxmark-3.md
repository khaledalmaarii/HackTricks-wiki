...
```
Proxmark3 允许执行其他操作，如**窃听** **标签到读卡器的通信**，以尝试找到敏感数据。在这种情况下，您可以仅仅通过嗅探通信并计算使用的密钥，因为使用的**加密操作较弱**，知道明文和密文后，您可以计算出它（`mfkey64` 工具）。
...
```
您可以创建一个脚本来**模糊标签读卡器**，复制一个**有效卡**的数据，然后编写一个**Lua脚本**来**随机化**一个或多个随机**字节**，并检查**读卡器**在任何迭代中是否**崩溃**。
