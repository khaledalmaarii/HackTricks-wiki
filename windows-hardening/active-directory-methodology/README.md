...
더 자세한 설명은 다음을 참조하세요: [**TechTerms - Active Directory 정의**](https://techterms.com/definition/active_directory)
...
* **익명 SMB/LDAP 열거:** [**펜테스팅 SMB**](../../network-services-pentesting/pentesting-smb.md) 및 [**펜테스팅 LDAP**](../../network-services-pentesting/pentesting-ldap.md) 페이지를 확인하세요.
* **Kerbrute 열거**: **유효하지 않은 사용자 이름이 요청**되면 서버는 **Kerberos 오류** 코드 _KRB5KDC\_ERR\_C\_PRINCIPAL\_UNKNOWN_를 사용하여 사용자 이름이 잘못되었음을 확인할 수 있습니다. **유효한 사용자 이름**은 AS-REP 응답에서 **TGT**를 나타내거나 사용자가 사전 인증을 수행해야 함을 나타내는 _KRB5KDC\_ERR\_PREAUTH\_REQUIRED_ 오류를 일으킬 것입니다.
```bash
...
* _userPassword_ 및 _unixUserPassword_ 필드에서 자격 증명을 찾거나 _Description_을 위해 **LDAP 데이터베이스에서 ldapsearch**를 사용할 수도 있습니다. 다른 방법은 [PayloadsAllTheThings의 AD 사용자 코멘트에 있는 Password](https://github.com/swisskyrepo/PayloadsAllTheThings/blob/master/Methodology%20and%20Resources/Active%20Directory%20Attack.md#password-in-ad-user-comment)를 참조하세요.
...
[**다음 링크를 따라가서 사용할 수 있는 도구에 대해 알아보세요.**](../../network-services-pentesting/pentesting-smb.md#domain-shared-folders-search)
...
[**다양한 방법으로 해시를 얻는 방법에 대해 이 페이지를 읽어보세요.**](broken-reference/)
...
**도메인 관리자** 또는 더 나은 **엔터프라이즈 관리자** 권한을 획득하면, 도메인 데이터베이스인 _ntds.dit_를 **덤프**할 수 있습니다.

[**DCSync 공격에 대한 자세한 정보는 여기에서 찾을 수 있습니다**](dcsync.md).

[**NTDS.dit를 탈취하는 방법에 대한 자세한 정보는 여기에서 찾을 수 있습니다**](broken-reference/)
...
[여기에서 SSP (보안 지원 공급자)가 무엇인지 알아보세요.](../authentication-credentials-uac-and-efs.md#security-support-provider-interface-sspi)\
...
1. 이 경우 kerberoast도 다른 옵션일 수 있습니다.
3. 도메인을 통해 **피벗**할 수 있는 **계정**을 **침해**합니다.
...
- **액세스 제어 목록 (ACL)**: 주체는 특정 리소스에 액세스할 수 있도록 **ACL**에 지정될 수 있습니다. 특히 **DACL** 내의 **ACE**로서 지정될 수 있습니다. ACL, DACL 및 ACE의 작동 원리에 대해 자세히 알아보려면 "[An ACE Up The Sleeve](https://specterops.io/assets/resources/an_ace_up_the_sleeve.pdf)"라는 백서를 참조하면 됩니다.
...
```
{% hint style="warning" %}
**2개의 신뢰할 수 있는 키**가 있습니다. 하나는 _자식 --> 부모_를 위한 것이고, 다른 하나는 _부모_ --> _자식_을 위한 것입니다.\
