...
**Ovo je sažetak sekcija tehnika eskalacije iz sledećih postova:**
...
Ukoliko **napadač** poseduje potrebne **dozvole** da **izmeni** **predložak** i **uspostavi** bilo koje **iskoristive loše konfiguracije** navedene u **prethodnim sekcijama**, olakšano bi bilo preuzimanje privilegija.
...
* **FullControl:** Omogućava potpunu kontrolu nad objektom, uključujući sposobnost izmene bilo kojih atributa.
...
Obimna mreža međusobno povezanih odnosa zasnovanih na ACL-u, koja uključuje nekoliko objekata izvan predložaka sertifikata i sertifikacionog tela, može uticati na bezbednost celog AD CS sistema. Ovi objekti, koji mogu značajno uticati na bezbednost, obuhvataju:
...
Da bi se iskoristio ESC6, sistem mora biti podložan ESC10 (Slaba mapiranja sertifikata), koji daje prioritet **SAN-u nad novim bezbednosnim proširenjem**.
{% endhint %}
...
Imajući prava **`ManageCA`** na autoritetu za sertifikate omogućava subjektu da daljinski manipuliše podešavanjima koristeći PSPKI. To uključuje prebacivanje zastave **`EDITF_ATTRIBUTESUBJECTALTNAME2`** kako bi se omogućila specifikacija SAN-a u bilo kom obrascu, što je ključni aspekt eskalacije domena.
...
* **`Manage Certificates`** dozvola (može biti dodeljena iz **`ManageCA`**)
* Šablon sertifikata **`SubCA`** mora biti **omogućen** (može biti omogućen iz **`ManageCA`**)

Tehnika se oslanja na činjenicu da korisnici sa pravom pristupa `Manage CA` _i_ `Manage Certificates` mogu **izdati neuspele zahteve za sertifikatima**. Šablon sertifikata **`SubCA`** je **ranjiv na ESC1**, ali **samo administratori** mogu upisati u šablon. Dakle, **korisnik** može **zatražiti** upis u **`SubCA`** - što će biti **odbijeno** - ali će **zatim biti izdato od strane menadžera**.
...
```
Ako smo ispunili preduslove za ovaj napad, možemo početi sa **zahtevom za sertifikat zasnovan na `SubCA` obrascu**.
...
Ipak, ova ograničenja se prevazilaze eksploatisanjem NTLM relay napada da bi se stekao sertifikat za korisnika, jer period važenja sertifikata određuje trajanje sesije, i sertifikat može biti korišćen sa servisima koji **zahtevaju NTLM potpisivanje**. Za uputstva o korišćenju ukradenog sertifikata, pogledajte:

{% content-ref url="account-persistence.md" %}
[account-persistence.md](account-persistence.md)
{% endcontent-ref %}

Još jedno ograničenje NTLM relay napada je da **napadački kontrolisani računar mora biti autentifikovan od strane žrtvenog naloga**. Napadač može sačekati ili pokušati **prisiliti** ovu autentifikaciju:
...
Zahtev za sertifikat se podrazumevano pravi pomoću Certipy-ja na osnovu šablona `Machine` ili `User`, određenog na osnovu toga da li ime naloga završava znakom `$`. Specifikacija alternativnog šablona može se postići korišćenjem parametra `-template`.
...
* Bilo koja EKU za autentikaciju klijenta je navedena sertifikatom.
...
Prvo, `Jane`-in heš se dobija korišćenjem Senki Credentials, zahvaljujući `John`-ovom `GenericWrite`:
```bash
...
Nakon toga, šablona za sertifikat `ESC9`, označenog kao ranjivog, zahteva se kao `Jane`:
```bash
...
Prvo se dobija `Jane`-ov heš korišćenjem Senki akreditiva, iskorišćavajući `GenericWrite`.
```bash
...
Sa `CertificateMappingMethods` koji sadrži `UPN` bit flag (`0x4`), nalog A sa `GenericWrite` dozvolama može kompromitovati bilo koji nalog B koji nema svojstvo `userPrincipalName`, uključujući mašinske naloge i ugrađenog administratorskog `Administrator`-a domena.
...
### Povreda poverenja šuma pomoću kompromitovanih CA
...
U okruženjima sa više šuma, potrebna je opreznost u vezi sa Enterprise CA koje **objavljuju šablone sertifikata** koji omogućavaju **Autentifikovanim korisnicima ili stranim principima** (korisnicima/grupama van šuma kojem Enterprise CA pripada) **prava registracije i izmene**.\
...
Oba scenarija dovode do **povećanja površine napada** iz jednog šuma u drugi. Postavke šablona sertifikata mogu biti iskorišćene od strane napadača kako bi se stekle dodatne privilegije u stranom domenu.
