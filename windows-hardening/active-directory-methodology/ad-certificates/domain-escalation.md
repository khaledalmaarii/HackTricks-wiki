...
* **인증서 템플릿은 인증을 용이하게 하는 EKU를 정의하도록 구성되어 있습니다:**
...
{% hint style="danger" %}
위에 설명된 구성은 낮은 권한의 사용자가 선택한 SAN으로 인증서를 요청할 수 있도록 하여 Kerberos 또는 SChannel을 통해 어떤 도메인 주체로도 인증할 수 있게 합니다.
{% endhint %}

이 기능은 때때로 HTTPS 또는 호스트 인증서의 즉석 생성 지원을 위해 제품이나 배포 서비스에 의해 활성화되거나 이해 부족으로 인해 활성화됩니다.

이 옵션으로 인증서를 생성하면 경고가 발생하지만, 기존 인증서 템플릿(예: `WebServer` 템플릿, `CT_FLAG_ENROLLEE_SUPPLIES_SUBJECT`가 활성화된 경우)을 복제한 후 인증 OID를 포함하도록 수정할 때는 그렇지 않습니다.
...
```
그런 다음 생성된 **인증서를 `.pfx`** 형식으로 변환하고 **Rubeus 또는 certipy**를 사용하여 다시 **인증**할 수 있습니다:
```bash
...
AD 포리스트의 구성 스키마 내에서 인증서 템플릿을 열거할 수 있으며, 특히 승인이나 서명이 필요하지 않고, 클라이언트 인증 또는 스마트 카드 로그온 EKU를 보유하며, `CT_FLAG_ENROLLEE_SUPPLIES_SUBJECT` 플래그가 활성화된 템플릿을 다음 LDAP 쿼리를 실행하여 수행할 수 있습니다:
```
...
1. 엔터프라이즈 CA에 의해 저권한 사용자에게 등록 권한이 부여됩니다.
...
그러나 **도메인 인증**을 위해 생성된 새로운 인증서는 하위 CA가 **`NTAuthCertificates`** 객체에 의해 신뢰되지 않는 경우 작동하지 않습니다. 이는 기본 설정입니다. 그럼에도 불구하고 공격자는 여전히 **모든 EKU**와 임의의 인증서 값을 가진 **새로운 인증서**를 생성할 수 있습니다. 이러한 인증서는 잠재적으로 **남용**될 수 있는 다양한 목적(예: 코드 서명, 서버 인증 등)에 사용될 수 있으며, SAML, AD FS 또는 IPSec과 같은 네트워크의 다른 애플리케이션에 중대한 영향을 미칠 수 있습니다.

AD 포리스트의 구성 스키마 내에서 이 시나리오와 일치하는 템플릿을 나열하기 위해 다음 LDAP 쿼리를 실행할 수 있습니다:
```
...
```
The **사용자**는 **등록 에이전트 인증서**를 **획득**할 수 있으며, 등록 **에이전트**가 등록할 수 있는 템플릿과 등록 에이전트가 대신하여 행동할 수 있는 **계정**은 엔터프라이즈 CA에 의해 제한될 수 있습니다. 이는 `certsrc.msc` **스냅인**을 열고, **CA를 마우스 오른쪽 버튼으로 클릭**한 다음, **속성 클릭** 후 “등록 에이전트” 탭으로 **이동**하여 달성됩니다.
...
**인증서 템플릿**에 대한 **보안 설명자**는 템플릿에 대해 특정 **AD 주체**가 가지는 **권한**을 정의합니다.
...
* **소유자:** 객체에 대한 암묵적인 제어를 부여하며, 모든 속성을 수정할 수 있습니다.
* **전체 제어:** 객체에 대한 완전한 권한을 부여하며, 모든 속성을 변경할 수 있는 능력을 포함합니다.
* **소유자 쓰기:** 객체의 소유자를 공격자가 제어하는 주체로 변경할 수 있습니다.
* **DACL 쓰기:** 접근 제어를 조정할 수 있으며, 공격자에게 전체 제어를 부여할 수 있습니다.
* **속성 쓰기:** 모든 객체 속성을 편집할 수 있는 권한을 부여합니다.
...
```
**Certipy**는 단일 명령으로 인증서 템플릿의 구성을 덮어쓸 수 있습니다. **기본적으로**, Certipy는 구성을 **ESC1에 취약하게** 만들기 위해 **덮어씁니다**. 또한 **`-save-old` 매개변수를 지정하여 이전 구성을 저장할 수 있으며**, 이는 공격 후 구성을 **복원하는 데** 유용합니다.
```bash
...
상호 연결된 ACL 기반 관계의 광범위한 웹은 인증서 템플릿과 인증 기관을 넘어서는 여러 객체를 포함하며, 전체 AD CS 시스템의 보안에 영향을 미칠 수 있습니다. 보안에 상당한 영향을 미칠 수 있는 이러한 객체는 다음과 같습니다:
...
[**CQure Academy 포스트**](https://cqureacademy.com/blog/enhanced-key-usage)에서 논의된 주제는 Microsoft에서 설명한 **`EDITF_ATTRIBUTESUBJECTALTNAME2`** 플래그의 의미를 다룹니다. 이 구성은 인증 기관(CA)에서 활성화되면 **사용자 정의 값**을 **주체 대체 이름**에 포함할 수 있도록 허용합니다. 이는 Active Directory®에서 구성된 요청을 포함하여 **모든 요청**에 해당합니다. 결과적으로 이 조항은 **침입자**가 도메인 **인증**을 위해 설정된 **모든 템플릿**을 통해 등록할 수 있도록 하며, 특히 표준 사용자 템플릿과 같이 **저권한** 사용자 등록이 가능한 템플릿에 해당합니다. 그 결과, 인증서를 확보하여 침입자가 도메인 관리자 또는 도메인 내의 **다른 활성 엔터티**로 인증할 수 있게 됩니다.

**Note**: `certreq.exe`에서 `-attrib "SAN:"` 인수를 통해 인증서 서명 요청(CSR)에 **대체 이름**을 추가하는 방법은 ESC1의 SANs 악용 전략과 **대조적**입니다. 여기서의 차이는 **계정 정보가 캡슐화되는 방식**에 있으며, 확장 대신 인증서 속성 내에 포함됩니다.
...
```
이 설정을 변경하려면 **도메인 관리** 권한 또는 동등한 권한이 있다고 가정할 때, 다음 명령을 모든 워크스테이션에서 실행할 수 있습니다:
```bash
...
ESC6를 악용하기 위해서는 시스템이 ESC10(Weak Certificate Mappings)에 취약해야 하며, 이는 **새로운 보안 확장**보다 **SAN**을 우선시합니다.
{% endhint %}
...
{% hint style="warning" %}
이 **이전 공격**에서 **`Manage CA`** 권한이 **EDITF\_ATTRIBUTESUBJECTALTNAME2** 플래그를 **활성화**하여 **ESC6 공격**을 수행하는 데 사용되었지만, CA 서비스(`CertSvc`)가 재시작될 때까지는 아무런 효과가 없습니다. 사용자가 `Manage CA` 접근 권한을 가지고 있을 경우, 해당 사용자는 **서비스를 재시작할 수 있습니다**. 그러나, 이는 **사용자가 원격으로 서비스를 재시작할 수 있다는 의미는 아닙니다**. 또한, E**SC6는 대부분의 패치된 환경에서 기본적으로 작동하지 않을 수 있습니다**. 이는 2022년 5월 보안 업데이트로 인한 것입니다.
{% endhint %}
...
* **`ManageCA` 권한만 필요**
* **`Manage Certificates`** 권한 (이는 **`ManageCA`**에서 부여될 수 있음)
* 인증서 템플릿 **`SubCA`**는 **활성화**되어야 함 (이는 **`ManageCA`**에서 활성화될 수 있음)

이 기술은 `Manage CA` _및_ `Manage Certificates` 접근 권한을 가진 사용자가 **실패한 인증서 요청을 발급할 수 있다는 사실**에 의존합니다. **`SubCA`** 인증서 템플릿은 **ESC1에 취약하지만**, **오직 관리자만** 템플릿에 등록할 수 있습니다. 따라서, **사용자**는 **`SubCA`**에 등록 요청을 할 수 있으며 - 이는 **거부될 것**이지만 - **그 후 관리자에 의해 발급될 것입니다**.

#### Abuse

사용자는 새로운 담당자로 자신의 사용자를 추가하여 **`Manage Certificates`** 접근 권한을 **부여할 수 있습니다**.
```bash
...
```
**`SubCA`** 템플릿은 `-enable-template` 매개변수로 CA에서 **활성화**할 수 있습니다. 기본적으로 `SubCA` 템플릿은 활성화되어 있습니다.
```bash
# List templates
...
```
이 공격을 위한 전제 조건을 충족했다면, **`SubCA` 템플릿을 기반으로 인증서를 요청하는 것부터 시작할 수 있습니다**.
...
```
우리의 **`Manage CA` 및 `Manage Certificates`**를 사용하여 `ca` 명령과 `-issue-request <request ID>` 매개변수를 사용하여 **실패한 인증서** 요청을 **발급**할 수 있습니다.
```bash
...
{% hint style="info" %}
**AD CS가 설치된** 환경에서는 **취약한 웹 등록 엔드포인트**가 존재하고, **도메인 컴퓨터 등록 및 클라이언트 인증**을 허용하는 **인증서 템플릿이 게시**되어 있는 경우(기본 **`Machine`** 템플릿과 같은), **스풀러 서비스가 활성화된 모든 컴퓨터가 공격자에 의해 손상될 수 있는** 가능성이 있습니다!
{% endhint %}
...
Certipy는 기본적으로 계정 이름이 `$`로 끝나는지 여부에 따라 `Machine` 또는 `User` 템플릿을 기반으로 인증서 요청을 합니다. 대체 템플릿을 지정하려면 `-template` 매개변수를 사용할 수 있습니다.
...
```
이 수정은 `Administrator@corp.local`이 `Administrator`의 `userPrincipalName`으로서 구별된 상태를 유지하므로 제약을 위반하지 않습니다.
...
```
인증서로 인증을 시도하면 이제 `Administrator@corp.local`의 NT 해시가 생성됩니다. 인증서에 도메인 지정이 없기 때문에 명령어에는 `-domain <domain>`이 포함되어야 합니다:
```bash
...
ESC10에서 언급하는 도메인 컨트롤러의 두 레지스트리 키 값은 다음과 같습니다:

* `HKEY_LOCAL_MACHINE\System\CurrentControlSet\Control\SecurityProviders\Schannel` 아래의 `CertificateMappingMethods`의 기본 값은 `0x18` (`0x8 | 0x10`)이며, 이전에는 `0x1F`로 설정되어 있었습니다.
...
`CertificateMappingMethods`에 `UPN` 비트 플래그(`0x4`)가 포함된 경우, `GenericWrite` 권한이 있는 계정 A는 `userPrincipalName` 속성이 없는 모든 계정 B를 손상시킬 수 있으며, 여기에는 머신 계정과 내장 도메인 관리자 `Administrator`가 포함됩니다.
...
```
`Jane`으로 클라이언트 인증을 위한 인증서가 기본 `User` 템플릿을 사용하여 요청됩니다.
```bash
certipy req -ca 'corp-DC-CA' -username Jane@corp.local -hashes <hash>
```
...
```
LDAP 셸을 통해 `set_rbcd`와 같은 명령은 리소스 기반 제약 위임(RBCD) 공격을 가능하게 하여 도메인 컨트롤러를 위험에 빠뜨릴 수 있습니다.
```bash
certipy auth -pfx dc.pfx -dc-ip 172.16.126.128 -ldap-shell
```
이 취약점은 `userPrincipalName`이 없거나 `sAMAccountName`과 일치하지 않는 모든 사용자 계정에도 확장됩니다. 기본 `Administrator@corp.local`은 LDAP 권한이 높고 기본적으로 `userPrincipalName`이 없기 때문에 주요 타겟입니다.
...
[*] Exiting...
```
Note: 도메인 컨트롤러의 경우, DomainController에서 `-template`을 지정해야 합니다.

또는 [sploutchy의 impacket 포크](https://github.com/sploutchy/impacket)를 사용하여:
```bash
...
[여기](https://pkiblog.knobloch.info/esc12-shell-access-to-adcs-ca-with-yubihsm)에서 참조하십시오.
...
템플릿만 지정하면 OIDToGroupLink 권한이 있는 인증서를 받을 수 있습니다.
```bash
...
**크로스 포리스트 등록**을 위한 구성은 상대적으로 간단하게 이루어집니다. **리소스 포리스트의 루트 CA 인증서**는 관리자가 **계정 포리스트에 게시**하고, **리소스 포리스트의 엔터프라이즈 CA** 인증서는 **각 계정 포리스트의 `NTAuthCertificates` 및 AIA 컨테이너에 추가**됩니다. 이 arrangement는 **리소스 포리스트의 CA가 PKI를 관리하는 모든 다른 포리스트에 대한 완전한 제어 권한**을 부여합니다. 만약 이 CA가 **공격자에 의해 손상된다면**, 리소스 및 계정 포리스트의 모든 사용자에 대한 인증서가 **위조될 수 있으며**, 이로 인해 포리스트의 보안 경계가 깨질 수 있습니다.
...
신뢰를 통해 인증이 이루어지면, **인증된 사용자 SID**가 AD에 의해 사용자의 토큰에 추가됩니다. 따라서 도메인에 **인증된 사용자 등록 권한을 허용하는 템플릿**이 있는 엔터프라이즈 CA가 있다면, **다른 포리스트의 사용자가 템플릿에 등록할 수 있는 가능성**이 있습니다. 마찬가지로, **템플릿에 의해 외부 주체에게 명시적으로 등록 권한이 부여되면**, **크로스 포리스트 접근 제어 관계가 생성되어** 한 포리스트의 주체가 **다른 포리스트의 템플릿에 등록할 수 있게 됩니다**.

두 시나리오는 한 포리스트에서 다른 포리스트로의 **공격 표면 증가**로 이어집니다. 공격자는 인증서 템플릿의 설정을 악용하여 외부 도메인에서 추가 권한을 얻을 수 있습니다.
