...
**DCSync** 권한은 도메인 자체에 대한 다음 권한을 의미합니다: **DS-Replication-Get-Changes**, **Replicating Directory Changes All** 및 **Replicating Directory Changes In Filtered Set**.
...
* 기본적으로 **도메인 관리자, 엔터프라이즈 관리자, 관리자 및 도메인 컨트롤러** 그룹만 필요한 권한을 갖습니다.
...
* NTDS의 암호를 평문으로 표시하는 파일 하나. [**가역 암호화**](https://docs.microsoft.com/en-us/windows/security/threat-protection/security-policy-settings/store-passwords-using-reversible-encryption)가 활성화된 계정에 대한 NTDS의 암호를 얻을 수 있습니다. 가역 암호화가 설정된 사용자는 다음과 같이 확인할 수 있습니다.
...
```
그럼, **사용자가 올바르게 할당되었는지 확인할 수 있습니다**. 이를 확인하기 위해 출력에서 해당 권한의 이름을 찾아보세요 ("ObjectType" 필드 내에서 권한 이름을 볼 수 있어야 함):
```powershell
