...
* **NTLM 해시**가 있는 파일
* **Kerberos 키**가 있는 파일
* NTDS의 암호를 평문으로 표시하는 파일, [**가역 암호화**](https://docs.microsoft.com/en-us/windows/security/threat-protection/security-policy-settings/store-passwords-using-reversible-encryption)가 활성화된 계정에 대한 것입니다. 가역 암호화가 설정된 사용자는 다음과 같이 얻을 수 있습니다.
...
```
그럼, **사용자가 올바르게 할당되었는지 확인할 수 있습니다**. 이를 확인하기 위해 (권한 이름을 "ObjectType" 필드 내에서 볼 수 있어야 함) 출력에서 해당 권한을 찾아보세요:
```powershell
