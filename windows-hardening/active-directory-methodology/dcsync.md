...
* **DCSync 공격은 도메인 컨트롤러의 동작을 모방하고 다른 도메인 컨트롤러에게 정보를 복제하도록 요청**하는 것을 의미합니다. 이는 디렉터리 복제 서비스 원격 프로토콜(MS-DRSR)을 사용합니다. MS-DRSR은 Active Directory의 유효하고 필수적인 기능이므로 끌 수나 비활성화할 수 없습니다.
...
```
`-just-dc`은 3개의 파일을 생성합니다:

* **NTLM 해시**가 있는 파일
* **Kerberos 키**가 있는 파일
* NTDS의 암호를 평문으로 포함하는 파일, [**가역 암호화**](https://docs.microsoft.com/en-us/windows/security/threat-protection/security-policy-settings/store-passwords-using-reversible-encryption)가 활성화된 계정에 대한 것입니다. 가역 암호화가 설정된 사용자는 다음과 같이 확인할 수 있습니다.
...
```
그럼, **사용자가 올바르게 할당되었는지 확인할 수 있습니다**. 이를 확인하기 위해 다음 출력에서 해당 권한 이름을 찾아보세요 ("ObjectType" 필드 내에서 권한 이름을 볼 수 있어야 함):
```powershell
