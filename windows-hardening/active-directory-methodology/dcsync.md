...
* O ataque **DCSync simula o comportamento de um Controlador de Domínio e solicita a outros Controladores de Domínio a replicação de informações** usando o Protocolo Remoto de Serviço de Replicação de Diretório (MS-DRSR). Como o MS-DRSR é uma função válida e necessária do Active Directory, não pode ser desativado.
...
* Se alguma senha de conta for armazenada com criptografia reversível, há uma opção no Mimikatz para retornar a senha em texto claro.
