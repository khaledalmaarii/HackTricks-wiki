...
Pour exécuter le **Kerberoasting**, un compte de domaine capable de demander des **tickets TGS** est essentiel ; cependant, ce processus ne nécessite pas de **privilèges spéciaux**, le rendant accessible à toute personne disposant de **justes informations d'identification de domaine**.
...
* Aucun **privilège spécial** n'est nécessaire, juste des **informations d'identification de domaine valides**.
...
* **Énumérer les utilisateurs pouvant être ciblés pour une attaque Kerberoast**
```powershell
...
* **Le plus important**, le type de chiffrement du ticket doit être **0x17**, ce qui est souvent utilisé dans les attaques de Kerberoasting.
```bash
