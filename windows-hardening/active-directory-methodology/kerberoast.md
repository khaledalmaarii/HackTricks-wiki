...
因此，要执行Kerberoasting，只需要一个可以请求TGS的域账户，这个账户可以是任何人，因为不需要特殊权限。
...
{% hint style="warning" %}
**Kerberoasting工具**通常在执行攻击和发起TGS-REQ请求时会请求**`RC4加密`**。这是因为**RC4**比其他加密算法如AES-128和AES-256更容易通过离线工具（如Hashcat）破解。\
RC4（类型23）的哈希以**`$krb5tgs$23$*`**开头，而AES-256（类型18）的哈希以**`$krb5tgs$18$*`**开头。
{% endhint %}
