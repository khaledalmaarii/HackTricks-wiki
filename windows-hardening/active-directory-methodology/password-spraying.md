...
Birkaç **geçerli kullanıcı adı** bulduktan sonra, her bir keşfedilen kullanıcı için en **sık kullanılan parolaları** deneyebilirsiniz (**ortamın parola politikasını** göz önünde bulundurun).\
...
Birkaç yanlış parola denerseniz bazı hesapları **kilitliyor olabileceğinizi unutmayın** (varsayılan olarak 10'dan fazla).
...
```
* [**sprey**](https://github.com/Greenwolf/Spray) _**(kilitlenmeleri önlemek için deneme sayısını belirtebilirsiniz):**_
```bash
spray.sh -smb <targetIP> <usernameList> <passwordList> <AttemptsPerLockoutPeriod> <LockoutPeriodInMinutes> <DOMAIN>
```
* [**kerbrute**](https://github.com/TarlogicSecurity/kerbrute) kullanarak (python) - SOMETIMES DOESN'T WORK ÖNERİLMEZ
```bash
...
* [Rubeus](https://github.com/Zer1t0/Rubeus) brute modülü ile birlikte sürümü:
```bash
...
```
* [**Invoke-DomainPasswordSpray**](https://github.com/dafthack/DomainPasswordSpray/blob/master/DomainPasswordSpray.ps1) ile (Varsayılan olarak etki alanından kullanıcılar oluşturabilir ve şifre politikasını etki alanından alabilir ve denemeleri buna göre sınırlayabilir):
```powershell
Invoke-DomainPasswordSpray -UserList .\users.txt -Password 123456 -Verbose
```
* [**Invoke-SprayEmptyPassword.ps1**](https://github.com/S3cur3Th1sSh1t/Creds/blob/master/PowershellScripts/Invoke-SprayEmptyPassword.ps1) betiği ile
```
...
Outlook için p**assword spraying** yapmak için çeşitli araçlar bulunmaktadır.
...
Bu araçlardan herhangi birini kullanabilmek için bir kullanıcı listesine ve bir şifre / püskürtmek için küçük bir şifre listesine ihtiyacınız vardır.
```bash
