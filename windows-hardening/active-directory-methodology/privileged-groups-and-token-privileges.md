...
```
स्पॉटलेस उपयोगकर्ता की सदस्यता पर ध्यान दें:
...
**AdminSDHolder** ऑब्जेक्ट की एक्सेस कंट्रोल लिस्ट (ACL) का उपयोग सभी "सुरक्षित समूहों" और उनके सदस्यों को **अनुमतियां** **कॉपी** करने के लिए एक टेम्पलेट के रूप में किया जाता है। सुरक्षित समूहों में विशेषाधिकार प्राप्त समूह जैसे कि डोमेन एडमिन्स, एडमिनिस्ट्रेटर्स, एंटरप्राइज एडमिन्स, और स्कीमा एडमिन्स शामिल हैं।\
डिफ़ॉल्ट रूप से, इस समूह की ACL को सभी "सुरक्षित समूहों" के अंदर कॉपी किया जाता है। यह इन महत्वपूर्ण समूहों में जानबूझकर या आकस्मिक परिवर्तनों से बचने के लिए किया जाता है। हालांकि, यदि कोई हमलावर **AdminSDHolder** समूह की ACL को संशोधित करता है, उदाहरण के लिए एक सामान्य उपयोगकर्ता को पूरी अनुमतियां देता है, तो यह उपयोगकर्ता सुरक्षित समूह के अंदर सभी समूहों पर पूरी अनुमतियां प्राप्त कर लेगा (एक घंटे में)।\
और यदि कोई इस उपयोगकर्ता को डोमेन एडमिन्स से (उदाहरण के लिए) एक घंटे या उससे कम समय में हटाने की कोशिश करता है, तो उपयोगकर्ता समूह में वापस आ जाएगा।
...
[**ired.team पर अधिक जानकारी।**](https://ired.team/offensive-security-experiments/active-directory-kerberos-abuse/how-to-abuse-and-backdoor-adminsdholder-to-obtain-domain-admin-persistence)
...
```
```markdown
यह पुष्टि करता है कि Server Operators समूह के पास [SERVICE\_ALL\_ACCESS](https://docs.microsoft.com/en-us/windows/win32/services/service-security-and-access-rights) पहुंच अधिकार है, जो हमें इस सेवा पर पूर्ण नियंत्रण प्रदान करता है।
...
`Server Operators` सदस्यता की तरह, यदि हम `Backup Operators` के सदस्य हैं तो हम **`DC01` फाइल सिस्टम तक पहुंच सकते हैं**।

ऐसा इसलिए है क्योंकि यह समूह अपने **सदस्यों** को [**`SeBackup`**](../windows-local-privilege-escalation/privilege-escalation-abusing-tokens/#sebackupprivilege-3.1.4) और [**`SeRestore`**](../windows-local-privilege-escalation/privilege-escalation-abusing-tokens/#serestoreprivilege-3.1.5) विशेषाधिकार प्रदान करता है। **SeBackupPrivilege** हमें किसी भी फोल्डर को पार करने और फोल्डर सामग्री की सूची बनाने की अनुमति देता है। यह हमें **एक फाइल को फोल्डर से कॉपी करने** की अनुमति देगा, भले ही कुछ और आपको अनुमति न दे रहा हो। हालांकि, इस अनुमति का दुरुपयोग करके एक फाइल को कॉपी करने के लिए [**FILE\_FLAG\_BACKUP\_SEMANTICS**](https://docs.microsoft.com/en-us/windows/win32/api/fileapi/nf-fileapi-createfilea) \*\*\*\* फ्लैग का उपयोग किया जाना चाहिए। इसलिए, विशेष उपकरणों की आवश्यकता होती है।
...
DISKSHADOW> exit
```
जैसा कि स्थानीय हमले में, आप अब विशेषाधिकार प्राप्त फ़ाइल **`NTDS.dit`** की प्रतिलिपि बना सकते हैं:
```
Copy-FileSeBackupPrivilege E:\Windows\NTDS\ntds.dit C:\Tools\ntds.dit
```
फाइलों को कॉपी करने का एक और तरीका है [**robocopy**](https://docs.microsoft.com/en-us/windows-server/administration/windows-commands/robocopy)**:**
```
...
इसका उपयोग करने से पहले, आपको हमलावर मशीन पर [**smb सर्वर के लिए ntfs फाइलसिस्टम सेटअप**](https://gist.github.com/manesec/9e0e8000446b966d0f0ef74000829801) करने की आवश्यकता है।
...
जो उपयोगकर्ता **DNSAdmins** समूह का सदस्य है या **DNS** सर्वर ऑब्जेक्ट के लिए **लिखने की अनुमति** रखता है, वह **DNS सर्वर** पर **SYSTEM** विशेषाधिकारों के साथ **मनमानी DLL** लोड कर सकता है।\
...
* [**ServerLevelPluginDll**](https://docs.microsoft.com/en-us/openspecs/windows_protocols/ms-dnsp/c9d38538-8827-44e6-aa5e-022a016ed723) हमें DLL के पथ की **कोई जांच** के बिना एक कस्टम **DLL** **लोड** करने की अनुमति देता है। यह `dnscmd` टूल के साथ कमांड लाइन से किया जा सकता है
...
* जब **DNS सेवा को पुनः आरंभ** किया जाता है, तो इस पथ में **DLL** **लोड** की जाएगी (अर्थात्, एक नेटवर्क शेयर जिसे डोमेन कंट्रोलर का मशीन खाता एक्सेस कर सकता है)
...
**ग्लोबल क्वेरी** ब्लॉक सूची को अक्षम करने और **WPAD रिकॉर्ड** बनाने के बाद, **हर मशीन** जो WPAD को डिफ़ॉल्ट सेटिंग्स के साथ चला रही है, उसका **ट्रैफिक हमारी अटैक मशीन के माध्यम से प्रॉक्सी किया जाएगा**। हम [**Responder**](https://github.com/lgandx/Responder) **या** [**Inveigh**](https://github.com/Kevin-Robertson/Inveigh) जैसे टूल का उपयोग करके **ट्रैफिक स्पूफिंग कर सकते हैं**, और पासवर्ड हैशेज को कैप्चर करने और उन्हें ऑफलाइन क्रैक करने या SMBRelay हमला करने का प्रयास कर सकते हैं।
...
यह [**GitHub रेपो**](https://github.com/gdedrouas/Exchange-AD-Privesc) कुछ **तकनीकों** को समझाता है जिनका उपयोग करके इस समूह की अनुमतियों का दुरुपयोग करके विशेषाधिकारों को **बढ़ाया** जा सकता है।
```powershell
#Get members of the group
...
यह भी इस [ब्लॉग](https://decoder.cloud/2020/01/20/from-hyper-v-admin-to-system/) पर अच्छी तरह से दस्तावेज़ीकृत है कि एक वर्चुअल मशीन को **हटाने** पर, `vmms.exe` संबंधित **`.vhdx` फाइल** पर **मूल फाइल अनुमतियों को पुनःस्थापित** करने का प्रयास करता है और वह `NT AUTHORITY\SYSTEM` के रूप में ऐसा करता है, उपयोगकर्ता की नकल किए बिना। हम **`.vhdx` फाइल को हटा सकते हैं** और एक मूल **हार्ड लिंक बना सकते हैं** जो इस फाइल को एक **सुरक्षित SYSTEM फाइल** की ओर इंगित करता है, और आपको इस पर पूरी अनुमतियां दी जाएंगी।

यदि ऑपरेटिंग सिस्टम [CVE-2018-0952](https://www.tenable.com/cve/CVE-2018-0952) या [CVE-2019-0841](https://www.tenable.com/cve/CVE-2019-0841) के लिए संवेदनशील है, तो हम इसका लाभ उठाकर SYSTEM विशेषाधिकार प्राप्त कर सकते हैं। अन्यथा, हम **सर्वर पर एक ऐसे एप्लिकेशन का लाभ उठा सकते हैं जिसमें SYSTEM के संदर्भ में चलने वाली एक सेवा स्थापित है**, जिसे अधिकारहीन उपयोगकर्ता द्वारा शुरू किया जा सकता है।
...
```
#### **मोज़िला मेंटेनेंस सर्विस शुरू करना**
...
इस समूह को `Microsoft Exchange Security Groups` नामक OU का भी **पूर्ण नियंत्रण** होता है, जिसमें समूह [**`Exchange Windows Permissions`**](privileged-groups-and-token-privileges.md#exchange-windows-permissions) \*\*\*\* (इस समूह का दुरुपयोग करके privesc कैसे करें, इसके लिए लिंक का अनुसरण करें) शामिल है।
...
* **डोमेन कंट्रोलर पर स्थानीय रूप से लॉग ऑन** करना और इसे बंद करना
