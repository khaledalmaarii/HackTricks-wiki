...
```
ध्यान दें कि स्पॉटलेस उपयोगकर्ता की सदस्यता:
...
**AdminSDHolder** ऑब्जेक्ट की एक्सेस कंट्रोल लिस्ट (ACL) का उपयोग **सभी “सुरक्षित समूहों”** में **अनुमतियों** की **प्रतिलिपि** बनाने के लिए किया जाता है और उनके सदस्यों में भी। सुरक्षित समूहों में विशेषाधिकार प्राप्त समूह जैसे कि डोमेन एडमिन्स, एडमिनिस्ट्रेटर्स, एंटरप्राइज एडमिन्स, और स्कीमा एडमिन्स शामिल हैं।\
डिफ़ॉल्ट रूप से, इस समूह की ACL की प्रतिलिपि सभी "सुरक्षित समूहों" के अंदर की जाती है। यह इन महत्वपूर्ण समूहों में जानबूझकर या आकस्मिक परिवर्तनों से बचने के लिए किया जाता है। हालांकि, यदि कोई हमलावर **AdminSDHolder** समूह की ACL को संशोधित करता है, उदाहरण के लिए एक सामान्य उपयोगकर्ता को पूर्ण अनुमतियां देता है, तो यह उपयोगकर्ता सुरक्षित समूह के अंदर सभी समूहों पर पूर्ण अनुमतियां प्राप्त कर लेगा (एक घंटे में)।\
...
सेवा पर अनुमतियों की जांच के लिए Sysinternals से [`PsService`](https://docs.microsoft.com/en-us/sysinternals/downloads/psservice) या `sc` का उपयोग करें।
```
...
All
```
इससे पुष्टि होती है कि Server Operators समूह के पास [SERVICE\_ALL\_ACCESS](https://docs.microsoft.com/en-us/windows/win32/services/service-security-and-access-rights) पहुँच अधिकार है, जो हमें इस सेवा पर पूर्ण नियंत्रण प्रदान करता है।
आप इस सेवा का दुरुपयोग करके [**सेवा को मनमाने आदेश निष्पादित करने के लिए प्रेरित कर सकते हैं**](https://book.hacktricks.xyz/windows-hardening/windows-local-privilege-escalation#modify-service-binary-path) और विशेषाधिकारों को बढ़ा सकते हैं।

## Backup Operators <a href="#backup-operators" id="backup-operators"></a>

`Server Operators` सदस्यता की तरह, यदि हम `Backup Operators` के सदस्य हैं तो हम **`DC01` फाइल सिस्टम तक पहुँच सकते हैं**।

ऐसा इसलिए है क्योंकि यह समूह अपने **सदस्यों** को [**`SeBackup`**](../windows-local-privilege-escalation/privilege-escalation-abusing-tokens/#sebackupprivilege-3.1.4) और [**`SeRestore`**](../windows-local-privilege-escalation/privilege-escalation-abusing-tokens/#serestoreprivilege-3.1.5) विशेषाधिकार प्रदान करता है। **SeBackupPrivilege** हमें किसी भी फोल्डर को **पार करने और फोल्डर सामग्री की सूची बनाने** की अनुमति देता है। इससे हमें **एक फाइल को फोल्डर से कॉपी करने** की अनुमति मिलेगी, भले ही कोई और आपको अनुमति न दे रहा हो। हालांकि, इस अनुमति का दुरुपयोग करके एक फाइल को कॉपी करने के लिए झंडा [**FILE\_FLAG\_BACKUP\_SEMANTICS**](https://docs.microsoft.com/en-us/windows/win32/api/fileapi/nf-fileapi-createfilea) \*\*\*\* का उपयोग करना होगा। इसलिए, विशेष उपकरणों की आवश्यकता होती है।

इस उद्देश्य के लिए आप [**इन स्क्रिप्ट्स**](https://github.com/giuliano108/SeBackupPrivilege)** का उपयोग कर सकते हैं।**
...
उदाहरण के लिए, आप सीधे Domain Controller फाइल सिस्टम तक पहुँच सकते हैं:

![](../../.gitbook/assets/a7.png)

आप इस पहुँच का दुरुपयोग करके सक्रिय निर्देशिका डेटाबेस **`NTDS.dit`** को **चुरा** सकते हैं ताकि डोमेन में सभी उपयोगकर्ता और कंप्यूटर ऑब्जेक्ट्स के लिए सभी **NTLM हैशेज** प्राप्त कर सकें।
...
DISKSHADOW> exit
```
स्थानीय हमले की तरह, आप अब विशेषाधिकार प्राप्त फ़ाइल **`NTDS.dit`** की प्रतिलिपि बना सकते हैं:
```
...
wbadmin.exe का उपयोग diskshadow.exe के समान है, wbadmin.exe उपकरण एक कमांड लाइन उपयोगिता है जो Windows में निर्मित है, Windows Vista/Server 2008 से शुरू होकर।

इसका उपयोग करने से पहले, आपको हमलावर मशीन पर [**smb सर्वर के लिए ntfs फाइल सिस्टम सेटअप**](https://gist.github.com/manesec/9e0e8000446b966d0f0ef74000829801) करना होगा।

smb सर्वर सेटअप करने के बाद, आपको लक्ष्य मशीन पर smb क्रेडेंशियल को कैश करने की आवश्यकता है:
```
...
dir X:\
```
यदि कोई त्रुटि नहीं है, तो इसका उपयोग करने के लिए wbadmin.exe का उपयोग करें:
```
...
* जब **DNS सेवा को पुनः आरंभ किया जाता है**, इस पथ में **DLL** **लोड** की जाएगी (अर्थात्, एक नेटवर्क शेयर जिसे डोमेन कंट्रोलर के मशीन खाते तक पहुँच हो सकती है)
...
फिर, यदि आपके पास **DNSAdmins समूह** में एक उपयोगकर्ता है, तो आप **DNS सर्वर को SYSTEM विशेषाधिकारों के साथ मनमानी DLL लोड करने के लिए बना सकते हैं** (DNS सेवा `NT AUTHORITY\SYSTEM` के रूप में चलती है)। आप DNS सर्वर को एक **स्थानीय या दूरस्थ** (SMB द्वारा साझा की गई) DLL फ़ाइल निष्पादित करने के लिए लोड कर सकते हैं:
```
...
```
तो, जब **DNSservice** शुरू होती है या पुनः आरंभ होती है, एक नया उपयोगकर्ता बनाया जाएगा।
...
इस [**पोस्ट**](http://www.labofapenetrationtester.com/2017/05/abusing-dnsadmins-privilege-for-escalation-in-active-directory.html) में विस्तार से बताया गया है कि `Mimikatz` टूल के निर्माता द्वारा [**mimilib.dll**](https://github.com/gentilkiwi/mimikatz/tree/master/mimilib) का उपयोग करके [**kdns.c**](https://github.com/gentilkiwi/mimikatz/blob/master/mimilib/kdns.c) **** फाइल को **संशोधित** करके **रिवर्स शेल** वन-लाइनर या हमारी पसंद का कोई अन्य कमांड निष्पादित करने के लिए कमांड निष्पादन प्राप्त करना संभव है।

### WPAD रिकॉर्ड MitM के लिए

DnsAdmins समूह की विशेषाधिकारों का **दुरुपयोग** करने का एक अन्य तरीका **WPAD रिकॉर्ड** बनाना है। इस समूह की सदस्यता हमें [ग्लोबल क्वेरी ब्लॉक सुरक्षा को अक्षम करने](https://docs.microsoft.com/en-us/powershell/module/dnsserver/set-dnsserverglobalqueryblocklist?view=windowsserver2019-ps) का अधिकार देती है, जो डिफ़ॉल्ट रूप से इस हमले को ब्लॉक करती है। सर्वर 2008 ने पहली बार DNS सर्वर पर ग्लोबल क्वेरी ब्लॉक सूची में जोड़ने की क्षमता पेश की। डिफ़ॉल्ट रूप से, वेब प्रॉक्सी ऑटोमैटिक डिस्कवरी प्रोटोकॉल (WPAD) और इंट्रा-साइट ऑटोमैटिक टनल एड्रेसिंग प्रोटोकॉल (ISATAP) ग्लोबल क्वेरी ब्लॉक सूची में होते हैं। ये प्रोटोकॉल हाइजैकिंग के लिए काफी संवेदनशील होते हैं, और कोई भी डोमेन उपयोगकर्ता इन नामों वाली कंप्यूटर ऑब्जेक्ट या DNS रिकॉर्ड बना सकता है।

**ग्लोबल क्वेरी** ब्लॉक सूची को अक्षम करने और **WPAD रिकॉर्ड** बनाने के बाद, **हर मशीन** जो WPAD को डिफ़ॉल्ट सेटिंग्स के साथ चला रही है, उसका **ट्रैफिक हमारी हमला मशीन के माध्यम से प्रॉक्सी किया जाएगा**। हम ट्रैफिक स्पूफिंग करने के लिए \*\*\*\* [**Responder**](https://github.com/lgandx/Responder) **या** [**Inveigh**](https://github.com/Kevin-Robertson/Inveigh) **जैसे टूल का उपयोग कर सकते हैं**, और पासवर्ड हैशेज को कैप्चर करने और उन्हें ऑफ़लाइन क्रैक करने या SMBRelay हमला करने का प्रयास कर सकते हैं।
...
सदस्यों को **डोमेन ऑब्जेक्ट पर एक DACL लिखने** की क्षमता प्रदान की जाती है। एक हमलावर इसका दुरुपयोग करके **एक उपयोगकर्ता को** [**DCSync**](dcsync.md) विशेषाधिकार देने के लिए कर सकता है।\
यदि माइक्रोसॉफ्ट एक्सचेंज AD वातावरण में स्थापित है, तो इस समूह के सदस्य के रूप में उपयोगकर्ता खातों और यहां तक कि कंप्यूटरों को पाया जाना सामान्य है।
...
[**Hyper-V प्रशासक**](https://docs.microsoft.com/en-us/windows/security/identity-protection/access-control/active-directory-security-groups#hyper-v-administrators) समूह को [Hyper-V सुविधाओं](https://docs.microsoft.com/en-us/windows-server/manage/windows-admin-center/use/manage-virtual-machines) पर पूर्ण पहुंच होती है। यदि **डोमेन कंट्रोलर्स** को **वर्चुअलाइज़** किया गया है, तो **वर्चुअलाइज़ेशन प्रशासकों** को **डोमेन प्रशासकों** के रूप में माना जाना चाहिए। वे आसानी से **जीवित डोमेन कंट्रोलर की एक क्लोन बना सकते हैं** और वर्चुअल **डिस्क** को ऑफलाइन **माउंट** करके **`NTDS.dit`** फाइल प्राप्त कर सकते हैं और डोमेन में सभी उपयोगकर्ताओं के लिए NTLM पासवर्ड हैशेज निकाल सकते हैं।

इस [ब्लॉग](https://decoder.cloud/2020/01/20/from-hyper-v-admin-to-system/) पर भी अच्छी तरह से दस्तावेज़ीकरण किया गया है कि एक वर्चुअल मशीन को **हटाने** पर, `vmms.exe` संबंधित **`.vhdx` फाइल** पर मूल फाइल अनुमतियों को **पुनर्स्थापित** करने का प्रयास करता है और वह `NT AUTHORITY\SYSTEM` के रूप में ऐसा करता है, उपयोगकर्ता की नकल किए बिना। हम **`.vhdx`** फाइल को **हटा सकते हैं** और एक मूल **हार्ड लिंक** बना सकते हैं जो इस फाइल को एक **सुरक्षित SYSTEM फाइल** की ओर इंगित करता है, और आपको इस पर पूर्ण अनुमतियां दी जाएंगी।

यदि ऑपरेटिंग सिस्टम [CVE-2018-0952](https://www.tenable.com/cve/CVE-2018-0952) या [CVE-2019-0841](https://www.tenable.com/cve/CVE-2019-0841) के लिए संवेदनशील है, तो हम इसका उपयोग करके SYSTEM विशेषाधिकार प्राप्त कर सकते हैं। अन्यथा, हम **सर्वर पर एक ऐसे एप्लिकेशन का लाभ उठा सकते हैं जिसमें SYSTEM के संदर्भ में चलने वाली एक सेवा स्थापित है**, जिसे अधिकारहीन उपयोगकर्ता द्वारा शुरू किया जा सकता है।

### **शोषण उदाहरण**

इसका एक उदाहरण **Firefox** है, जो **`Mozilla Maintenance Service`** स्थापित करता है। हम [इस शोषण](https://raw.githubusercontent.com/decoder-it/Hyper-V-admin-EOP/master/hyperv-eop.ps1) (NT हार्ड लिंक के लिए एक प्रमाण-संकल्प) को अपडेट कर सकते हैं ताकि हमारे वर्तमान उपयोगकर्ता को नीचे दी गई फाइल पर पूर्ण अनुमतियां प्रदान की जा सकें:
```bash
C:\Program Files (x86)\Mozilla Maintenance Service\maintenanceservice.exe
```
#### **फ़ाइल का स्वामित्व लेना**

PowerShell स्क्रिप्ट चलाने के बाद, हमें **इस फ़ाइल पर पूर्ण नियंत्रण मिल जाना चाहिए और हम इसका स्वामित्व ले सकते हैं**।
```bash
...
यह समूह **Microsoft Exchange** स्थापित किए गए पर्यावरणों में भी होता है।\
इस समूह के सदस्य **सभी** डोमेन उपयोगकर्ताओं के **मेलबॉक्स** तक **पहुँच** सकते हैं।\
इस समूह को `Microsoft Exchange Security Groups` नामक OU का **पूर्ण नियंत्रण** भी होता है, जिसमें समूह [**`Exchange Windows Permissions`**](privileged-groups-and-token-privileges.md#exchange-windows-permissions) \*\*\*\* (इस समूह का दुरुपयोग करके privesc कैसे करें, लिंक का अनुसरण करें) शामिल है।
...
* **डोमेन कंट्रोलर पर स्थानीय रूप से लॉग ऑन** करना और उसे बंद करना
* डोमेन कंट्रोलर से जुड़े **प्रिंटर्स को प्रबंधित**, बनाने, साझा करने और हटाने की अनुमतियाँ

{% hint style="warning" %}
यदि कमांड `whoami /priv`, एक अनउच्चित संदर्भ से **`SeLoadDriverPrivilege`** नहीं दिखाता है, तो आपको UAC को बायपास करने की आवश्यकता है।
{% endhint %}
...
```
इस पृष्ठ पर जांचें कि SeLoadDriverPrivilege का दुरुपयोग करके privesc कैसे करें:
