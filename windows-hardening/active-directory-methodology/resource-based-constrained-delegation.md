# Delegação Constrain Baseada em Recursos
...
## Conceitos Básicos de Delegação Constrain Baseada em Recursos

Isso é semelhante à [Delegação Constrain](constrained-delegation.md) básica, mas **em vez** de conceder permissões a um **objeto** para **fingir ser qualquer usuário em relação a um serviço**. A Delegação Constrain Baseada em Recursos **define** no **objeto quem pode fingir ser qualquer usuário em relação a ele**.
...
Outra diferença importante desta Delegação Constrain para as outras delegações é que qualquer usuário com **permissões de escrita sobre uma conta de máquina** (_GenericAll/GenericWrite/WriteDacl/WriteProperty/etc_) pode definir o _**msDS-AllowedToActOnBehalfOfOtherIdentity**_ (Nas outras formas de Delegação você precisava de privilégios de administrador de domínio).

### Novos Conceitos

Voltando à Delegação Constrain, foi dito que a flag **`TrustedToAuthForDelegation`** dentro do valor _userAccountControl_ do usuário é necessária para realizar um **S4U2Self**. Mas isso não é completamente verdade.\
...
No entanto, se o **TGS** usado em **S4U2Proxy** **NÃO for Forwardable** tentar abusar de uma **Delegação Constrain básica** **não funcionará**. Mas se você estiver tentando explorar uma **delegação Constrain baseada em recursos, funcionará** (isso não é uma vulnerabilidade, é um recurso, aparentemente).
...
1. O atacante **compromete** uma conta que tem um **SPN** ou **cria um** (“Serviço A”). Note que **qualquer** _Usuário Administrador_ sem nenhum outro privilégio especial pode **criar** até 10 **objetos de Computador (**_**MachineAccountQuota**_**)** e definir um **SPN**. Então o atacante pode simplesmente criar um objeto de Computador e definir um SPN.
2. O atacante **abusa de seu privilégio de ESCRITA** sobre o computador da vítima (ServiçoB) para configurar **delegação constrain baseada em recursos para permitir que o ServiçoA finja ser qualquer usuário** contra esse computador da vítima (ServiçoB).
...
3. Mesmo se você estiver usando um TGS não Forwardable, como você está explorando a delegação constrain baseada em recursos, funcionará.
4. O atacante pode **pass-the-ticket** e **fingir ser** o usuário para obter **acesso ao ServiçoB da vítima**.
