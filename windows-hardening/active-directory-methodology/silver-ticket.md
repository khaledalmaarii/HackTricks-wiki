...
**CIFS**服务是允许您访问受害者的文件系统的服务。您可以在此处找到其他服务：[https://adsecurity.org/?page\_id=183](https://adsecurity.org/?page\_id=183)。例如，您可以使用**HOST服务**在计算机上创建一个_schtask_。然后，您可以尝试列出受害者的任务来检查是否成功：`schtasks /S <hostname>`，或者您可以使用**HOST和RPCSS服务**在计算机上执行**WMI**查询，测试方法如下：`Get-WmiObject -Class win32_operatingsystem -ComputerName <hostname>`
...
在以下示例中，假设使用管理员帐户模拟检索到票据。

### CIFS

使用此票据，您将能够通过**SMB**访问`C$`和`ADMIN$`文件夹（如果它们被公开），并将文件复制到远程文件系统的某个位置，只需执行以下操作：
```bash
