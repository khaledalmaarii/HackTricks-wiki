...
[Benutzerkontensteuerung (UAC)](https://docs.microsoft.com/en-us/windows/security/identity-protection/user-account-control/how-user-account-control-works) ist eine Funktion, die eine **Zustimmungsaufforderung für erhöhte Aktivitäten** ermöglicht. Anwendungen haben unterschiedliche `Integritäts`-Ebenen, und ein Programm mit einer **hohen Ebene** kann Aufgaben ausführen, die **potenziell das System gefährden könnten**. Wenn UAC aktiviert ist, werden Anwendungen und Aufgaben immer unter dem Sicherheitskontext eines Nicht-Administrator-Kontos ausgeführt, es sei denn, ein Administrator autorisiert explizit diese Anwendungen/Aufgaben, um Administratorzugriff auf das System zu erhalten. Es handelt sich um eine Komfortfunktion, die Administratoren vor unbeabsichtigten Änderungen schützt, aber nicht als Sicherheitsgrenze betrachtet wird.

Für weitere Informationen zu Integritätsebenen:
...
| [Benutzerkontensteuerung: Nur ausführen von ausführbaren Dateien, die signiert und validiert sind](https://docs.microsoft.com/en-us/windows/security/identity-protection/user-account-control/user-account-control-group-policy-and-registry-key-settings#user-account-control-only-elevate-executables-that-are-signed-and-validated)                                                             | ValidateAdminCodeSignatures | Deaktiviert                                                  |
...
Einige Programme werden automatisch **autoelevated**, wenn der **Benutzer zur Administratorgruppe gehört**. Diese Binärdateien enthalten in ihren _**Manifesten**_ die Option _**autoElevate**_ mit dem Wert _**True**_. Die Binärdatei muss auch von **Microsoft signiert** sein.

Dann verwenden einige Angreifer diese Art von Binärdateien, um die **UAC zu umgehen** (von **mittlerem** Integritätsniveau auf **hohes** Niveau zu heben) und beliebigen Code auszuführen, da er von einem Prozess mit **hohem Integritätsniveau** ausgeführt wird.
...
```
Wenn es **`1`** ist, dann ist UAC **aktiviert**, wenn es **`0`** ist oder **nicht existiert**, dann ist UAC **deaktiviert**.
...
* Wenn **`2`** (**Immer benachrichtigen**), wird UAC den Administrator immer um Bestätigung bitten, wenn er versucht, etwas mit hohen Rechten auszuführen (auf dem sicheren Desktop)
...
Dann müssen Sie den Wert von **`LocalAccountTokenFilterPolicy`** überprüfen. Wenn der Wert **`0`** ist, kann nur der Benutzer mit der **RID 500** (**eingebauter Administrator**) **Admin-Aufgaben ohne UAC** ausführen, und wenn es `1` ist, können **alle Konten innerhalb der Gruppe "Administratoren"** dies tun.
...
{% hint style="info" %}
Beachten Sie, dass bei grafischem Zugriff auf das Opfer das UAC-Bypass einfach ist, da Sie einfach auf "Ja" klicken können, wenn das UAC-Prompt erscheint.
{% endhint %}

Der UAC-Bypass ist in folgender Situation erforderlich: **das UAC ist aktiviert, Ihr Prozess läuft in einem mittleren Integritätskontext und Ihr Benutzer gehört zur Administratorengruppe**.

Es ist wichtig zu erwähnen, dass es **viel schwieriger ist, das UAC zu umgehen, wenn es auf dem höchsten Sicherheitsniveau (Immer) liegt, als wenn es auf einem der anderen Ebenen (Standard) liegt**.

### UAC deaktiviert

Wenn das UAC bereits deaktiviert ist (`ConsentPromptBehaviorAdmin` ist **`0`**), können Sie **eine Reverse-Shell mit Administratorrechten** (hohes Integritätsniveau) ausführen, indem Sie beispielsweise:
```bash
...
Wenn Sie eine Shell mit einem Benutzer haben, der zur Gruppe der Administratoren gehört, können Sie das C$-Laufwerk über SMB (Dateisystem) lokal als neuen Datenträger einbinden und Sie werden **Zugriff auf alles im Dateisystem haben** (sogar auf den Administrator-Startordner).
...
Sie sollten **vorsichtig sein**, da einige Umgehungen einige andere Programme **auffordern**, die **Benutzer** darauf hinweisen, dass etwas passiert.

UACME verfügt über die **Build-Version, ab der jede Technik zu funktionieren begann**. Sie können nach einer Technik suchen, die Ihre Versionen betrifft:
```
...
**Alle** hier verwendeten Techniken zum Umgehen von UAC erfordern eine **vollständige interaktive Shell** mit dem Opfer (eine gewöhnliche nc.exe-Shell reicht nicht aus).

Sie können eine **Meterpreter**-Sitzung verwenden. Wechseln Sie zu einem **Prozess**, dessen **Sitzungs**-Wert gleich **1** ist:
...
1. Finden Sie eine ausführbare Datei, die **automatisch erhöht** (überprüfen Sie, ob sie beim Ausführen auf einem hohen Integritätsniveau ausgeführt wird).
2. Suchen Sie mit Procmon nach "**NAME NOT FOUND**"-Ereignissen, die anfällig für **DLL-Hijacking** sein könnten.
3. Sie müssen wahrscheinlich die DLL in einigen **geschützten Pfaden** (wie C:\Windows\System32) schreiben, in denen Sie keine Schreibberechtigungen haben. Sie können dies umgehen, indem Sie:
...
4. Erstellen Sie ein **Skript**, um Ihre DLL in den geschützten Pfad zu kopieren und die anfällige und automatisch erhöhte ausführbare Datei auszuführen.
