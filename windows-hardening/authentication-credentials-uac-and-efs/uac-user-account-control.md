...
Quando UAC è attivo, a un utente amministratore vengono forniti 2 token: una chiave utente standard, per eseguire azioni regolari a livello regolare, e una con i privilegi di amministratore.
...
{% hint style="info" %}
Si noti che se si ha accesso grafico alla vittima, il bypass di UAC è semplice poiché è sufficiente fare clic su "Sì" quando compare il prompt UAC.
{% endhint %}
...
Se dai un'occhiata a **UACME** noterai che **la maggior parte dei bypass di UAC sfruttano una vulnerabilità di Dll Hijacking** (principalmente scrivendo il dll dannoso su _C:\Windows\System32_). [Leggi questo per imparare come trovare una vulnerabilità di Dll Hijacking](../windows-local-privilege-escalation/dll-hijacking/).
...
6. Prepara uno **script** per copiare il tuo DLL all'interno del percorso protetto ed eseguire il binario vulnerabile e autoelevato.
