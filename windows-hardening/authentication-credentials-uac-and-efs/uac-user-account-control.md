...
Niektóre programy są **automatycznie ustawione na wyższy poziom uprawnień**, jeśli **użytkownik należy** do **grupy administratorów**. Te pliki wykonywalne mają w swoich _**Manifestach**_ opcję _**autoElevate**_ ustawioną na wartość _**True**_. Plik wykonywalny musi być również **podpisany przez Microsoft**.
...
* Jeśli **`2`** (**Zawsze informuj mnie**), UAC zawsze będzie prosić o potwierdzenie dla administratora, gdy próbuje on wykonać coś z wysokimi uprawnieniami (na Bezpiecznym Pulpicie)
...
Jeśli **`0`**(domyślnie), to **wbudowane konto Administratora** może wykonywać zadania administracyjne zdalnie, a jeśli **`1`**, wbudowane konto Administratora **nie może** wykonywać zadań administracyjnych zdalnie, chyba że `LocalAccountTokenFilterPolicy` jest ustawione na `1`.
...
### Wykorzystania omijania UAC

[**UACME**](https://github.com/hfiref0x/UACME) to **kompilacja** kilku wykorzystań omijania UAC. Zauważ, że będziesz musiał **skompilować UACME za pomocą Visual Studio lub msbuild**. Kompilacja stworzy kilka plików wykonywalnych (np. `Source\Akagi\outout\x64\Debug\Akagi.exe`), będziesz musiał wiedzieć, **który z nich potrzebujesz.**\
Powinieneś **być ostrożny**, ponieważ niektóre omijania spowodują, że inne programy **wyświetlą komunikaty**, które **poinformują użytkownika**, że coś się dzieje.
...
**Wszystkie** techniki używane tutaj do bypassowania AUC **wymagają** **pełnej interaktywnej powłoki** z ofiarą (zwykła powłoka nc.exe nie wystarczy).

Możesz uzyskać dostęp za pomocą sesji **meterpreter**. Przejdź do **procesu**, który ma wartość **Session** równą **1**:
...
Jeśli masz dostęp do **interfejsu graficznego, możesz po prostu zaakceptować monit UAC**, gdy się pojawi, nie potrzebujesz naprawdę bypassu. Dlatego uzyskanie dostępu do interfejsu graficznego pozwoli Ci ominąć UAC.

Co więcej, jeśli uzyskasz sesję interfejsu graficznego, którą ktoś używał (potencjalnie za pośrednictwem RDP), istnieją **niektóre narzędzia, które będą działać jako administrator**, z których można **uruchomić** na przykład **cmd** jako administratora bez ponownego pytania o UAC, takie jak [**https://github.com/oski02/UAC-GUI-Bypass-appverif**](https://github.com/oski02/UAC-GUI-Bypass-appverif). Może to być trochę bardziej **ukryte**.

### Głośny bypass UAC metodą siłową

Jeśli nie zależy Ci na tym, aby być głośnym, zawsze możesz **uruchomić coś w rodzaju** [**https://github.com/Chainski/ForceAdmin**](https://github.com/Chainski/ForceAdmin), które **wymaga podniesienia uprawnień, dopóki użytkownik nie zaakceptuje**.

### Twój własny bypass - Podstawowa metodologia bypassu UAC

Jeśli spojrzysz na **UACME**, zauważysz, że **większość bypassów UAC wykorzystuje podatność na Dll Hijacking** (głównie poprzez zapisanie złośliwej dll w _C:\Windows\System32_). [Przeczytaj to, aby dowiedzieć się, jak znaleźć podatność na Dll Hijacking](../windows-local-privilege-escalation/dll-hijacking/).
...
3. Prawdopodobnie będziesz musiał **zapisać** DLL w **chronionych ścieżkach** (takich jak C:\Windows\System32), gdzie nie masz uprawnień do zapisu. Możesz ominąć to, używając:
4. **wusa.exe**: Windows 7, 8 i 8.1. Pozwala na wypakowanie zawartości pliku CAB w chronionych ścieżkach (ponieważ to narzędzie jest uruchamiane na wysokim poziomie integralności).
5. **IFileOperation**: Windows 10.
6. Przygotuj **skrypt**, aby skopiować swoje DLL do chronionej ścieżki i uruchomić podatny i automatycznie podniesiony binarny plik.

### Inna technika bypassu UAC

Polega na obserwowaniu, czy **binarny plik autoElevated** próbuje **odczytać** z **rejestru** **nazwę/ścieżkę** **binarnego** pliku lub **polecenia** do **wykonania** (to jest bardziej interesujące, jeśli binarny plik wyszukuje te informacje wewnątrz **HKCU**).
