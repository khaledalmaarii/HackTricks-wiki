...
**MMC20.Application** 对象被发现缺乏显式的“LaunchPermissions”，默认权限允许管理员访问。有关更多详细信息，请查看[此处](https://twitter.com/tiraniddo/status/817532039771525120)，建议使用[@tiraniddo](https://twitter.com/tiraniddo)的 OleView .NET 进行过滤没有显式启动权限的对象。
...
```
### 通过 Excel DCOM 对象进行横向移动

可以通过利用 DCOM Excel 对象来实现横向移动。有关详细信息，建议阅读Cybereason博客上关于通过 DCOM 利用 Excel DDE 进行横向移动的讨论。

Empire项目提供了一个PowerShell脚本，演示了通过操纵 DCOM 对象利用Excel进行远程代码执行（RCE）的方法。以下是从[Empire的GitHub存储库](https://github.com/EmpireProject/Empire/blob/master/data/module_source/lateral_movement/Invoke-DCOM.ps1)中提取的脚本片段，展示了滥用Excel进行RCE的不同方法：
```powershell
