...
分布式组件对象模型（DCOM）对象提供了与对象进行基于网络的交互的有趣功能。Microsoft为DCOM和组件对象模型（COM）提供了全面的文档，可在[此处查看DCOM](https://msdn.microsoft.com/en-us/library/cc226801.aspx)和[此处查看COM](https://msdn.microsoft.com/en-us/library/windows/desktop/ms694363\(v=vs.85\).aspx)。可以使用PowerShell命令检索DCOM应用程序的列表：
```bash
...
## ShellWindows 和 ShellBrowserWindow

**MMC20.Application** 对象被发现缺乏显式的“LaunchPermissions”，默认权限允许管理员访问。有关更多详细信息，请查看[此处](https://twitter.com/tiraniddo/status/817532039771525120)，建议使用[@tiraniddo](https://twitter.com/tiraniddo)的 OleView .NET 过滤没有显式 Launch Permission 的对象。
