...
Zwei spezifische Objekte, `ShellBrowserWindow` und `ShellWindows`, wurden aufgrund ihres Fehlens expliziter Startberechtigungen hervorgehoben. Das Fehlen eines `LaunchPermission`-Registrierungseintrags unter `HKCR:\AppID\{guid}` deutet auf fehlende explizite Berechtigungen hin.
