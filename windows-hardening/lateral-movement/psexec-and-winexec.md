...
1. **Copie d'un binaire de service vers le partage ADMIN$ via SMB** est effectuée.
2. **Création d'un service sur la machine distante** est faite en pointant vers le binaire.
...
En supposant qu'il y a une charge utile exécutable (créée avec msfvenom et obfusquée à l'aide de Veil pour éviter la détection par les antivirus), nommée 'met8888.exe', représentant une charge utile meterpreter reverse\_http, les étapes suivantes sont prises :

* **Copie du binaire** : L'exécutable est copié vers le partage ADMIN$ à partir d'une invite de commande, bien qu'il puisse être placé n'importe où sur le système de fichiers pour rester caché.
* **Création d'un service** : En utilisant la commande Windows `sc`, qui permet d'interroger, de créer et de supprimer des services Windows à distance, un service nommé "meterpreter" est créé pour pointer vers le binaire téléchargé.
