...
기본적으로 **Kerberos** 인증 프로토콜이 주요 방법으로 사용됩니다. NTLM (NT LAN Manager)은 특정 상황에서 사용됩니다: Active Directory의 부재, 도메인의 미존재, 잘못된 구성으로 인한 Kerberos의 기능 장애, 또는 유효한 호스트 이름 대신 IP 주소를 사용하여 연결을 시도할 때.
...
* Kerberos가 기본 인증 방법이며, NTLM은 특정 조건에서만 사용됩니다.
...
3. **서버**가 **도전 과제**를 보냅니다.
4. 클라이언트가 **도전 과제**를 **비밀번호의 해시를 키로 사용하여 암호화**하고 응답으로 보냅니다.
5. **서버**가 **도메인 컨트롤러**에 **도메인 이름, 사용자 이름, 도전 과제 및 응답**을 보냅니다. Active Directory가 구성되어 있지 않거나 도메인 이름이 서버 이름인 경우 자격 증명은 **로컬로 확인**됩니다.
6. **도메인 컨트롤러가 모든 것이 올바른지 확인**하고 정보를 서버로 보냅니다.

**서버**와 **도메인 컨트롤러**는 **NTDS.DIT** 데이터베이스 내에 서버의 암호를 알고 있기 때문에 **Netlogon** 서버를 통해 **안전한 채널**을 생성할 수 있습니다.

### 로컬 NTLM 인증 체계

인증은 **이전에 언급된 것처럼** 진행되지만 **서버**는 **SAM** 파일 내에서 인증을 시도하는 사용자의 **해시를 알고** 있습니다. 따라서 도메인 컨트롤러에 요청하는 대신 **서버가 직접 확인**하여 사용자가 인증할 수 있는지 확인합니다.

### NTLMv1 도전

**도전의 길이는 8바이트**이며 **응답은 24바이트**입니다.

**해시 NT(16바이트)**는 **각각 7바이트로 구성된 3부분**으로 나뉩니다 (7B + 7B + (2B+0x00\*5)): **마지막 부분은 0으로 채워집니다**. 그런 다음 **도전**은 각 부분별로 **별도로 암호화**되고 결과 암호화된 바이트가 **결합**됩니다. 총: 8B + 8B + 8B = 24바이트.
...
* **DES가 해독 가능**
* 3번째 키는 항상 **5개의 0으로 구성**
* **같은 도전**이 주어지면 **응답**도 **동일**합니다. 따라서 피해자에게 문자열 "**1122334455667788**"을 **도전**으로 제공하고 **사전 계산된 무지개 테이블**을 사용하여 사용된 응답을 공격할 수 있습니다.

### NTLMv1 공격

현재는 Unconstrained Delegation이 구성된 환경을 찾기가 점점 더 어려워지고 있지만, 이는 구성된 **Print Spooler 서비스를 악용**할 수 없다는 의미는 아닙니다.

AD에서 이미 가지고 있는 일부 자격 증명/세션을 악용하여 **프린터에게** 일부 **호스트**에 대해 **인증을 요청**할 수 있습니다. 그런 다음 `metasploit auxiliary/server/capture/smb` 또는 `responder`를 사용하여 **인증 도전을 1122334455667788**로 설정하고 인증 시도를 캡처하면, **NTLMv1**을 사용하여 수행된 경우 **해독**할 수 있습니다.\
`responder`를 사용하는 경우 **인증을 다운그레이드**하기 위해 `--lm` 플래그를 시도할 수 있습니다.\
_이 기술을 위해 인증은 NTLMv1을 사용하여 수행되어야 합니다 (NTLMv2는 유효하지 않음)._

프린터는 인증 중에 컴퓨터 계정을 사용하며, 컴퓨터 계정은 **긴 무작위 암호**를 사용하므로 일반 **사전**을 사용하여 해독하기 어려울 수 있습니다. 그러나 **NTLMv1** 인증은 **DES를 사용**합니다([자세한 정보는 여기를 참조](./#ntlmv1-challenge)), 따라서 DES를 해독하는 데 특히 전용 서비스를 사용하여 해독할 수 있습니다([https://crack.sh/](https://crack.sh) 또는 [https://ntlmv1.com/](https://ntlmv1.com)를 사용할 수 있습니다).

### hashcat을 사용한 NTLMv1 공격

NTLMv1은 NTLMv1 메시지를 해독할 수 있는 방식으로 형식을 지정하는 NTLMv1 Multi Tool [https://github.com/evilmog/ntlmv1-multi](https://github.com/evilmog/ntlmv1-multi)를 사용하여 해독할 수 있습니다.
...
NTLM relay attacks are a common technique used by attackers to exploit the NTLM authentication protocol. This attack involves relaying NTLM authentication messages from a victim machine to a target machine, allowing the attacker to impersonate the victim and potentially gain unauthorized access to the target system.
...
3. The target machine processes the authentication request, believing it to be from the victim machine.
4. If successful, the attacker can access resources on the target machine using the victim's credentials.
...
- **Use Kerberos**: Consider using Kerberos authentication as an alternative to NTLM to prevent relay attacks.
...
A52B9CDEDAE86934:1122334455667788
```
해시캣을 실행하십시오 (hashtopolis와 같은 도구를 통해 분산하는 것이 가장 좋습니다) 그렇지 않으면 몇 일이 걸릴 것입니다.
```bash
./hashcat -m 14000 -a 3 -1 charsets/DES_full.charset --hex-charset hashes.txt ?1?1?1?1?1?1?1?1
```
...
- Implement Extended Protection for Authentication to protect against relay attacks in Windows environments.
...
**첫 번째 응답**은 **클라이언트와 도메인**으로 구성된 **문자열을 사용하여 HMAC\_MD5**를 사용하여 생성되며 **NT 해시**의 **해시 MD4**를 **키**로 사용합니다. 그런 다음 **결과**는 **도전**을 암호화하는 데 사용됩니다. 여기에 **8바이트의 클라이언트 도전이 추가**됩니다. 총: 24 B.
...
**피해자의 해시를 획득한 후**, 해당 해시를 사용하여 **가장하는** 것이 가능합니다.\
**해시를 사용하여 NTLM 인증을 수행하는 도구**를 사용해야 하며, **또는** 새 **세션로그온**을 만들고 해당 **해시를 LSASS에 삽입**하여 **NTLM 인증이 수행될 때 해당 해시가 사용**되도록 할 수 있습니다. 마지막 옵션은 mimikatz가 하는 일입니다.

**Pass-the-Hash 공격은 컴퓨터 계정을 사용하여도 수행할 수 있다는 것을 기억해 주세요.**
...
```
이것은 mimikatz를 실행한 사용자에 속하는 프로세스를 시작합니다. 그러나 내부적으로 LSASS에서 저장된 자격 증명은 mimikatz 매개 변수 내에 있습니다. 그럼으로 네트워크 리소스에 액세스할 수 있습니다. 마치 해당 사용자인 것처럼(일반 텍스트 암호를 알 필요 없이 `runas /netonly` 트릭과 유사).

### 리눅스에서 Pass-the-Hash

리눅스를 통해 Pass-the-Hash를 사용하여 Windows 기기에서 코드 실행을 얻을 수 있습니다.\
...
Windows용 impacket 이진 파일을 [여기에서 다운로드할 수 있습니다.](https://github.com/ropnop/impacket_static_binaries/releases/tag/0.9.21-dev-binaries).
