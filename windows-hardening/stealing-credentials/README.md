...
Al analizar estos tres componentes, la DLL se involucra en la creación del archivo de volcado y transfiere la memoria del proceso especificado a este archivo.\
...
[Procdump](https://docs.microsoft.com/en-us/sysinternals/downloads/procdump) es un binario firmado por Microsoft que forma parte del conjunto [sysinternals](https://docs.microsoft.com/en-us/sysinternals/).
```
...
[**PPLBlade**](https://github.com/tastypepperoni/PPLBlade) es una herramienta de volcado de procesos protegidos que soporta la ofuscación de volcado de memoria y su transferencia a estaciones de trabajo remotas sin guardarlo en el disco.
...
3. Subir el volcado de memoria con métodos de carga RAW y SMB sin guardarlo en el disco (volcado sin archivo)
...
El archivo **NTDS.dit** es conocido como el corazón de **Active Directory**, ya que contiene datos cruciales sobre objetos de usuario, grupos y sus membresías. Es donde se almacenan los **hashes de contraseñas** de los usuarios del dominio. Este archivo es una base de datos **Extensible Storage Engine (ESE)** y reside en **_%SystemRoom%/NTDS/ntds.dit_**.
...
- **Link Table**: Lleva un registro de las relaciones, como las membresías de grupos.
- **SD Table**: Aquí se mantienen los **descriptores de seguridad** para cada objeto, asegurando la seguridad y el control de acceso para los objetos almacenados.
...
Windows usa _Ntdsa.dll_ para interactuar con ese archivo y es utilizado por _lsass.exe_. Entonces, **parte** del archivo **NTDS.dit** podría estar ubicado **dentro de la memoria de `lsass`** (puedes encontrar los datos accedidos más recientemente probablemente debido a la mejora de rendimiento al usar una **caché**).
...
```
También podrías usar el truco de [**volume shadow copy**](./#stealing-sam-and-system) para copiar el archivo **ntds.dit**. Recuerda que también necesitarás una copia del **SYSTEM file** (nuevamente, [**extráelo del registro o usa el truco de volume shadow copy**](./#stealing-sam-and-system)).
...
```
Para **archivos NTDS.dit grandes** se recomienda extraerlo usando [gosecretsdump](https://github.com/c-sto/gosecretsdump).

Finalmente, también puedes usar el **módulo de metasploit**: _post/windows/gather/credentials/domain\_hashdump_ o **mimikatz** `lsadump::lsa /inject`

### **Extrayendo objetos de dominio de NTDS.dit a una base de datos SQLite**

Los objetos NTDS pueden ser extraídos a una base de datos SQLite con [ntdsdotsqlite](https://github.com/almandin/ntdsdotsqlite). No solo se extraen secretos, sino también los objetos completos y sus atributos para una mayor extracción de información cuando el archivo NTDS.dit en bruto ya ha sido recuperado.
```
ntdsdotsqlite ntds.dit -o ntds.sqlite --system SYSTEM.hive
```
El `SYSTEM` hive es opcional pero permite la descryptación de secretos (hashes NT & LM, credenciales suplementarias como contraseñas en texto claro, claves kerberos o de confianza, historiales de contraseñas NT & LM). Junto con otra información, se extraen los siguientes datos: cuentas de usuario y máquina con sus hashes, banderas UAC, marca de tiempo del último inicio de sesión y cambio de contraseña, descripción de cuentas, nombres, UPN, SPN, grupos y membresías recursivas, árbol de unidades organizativas y membresía, dominios de confianza con tipo de confianza, dirección y atributos...
...
Esta herramienta se puede usar para extraer credenciales de la memoria. Descárgala desde: [http://www.ampliasecurity.com/research/windows-credentials-editor/](https://www.ampliasecurity.com/research/windows-credentials-editor/)
...
Descárgalo de: [http://www.tarasco.org/security/pwdump\_7](http://www.tarasco.org/security/pwdump\_7) y simplemente **ejecútalo** y las contraseñas serán extraídas.
