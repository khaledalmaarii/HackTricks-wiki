...
* **사이버 보안 회사**에서 일하시나요? **회사가 HackTricks에 광고**되길 원하시나요? 또는 **PEASS의 최신 버전에 액세스하거나 HackTricks를 PDF로 다운로드**하고 싶으신가요? [**구독 요금제**](https://github.com/sponsors/carlospolop)를 확인해보세요!
...
### **Windows 지역 권한 상승 벡터를 찾는 데 가장 좋은 도구:** [**WinPEAS**](https://github.com/carlospolop/privilege-escalation-awesome-scripts-suite/tree/master/winPEAS)
...
Windows에는 **시스템 열거를 방지하거나 실행 파일을 실행하지 못하게 하거나 심지어 활동을 감지하는** 다양한 요소가 있습니다. 권한 상승 열거를 시작하기 전에 다음 **페이지**를 **읽고** 이러한 **방어 메커니즘**을 **모두 열거**해야 합니다:
...
이 [사이트](https://msrc.microsoft.com/update-guide/vulnerability)는 Microsoft 보안 취약점에 대한 자세한 정보를 검색하는 데 유용합니다. 이 데이터베이스에는 4,700개 이상의 보안 취약점이 있어 Windows 환경이 제공하는 **대규모 공격 표면**을 보여줍니다.
...
이를 켜는 방법은 [https://sid-500.com/2017/11/07/powershell-enabling-transcription-logging-by-using-group-policy/](https://sid-500.com/2017/11/07/powershell-enabling-transcription-logging-by-using-group-policy/)에서 배울 수 있습니다.
```bash
...
이를 활성화하려면 설명서의 "Transcript files" 섹션의 지침을 따르고 **"Powershell Transcription"** 대신 **"모듈 로깅"**을 선택하십시오.
```bash
...
```
마지막 15개의 이벤트를 PowersShell 로그에서 보려면 다음을 실행하십시오:
```bash
Get-WinEvent -LogName "windows Powershell" | select -First 15 | Out-GridView
```
### PowerShell **스크립트 블록 로깅**

스크립트의 실행에 대한 완전한 활동 및 전체 내용 기록이 캡처되어 각 코드 블록이 실행될 때마다 문서화됩니다. 이 프로세스는 각 활동에 대한 포괄적인 감사 트레일을 보존하여 포렌식 및 악의적인 행위 분석에 유용합니다. 실행 시 모든 활동을 문서화함으로써 프로세스에 대한 자세한 통찰력을 제공합니다.
```bash
...
```
스크립트 블록에 대한 로깅 이벤트는 Windows 이벤트 뷰어에서 다음 경로에 위치할 수 있습니다: **Application and Services Logs > Microsoft > Windows > PowerShell > Operational**.\
...
> 만약 로컬 사용자 프록시를 수정할 수 있는 권한이 있다면, 그리고 Windows 업데이트가 인터넷 익스플로러 설정에 구성된 프록시를 사용한다면, 우리는 [PyWSUS](https://github.com/GoSecure/pywsus)를 로컬로 실행하여 자체 트래픽을 가로채고 자산에서 상위 사용자로 코드를 실행할 수 있는 권한을 가지게 됩니다.
>
> 게다가, WSUS 서비스는 현재 사용자의 설정을 사용하므로, 현재 사용자의 인증서 저장소도 사용합니다. WSUS 호스트 이름을 위한 자체 서명 인증서를 생성하고 이 인증서를 현재 사용자의 인증서 저장소에 추가하면 HTTP 및 HTTPS WSUS 트래픽을 모두 가로챌 수 있습니다. WSUS는 인증서에 대한 신뢰성 검증을 구현하기 위해 HSTS와 같은 메커니즘을 사용하지 않습니다. 사용자가 신뢰하고 올바른 호스트 이름을 가진 인증서를 제시하면 서비스에서 인증됩니다.

이 취약점을 악용하기 위해 [**WSUSpicious**](https://github.com/GoSecure/wsuspicious) 도구를 사용할 수 있습니다 (해제되면).

## KrbRelayUp

특정 조건에서 Windows **도메인** 환경에서 **로컬 권한 상승** 취약점이 존재합니다. 이러한 조건에는 **LDAP 서명이 강제되지 않는** 환경, 사용자가 **Resource-Based Constrained Delegation (RBCD)를 구성할 수 있는** 자체 권한을 가지고 있으며 도메인 내에서 컴퓨터를 생성할 수 있는 능력이 포함됩니다. 이러한 **요구 사항**은 **기본 설정**을 사용하여 충족되는 것이 중요합니다.

[**https://github.com/Dec0ne/KrbRelayUp**](https://github.com/Dec0ne/KrbRelayUp)에서 **악용**을 찾을 수 있습니다.
...
이 도구를 사용하여 MSI 래퍼를 만드는 방법을 배우려면이 자습서를 읽으십시오. **명령줄을 실행**하려면 "**.bat**" 파일을 랩핑할 수 있다는 점에 유의하십시오.
...
* **AlwaysPrivesc**와 같은 이름의 프로젝트를 지정하고, 위치로 **`C:\privesc`**를 사용하고, **솔루션 및 프로젝트를 동일한 디렉토리에 배치**를 선택하고 **만들기**를 클릭합니다.
...
* 설치된 앱을 더 신뢰할 수 있게 만들 수 있는 **Author** 및 **Manufacturer**과 같은 다른 속성을 변경할 수 있습니다.
...
악의적인 `.msi` 파일을 **백그라운드에서** 실행하려면:
```
msiexec /quiet /qn /i C:\Users\Steve.INFERNO\Downloads\alwe.msi
```
## 취약점을 악용하려면 다음을 사용할 수 있습니다: _exploit/windows/local/always\_install\_elevated_
...
**Windows 8.1**부터 Microsoft는 로컬 보안 권한(LSA)에 대한 강화된 보호 기능을 도입하여 신뢰되지 않는 프로세스가 해당 메모리를 읽거나 코드를 삽입하는 시도를 차단하여 시스템을 더욱 안전하게 보호합니다.\
...
[**캐시된 자격 증명에 대한 자세한 정보는 여기를 참조하십시오**](../stealing-credentials/credentials-protections.md#cached-credentials).
```bash
...
만약 **특권 그룹에 속해 있다면 권한 상승을 할 수 있을 수 있습니다**. 특권 그룹에 대해 알아보고 권한 상승을 위해 그들을 어떻게 남용할 수 있는지 여기에서 배워보세요:
...
**더 알아보세요** 토큰이 무엇인지에 대해 이 페이지에서: [**Windows Tokens**](../authentication-credentials-uac-and-efs/#access-tokens).\
다음 페이지를 확인하여 **흥미로운 토큰에 대해 배우고** 그들을 어떻게 남용할 수 있는지 알아보세요:
...
먼저, 프로세스를 나열하여 **프로세스의 명령줄 내에 비밀번호가 있는지 확인**합니다.\
...
```
항상 실행 중인 [**electron/cef/chromium 디버거**를 확인하여 권한 상승에 악용할 수 있습니다](../../linux-hardening/privilege-escalation/electron-cef-chromium-debugger-abuse.md).
...
```
**프로세스 이진 파일의 폴더 권한 확인 (**[**DLL Hijacking**](dll-hijacking/)**)**
```bash
...
예: "Windows 도움말 및 지원" (Windows + F1), "명령 프롬프트 열기"를 클릭하여 "명령 프롬프트 열기"
...
sc qc <service_name>
```
이 각 서비스에 필요한 권한 수준을 확인하기 위해 _Sysinternals_의 **accesschk** 이진 파일을 추천합니다.
```bash
accesschk.exe -ucqv <Service_Name> #Check rights for different groups
```
다음을 확인하는 것이 좋습니다: "인증된 사용자"가 어떤 서비스든 수정할 수 있는지:
```bash
...
_서비스를 시작할 수 없습니다. 이는 비활성화되었거나 연결된 장치가 없기 때문일 수 있습니다._
...
**이 문제의 또 다른 해결책**은 다음을 실행하는 것입니다:
```
...
**서비스에서 실행되는 이진 파일을 수정할 수 있는지 확인**하거나 이진 파일이 위치한 폴더에 **쓰기 권한**이 있는지 확인하세요 ([**DLL Hijacking**](dll-hijacking/))**.**\
**wmic** (system32에 없음)을 사용하여 서비스에서 실행되는 모든 이진 파일을 가져올 수 있으며 **icacls**를 사용하여 권한을 확인할 수 있습니다:
```bash
...
레지스트리에 대한 이 권한을 가지고 있다면 **이것은 **이 레지스트리에서 하위 레지스트리를 생성할 수 있음을 의미합니다**. Windows 서비스의 경우 **임의의 코드를 실행하는 데 충분합니다:**
...
**바이너리의 권한** (하나를 덮어쓸 수 있고 권한을 상승할 수도 있음) 및 **폴더**의 권한을 확인하세요 ([DLL Hijacking](dll-hijacking/)).
```bash
...
특정한 파일을 읽을 수 있도록 구성 파일을 수정하거나 관리자 계정 (schedtasks)에 의해 실행될 이진 파일을 수정할 수 있는지 확인합니다.
...
만약 PATH에 있는 폴더 중 하나에 **쓰기 권한**이 있다면, 프로세스에서 로드되는 DLL을 **해킹**하여 **권한 상승**이 가능할 수 있습니다.

PATH 내 모든 폴더의 권한을 확인하세요:
```bash
for %%A in ("%path:;=";"%") do ( cmd.exe /c icacls "%%~A" 2>nul | findstr /i "(F) (M) (W) :\" | findstr /i ":\\ everyone authenticated users todos %username%" && echo. )
```
더 많은 정보를 얻으려면 이 체크를 악용하는 방법에 대해 확인하십시오:
...
루트 사용자를 얻으면 어떤 포트에서든 들을 수 있습니다 (`nc.exe`를 사용하여 처음으로 포트에서 듣기를 시도하면 방화벽에서 `nc`를 허용할지 GUI를 통해 물어봅니다).
```bash
wsl whoami
...
Windows Vault는 **Windows**가 **사용자를 자동으로 로그인**할 수 있는 서버, 웹사이트 및 다른 프로그램의 사용자 자격 증명을 저장합니다. 처음에는 사용자가 Facebook 자격 증명, Twitter 자격 증명, Gmail 자격 증명 등을 브라우저를 통해 자동으로 로그인할 수 있도록 저장할 수 있다고 생각할 수 있지만, 실제로는 그렇지 않습니다.

Windows Vault는 Windows가 사용자를 자동으로 로그인할 수 있는 자격 증명을 저장하며, 이는 **자격 증명이 필요한 Windows 애플리케이션**이 사용자가 계속해서 사용자 이름과 암호를 입력하는 대신 제공된 자격 증명을 사용할 수 있도록 **자격 증명 관리자** 및 Windows Vault를 활용할 수 있다는 것을 의미합니다.

애플리케이션이 자격 증명 관리자와 상호 작용하지 않는 한, 해당 리소스의 자격 증명을 사용할 수 없을 것으로 생각됩니다. 따라서 애플리케이션이 보관함을 활용하려면 어떻게든 **자격 증명 관리자와 통신하고 해당 리소스의 자격 증명을 기본 저장 보관함에서 요청해야** 합니다.
...
**데이터 보호 API (DPAPI)**는 주로 Windows 운영 체제 내에서 대칭 키를 사용하여 데이터를 암호화하는 방법을 제공합니다. 이 암호화는 사용자 또는 시스템 비밀을 엔트로피에 상당히 기여하는 대칭 키를 활용합니다.

**DPAPI는 사용자의 로그인 비밀로부터 파생된 대칭 키를 통해 키를 암호화하는 기능을 제공합니다**. 시스템 암호화가 관련된 시나리오에서는 시스템의 도메인 인증 비밀을 활용합니다.

DPAPI를 사용하여 암호화된 사용자 RSA 키는 `%APPDATA%\Microsoft\Protect\{SID}` 디렉토리에 저장됩니다. 여기서 `{SID}`는 사용자의 [보안 식별자](https://en.wikipedia.org/wiki/Security\_Identifier)를 나타냅니다. **DPAPI 키는 사용자의 개인 키를 보호하는 마스터 키와 함께 동일한 파일에 위치하며, 일반적으로 64바이트의 무작위 데이터로 구성**됩니다. (이 디렉토리에 대한 액세스는 제한되어 있어 CMD의 `dir` 명령을 통해 내용을 나열하는 것을 방지하지만 PowerShell을 통해 나열할 수 있습니다).
```powershell
...
설치 프로그램은 **SYSTEM 권한으로 실행**되며, 많은 프로그램이 **DLL Sideloading 취약점**에 취약합니다 ([**https://github.com/enjoiz/Privesc**](https://github.com/enjoiz/Privesc)에서 정보 확인).
```bash
...
```
당신은 **metasploit**을 사용하여 이러한 파일을 검색할 수도 있습니다: _post/windows/gather/enum\_unattend_
...
이전에 사용 가능했던 기능으로, 그룹 정책 환경 설정 (GPP)을 통해 여러 대의 컴퓨터에 사용자 정의 로컬 관리자 계정을 배포할 수 있었습니다. 그러나 이 방법에는 중대한 보안 결함이 있었습니다. 첫째, SYSVOL에 XML 파일로 저장된 그룹 정책 개체(GPO)에는 모든 도메인 사용자가 액세스할 수 있었습니다. 둘째, 이러한 GPP 내의 암호는 공개 문서화된 기본 키를 사용하여 AES256로 암호화되었으며, 이를 인증된 사용자가 해독할 수 있었습니다. 이는 사용자가 권한을 상승시킬 수 있게 해 심각한 위험을 초래할 수 있었습니다.

이 위험을 완화하기 위해, "cpassword" 필드가 비어 있지 않은 로컬로 캐시된 GPP 파일을 스캔하는 기능이 개발되었습니다. 이러한 파일을 찾으면, 해당 기능은 암호를 해독하고 사용자 정의 PowerShell 객체를 반환합니다. 이 객체에는 GPP 및 파일 위치에 대한 세부 정보가 포함되어 있어 이 보안 취약점의 식별 및 해결에 도움이 됩니다.
...
항상 사용자에게 자신의 자격 증명 또는 다른 사용자의 자격 증명을 입력하도록 요청할 수 있습니다 (클라이언트에게 직접 자격 증명을 요청하는 것은 정말 위험하다는 것을 주의하십시오):
```bash
...
알려진 파일들은 이전에 **암호**를 **평문**이나 **Base64**로 포함하고 있었습니다.
```bash
...
```
### RecycleBin(휴지통) 안의 자격 증명

당신은 또한 내부에 있는 자격 증명을 확인하기 위해 휴지통을 확인해야 합니다.

여러 프로그램에 의해 저장된 **암호를 복구**하려면 다음을 사용할 수 있습니다: [http://www.nirsoft.net/password\_recovery\_tools.html](http://www.nirsoft.net/password\_recovery\_tools.html)
...
또한 브라우저의 히스토리, 즐겨찾기 및 즐겨찾기를 확인하여 **비밀번호가** 저장되어 있을 수 있습니다.
...
이 레지스트리의 CLSIDs 내부에는 **InProcServer32**라는 하위 레지스트리가 있으며, 이는 **DLL**을 가리키는 **기본 값**과 **Apartment** (단일 스레드), **Free** (다중 스레드), **Both** (단일 또는 다중) 또는 **Neutral** (스레드 중립)일 수 있는 **ThreadingModel**이라는 값을 포함합니다.

![](<../../.gitbook/assets/image (729).png>)

기본적으로, 실행될 DLL 중 하나를 **덮어쓸 수 있다면**, 다른 사용자가 실행할 DLL이라면 **권한 상승**이 가능합니다.

공격자가 COM Hijacking을 지속성 메커니즘으로 사용하는 방법을 알아보려면:
...
**SYSTEM으로 실행되는 프로세스가 전체 액세스 권한으로 새 프로세스를 열고** (`OpenProcess()`) **같은 프로세스가 낮은 권한으로 새 프로세스를 생성** (`CreateProcess()`) **하지만 주 프로세스의 모든 열린 핸들을 상속**한다고 상상해보십시오.\
그런 다음, **낮은 권한을 가진 프로세스에 완전한 액세스**가 있다면, `OpenProcess()`로 생성된 **특권 프로세스에 대한 열린 핸들을 잡아서 쉘코드를 삽입**할 수 있습니다.\
[**이 취약점을 감지하고 악용하는 방법에 대한 자세한 내용은 여기를 참조하십시오.**](leaked-handle-exploitation.md)\
[**다른 게시물에서는 다른 수준의 권한(전체 액세스뿐만 아니라)으로 상속된 프로세스 및 스레드의 더 많은 열린 핸들러를 테스트하고 악용하는 방법에 대해 더 자세히 설명하고 있습니다.**](http://dronesec.pw/blog/2019/08/22/exploiting-leaked-process-and-thread-handles/).

## Named Pipe 클라이언트 임퍼슨테이션

**파이프**로 알려진 **공유 메모리 세그먼트**는 프로세스 간 통신과 데이터 전송을 가능하게 합니다.
...
**클라이언트**가 파이프를 통해 데이터를 보낼 때, 파이프를 설정한 **서버**는 **SeImpersonate** 권한이 필요하다면 **클라이언트**의 **아이덴티티를 취할 수 있습니다**. 설정한 파이프를 통해 상호작용하는 특권 프로세스를 모방할 수 있는 기회를 제공하며, 이를 통해 해당 프로세스의 아이덴티티를 취함으로써 **더 높은 권한을 얻을 수 있습니다**. 이러한 공격을 실행하는 방법에 대한 지침은 [**여기**](named-pipe-client-impersonation.md) 및 [**여기**](./#from-high-integrity-to-system)에서 찾을 수 있습니다.

또한 다음 도구를 사용하여 **버프와 같은 도구로 Named Pipe 통신을 가로챌 수 있습니다:** [**https://github.com/gabriel-sztejnworcel/pipe-intercept**](https://github.com/gabriel-sztejnworcel/pipe-intercept) **그리고 이 도구를 사용하여 모든 파이프를 나열하고 특권 상승을 찾을 수 있습니다** [**https://github.com/cyberark/PipeViewer**](https://github.com/cyberark/PipeViewer)

## 기타

### **비밀번호를 위한 명령줄 모니터링**

사용자로서 쉘을 획들했을 때, 예약된 작업이나 다른 프로세스가 실행되어 **명령줄에 자격 증명을 전달**할 수 있습니다. 아래 스크립트는 프로세스 명령줄을 매 2초마다 캡처하고 현재 상태와 이전 상태를 비교하여 차이가 있는 경우 출력합니다.
```powershell
while($true)
{
...
이를 통해 권한 상승과 UAC 우회를 동시에 할 수 있으며, 프로세스 중에 사용된 이진 파일은 Microsoft에 의해 서명되고 발급됩니다.
...
## 관리자 Medium에서 High Integrity Level / UAC Bypass로
...
그런 다음 **UAC 및 UAC 바이패스에 대해 알아보려면** 이것을 읽으세요:
...
고정 프로세스에서 **AlwaysInstallElevated 레지스트리 항목을 활성화**하고 **.msi 래퍼**를 사용하여 **역술 쉘을 설치**해 볼 수 있습니다.\
[레지스트리 키 및 .msi 패키지 설치에 관한 자세한 정보는 여기를 참조하십시오.](./#alwaysinstallelevated)
...
이러한 토큰 권한을 가지고 있다면 (아마도 이미 고정 프로세스에서 찾을 수 있을 것입니다), SeDebug 권한으로 (보호되지 않은 프로세스가 아닌) 거의 모든 프로세스를 열고, 프로세스의 토큰을 복사하고 해당 토큰으로 임의의 프로세스를 생성할 수 있습니다.\
...
이 기술은 meterpreter가 `getsystem`에서 승격하는 데 사용됩니다. 이 기술은 **파이프를 생성한 다음 서비스를 사용하여 해당 파이프에 쓰기 위해 서비스를 생성/남용하는 것**으로 구성됩니다. 그런 다음 **SeImpersonate** 권한을 사용하여 파이프 클라이언트(서비스)의 토큰을 **표시**할 수 있는 **서버**가 SYSTEM 권한을 획들할 수 있습니다.\
...
**SYSTEM**으로 실행 중인 **프로세스**에서 **로드되는 dll을 탈취**하면 해당 권한으로 임의의 코드를 실행할 수 있습니다. 따라서 Dll Hijacking은 이러한 종류의 권한 상승에도 유용하며, 더욱이 고정 프로세스에서는 **dll을 로드하는 데 사용되는 폴더에 쓰기 권한**이 있기 때문에 이를 달성하기가 **훨씬 더 쉽습니다**.\
...
[**Watson**](https://github.com/rasta-mouse/Watson) -- 알려진 권한 상승 취약점 검색 (VisualStudio를 사용하여 컴파일해야 함) ([**사전 컴파일된**](https://github.com/carlospolop/winPE/tree/master/binaries/watson))\
[**SeatBelt**](https://github.com/GhostPack/Seatbelt) -- 구성 오류를 검색하기 위해 호스트를 열거하는 도구 (권한 상승보다는 정보 수집 도구) (컴파일 필요) **(**[**사전 컴파일된**](https://github.com/carlospolop/winPE/tree/master/binaries/seatbelt)**)**\
[**LaZagne**](https://github.com/AlessandroZ/LaZagne) **-- 다양한 소프트웨어에서 자격 증명을 추출합니다 (github에 사전 컴파일된 exe)**\
[**SharpUP**](https://github.com/GhostPack/SharpUp) **-- C#로 PowerUp의 포트**\
[~~**Beroot**~~](https://github.com/AlessandroZ/BeRoot) **\~\~**\~\~ -- 구성 오류 확인 (github에서 사전 컴파일된 실행 파일). 권장하지 않습니다. Win10에서 잘 작동하지 않습니다.\
...
[**Windows-Exploit-Suggester**](https://github.com/GDSSecurity/Windows-Exploit-Suggester) -- **systeminfo**의 출력을 읽고 작동하는 exploits을 추천합니다 (로컬 파이썬)\
[**Windows Exploit Suggester Next Generation**](https://github.com/bitsadmin/wesng) -- **systeminfo**의 출력을 읽고 작동하는 exploits을 추천합니다 (로컬 파이썬)
...
* **사이버 보안 회사**에서 일하시나요? **HackTricks에 귀사를 광고하고 싶으신가요**? 아니면 **최신 버전의 PEASS를 보거나 HackTricks를 PDF로 다운로드**하고 싶으신가요? [**구독 요금제**](https://github.com/sponsors/carlospolop)를 확인하세요!
