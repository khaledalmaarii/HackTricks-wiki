...
* **हैकिंग ट्रिक्स साझा करें और** [**हैकट्रिक्स रेपो**](https://github.com/carlospolop/hacktricks) **और** [**हैकट्रिक्स-क्लाउड रेपो**](https://github.com/carlospolop/hacktricks-cloud) **को** PRs **सबमिट करके**।
...
Windows में विभिन्न चीजें हैं जो **आपको सिस्टम की जाँच करने से रोक सकती हैं**, कार्यक्रम चला सकती हैं या आपकी गतिविधियों को भी **पता लगा सकती हैं**। आपको **निम्नलिखित पृष्ठ** पढ़ना चाहिए और इन सभी **सुरक्षा प्रणालियों** की **जाँच** करनी चाहिए **प्राधिकरण उन्नयन** शुरू करने से पहले:
...
इसे सक्षम करने के लिए, "ट्रांस्क्रिप्ट फ़ाइल" अनुभाग में दिए गए निर्देशों का पालन करें, **"पावरशेल ट्रांस्क्रिप्शन"** की बजाय **"मॉड्यूल लॉगिंग"** का चयन करें।
```bash
...
स्क्रिप्ट के निष्पादन की पूरी गतिविधि और सामग्री रिकॉर्ड को दर्ज किया जाता है, इस से यह सुनिश्चित होता है कि प्रत्येक कोड ब्लॉक को जब वह चलता है, उसकी पूरी जानकारी दर्ज की जाती है। यह प्रक्रिया प्रत्येक गतिविधि का व्यापक अडिट ट्रेल बनाए रखती है, जो फोरेंसिक्स और दुराचारी व्यवहार का विश्लेषण के लिए मूल्यवान होता है। निष्पादन के समय सभी गतिविधि को दर्ज करके, प्रक्रिया के बारे में विस्तृत अंदरूनी दृष्टिकोण प्रदान किया जाता है।
```bash
...
```
विंडोज इवेंट व्यूअर में स्क्रिप्ट ब्लॉक के लिए लॉगिंग घटनाएं इस पथ पर उपलब्ध हो सकती हैं: **एप्लिकेशन और सेवाएं लॉग > माइक्रोसॉफ्ट > विंडोज > पावरशेल > ऑपरेशनल**।\
...
मुख्य रूप से, यह गड़बड़ी उस दोष का शिकार है जिसे यह बग एक्सप्लॉइट करता है:
...
>
> इसके अतिरिक्त, क्योंकि WSUS सेवा वर्तमान उपयोगकर्ता की सेटिंग्स का उपयोग करती है, इसका उपयोग भी इसके सर्टिफिकेट स्टोर का उपयोग करेगा। यदि हम WSUS होस्टनाम के लिए एक स्व-साइन किया गया सर्टिफिकेट उत्पन्न करते हैं और इस सर्टिफिकेट को वर्तमान उपयोगकर्ता के सर्टिफिकेट स्टोर में जोड़ते हैं, तो हम HTTP और HTTPS WSUS ट्रैफिक दोनों को अंतर्दृष्टि कर सकेंगे। WSUS एक भीएचटीएस-जैसी तंत्रिकाएँ नहीं उपयोग करता है जो पहली उपयोग पर सर्टिफिकेट पर भरोसा करने वाले प्रकार की मान्यता लागू करने के लिए हो। यदि उपयोगकर्ता द्वारा विश्वसनीय माना गया सर्टिफिकेट प्रस्तुत किया गया है और सही होस्टनाम है, तो सेवा द्वारा स्वीकार किया जाएगा।

आप इस दोष को उपयोग करके उपकरण [**WSUSpicious**](https://github.com/GoSecure/wsuspicious) का उपयोग कर सकते हैं (जब यह मुक्त होगा)।

## KrbRelayUp

विशेष परिस्थितियों के तहत Windows **डोमेन** वातावरण में एक **स्थानीय वर्चस्व उन्नति** दोष मौजूद है। इन परिस्थितियों में शामिल हैं: **LDAP साइनिंग नहीं लागू है,** उपयोगकर्ताओं के पास स्व-अधिकार हैं जो उन्हें **संसाधन-आधारित सीमित अधिकार देने की** अनुमति है, और उपयोगकर्ताओं को डोमेन के भीतर कंप्यूटर बनाने की क्षमता है। यह महत्वपूर्ण है कि ये **आवश्यकताएं** **डिफ़ॉल्ट सेटिंग्स** का उपयोग करके पूरी होती हैं।

दोष को खोजें [**https://github.com/Dec0ne/KrbRelayUp**](https://github.com/Dec0ne/KrbRelayUp)

हमले की विवरण के बारे में अधिक जानकारी के लिए देखें [https://research.nccgroup.com/2019/08/20/kerberos-resource-based-constrained-delegation-when-an-image-change-leads-to-a-privilege-escalation/](https://research.nccgroup.com/2019/08/20/kerberos-resource-based-constrained-delegation-when-an-image-change-leads-to-a-privilege-escalation/)

## AlwaysInstallElevated

यदि ये 2 रजिस्टर **सक्षम** हैं (मान **0x1** है), तो किसी भी विशेषाधिकार वाले उपयोगकर्ता NT AUTHORITY\\**SYSTEM** के रूप में `*.msi` फ़ाइलें स्थापित (चलाई) कर सकते हैं।
```bash
...
इस टूल का उपयोग करके एमएसआई रैपर बनाने के लिए इस ट्यूटोरियल को पढ़ें। ध्यान दें कि आप एक "**.bat**" फ़ाइल को रैप कर सकते हैं अगर आप **केवल** **कमांड लाइन्स** **को चलाना** चाहते हैं।

{% content-ref url="msi-wrapper.md" %}
[msi-wrapper.md](msi-wrapper.md)
{% endcontent-ref %}

### WIX के साथ एमएसआई बनाएं

{% content-ref url="create-msi-with-wix.md" %}
[create-msi-with-wix.md](create-msi-with-wix.md)
{% endcontent-ref %}

### Visual Studio के साथ एमएसआई बनाएं
...
* **समाधान एक्सप्लोरर** में **AlwaysPrivesc** परियोजना को हाइलाइट करें और **गुण** में से **TargetPlatform** को **x86** से **x64** में बदलें।
* आप इंस्टॉल किए गए ऐप को अधिक वैध दिखने के लिए **लेखक** और **निर्माता** जैसी अन्य गुण बदल सकते हैं।
* परियोजना पर दायां क्लिक करें और **दृश्य > कस्टम क्रियाएँ** चुनें।
* **स्थापित** पर दायां क्लिक करें और **कस्टम क्रिया जोड़ें** चुनें।
* **एप्लिकेशन फ़ोल्डर** पर डबल-क्लिक करें, अपनी **beacon.exe** फ़ाइल का चयन करें और **ठीक** पर क्लिक करें। इससे यह सुनिश्चित होगा कि बीकन पेलोड को तुरंत चलाया जाए जब इंस्टॉलर चलाया जाता है।
...
**LAPS** का निर्माण स्थानीय प्रशासक पासवर्डों का प्रबंधन के लिए किया गया है, यह सुनिश्चित करता है कि प्रत्येक पासवर्ड कंप्यूटर्स जोड़े गए डोमेन पर **अद्वितीय, यादृच्छिक और नियमित रूप से अपडेट** किया जाता है। ये पासवर्ड सुरक्षित रूप से Active Directory में संग्रहीत होते हैं और केवल उन उपयोगकर्ताओं द्वारा पहुंचा जा सकता है जिन्हें ACLs के माध्यम से पर्याप्त अनुमति प्रदान की गई है, जो उन्हें स्थानीय व्यवस्थापक पासवर्ड देखने की अनुमति देती है।
...
**Windows 8.1** से शुरू करके, Microsoft ने स्थानीय सुरक्षा प्राधिकरण (LSA) के लिए उन्नत सुरक्षा शुरू की है ताकि अविश्वसनीय प्रक्रियाओं द्वारा इसकी मेमोरी को पढ़ने या कोड इंजेक्ट करने की कोशिशें रोकी जा सकें, जो व्यवस्था को और अधिक सुरक्षित बनाता है।\
...
**क्रेडेंशियल गार्ड** को **Windows 10** में शामिल किया गया था। इसका उद्देश्य डिवाइस पर स्टोर की गई क्रेडेंशियल्स को पास-द-हैश हमलों जैसी खतरों से बचाना है।|
...
**डोमेन क्रेडेंशियल** को **स्थानीय सुरक्षा प्राधिकरण** (LSA) द्वारा प्रमाणित किया जाता है और ऑपरेटिंग सिस्टम घटकों द्वारा उपयोग किया जाता है। जब किसी उपयोगकर्ता के लॉगऑन डेटा को एक पंजीकृत सुरक्षा पैकेज द्वारा प्रमाणित किया जाता है, तो उपयोगकर्ता के लिए डोमेन क्रेडेंशियल सामान्यत: स्थापित होते हैं।\
...
आपको यह देखना चाहिए कि आप किसी भी समूह में शामिल हैं जिसमें दिलचस्प अनुमतियाँ हैं।
```bash
# CMD
...
इन्टरेस्टिंग टोकन्स के बारे में और उन्हें उन्नत करने के तरीके के बारे में जानने के लिए निम्नलिखित पृष्ठ की जाँच करें:
...
जांचें कि क्या आप **कुछ बाइनरी को ओवरराइट कर सकते हैं** या यदि आपके पास बाइनरी फ़ोल्डर की लेखन अनुमतियाँ हैं तो संभावित [**DLL हाइजैकिंग हमलों**](dll-hijacking.md) का शोषण करने के लिए।
```bash
...
```
सदैव संभावित [**electron/cef/chromium debuggers** चल रहे हैं, आप इसका दुरुपयोग करके वर्चस्व उन्नति कर सकते हैं](../../linux-hardening/privilege-escalation/electron-cef-chromium-debugger-abuse.md).
...
sc qc <service_name>
```
यह सुझाव दिया जाता है कि आप **_Sysinternals_** से बाइनरी **accesschk** को जांचने के लिए उपयोग करें ताकि प्रत्येक सेवा के लिए आवश्यक प्रिविलेज स्तर की जांच की जा सके।
```bash
accesschk.exe -ucqv <Service_Name> #Check rights for different groups
```
**हाल के उपयोगकर्ताओं** को किसी भी सेवा को संशोधित करने की अनुमति है, यह जांचना अनुशंसित है:
```bash
...
_सेवा शुरू नहीं की जा सकती, या तो इसे निष्क्रिय किया गया है या इसके साथ कोई सक्षम उपकरण जुड़े हुए नहीं हैं।_
...
- **SERVICE_CHANGE_CONFIG**: सेवा बाइनरी का पुनर्विन्यास संभव बनाता है।
...
```
आप यहाँ **sc** और **icacls** का भी उपयोग कर सकते हैं:
```bash
...
यदि किसी एक्जीक्यूटेबल का पथ उद्धरण के भीतर नहीं है, तो Windows हर स्थान को क्रियान्वित करने की कोशिश करेगा जो एक अंत में एक अंतरिक्ष से पहले है।
...
```
### सभी अन-उद्धृत सेवा पथों की सूची, जो कि विंडोज की अंदरूनी सेवाओं के नहीं हैं:
```bash
...
```
**आप इस वंरणशीलता को** मेटास्प्लॉइट के साथ पता लगा सकते हैं और इसका शोध कर सकते हैं: `exploit/windows/local/trusted\_service\_path`
...
Windows उपयोगकर्ताओं को सेवा विफल होने पर कार्रवाई निर्दिष्ट करने की अनुमति देता है। इस सुविधा को एक बाइनरी की ओर संकेतित किया जा सकता है। यदि यह बाइनरी प्रतिस्थापनीय है, तो विशेषाधिकार उन्नयन संभव हो सकता है। अधिक विवरण [आधिकारिक दस्तावेज़ीकरण](https://docs.microsoft.com/en-us/previous-versions/windows/it-pro/windows-server-2008-R2-and-2008/cc753662\(v=ws.11\)?redirectedfrom=MSDN) में उपलब्ध है।

## अनुप्रयोग

### स्थापित अनुप्रयोग

**बाइनरी की अनुमतियों** की जाँच करें (शायद आप एक को अधिक कर सकते हैं और विशेषाधिकारों को उन्नयन कर सकते हैं) और **फोल्डरों** की ([DLL हाइजैकिंग](dll-hijacking.md))।
```bash
...
**जांचें कि क्या आप किसी रजिस्ट्री या बाइनरी को अधिकार देने वाले किसी अलग उपयोगकर्ता द्वारा चलाए जाने वाले को अधिकार बढ़ाने के लिए ओवरराइट कर सकते हैं।**\
**निम्नलिखित पृष्ठ** को पढ़ें और अधिक जानें **उच्चाधिकार बढ़ाने के लिए दिलचस्प ऑटोरन्स स्थानों** के बारे में:
...
**संभावित** **तीसरे पक्ष के अजीब/वंशाजनक** **ड्राइवर्स** की खोज करें
```bash
driverquery
...
अगर आप रूट उपयोगकर्ता प्राप्त करते हैं तो आप किसी भी पोर्ट पर सुन सकते हैं (पहली बार जब आप `nc.exe` का उपयोग करके किसी पोर्ट पर सुनना चाहेंगे तो यह GUI के माध्यम से पूछेगा कि क्या फ़ायरवॉल द्वारा `nc` को अनुमति दी जानी चाहिए)।
```bash
wsl whoami
...
Windows Vault उन क्रेडेंशियल्स को संग्रहित करता है जिन्हें Windows उपयोगकर्ताओं को स्वतः लॉग इन करने में सक्षम है, जिसका मतलब है कि कोई भी **Windows एप्लिकेशन जो किसी संसाधन** (सर्वर या वेबसाइट) **तक पहुँचने के लिए क्रेडेंशियल्स की आवश्यकता होती है**, **इस Credential Manager** और Windows Vault का उपयोग कर सकता है और उपयोगकर्ताओं को प्रवेश नाम और पासवर्ड बार-बार दर्ज करने की आवश्यकता नहीं होती।

जब तक एप्लिकेशन Credential Manager के साथ संवाद न करें, मुझे लगता है कि उन्हें दिए गए संसाधन के लिए क्रेडेंशियल्स का उपयोग करना संभव नहीं है। इसलिए, यदि आपकी एप्लिकेशन वॉल्ट का उपयोग करना चाहती है, तो वह किसी प्रकार से **क्रेडेंशियल मैनेजर के साथ संवाद करना चाहिए और डिफ़ॉल्ट स्टोरेज वॉल्ट से उस संसाधन के लिए क्रेडेंशियल्स का अनुरोध करना चाहिए।**
...
```
तो आप `runas` का उपयोग `/savecred` विकल्प के साथ कर सकते हैं ताकि सहेजे गए क्रेडेंशियल का उपयोग किया जा सके। निम्नलिखित उदाहरण एक रिमोट बाइनरी को SMB शेयर के माध्यम से कॉल कर रहा है।
```bash
...
**डेटा संरक्षण API (DPAPI)** डेटा का सममित एन्क्रिप्शन के लिए एक विधि प्रदान करता है, जो अधिकांश रूप से एसिमेट्रिक प्राइवेट की तुलना में विंडोज ऑपरेटिंग सिस्टम के भीतर उपयोग किया जाता है। यह एन्क्रिप्शन एक उपयोगकर्ता या सिस्टम रहस्य का उपयोग करता है जो अंतर्विक्षता में महत्वपूर्ण योगदान देता है।

**DPAPI उपयोगकर्ता के लॉगिन रहस्य से निकाले गए एक सममित कुंजी के माध्यम से कुंजियों का एन्क्रिप्शन संभव बनाता है**। सिस्टम एन्क्रिप्शन से जुड़े स्थितियों में, यह सिस्टम के डोमेन प्रमाणीकरण रहस्यों का उपयोग करता है।

DPAPI का उपयोग करके एन्क्रिप्टेड उपयोगकर्ता RSA कुंजियाँ, `%APPDATA%\Microsoft\Protect\{SID}` निर्देशिका में संग्रहीत की जाती है, जहाँ `{SID}` उपयोगकर्ता को [सुरक्षा पहचानकर्ता](https://en.wikipedia.org/wiki/Security\_Identifier) का प्रतिनिधित्व करता है। **DPAPI कुंजी, जो उपयोगकर्ता की निजी कुंजियों की सुरक्षा करने वाली मास्टर कुंजी के साथ सहयोगी है, सामान फ़ाइल में संग्रहीत होती है**, सामान्यत: 64 बाइट के यादृच्छिक डेटा से बनी होती है। (इस निर्देशिका तक पहुंच की पहुंच प्रतिबंधित है, जिससे `CMD` में `dir` कमांड के माध्यम से इसकी सामग्री की सूचीकरण रोकी जाती है, हालांकि यह पावरशेल के माध्यम से सूचीबद्ध की जा सकती है)।
```powershell
...
आप उन्हें `HKEY_USERS\<SID>\Software\Microsoft\Terminal Server Client\Servers\` में और `HKCU\Software\Microsoft\Terminal Server Client\Servers\` में पा सकते हैं
...
**ध्यान दें कि AppCmd.exe से पासवर्ड पुनर्प्राप्त करने के लिए आपको प्रशासक होना चाहिए और उच्च अभियांत्रिकी स्तर के तहत चलाना चाहिए।**\
...
SSH निजी कुंजी को रजिस्ट्री कुंजी `HKCU\Software\OpenSSH\Agent\Keys` में संग्रहीत किया जा सकता है, इसलिए आपको यह जांचना चाहिए कि क्या वहां कुछ दिलचस्प है:
```bash
reg query 'HKEY_CURRENT_USER\Software\OpenSSH\Agent\Keys'
```
यदि आप उस पथ में कोई एंट्री पाते हैं तो शायद वहाँ एक सहेजी गई SSH कुंजी हो। यह एन्क्रिप्टेड रूप में संग्रहित है लेकिन [https://github.com/ropnop/windows_sshagent_extract](https://github.com/ropnop/windows_sshagent_extract) का उपयोग करके आसानी से डिक्रिप्ट किया जा सकता है।\
...
एक सुविधा पहले उपलब्ध थी जो ग्रुप नीति प्राथमिकताएँ (GPP) के माध्यम से कस्टम स्थानीय प्रशासक खातों का डिप्लॉयमेंट संभव बनाती थी। हालांकि, इस विधि में महत्वपूर्ण सुरक्षा दोष थे। पहले, ग्रुप नीति ऑब्जेक्ट्स (GPOs), SYSVOL में XML फ़ाइल्स के रूप में संग्रहीत, किसी भी डोमेन उपयोगकर्ता द्वारा एक्सेस किए जा सकते थे। दूसरा, इन GPPs में रखे गए पासवर्ड, AES256 का उपयोग करके एन्क्रिप्ट किए गए, जो एक सार्वजनिक दस्तावेज़ीकृत डिफ़ॉल्ट कुंजी का उपयोग करके डिक्रिप्ट किए जा सकते थे। यह एक गंभीर जोखिम पैदा करता था, क्योंकि यह उपयोगकर्ताओं को उच्च विशेषाधिकार प्राप्त करने की अनुमति दे सकता था।

इस जोखिम को कम करने के लिए, एक कार्य विकसित किया गया था जो स्थानीय रूप से कैश किए गए GPP फ़ाइलों की खोज करता है जिसमें "cpassword" फ़ील्ड खाली नहीं है। ऐसी फ़ाइल मिलने पर, फ़ंक्शन पासवर्ड को डिक्रिप्ट करता है और एक कस्टम PowerShell ऑब्जेक्ट वापस करता है। यह ऑब्ज
```bash
...
```
### **पासवर्ड वाली संभावित फ़ाइलों के नाम**

जिन फ़ाइलों में कभी-कभी **पासवर्ड** **साफ-टेक्स्ट** या **Base64** में थे, उनके नाम
```bash
...
आपको डीबी की जाँच करनी चाहिए जहाँ से **Chrome या Firefox** में स्टोर किए गए पासवर्ड हो सकते हैं।\
इसके अलावा, ब्राउज़रों के इतिहास, बुकमार्क और पसंदीदा जाँचें ताकि कुछ **पासवर्ड स्टोर** हो सकें।
...
### **COM DLL अधिकार उन्नति**

**Component Object Model (COM)** एक प्रौद्योगिकी है जो Windows ऑपरेटिंग सिस्टम के भीतर बनाई गई है जो विभिन्न भाषाओं के सॉफ़्टवेयर घटकों के बीच **अंतरसंचार** को संभावित बनाती है। प्रत्येक COM घटक को एक कक्षा आईडी (CLSID) द्वारा **पहचाना जाता है** और प्रत्येक घटक एक या एक से अधिक इंटरफेस के माध्यम से कार्यक्षमता प्रकट करता है, जिन्हें इंटरफेस आईडी (IIDs) द्वारा पहचाना जाता है।
...
इस रजिस्ट्री के CLSIDs के अंदर आपको एक डिफ़ॉल्ट मान वाली **DLL** को इंजेक्ट करने वाली बच्ची रजिस्ट्री **InProcServer32** मिल सकती है और एक मान जिसे **ThreadingModel** कहा जाता है जो **Apartment** (Single-Threaded), **Free** (Multi-Threaded), **Both** (Single या Multi) या **Neutral** (Thread Neutral) हो सकता है।

![](<../../.gitbook/assets/image (638).png>)

आम तौर पर, यदि आप किसी भी DLL को **अधिकार उन्नति** कर सकते हैं जो किसी अलग उपयोगकर्ता द्वारा निष्पादित किया जाएगा, तो आप उस DLL को निष्पादित करने के लिए अगर उस DLL को निष्पादित किया जाएगा तो आप **अधिकार उन्नति** कर सकते हैं।

आक्रमणकर्ताओं को COM Hijacking का एक स्थायित्व तंत्र के रूप में कैसे उपयोग करते हैं इसे सीखने के लिए जांचें:
...
```
**किसी विशेष फ़ाइल नाम की फ़ाइल की खोज करें**
```bash
...
फिर, अगर आपके पास **कम विशेषाधिकार वाली प्रक्रिया के लिए पूर्ण अधिकार** है, तो आप **खुले हैंडल को प्राप्त कर सकते हैं जो उच्च विशेषाधिकार वाली प्रक्रिया ने बनाई** थी `OpenProcess()` और **शेलकोड इंजेक्ट** कर सकते हैं।\
[इस उदाहरण को पढ़ें और इस व्यवस्था को **कैसे पता लगाएं और इस कमजोरी का शोधन करें** के बारे में अधिक जानकारी के लिए।](leaked-handle-exploitation.md)\
[इस **अन्य पोस्ट को पढ़ें जिसमें विभिन्न स्तरों की अनुमतियों के साथ विरासत में मिली प्रक्रियाओं और धागों के अधिक हैंडलर्स का परीक्षण और दुरुपयोग कैसे करें** के बारे में एक अधिक पूर्ण व्याख्या के लिए](http://dronesec.pw/blog/2019/08/22/exploiting-leaked-process-and-thread-handles/).
...
Windows एक सुविधा प्रदान करता है जिसे **नेम्ड पाइप्स** कहा जाता है, जो असंबंधित प्रक्रियाओं को डेटा साझा करने की अनुमति देता है, यहां तक ​​कि विभिन्न नेटवर्कों पर भी। यह एक क्लाइंट/सर्वर संरचना की तरह है, जिसमें भूमिकाएँ **नेम्ड पाइप सर्वर** और **नेम्ड पाइप क्लाइंट** के रूप में परिभाषित हैं।

जब डेटा एक पाइप के माध्यम से एक **क्लाइंट** द्वारा भेजा जाता है, तो पाइप स्थापित करने वाला **सर्वर** उसकी **पहचान ग्रहण करने की क्षमता** रखता है, यदि उसके पास आवश्यक **SeImpersonate** अधिकार हैं। एक **उच्च विशेषाधिकार वाली प्रक्रिया** की पहचान करना जो एक पाइप के माध्यम से संवाद करती है और जिसे आप अनुकरण कर सकते हैं, उसे एक अवसर प्रदान करता है **उच्च विशेषाधिकार प्राप्त करने** के लिए जब वह पाइप के साथ आपके द्वारा स्थापित पाइप के साथ बातचीत करती है। इस प्रकार के हमले को कैसे कार्यान्वित करने के लिए निर्देशों के लिए मददगार गाइड [**यहां**](named-pipe-client-impersonation.md) और [**यहां**](./#from-high-integrity-to-system) मिल सकते हैं।

इसके अलावा निम्नलिखित उपकरण एक नेम्ड पाइप संचार को एक उपकरण जैसे बर्प के साथ आवरण करने की अनुमति देते हैं: [**https://github.com/gabriel-sztejnworcel/pipe-intercept**](https://github.com/gabriel-sztejnworcel/pipe-intercept) **और यह उपकरण सभी पाइपों को सूचीबद्ध करने और देखने की अनुमति देता है ताकि प्राइवेस्क्स खोजें** [**https://github.com/cyberark/PipeViewer**](https://github.com/cyberark/PipeViewer)

## विविध

### **पासवर्ड के लिए कमांड लाइन की निगरानी**

जब आप एक उपयोगकर्ता के रूप में एक शैल प्राप्त करते हैं, तो शेड्यूल किए गए कार्य या अन्य प्रक्रियाएं भी चल रही हो सकती हैं जो **कमांड लाइन पर प्रमाण पास करती हैं**। नीचे दिया गया स्क्रिप्ट प्रत्येक दो सेकंड में प्रक्रिया कमांड लाइन को कैप्चर करता है और वर्तमान स्थिति को पिछली स्थिति के साथ तुलना करता है, किसी भी अंतर को आउटपुट करता है।
```powershell
while($true)
{
...
इससे यह संभव होता है कि विशेषाधिकारों को उन्नत किया जा सके और एक साथ ही UAC को भी बायपास किया जा सके एक ही सुरक्षा दोष के साथ। इसके अतिरिक्त, कुछ भी स्थापित करने की आवश्यकता नहीं है और प्रक्रिया के दौरान उपयोग किया जाने वाला बाइनरी, माइक्रोसॉफ्ट द्वारा हस्ताक्षरित और जारी किया गया है।
...
```
1) HHUPD.EXE फ़ाइल पर राइट क्लिक करें और इसे व्यवस्थापक के रूप में चलाएं।

2) UAC प्रॉम्प्ट प्रकट होने पर, "अधिक विवरण दिखाएं" का चयन करें।

3) "प्रकाशक प्रमाणपत्र जानकारी दिखाएं" पर क्लिक करें।

4) यदि सिस्टम वंरूपनीय है, तो "जारी किया गया" URL लिंक पर क्लिक करने पर डिफ़ॉल्ट वेब ब्राउज़र प्रकट हो सकता है।

5) साइट पूरी तरह से लोड होने का इंतजार करें और "इसे बचाएं" का चयन करें ताकि एक explorer.exe विंडो प्रकट हो।
...
यदि आप पहले से हाई अखंडता प्रक्रिया पर चल रहे हैं, तो **सिस्टम तक पहुंचना** आसान हो सकता है बस **एक नई सेवा बनाकर और निष्पादित करके**:
```
...
एक उच्च अवधारणा प्रक्रिया से आप **AlwaysInstallElevated रजिस्ट्री एंट्री** को सक्षम करने की कोशिश कर सकते हैं और एक _**.msi**_ रैपर का उपयोग करके एक पलटी हुई शैली को **इंस्टॉल** कर सकते हैं।\
[इसमें शामिल रजिस्ट्री कुंजीयों और एक _.msi_ पैकेज को कैसे इंस्टॉल करें के बारे में अधिक जानकारी यहाँ मिलेगी।](./#alwaysinstallelevated)
...
यदि आपके पास वह टोकन विशेषाधिकार हैं (संभावना है कि आप इसे पहले से ही उच्च अवधारणा प्रक्रिया में पाएंगे), तो आप **SeDebug विशेषाधिकार के साथ लगभग किसी भी प्रक्रिया को खोल सकेंगे** (संरक्षित प्रक्रियाएं नहीं) , **प्रक्रिया का टोकन कॉपी** करें, और उस टोकन के साथ **एक अनियमित प्रक्रिया बनाएं**।\
इस तकनीक का उपयोग आम तौर पर **सिस्टम के रूप में चल रही किसी भी प्रक्रिया को चुना जाता है जिसमें सभी टोकन विशेषाधिकार होते हैं** (_हाँ, आप सभी टोकन विशेषाधिकारों के बिना सिस्टम प्रक्रियाएं पा सकते हैं_)।\
**आप** [**यहाँ प्रस्तावित तकनीक को अमल में लाने वाले कोड का उदाहरण पा सकते हैं**](sedebug-+-seimpersonate-copy-token.md)**।**

### **Named Pipes**

यह तकनीक मीटरप्रेटर द्वारा `getsystem` में उन्नति के लिए उपयोग की जाती है। तकनीक इस पर निर्भर करती है कि **एक पाइप बनाएं और फिर उस पाइप पर लिखने के लिए एक सेवा बनाएं/दुरुपयोग करें**। फिर, पाइप क्लाइंट (सेवा) के टोकन का अनुकरण करने के लिए **`SeImpersonate`** विशेषाधिकार का उपयोग करने वाला **सर्वर** पाइप बनाने वाला **सर्वर** सिस्टम विशेषाधिकार प्राप्त कर सकेगा।\
...
यदि आप **सिस्टम** के रूप में चल रही **प्रक्रिया** द्वारा **लोड** किए जा रहे **एक dll को हाइजैक** करने में सफल होते हैं तो आप उन अनुमतियों के साथ विचारशील कोड को उसके अनुमतियों के साथ क्रियान्वित कर सकेंगे। इसलिए Dll Hijacking इस प्रकार की विशेषाधिकार उन्नति के लिए भी उपयोगी है, और, इसके अतिरिक्त, यदि उच्च अवधारणा प्रक्रिया से यह सरल होगा क्योंकि इसके पास dlls लोड करने के लिए उपयोग किए जाने वाले फोल्डरों पर **लेखन अनुमतियाँ** होंगी।\
...
[**SeatBelt**](https://github.com/GhostPack/Seatbelt) -- मिसकॉन्फिगरेशन की खोज के लिए होस्ट की जांच करता है (एक जानकारी एकत्र करने का उपकरण है ज्यादा विशेषाधिकार नहीं) (कंपाइल करने की आवश्यकता है) **(**[**पूर्व-कंपाइल**](https://github.com/carlospolop/winPE/tree/master/binaries/seatbelt)**)**\
...
[**Windows-Exploit-Suggester**](https://github.com/GDSSecurity/Windows-Exploit-Suggester) -- **systeminfo** का आउटपुट पढ़ता है और क
```
...
* क्या आप **साइबर सुरक्षा कंपनी** में काम करते हैं? क्या आप चाहते हैं कि आपकी **कंपनी को हैकट्रिक्स में विज्ञापित** किया जाए? या क्या आप **नवीनतम संस्करण का पीईएएस देखना चाहते हैं या हैकट्रिक्स को पीडीएफ में डाउनलोड करना चाहते हैं**? [**सब्सक्रिप्शन प्लान्स**](https://github.com/sponsors/carlospolop) की जाँच करें!
* अपने [**NFTs**](https://opensea.io/collection/the-peass-family) के लिए हमारे विशेष संग्रह [**The PEASS Family**](https://opensea.io/collection/the-peass-family) की खोज करें
