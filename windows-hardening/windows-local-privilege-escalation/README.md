...
जांचें कि Windows संस्करण में कोई ज्ञात कमजोरी है या नहीं (लागू पैचों की भी जांच करें)।
```bash
systeminfo
...
यह [साइट](https://msrc.microsoft.com/update-guide/vulnerability) Microsoft सुरक्षा संवेदनशीलताओं के बारे में विस्तृत जानकारी खोजने के लिए उपयोगी है। इस डेटाबेस में 4,700 से अधिक सुरक्षा संवेदनशीलताएँ हैं, जो दिखाती हैं कि एक Windows वातावरण कितना **विशाल आक्रमण सतह** प्रस्तुत करता है।
...
क्या कोई प्रमाणपत्र/रसीली जानकारी env चर में सहेजी गई है?
```bash
set
dir env:
...
आप इसे कैसे चालू करें, इसके बारे में जान सकते हैं [https://sid-500.com/2017/11/07/powershell-enabling-transcription-logging-by-using-group-policy/](https://sid-500.com/2017/11/07/powershell-enabling-transcription-logging-by-using-group-policy/)
```bash
...
यह PowerShell के पाइपलाइन निष्पादन के विवरण को रिकॉर्ड करता है। इसमें निष्पादित किए गए आदेश शामिल होते हैं जिसमें आदेश आह्वान और स्क्रिप्ट्स का कुछ भाग शामिल होता है। इसमें निष्पादन के पूरे विवरण और आउटपुट परिणाम नहीं हो सकते हैं।\
...
```
Script Block लॉगिंग इवेंट्स को Windows Event viewer में निम्नलिखित पथ के अंतर्गत पाया जा सकता है: _Application and Services Logs > Microsoft > Windows > Powershell > Operational_\
अंतिम 20 इवेंट्स देखने के लिए आप इसका उपयोग कर सकते हैं:
```bash
...
इन कमजोरियों का शोषण करने के लिए आप उपकरणों का उपयोग कर सकते हैं जैसे: [Wsuxploit](https://github.com/pimps/wsuxploit), [pyWSUS](https://github.com/GoSecure/pywsus) - ये MiTM हथियारबंद शोषण स्क्रिप्ट हैं जो गैर-SSL WSUS ट्रैफिक में 'नकली' अपडेट्स इंजेक्ट करते हैं।
...
>
> इसके अलावा, चूंकि WSUS सेवा वर्तमान उपयोगकर्ता की सेटिंग्स का उपयोग करती है, यह उसके सर्टिफिकेट स्टोर का भी उपयोग करेगी। यदि हम WSUS होस्टनेम के लिए एक स्व-हस्ताक्षरित सर्टिफिकेट जनरेट करते हैं और इस सर्टिफिकेट को वर्तमान उपयोगकर्ता के सर्टिफिकेट स्टोर में जोड़ते हैं, तो हम HTTP और HTTPS WSUS ट्रैफिक दोनों को इंटरसेप्ट करने में सक्षम होंगे। WSUS में सर्टिफिकेट पर विश्वास-पर-पहली-उपयोग प्रकार की मान्यता को लागू करने के लिए HSTS-जैसी कोई तंत्र नहीं है। यदि प्रस्तुत सर्टिफिकेट उपयोगकर्ता द्वारा विश्वसनीय है और उसके पास सही होस्टनेम है, तो यह सेवा द्वारा स्वीकार किया जाएगा।
...
सभी **आवश्यकताएँ** **डिफ़ॉल्ट सेटिंग्स** के साथ संतुष्ट होती हैं।

**शोषण को यहाँ पाएं** [**https://github.com/Dec0ne/KrbRelayUp**](https://github.com/Dec0ne/KrbRelayUp)

यदि हमला है तो हमले के प्रवाह के बारे में अधिक जानकारी के लिए देखें [https://research.nccgroup.com/2019/08/20/kerberos-resource-based-constrained-delegation-when-an-image-change-leads-to-a-privilege-escalation/](https://research.nccgroup.com/2019/08/20/kerberos-resource-based-constrained-delegation-when-an-image-change-leads-to-a-privilege-escalation/)

## AlwaysInstallElevated

**यदि** ये 2 रजिस्ट्री **सक्षम** हैं (मान **0x1** है), तो किसी भी विशेषाधिकार के उपयोगकर्ता `*.msi` फाइलों को NT AUTHORITY\\**SYSTEM** के रूप में **स्थापित** (निष्पादित) कर सकते हैं।
```bash
...
Power-up से `Write-UserAddMSI` कमांड का उपयोग करके वर्तमान डायरेक्टरी में एक Windows MSI बाइनरी बनाएं जो विशेषाधिकार बढ़ाने के लिए है। यह स्क्रिप्ट एक पूर्व-संकलित MSI इंस्टॉलर लिखती है जो उपयोगकर्ता/समूह जोड़ने के लिए संकेत देती है (इसलिए आपको GIU पहुँच की आवश्यकता होगी):
```
```
Write-UserAddMSI
```
...
* Cobalt Strike या Metasploit के साथ एक **नया Windows EXE TCP पेलोड** `C:\privesc\beacon.exe` में **जनरेट** करें।
* **Visual Studio** खोलें, **Create a new project** चुनें और सर्च बॉक्स में "installer" टाइप करें। **Setup Wizard** प्रोजेक्ट को चुनें और **Next** पर क्लिक करें।
* प्रोजेक्ट को एक नाम दें, जैसे कि **AlwaysPrivesc**, स्थान के लिए **`C:\privesc`** का उपयोग करें, **place solution and project in the same directory** का चयन करें, और **Create** पर क्लिक करें।
* चरण 3 के 4 (शामिल करने के लिए फाइलों का चयन) तक **Next** पर क्लिक करते रहें। **Add** पर क्लिक करें और अभी जनरेट किए गए Beacon पेलोड को चुनें। फिर **Finish** पर क्लिक करें।
...
**LAPS** आपको डोमेन-जुड़े कंप्यूटरों पर **लोकल एडमिनिस्ट्रेटर पासवर्ड का प्रबंधन** करने की अनुमति देता है (जो **रैंडमाइज्ड**, अद्वितीय होता है और **नियमित रूप से बदला जाता है**). ये पासवर्ड्स केंद्रीय रूप से Active Directory में संग्रहीत होते हैं और ACLs का उपयोग करके अधिकृत उपयोगकर्ताओं तक सीमित होते हैं। यदि आपके उपयोगकर्ता को पर्याप्त अनुमतियां दी गई हैं, तो आप लोकल एडमिन्स के पासवर्ड पढ़ सकते हैं।
...
Microsoft ने **Windows 8.1 और बाद के संस्करणों** में LSA के लिए अतिरिक्त सुरक्षा प्रदान की है ताकि अविश्वसनीय प्रक्रियाओं को इसकी मेमोरी को **पढ़ने** से या कोड इंजेक्ट करने से **रोका** जा सके।\
...
**क्रेडेंशियल गार्ड** Windows 10 (एंटरप्राइज और एजुकेशन संस्करण) में एक नई सुविधा है जो आपके मशीन पर क्रेडेंशियल्स की सुरक्षा करती है, जैसे कि पास द हैश से खतरों से।\
...
### उपयोगकर्ता और समूहों का परिगणन करें

आपको यह जांचना चाहिए कि क्या आपके द्वारा संबंधित किसी समूह में दिलचस्प अनुमतियां हैं
```bash
# CMD
...
इस पृष्ठ पर **और जानें** कि **टोकन** क्या है: [**Windows Tokens**](../authentication-credentials-uac-and-efs.md#access-tokens).\
निम्नलिखित पृष्ठ को देखें ताकि **रोचक टोकनों के बारे में जानें** और उनका दुरुपयोग कैसे करें:
...
```
हमेशा संभावित [**electron/cef/chromium डिबगर्स** की जांच करें जो चल रहे हों, आप इसका दुरुपयोग करके विशेषाधिकार बढ़ा सकते हैं](../../linux-hardening/privilege-escalation/electron-cef-chromium-debugger-abuse.md).

**प्रक्रियाओं के बाइनरीज की अनुमतियों की जांच करना**
```bash
...
आप सिसइंटरनल्स से **procdump** का उपयोग करके चल रही प्रक्रिया का मेमोरी डंप बना सकते हैं। FTP जैसी सेवाओं में **मेमोरी में स्पष्ट पाठ में क्रेडेंशियल्स होते हैं**, मेमोरी डंप करने की कोशिश करें और क्रेडेंशियल्स पढ़ें।
```
procdump.exe -accepteula -ma <proc_name_tasklist>
```
### असुरक्षित GUI ऐप्स

**SYSTEM के रूप में चल रहे ऐप्लिकेशन्स एक उपयोगकर्ता को CMD शुरू करने या डायरेक्टरीज़ ब्राउज़ करने की अनुमति दे सकते हैं।**
...
```
यह सिफारिश की जाती है कि जांचें कि क्या "Authenticated Users" किसी भी सेवा में परिवर्तन कर सकते हैं:
```bash
...
_सेवा शुरू नहीं की जा सकती, या तो इसे अक्षम किया गया है या इसके साथ जुड़े हुए सक्षम उपकरण नहीं हैं।_
...
यदि "Authenticated users" समूह को किसी सेवा में **SERVICE\_ALL\_ACCESS** की अनुमति प्राप्त हो, तो वह उस बाइनरी को संशोधित कर सकता है जिसे सेवा द्वारा निष्पादित किया जा रहा है। **nc** को निष्पादित करने और इसे संशोधित करने के लिए आप यह कर सकते हैं:
```bash
...
**SERVICE_CHANGE_CONFIG** सेवा बाइनरी को पुनः कॉन्फ़िगर कर सकते हैं
**WRITE_DAC:** अनुमतियों को पुनः कॉन्फ़िगर कर सकते हैं, जिससे SERVICE_CHANGE_CONFIG हो सकता है
**WRITE_OWNER:** मालिक बन सकते हैं, अनुमतियों को पुनः कॉन्फ़िगर कर सकते हैं
**GENERIC_WRITE:** SERVICE_CHANGE_CONFIG को विरासत में मिलता है
**GENERIC_ALL:** SERVICE_CHANGE_CONFIG को विरासत में मिलता है

इस संवेदनशीलता का पता लगाने और उपयोग करने के लिए आप _exploit/windows/local/service_permissions_ का उपयोग कर सकते हैं

### सेवाओं की बाइनरीज में कमजोर अनुमतियां

**जांचें कि क्या आप उस बाइनरी को संशोधित कर सकते हैं जो किसी सेवा द्वारा निष्पादित की जाती है** या यदि आपके पास उस फ़ोल्डर पर **लिखने की अनुमति है** जहां बाइनरी स्थित है ([**DLL Hijacking**](dll-hijacking.md))**.**\
आप **wmic** का उपयोग करके हर बाइनरी जो किसी सेवा द्वारा निष्पादित की जाती है (system32 में नहीं) प्राप्त कर सकते हैं और अपनी अनुमतियों की जांच **icacls** का उपयोग करके कर सकते हैं:
```bash
...
```
### सेवाओं के रजिस्ट्री संशोधन अनुमतियाँ

आपको जांचना चाहिए कि क्या आप किसी सेवा रजिस्ट्री को संशोधित कर सकते हैं।\
...
```
जांचें कि क्या **Authenticated Users** या **NT AUTHORITY\INTERACTIVE** के पास FullControl है। ऐसी स्थिति में आप सेवा द्वारा निष्पादित होने वाले बाइनरी को बदल सकते हैं।

बाइनरी के Path को बदलने के लिए:
```bash
reg add HKLM\SYSTEM\CurrentControlSet\services\<service_name> /v ImagePath /t REG_EXPAND_SZ /d C:\path\new\binary /f
```
### सेवाओं के रजिस्ट्री AppendData/AddSubdirectory अनुमतियाँ

यदि आपके पास किसी रजिस्ट्री पर यह अनुमति है, इसका मतलब है **आप इससे उप रजिस्ट्री बना सकते हैं**। विंडोज सेवाओं के मामले में यह **मनमाने कोड को निष्पादित करने के लिए पर्याप्त है:**
...
यदि किसी निष्पादन योग्य फ़ाइल का पथ उद्धरण चिह्नों के अंदर नहीं है, तो विंडोज हर अंत को एक स्पेस से पहले निष्पादित करने का प्रयास करेगा।
...
यह संभव है कि Windows को यह निर्देशित किया जा सके कि [जब कोई सेवा का निष्पादन विफल हो जाए](https://docs.microsoft.com/en-us/previous-versions/windows/it-pro/windows-server-2008-R2-and-2008/cc753662\(v=ws.11\)?redirectedfrom=MSDN) तो वह क्या करे। यदि वह सेटिंग किसी बाइनरी की ओर इशारा कर रही है और इस बाइनरी को अधिलेखित किया जा सकता है, तो आपको विशेषाधिकार बढ़ाने में सक्षम हो सकते हैं।

## अनुप्रयोग

### स्थापित अनुप्रयोग

**बाइनरीज की अनुमतियाँ** जांचें (शायद आप एक को अधिलेखित कर सकते हैं और विशेषाधिकार बढ़ा सकते हैं) और **फोल्डर्स** की अनुमतियाँ ([DLL Hijacking](dll-hijacking.md))।
```bash
...
संभावित **तृतीय पक्ष अजीब/संवेदनशील** ड्राइवर्स की तलाश करें
```
driverquery
...
hosts फ़ाइल में हार्डकोडेड अन्य ज्ञात कंप्यूटरों की जाँच करें
```
...
[**फ़ायरवॉल से संबंधित कमांड्स के लिए यह पृष्ठ देखें**](../basic-cmd-for-pentesters.md#firewall) **(नियमों की सूची, नियम बनाना, बंद करना, बंद करना...)**

और [नेटवर्क गणना के लिए यहाँ कमांड्स](../basic-cmd-for-pentesters.md#network)
...
विंडोज वॉल्ट सर्वर, वेबसाइट और अन्य प्रोग्राम के लिए उपयोगकर्ता क्रेडेंशियल्स संग्रहीत करता है जिसे **विंडोज** उपयोगकर्ताओं को **स्वचालित रूप से लॉग इन कर सकता है**। पहली नजर में, ऐसा लग सकता है कि अब उपयोगकर्ता अपने फेसबुक क्रेडेंशियल्स, ट्विटर क्रेडेंशियल्स, जीमेल क्रेडेंशियल्स आदि को संग्रहीत कर सकते हैं, ताकि वे ब्राउज़र के माध्यम से स्वचालित रूप से लॉग इन कर सकें। लेकिन ऐसा नहीं है।

विंडोज वॉल्ट ऐसे क्रेडेंशियल्स संग्रहीत करता है जिसे विंडोज स्वचालित रूप से उपयोगकर्ताओं को लॉग इन कर सकता है, जिसका अर्थ है कि कोई भी **विंडोज एप्लिकेशन जिसे किसी संसाधन तक पहुँचने के लिए क्रेडेंशियल्स की आवश्यकता होती है** (सर्वर या वेबसाइट) **इस क्रेडेंशियल मैनेजर** और विंडोज वॉल्ट का उपयोग कर सकता है और हर बार उपयोगकर्ता द्वारा यूजरनेम और पासवर्ड दर्ज करने के बजाय संग्रहीत क्रेडेंशियल्स का उपयोग कर सकता है।
...
सिद्धांत रूप में, डेटा प्रोटेक्शन API किसी भी प्रकार के डेटा के सममित एन्क्रिप्शन को सक्षम कर सकता है; व्यवहार में, इसका प्राथमिक उपयोग विंडोज ऑपरेटिंग सिस्टम में असममित निजी कुंजियों के सममित एन्क्रिप्शन को प्रदर्शन करना है, उपयोगकर्ता या सिस्टम के रहस्य का उपयोग करके एंट्रोपी का एक महत्वपूर्ण योगदान के रूप में।
...
आप **मेमोरी** से कई **DPAPI** **masterkeys** निकाल सकते हैं `sekurlsa::dpapi` मॉड्यूल के साथ (यदि आप रूट हैं)।
...
**Mimikatz** `dpapi::rdg` मॉड्यूल का उपयोग करें और उचित `/masterkey` के साथ **किसी भी .rdg फाइलों को डिक्रिप्ट करें**\
...
```
{% hint style="info" %}
ऐसा लगता है कि यह तकनीक अब मान्य नहीं है। मैंने कुछ ssh कीज़ बनाने की कोशिश की, उन्हें `ssh-add` के साथ जोड़ा और ssh के माध्यम से एक मशीन में लॉगिन करने की कोशिश की। रजिस्ट्री HKCU\Software\OpenSSH\Agent\Keys मौजूद नहीं है और procmon ने असममित कुंजी प्रमाणीकरण के दौरान `dpapi.dll` के उपयोग की पहचान नहीं की।
{% endhint %}
...
```
सभी प्रस्तावित फाइलों की खोज करें:
```
cd C:\
...
आपको Bin की भी जांच करनी चाहिए ताकि उसमें मौजूद क्रेडेंशियल्स की तलाश की जा सके

कई प्रोग्राम्स द्वारा सहेजे गए **पासवर्ड्स को पुनः प्राप्त** करने के लिए आप इसका उपयोग कर सकते हैं: [http://www.nirsoft.net/password\_recovery\_tools.html](http://www.nirsoft.net/password\_recovery\_tools.html)

### रजिस्ट्री के अंदर

**क्रेडेंशियल्स के साथ अन्य संभावित रजिस्ट्री कीज**
```bash
...
आपको डीबीएस की जांच करनी चाहिए जहां **Chrome या Firefox** के पासवर्ड संग्रहीत होते हैं।\
साथ ही ब्राउज़र्स की हिस्ट्री, बुकमार्क्स और फेवरेट्स की जांच करें ताकि कुछ **पासवर्ड वहां** संग्रहीत हो सकते हैं।
...
इस रजिस्ट्री के CLSIDs के अंदर आपको बच्चे की रजिस्ट्री **InProcServer32** मिलेगी जिसमें एक **डिफ़ॉल्ट मान** होता है जो एक **DLL** की ओर इशारा करता है और एक मान **ThreadingModel** होता है जो **Apartment** (सिंगल-थ्रेडेड), **Free** (मल्टी-थ्रेडेड), **Both** (सिंगल या मल्टी) या **Neutral** (थ्रेड न्यूट्रल) हो सकता है।
...
जानने के लिए कि हमलावर COM Hijacking का उपयोग एक स्थायित्व तंत्र के रूप में कैसे करते हैं, देखें:
...
where /R C:\ *.ini
```
**रजिस्ट्री में की नामों और पासवर्डों की खोज करें**
```bash
...
[**Winpeas**](https://github.com/carlospolop/privilege-escalation-awesome-scripts-suite) स्वचालित रूप से इस पृष्ठ पर उल्लिखित सभी फाइलों में पासवर्ड की खोज करता है।\
...
कल्पना कीजिए कि **एक प्रक्रिया SYSTEM के रूप में चल रही है और एक नई प्रक्रिया खोलती है** (`OpenProcess()`) **पूरी पहुंच के साथ**। वही प्रक्रिया **कम विशेषाधिकारों के साथ एक नई प्रक्रिया भी बनाती है** (`CreateProcess()`) **लेकिन मुख्य प्रक्रिया के सभी खुले हैंडल्स को विरासत में लेती है**।\
फिर, अगर आपके पास **कम विशेषाधिकार वाली प्रक्रिया की पूरी पहुंच है**, आप **खुले हैंडल को विशेषाधिकार प्राप्त प्रक्रिया से पकड़ सकते हैं** जो `OpenProcess()` के साथ बनाई गई थी और **शेलकोड इंजेक्ट कर सकते हैं**।\
[इस उदाहरण को पढ़ें जिसमें **इस कमजोरी का पता लगाने और इसका शोषण करने की जानकारी है**।](leaked-handle-exploitation.md)\
[इस **अन्य पोस्ट को पढ़ें जिसमें विभिन्न स्तरों की अनुमतियों के साथ विरासत में मिले प्रक्रियाओं और धागों के अधिक खुले हैंडलर्स का परीक्षण और दुरुपयोग कैसे करें (केवल पूरी पहुंच ही नहीं)** के लिए अधिक संपूर्ण व्याख्या है](http://dronesec.pw/blog/2019/08/22/exploiting-leaked-process-and-thread-handles/).

## नामित पाइप क्लाइंट इम्पर्सनेशन

`pipe` एक साझा मेमोरी का ब्लॉक है जिसका उपयोग प्रक्रियाएं संचार और डेटा आदान-प्रदान के लिए कर सकती हैं।

`Named Pipes` एक Windows तंत्र है जो दो असंबंधित प्रक्रियाओं को आपस में डेटा आदान-प्रदान करने की अनुमति देता है, भले ही प्रक्रियाएं दो अलग-अलग नेटवर्क पर स्थित हों। यह क्लाइंट/सर्वर आर्किटेक्चर के समान है क्योंकि `नामित पाइप सर्वर` और नामित `पाइप क्लाइंट` की धारणाएं मौजूद हैं।

जब एक **क्लाइंट पाइप पर लिखता है**, तो **सर्वर** जिसने पाइप बनाया है, **क्लाइंट का इम्पर्सनेशन** कर सकता है अगर उसके पास **SeImpersonate** विशेषाधिकार हैं। फिर, अगर आप एक **विशेषाधिकार प्राप्त प्रक्रिया को ढूंढ सकते हैं जो किसी भी पाइप पर लिखने वाली है जिसे आप इम्पर्सनेट कर सकते हैं**, आप **विशेषाधिकार बढ़ा सकते हैं** उस प्रक्रिया का इम्पर्सनेशन करके जब वह आपके बनाए पाइप के अंदर लिखती है। [**इस हमले को कैसे करें यह सीखने के लिए आप यह पढ़ सकते हैं**](named-pipe-client-impersonation.md) **या** [**यह**](./#from-high-integrity-to-system)**.**

**इसके अलावा निम्नलिखित टूल बर्प जैसे टूल के साथ नामित पाइप संचार को इंटरसेप्ट करने की अनुमति देता है:** [**https://github.com/gabriel-sztejnworcel/pipe-intercept**](https://github.com/gabriel-sztejnworcel/pipe-intercept) **और यह टूल प्रिवेस्क्स को ढूंढने के लिए सभी पाइप्स को सूचीबद्ध करने और देखने की अनुमति देता है** [**https://github.com/cyberark/PipeViewer**](https://github.com/cyberark/PipeViewer)****

## विविध

### **पासवर्ड्स के लिए कमांड लाइन्स की निगरानी**

जब आप एक यूजर के रूप में शेल प्राप्त करते हैं, तो हो सकता है कि निर्धारित कार्य या अन्य प्रक्रियाएं चल रही हों जो **कमांड लाइन पर क्रेडेंशियल्स पास करती हैं**। नीचे दी गई स्क्रिप्ट हर दो सेकंड में प्रक्रिया कमांड लाइन्स को कैप्चर करती है और वर्तमान स्थिति की तुलना पिछली स्थिति से करती है, किसी भी अंतर को आउटपुट करती है।
```powershell
while($true)
{
...
यदि आपके पास ग्राफिकल इंटरफेस (कंसोल या RDP के माध्यम से) तक पहुंच है और UAC सक्रिय है, तो Microsoft Windows के कुछ संस्करणों में यह संभव है कि एक टर्मिनल या कोई अन्य प्रक्रिया जैसे "NT\AUTHORITY SYSTEM" को एक अनाधिकृत यूजर से चलाया जा सके।
...
```
इस संवेदनशीलता का लाभ उठाने के लिए, निम्नलिखित चरणों को करना आवश्यक है:

```
1) HHUPD.EXE फ़ाइल पर राइट क्लिक करें और इसे एडमिनिस्ट्रेटर के रूप में चलाएं।

2) जब UAC प्रॉम्प्ट दिखाई दे, "अधिक विवरण दिखाएं" का चयन करें।

3) "प्रकाशक प्रमाणपत्र जानकारी दिखाएं" पर क्लिक करें।

4) यदि सिस्टम संवेदनशील है, तो "जारी करने वाले" URL लिंक पर क्लिक करने पर, डिफ़ॉल्ट वेब ब्राउज़र प्रकट हो सकता है।
...
**इंटेग्रिटी लेवल्स के बारे में जानने के लिए यह पढ़ें:**
...
यदि आपके पास ये टोकन विशेषाधिकार हैं (संभवतः आपको यह एक पहले से ही High Integrity प्रक्रिया में मिलेगा), तो आप SeDebug विशेषाधिकार के साथ **लगभग किसी भी प्रक्रिया को खोलने** में सक्षम होंगे (संरक्षित प्रक्रियाओं को छोड़कर), **प्रक्रिया के टोकन की प्रतिलिपि बनाएं**, और उस टोकन के साथ एक **मनमानी प्रक्रिया बनाएं**।\
इस तकनीक का उपयोग करते समय आमतौर पर **किसी भी प्रक्रिया को चुना जाता है जो SYSTEM के रूप में सभी टोकन विशेषाधिकारों के साथ चल रही होती है** (_हाँ, आपको SYSTEM प्रक्रियाएं सभी टोकन विशेषाधिकारों के बिना मिल सकती हैं_)।\
**आप यहाँ प्रस्तावित तकनीक को निष्पादित करने वाले कोड का एक** [**उदाहरण पा सकते हैं**](sedebug-+-seimpersonate-copy-token.md)**।**

### **Named Pipes**

यह तकनीक meterpreter द्वारा `getsystem` में वृद्धि के लिए उपयोग की जाती है। तकनीक में **एक पाइप बनाना और फिर एक सेवा बनाने/दुरुपयोग करना शामिल है जो उस पाइप पर लिखती है**। फिर, **सर्वर** जिसने **`SeImpersonate`** विशेषाधिकार का उपयोग करके पाइप बनाया है, वह पाइप क्लाइंट (सेवा) के टोकन को **अनुकरण करने** में सक्षम होगा, जिससे SYSTEM विशेषाधिकार प्राप्त होंगे।\
यदि आप [**नामित पाइपों के बारे में और जानना चाहते हैं तो आपको यह पढ़ना चाहिए**](./#named-pipe-client-impersonation)।\
यदि आप उदाहरण पढ़ना चाहते हैं कि [**कैसे उच्च अखंडता से System तक नामित पाइपों का उपयोग करके जाना जाता है तो आपको यह पढ़ना चाहिए**](from-high-integrity-to-system-with-name-pipes.md)।

### Dll Hijacking

यदि आप **SYSTEM के रूप में चल रही प्रक्रिया द्वारा लोड किए जा रहे dll को हाइजैक करने** में सफल होते हैं, तो आप उन अनुमतियों के साथ मनमाना कोड निष्पादित करने में सक्षम होंगे। इसलिए Dll Hijacking भी इस प्रकार के विशेषाधिकार वृद्धि के लिए उपयोगी है, और इसके अलावा, यह एक उच्च अखंडता प्रक्रिया से प्राप्त करना **अधिक आसान है** क्योंकि इसमें dlls लोड करने के लिए उपयोग किए जाने वाले फ़ोल्डरों पर **लिखने की अनुमतियां** होंगी।\
...
[**PowerSploit-Privesc(PowerUP)**](https://github.com/PowerShellMafia/PowerSploit) **-- मिसकॉन्फिगरेशन और संवेदनशील फ़ाइलों की जाँच करें (**[**यहाँ जाँचें**](../../windows/windows-local-privilege-escalation/broken-reference/)**). पता चला।**\
[**JAWS**](https://github.com/411Hall/JAWS) **-- कुछ संभावित मिसकॉन्फिगरेशन की जाँच करें और जानकारी एकत्र करें (**[**यहाँ जाँचें**](../../windows/windows-local-privilege-escalation/broken-reference/)**).**\
[**privesc** ](https://github.com/enjoiz/Privesc)**-- मिसकॉन्फिगरेशन की जाँच करें**\
...
[**Invoke-WCMDump**](https://github.com/peewpw/Invoke-WCMDump) **-- Credential Manager से crendentials निकालता है। पता चला।**\
[**DomainPasswordSpray**](https://github.com/dafthack/DomainPasswordSpray) **-- डोमेन भर में एकत्रित पासवर्ड स्प्रे करें**\
...
[~~**WINspect**~~](https://github.com/A-mIn3/WINspect) -- स्थानीय जाँचें **(एडमिन अधिकार चाहिए)**

**Exe**

[**Watson**](https://github.com/rasta-mouse/Watson) -- ज्ञात privesc कमजोरियों की खोज करें (VisualStudio का उपयोग करके संकलित करने की आवश्यकता है) ([**पूर्वसंकलित**](https://github.com/carlospolop/winPE/tree/master/binaries/watson))\
[**SeatBelt**](https://github.com/GhostPack/Seatbelt) -- मिसकॉन्फिगरेशन की खोज करता है (अधिक एक जानकारी एकत्र करने वाला उपकरण प्रिवेस्क से) (संकलित करने की आवश्यकता है) **(**[**पूर्वसंकलित**](https://github.com/carlospolop/winPE/tree/master/binaries/seatbelt)**)**\
[**LaZagne**](https://github.com/AlessandroZ/LaZagne) **-- बहुत सारे सॉफ्टवेयर से क्रेडेंशियल्स निकालता है (github में पूर्वसंकलित exe)**\
[**SharpUP**](https://github.com/GhostPack/SharpUp) **-- PowerUp का C# में पोर्ट**\
[~~**Beroot**~~](https://github.com/AlessandroZ/BeRoot) **\~\~**\~\~ -- मिसकॉन्फिगरेशन की जाँच करें (github में पूर्वसंकलित निष्पादनयोग्य)। अनुशंसित नहीं। Win10 में अच्छी तरह से काम नहीं करता।\
[~~**Windows-Privesc-Check**~~](https://github.com/pentestmonkey/windows-privesc-check) -- संभावित मिसकॉन्फिगरेशन की जाँच करें (python से exe)। अनुशंसित नहीं। Win10 में अच्छी तरह से काम नहीं करता।

**Bat**

[**winPEASbat** ](https://github.com/carlospolop/privilege-escalation-awesome-scripts-suite/tree/master/winPEAS)-- इस पोस्ट के आधार पर बनाया गया उपकरण (इसे ठीक से काम करने के लिए accesschk की आवश्यकता नहीं है लेकिन इसका उपयोग कर सकता है)।
...
आपको परियोजना को सही .NET संस्करण का उपयोग करके संकलित करना होगा ([इसे देखें](https://rastamouse.me/2018/09/a-lesson-in-.net-framework-versions/)). पीड़ित होस्ट पर .NET के स्थापित संस्करण क
```
