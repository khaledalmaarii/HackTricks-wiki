...
如果启用，明文密码将存储在LSASS（本地安全性局部子系统服务）中。\
...
Windows Vault 存储 Windows 可以自动登录用户的凭据，这意味着**任何需要凭据访问资源**（服务器或网站）的**Windows 应用程序**都可以利用此凭据管理器和 Windows Vault，并使用提供的凭据，而无需用户一直输入用户名和密码。
...
然后，如果你**对权限较低的进程有完全访问权限**，你可以获取通过`OpenProcess()`创建的**对特权进程的打开处理程序**，并**注入shellcode**。\
...
4) 如果系统存在漏洞，在点击“由...发布”URL链接时，可能会打开默认的网络浏览器。
