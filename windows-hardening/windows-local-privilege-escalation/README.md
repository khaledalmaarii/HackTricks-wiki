...
* **💬** [**Discord 그룹**](https://discord.gg/hRep4RUj7f)에 **가입**하거나 [**텔레그램 그룹**](https://t.me/peass)에 가입하거나 **트위터** 🐦[**@carlospolopm**](https://twitter.com/hacktricks\_live)**를 팔로우**하세요.
...
Windows에는 **시스템 열거를 방지하거나** 실행 파일을 실행하지 못하게 하거나 심지어 **활동을 감지하는 것을 방해할 수 있는 다양한 요소**가 있습니다. 권한 상승 열거를 시작하기 전에 다음 **페이지**를 **읽고** 이러한 **방어 메커니즘을 모두 열거**하세요:
...
스크립트의 실행 활동 및 전체 내용 기록이 캡처되어 각 코드 블록이 실행될 때마다 문서화됩니다. 이 프로세스는 각 활동에 대한 포괄적인 감사 트레일을 보존하여 포렌식 및 악의적인 행위 분석에 유용합니다. 실행 시 모든 활동을 문서화함으로써 프로세스에 대한 자세한 통찰력을 제공합니다.
```bash
...
```
스크립트 블록의 로깅 이벤트는 Windows 이벤트 뷰어에서 다음 경로에 위치할 수 있습니다: **Application and Services Logs > Microsoft > Windows > PowerShell > Operational**.\
...
시스템이 http 대신 http**S**를 사용하여 업데이트를 요청하지 않는 경우 시스템을 침해할 수 있습니다.
...
>
> 더 나아가, WSUS 서비스는 현재 사용자의 설정을 사용하므로, 현재 사용자의 인증서 저장소도 사용합니다. WSUS 호스트 이름을 위한 자체 서명 인증서를 생성하고 이 인증서를 현재 사용자의 인증서 저장소에 추가하면 HTTP 및 HTTPS WSUS 트래픽을 모두 가로챌 수 있습니다. WSUS는 인증서에 대한 신뢰성 검증을 구현하기 위해 HSTS와 같은 메커니즘을 사용하지 않습니다. 사용자가 신뢰하고 올바른 호스트 이름을 가진 인증서를 제시하면 서비스에서 인증됩니다.

이 취약점을 [**WSUSpicious**](https://github.com/GoSecure/wsuspicious) 도구를 사용하여 악용할 수 있습니다 (해제되면).

## KrbRelayUp

특정 조건에서 Windows **도메인** 환경에서 **로컬 권한 상승** 취약점이 존재합니다. 이러한 조건에는 **LDAP 서명이 강제되지 않는** 환경, 사용자가 **Resource-Based Constrained Delegation (RBCD)를 구성할 수 있는** 자체 권한이 있는 환경, 그리고 사용자가 도메인 내에서 컴퓨터를 생성할 수 있는 능력이 포함됩니다. 이러한 **요구 사항**은 **기본 설정**을 사용하여 충족되는 것이 중요합니다.
...
이 도구를 사용하여 MSI 래퍼를 만드는 방법을 배우려면이 자습서를 읽으십시오. **명령줄을 실행**하고 싶다면 "**.bat**" 파일을 랩핑할 수 있다는 점에 유의하십시오.
...
Windows Event Forwarding, 로그가 어디로 전송되는지 파악하는 것이 흥미로운 부분입니다.
```bash
reg query HKLM\Software\Policies\Microsoft\Windows\EventLog\EventForwarding\SubscriptionManager
```
### LAPS

**LAPS**는 로컬 관리자 암호를 관리하기 위해 설계되었으며, 도메인에 가입된 컴퓨터에서 각 암호가 고유하고 무작위로 생성되며 정기적으로 업데이트됨을 보장합니다. 이러한 암호는 Active Directory 내에서 안전하게 저장되며 ACL을 통해 충분한 권한을 부여받은 사용자만이 권한이 부여된 경우에만 로컬 관리자 암호를 볼 수 있도록 허용됩니다.
...
활성화된 경우, **평문 암호가 LSASS(로컬 보안 권한 부분 시스템 서비스)에 저장**됩니다.\
...
당신이 속한 그룹 중 흥미로운 권한을 가진 그룹이 있는지 확인해야 합니다.
```bash
# CMD
...
만약 **특권 그룹에 속한다면 권한 상승이 가능**할 수 있습니다. 특권 그룹에 대해 알아보고 권한 상승을 위해 그들을 어떻게 남용할 수 있는지 여기에서 배우세요:
...
**더 알아보세요**. 이 페이지에서 **토큰**이 무엇인지에 대해 알아보세요: [**Windows Tokens**](../authentication-credentials-uac-and-efs/#access-tokens).\
다음 페이지를 확인하여 **흥미로운 토큰** 및 그들을 어떻게 남용할 수 있는지에 대해 배우세요:
...
먼저, 프로세스 목록을 확인하여 **프로세스의 명령줄 내에 비밀번호가 있는지 확인**합니다.\
실행 중인 일부 이진 파일을 **덮어쓸 수 있는지** 또는 이진 파일 폴더에 쓰기 권한이 있는지 확인하여 가능한 [**DLL Hijacking 공격**](dll-hijacking/)을 악용할 수 있는지 확인합니다.
```bash
...
예: "Windows 도움말 및 지원" (Windows + F1), "명령 프롬프트 열기"를 클릭하여 "명령 프롬프트 열기"를 검색합니다.
...
sc qc <service_name>
```
다음은 _Sysinternals_에서 **accesschk** 바이너리를 사용하여 각 서비스의 필요한 권한 수준을 확인하는 것이 좋습니다.
```bash
...
```
다음을 확인해야 합니다. **인증된 사용자** 또는 **NT AUTHORITY\INTERACTIVE**가 `FullControl` 권한을 소유하는지. 그렇다면 서비스에 의해 실행된 이진 파일을 변경할 수 있습니다.
...
레지스트리에 대한 이 권한을 가지고 있다면 **이것은 **이 레지스트리에서 하위 레지스트리를 생성할 수 있다는 것을 의미합니다**. Windows 서비스의 경우, 이것은 **임의의 코드를 실행하는 데 충분합니다:**
...
Windows는 서비스가 실패할 경우 취해야 할 조치를 사용자가 지정할 수 있도록 합니다. 이 기능은 이진 파일을 가리키도록 구성할 수 있습니다. 이 이진 파일이 대체 가능하다면 권한 상승이 가능할 수 있습니다. 자세한 내용은 [공식 문서](https://docs.microsoft.com/en-us/previous-versions/windows/it-pro/windows-server-2008-R2-and-2008/cc753662\(v=ws.11\)?redirectedfrom=MSDN)에서 확인할 수 있습니다.

## 응용 프로그램

### 설치된 응용 프로그램

**이진 파일의 권한** (하나를 덮어쓸 수 있고 권한을 상승할 수 있음) 및 **폴더**의 권한을 확인하세요 ([DLL Hijacking](dll-hijacking/)).
```bash
...
특정 파일을 읽을 수 있는 구성 파일을 수정하거나 관리자 계정 (schedtasks)에 의해 실행될 이진 파일을 수정할 수 있는지 확인합니다.
...
**다른 사용자에 의해 실행될 레지스트리 또는 이진 파일을 덮어쓸 수 있는지 확인합니다.**\
...
만약 **PATH에 있는 폴더 내에 쓰기 권한**이 있다면, 프로세스에 의해 로드되는 DLL을 납치하고 **권한을 상승**시킬 수 있습니다.

PATH 내 모든 폴더의 권한을 확인하세요:
```bash
for %%A in ("%path:;=";"%") do ( cmd.exe /c icacls "%%~A" 2>nul | findstr /i "(F) (M) (W) :\" | findstr /i ":\\ everyone authenticated users todos %username%" && echo. )
```
이 확인을 악용하는 방법에 대한 자세한 정보는 다음을 참조하십시오:
...
응용 프로그램이 자격 증명 관리자와 상호 작용하지 않는 한 특정 리소스의 자격 증명을 사용할 수 없을 것으로 생각됩니다. 따라서 응용 프로그램이 보관함을 활용하려면 기본 저장 보관함에서 해당 리소스의 자격 증명을 요청하도록 자격 증명 관리자와 어떤 식으로든 **통신해야 합니다**.

`cmdkey`를 사용하여 기기에 저장된 자격 증명을 나열합니다.
```bash
cmdkey /list
...
```
그럼 저장된 자격 증명을 사용하기 위해 `runas`를 `/savecred` 옵션과 함께 사용할 수 있습니다. 다음 예는 SMB 공유를 통해 원격 이진 파일을 호출하는 것입니다.
```bash
...
```
참고로 mimikatz, lazagne, [credentialfileview](https://www.nirsoft.net/utils/credentials\_file\_view.html), [VaultPasswordView](https://www.nirsoft.net/utils/vault\_password\_view.html), 또는 [Empire Powershells 모듈](https://github.com/EmpireProject/Empire/blob/master/data/module\_source/credentials/dumpCredStore.ps1)에서 얻을 수 있습니다.

### DPAPI

**데이터 보호 API (DPAPI)**는 주로 Windows 운영 체제 내에서 사용되는 데이터의 대칭 암호화 방법을 제공하며 비대칭 개인 키의 대칭 암호화에 사용됩니다. 이 암호화는 사용자 또는 시스템 비밀을 엔트로피에 상당히 기여하는 데 사용합니다.

**DPAPI는 사용자의 로그인 비밀로부터 파생된 대칭 키를 통해 키의 암호화를 가능하게 합니다**. 시스템 암호화가 관련된 시나리오에서는 시스템의 도메인 인증 비밀을 활용합니다.

DPAPI를 사용하여 암호화된 사용자 RSA 키는 `%APPDATA%\Microsoft\Protect\{SID}` 디렉토리에 저장되며, 여기서 `{SID}`는 사용자의 [보안 식별자](https://en.wikipedia.org/wiki/Security\_Identifier)를 나타냅니다. **DPAPI 키는 사용자의 개인 키를 보호하는 마스터 키와 함께 동일한 파일에 공존하며, 일반적으로 64바이트의 무작위 데이터로 구성**됩니다. (이 디렉토리에 대한 액세스는 제한되어 있어 CMD의 `dir` 명령을 통해 내용을 나열하는 것을 방지하지만 PowerShell을 통해 나열할 수 있습니다).
```powershell
...
사람들은 종종 Windows 워크스테이션에서 StickyNotes 앱을 사용하여 **비밀번호** 및 기타 정보를 저장하는데, 이것이 데이터베이스 파일임을 깨닫지 못합니다. 이 파일은 `C:\Users\<user>\AppData\Local\Packages\Microsoft.MicrosoftStickyNotes_8wekyb3d8bbwe\LocalState\plum.sqlite`에 위치하며 항상 찾아보고 조사할 가치가 있습니다.
...
```
만약 해당 경로 내에 항목을 찾는다면 그것은 아마 저장된 SSH 키일 것입니다. 이는 암호화되어 저장되어 있지만 [https://github.com/ropnop/windows_sshagent_extract](https://github.com/ropnop/windows_sshagent_extract)를 사용하여 쉽게 복호화할 수 있습니다.\
...
만약 `ssh-agent` 서비스가 실행 중이 아니고 부팅 시 자동으로 시작하길 원한다면 다음을 실행하세요:
```bash
...
```
`crackmapexec`를 사용하여 비밀번호를 얻는 방법:
```bash
...
항상 사용자에게 자신의 자격 증명 또는 다른 사용자의 자격 증명을 입력하도록 요청할 수 있습니다 (클라이언트에게 직접 자격 증명을 요청하는 것은 정말 위험하다는 것을 주의하세요):
```bash
...
당신은 또한 내부에 자격 증명을 찾기 위해 휴지통을 확인해야 합니다.
...
공격자가 COM Hijacking을 지속성 메커니즘으로 사용하는 방법을 알아보려면 다음을 확인하십시오:
...
**SYSTEM으로 실행 중인 프로세스가 전체 액세스 권한으로 새 프로세스를 열고** (`OpenProcess()`) **같은 프로세스가 낮은 권한으로 새 프로세스를 생성** (`CreateProcess()`) **하지만 주 프로세스의 모든 열린 핸들을 상속**한다고 상상해보십시오.\
그런 다음, **낮은 권한을 가진 프로세스에 대한 전체 액세스**가 있다면, `OpenProcess()`로 생성된 **특권 프로세스에 대한 열린 핸들을 잡아서 쉘코드를 삽입**할 수 있습니다.\
[**이 취약점을 감지하고 악용하는 방법에 대한 자세한 내용을 보려면 여기를 클릭하십시오.**](leaked-handle-exploitation.md)\
[**다른 게시물에서는 다른 수준의 권한(전체 액세스뿐만 아니라)로 상속된 프로세스 및 스레드의 더 많은 열린 핸들러를 테스트하고 악용하는 방법에 대해 더 자세히 설명하고 있습니다.**](http://dronesec.pw/blog/2019/08/22/exploiting-leaked-process-and-thread-handles/).
...
**클라이언트**가 파이프를 통해 데이터를 보낼 때, 파이프를 설정한 **서버**는 **SeImpersonate** 권한이 필요하다면 **클라이언트**의 **아이덴티티를 취할 수 있습니다**. 특권 프로세스를 식별하여 해당 프로세스가 설정한 파이프를 통해 상호 작용할 때 해당 프로세스의 아이덴티티를 채택하여 **더 높은 권한을 얻을 수 있는 기회**가 생깁니다. 이러한 공격을 실행하는 방법에 대한 지침은 [**여기**](named-pipe-client-impersonation.md) 및 [**여기**](./#from-high-integrity-to-system)에서 찾을 수 있습니다.

또한 다음 도구를 사용하면 **burp와 같은 도구로 named pipe 통신을 가로챌 수 있습니다:** [**https://github.com/gabriel-sztejnworcel/pipe-intercept**](https://github.com/gabriel-sztejnworcel/pipe-intercept) **이 도구를 사용하면 모든 파이프를 나열하고 특권 상승을 찾을 수 있습니다** [**https://github.com/cyberark/PipeViewer**](https://github.com/cyberark/PipeViewer)

## 기타

### **비밀번호를 위한 명령줄 모니터링**

사용자로서 쉘을 획들한 경우 예약된 작업이나 다른 프로세스가 실행되어 **명령줄을 통해 자격 증명을 전달**할 수 있습니다. 아래 스크립트는 프로세스 명령줄을 매 2초마다 캡처하고 현재 상태를 이전 상태와 비교하여 차이가 있는 경우 출력합니다.
```powershell
while($true)
{
...
그래픽 인터페이스에 액세스할 수 있고 UAC가 활성화된 경우, 일부 버전의 Microsoft Windows에서 터미널 또는 "NT\AUTHORITY SYSTEM"과 같은 다른 프로세스를 권한이 없는 사용자로부터 실행할 수 있습니다.

이를 통해 권한 상슨을 승격하고 동시에 동일한 취약점을 통해 UAC를 우회할 수 있습니다. 게다가, 무언가를 설치할 필요가 없으며, 프로세스 중에 사용된 이진 파일은 Microsoft에 의해 서명되고 발급되었습니다.
...
## **높은 Integrity에서 시스템으로**

### **새로운 서비스**

이미 높은 Integrity 프로세스에서 실행 중인 경우, **SYSTEM으로의 경로**는 새로운 서비스를 **생성하고 실행**하는 것만으로 쉬울 수 있습니다:
```
...
고정 프로세스에서 **AlwaysInstallElevated 레지스트리 항목을 활성화**하고 **.msi 래퍼**를 사용하여 **역순 쉘을 설치**해 볼 수 있습니다.\
[관련 레지스트리 키 및 .msi 패키지 설치 방법에 대한 자세한 정보는 여기를 참조하십시오.](./#alwaysinstallelevated)
...
이 기술을 사용하면 일반적으로 **모든 토큰 권한을 가진 SYSTEM으로 실행 중인 프로세스가 선택**됩니다 (_네, 모든 토큰 권한을 가진 SYSTEM 프로세스를 찾을 수 있습니다_).\
[**여기에서 제안된 기술을 실행하는 코드 예제를 찾을 수 있습니다**](sedebug-+-seimpersonate-copy-token.md).

### Named Pipes

이 기술은 meterpreter가 `getsystem`에서 승격하는 데 사용됩니다. 이 기술은 **파이프를 생성한 다음 서비스를 사용하여 해당 파이프에 쓰기 위해 서비스를 생성/남용하는 것**으로 구성됩니다. 그런 다음 **SeImpersonate** 권한을 사용하여 파이프 클라이언트(서비스)의 토큰을 **표시**할 수 있는 **파이프를 생성한 서버**는 SYSTEM 권한을 획들할 수 있습니다.\
...
**SYSTEM**으로 실행 중인 **프로세스**에서 **로드되는 dll을 탈취**하면 해당 권한으로 임의의 코드를 실행할 수 있습니다. 따라서 Dll Hijacking은 이러한 종류의 권한 상승에도 유용하며, 더욱이 고정 프로세스에서는 dll을 로드하는 데 사용되는 폴더에 **쓰기 권한**이 있기 때문에 **더 쉽게 달성**할 수 있습니다.\
...
[**Invoke-WCMDump**](https://github.com/peewpw/Invoke-WCMDump) **-- 자격 증명을 Credential Manager에서 추출. 감지됨.**\
...
[~~**Sherlock**~~](https://github.com/rasta-mouse/Sherlock) **\~\~**\~\~ -- 알려진 권한 상승 취약점 검색 (Watson에 대해 폐기됨)\
...
[**SeatBelt**](https://github.com/GhostPack/Seatbelt) -- 구성 오류 검색을 위해 호스트를 열거하는 도구 (권한 상승보다는 정보 수집 도구) (컴파일 필요) **(**[**미리 컴파일된**](https://github.com/carlospolop/winPE/tree/master/binaries/seatbelt)**)**\
...
[~~**Beroot**~~](https://github.com/AlessandroZ/BeRoot) **\~\~**\~\~ -- 구성 오류 확인 (github에서 미리 컴파일된 실행 파일). 권장하지 않습니다. Win10에서 잘 작동하지 않습니다.\
...
* **사이버 보안 회사**에서 일하시나요? **회사를 HackTricks에서 광고하고 싶으세요**? 또는 **PEASS의 최신 버전을 확인하거나 HackTricks를 PDF로 다운로드**하고 싶으세요? [**구독 요금제**](https://github.com/sponsors/carlospolop)를 확인하세요!
* [**The PEASS Family**](https://opensea.io/collection/the-peass-family)를 발견하세요, 저희의 독점 [**NFTs**](https://opensea.io/collection/the-peass-family) 컬렉션
* [**공식 PEASS & HackTricks 스웨그**](https://peass.creator-spring.com)를 얻으세요
* **[💬](https://emojipedia.org/speech-balloon/) Discord 그룹**에 가입하거나 [텔레그램 그룹](https://t.me/peass)에 가입하거나 **트위터** 🐦[**@carlospolopm**](https://twitter.com/hacktricks_live)**를 팔로우하세요**.
* **해킹 요령을 공유하고 PR을 제출하여** [**hacktricks repo**](https://github.com/carlospolop/hacktricks) **및** [**hacktricks-cloud repo**](https://github.com/carlospolop/hacktricks-cloud) **를 팔로우하세요**.
