...
访问控制列表 (ACL) 包含一组有序的访问控制条目 (ACEs)，规定了对象及其属性的保护措施。实质上，ACL 定义了哪些安全主体 (用户或组) 可以或不可以在给定对象上执行哪些操作。

有两种类型的 ACL：

* **自主访问控制列表 (DACL):** 指定哪些用户和组可以或不可以访问对象。
...
* **SACL:** 用于审计对象的访问，其中 ACE 定义要在安全事件日志中记录的访问类型。这对于检测未经授权的访问尝试或解决访问问题非常有价值。
...
* **拒绝访问 ACE**: 此 ACE 明确拒绝指定用户或组对对象的访问权限 (在 DACL 中)。
...
2. 识别 ACE 类型 (拒绝访问、允许访问或系统审计) 的 **标志**。
...
* **允许访问 ACE(s)** 明确授予访问令牌中受托人的所有请求的权限。
* 在检查所有 ACE 后，如果任何请求的权限 **未被明确允许**，则访问将被隐式 **拒绝**。

### ACE 的顺序

将 **ACEs** (规定谁可以或不能访问某物的规则) 放入称为 **DACL** 的列表中的方式非常重要。这是因为一旦系统基于这些规则给予或拒绝访问权限，它就会停止查看其余内容。
...
* 在任何来自父文件夹或更远处的规则生效之前，让物品的所有者有最终决定权。

通过这种方式，文件或文件夹的所有者可以非常精确地控制谁可以访问，确保正确的人可以进入，错误的人则不能。
...
在管理资源（如文件夹）的访问权限时，我们使用称为访问控制列表（ACL）和访问控制条目（ACE）的列表和规则。这些规则定义了谁可以或不能访问特定数据。

#### 拒绝特定组的访问

假设您有一个名为成本的文件夹，并且希望每个人都可以访问它，除了营销团队。通过正确设置规则，我们可以确保在允许其他人访问之前，明确拒绝营销团队的访问。这是通过在允许所有人访问的规则之前放置拒绝营销团队访问的规则来实现的。

#### 允许拒绝组的特定成员访问

假设营销总监鲍勃需要访问成本文件夹，尽管通常不应允许营销团队访问。我们可以为鲍勃添加一个特定规则（ACE），授予他访问权限，并将其放置在拒绝营销团队访问的规则之前。这样，尽管对他的团队有一般限制，鲍勃仍然可以访问。
...
总之，ACL和ACE有助于定义精确的访问控制，确保只有正确的个人或组能够访问敏感信息或资源，并且可以将访问权限调整到个别属性或对象类型的级别。
