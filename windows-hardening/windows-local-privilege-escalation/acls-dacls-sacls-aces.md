...
2. Un **drapeau** qui identifie le type de ACE (accès refusé, autorisé ou audit système).
3. Des **drapeaux d'héritage** qui déterminent si les objets enfants peuvent hériter du ACE de leur parent.
...
- Tout d'abord, placez toutes les règles qui sont faites **spécifiquement pour cet élément** avant celles qui viennent d'ailleurs, comme un dossier parent.
...
Ainsi, cet **"ordre canonique"** vise à rendre les règles d'accès claires et efficaces, en plaçant d'abord les règles spécifiques et en organisant tout de manière intelligente.
...
Les ACE sont les règles individuelles dans une ACL. Elles identifient les utilisateurs ou groupes, spécifient quel accès est autorisé ou refusé, et déterminent comment ces règles s'appliquent aux sous-éléments (héritage). Il existe deux types principaux d'ACE :
