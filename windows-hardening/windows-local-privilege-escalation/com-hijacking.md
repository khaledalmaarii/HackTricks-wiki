...
一旦您决定要模拟的不存在的COM组件，请执行以下命令。_如果您决定模拟每隔几秒加载一次的COM组件，请小心，因为这可能会过度使用系统资源。_&#x20;
```bash
...
现在在 **HKEY\_**_**CLASSES\_**_**ROOT\CLSID** 和 HKLM 以及 HKCU 中搜索 CLSID **{1936ED8A-BD93-3213-E325-F38D112938EF}**，通常会发现该值在 HKCU 中不存在。
```bash
