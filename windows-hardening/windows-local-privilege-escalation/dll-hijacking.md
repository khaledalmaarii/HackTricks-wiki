...
首先，让我们先了解一下定义。DLL劫持是指在最广义的意义上，**欺骗一个合法/可信的应用程序加载任意DLL**。术语如_DLL搜索顺序劫持_、_DLL加载顺序劫持_、_DLL欺骗_、_DLL注入_和_DLL侧加载_经常被错误地用来表示相同的意思。
...
如果你正在寻找**一般的缺失DLL**，你可以让它运行**几秒钟**。\
如果你正在寻找**特定可执行文件中的缺失DLL**，你应该设置**另一个过滤器，如"进程名称" "包含" "\<exec name>"**，执行它，并停止捕获事件。
## 利用缺失的 DLL

为了提升权限，我们最好的机会是能够编写一个特权进程将尝试加载的 dll，而且这个 dll 将在某个被搜索的地方被搜索到。因此，我们将能够在一个在原始 dll 之前被搜索的文件夹中编写一个 dll（奇怪的情况），或者我们将能够在某个将要被搜索的文件夹中编写一个 dll，而原始的 dll 在任何文件夹中都不存在。
...
一般来说，Windows 应用程序将使用预定义的搜索路径来查找 DLL，并按照特定的顺序检查这些路径。DLL 劫持通常是通过将恶意 DLL 放置在其中一个文件夹中，同时确保在合法 DLL 之前找到该 DLL 来实现的。通过让应用程序指定所需 DLL 的绝对路径，可以缓解这个问题。

您可以在 32 位系统上看到 DLL 搜索顺序如下：
...
3. 16 位系统目录。没有函数可以获取此目录的路径，但会进行搜索。（_C:\Windows\System_）
4. Windows 目录。使用 [GetWindowsDirectory](https://docs.microsoft.com/en-us/windows/desktop/api/sysinfoapi/nf-sysinfoapi-getwindowsdirectorya) 函数获取此目录的路径。（_C:\Windows_）
5. 当前目录。
6. 在 PATH 环境变量中列出的目录。请注意，这不包括由 **App Paths** 注册表键指定的每个应用程序路径。计算 DLL 搜索路径时，不使用 **App Paths** 键。

这是启用了 **SafeDllSearchMode** 的默认搜索顺序。当禁用此功能时，当前目录将升至第二位。要禁用此功能，请创建 **HKEY\_LOCAL\_MACHINE\System\CurrentControlSet\Control\Session Manager**\\**SafeDllSearchMode** 注册表值，并将其设置为 0（默认启用）。

如果使用 [LoadLibraryEx](https://docs.microsoft.com/en-us/windows/desktop/api/LibLoaderAPI/nf-libloaderapi-loadlibraryexa) 函数调用 **LOAD\_WITH\_ALTERED\_SEARCH\_PATH**，搜索将从 **LoadLibraryEx** 加载的可执行模块的目录开始。

最后，请注意，可以通过指定绝对路径而不仅仅是名称来加载 DLL。在这种情况下，该 DLL 只会在该路径中搜索（如果 DLL 有任何依赖项，则会按名称搜索）。

还有其他改变搜索顺序的方法，但我不会在这里解释它们。

#### Windows 文档中的 DLL 搜索顺序异常

* 如果**已在内存中加载了具有相同模块名称的 DLL**，系统在解析到已加载的 DLL 之前，仅检查重定向和清单。**系统不会搜索该 DLL**，无论它位于哪个目录中。
* 如果 DLL 在应用程序运行的 Windows 版本的**已知 DLL 列表**上，系统将使用其自己的已知 DLL 的副本（以及已知 DLL 的依赖 DLL，如果有的话），而不是搜索该 DLL。有关当前系统上已知 DLL 的列表，请参阅以下注册表键：**HKEY\_LOCAL\_MACHINE\SYSTEM\CurrentControlSet\Control\Session Manager\KnownDLLs**。
* 如果 DLL 有依赖项，系统将按照它们只使用模块名称加载的方式搜索依赖 DLL。即使第一个 DLL 是通过指定完整路径加载的，这也是正确的。
...
* 在任何将要被**搜索**的**文件夹**上具有**写权限**（可能是可执行文件目录或系统路径中的某个文件夹）。

是的，这些先决条件很难找到，因为**默认情况下很难找到一个缺少 dll 的特权可执行文件**，而且**在系统路径文件夹中默认情况下无法获得写权限**。但是，在配置错误的环境中，这是可能的。\
如果你很幸运，发现自己符合要求，你可以查看 [UACME](https://github.com/hfiref0x/UACME) 项目。即使该项目的**主要目标是绕过 UAC**，您可能会在那里找到一个适用于您所使用的 Windows 版本的 DLL 劫持的 PoC（可能只需更改您具有写权限的文件夹的路径）。
...
如果您找到了一个可利用的场景，成功利用它的最重要的事情之一将是**创建一个至少导出可执行文件将从中导入的所有函数的dll**。无论如何，注意Dll劫持在从中间完整性级别升级到高级（绕过UAC）或从高级完整性升级到SYSTEM方面非常方便。您可以在此dll劫持研究中找到一个创建有效dll的示例，该研究专注于用于执行的dll劫持：[**https://www.wietzebeukema.nl/blog/hijacking-dlls-in-windows**](https://www.wietzebeukema.nl/blog/hijacking-dlls-in-windows)**.**\
此外，在下一个部分中，您可以找到一些可能有用的基本dll代码，可以用作模板或创建具有非必需导出函数的dll。

## **创建和编译Dlls**

### **Dll代理化**

基本上，**Dll代理**是一个能够在加载时执行恶意代码的Dll，但也能够以**中继所有调用到真实库**的方式**公开**和**工作**。
