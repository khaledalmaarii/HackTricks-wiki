...
1. (_C:\Windows_)
5. Bieżący katalog.
6. Katalogi wymienione w zmiennej środowiskowej PATH. Należy zauważyć, że nie obejmuje to ścieżki określonej dla aplikacji za pomocą klucza rejestru **App Paths**. Klucz **App Paths** nie jest używany podczas obliczania ścieżki wyszukiwania DLL-ek.
...
W przypadku znalezienia podatnego scenariusza jedną z najważniejszych rzeczy do pomyślnego wykorzystania byłoby **utworzenie dll, która eksportuje co najmniej wszystkie funkcje, które wykonywalny będzie importował z niej**. Niemniej jednak, zauważ, że Dll Hijacking przydaje się do [eskaltacji z poziomu Medium Integrity na High **(omijając UAC)**](../authentication-credentials-uac-and-efs.md#uac) lub z [**High Integrity na SYSTEM**](./#from-high-integrity-to-system)**.** Możesz znaleźć przykład **jak utworzyć prawidłową dll** w ramach tego studium dotyczącego Dll Hijacking skupionego na Dll Hijacking do wykonania: [**https://www.wietzebeukema.nl/blog/hijacking-dlls-in-windows**](https://www.wietzebeukema.nl/blog/hijacking-dlls-in-windows)**.**\
