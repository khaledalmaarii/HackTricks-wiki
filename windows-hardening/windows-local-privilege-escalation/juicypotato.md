...
* **RPC 서버 포트** _외부 서버에 인증하려는 경우 및 방화벽이 포트 `135`를 차단하는 경우 유용합니다..._
...
대부분 JuicyPotato가 사용하는 기본 CLSID가 **작동하지 않고** exploit이 실패합니다. 일반적으로 **작동하는 CLSID**를 찾기 위해 여러 번 시도해야 합니다. 특정 운영 체제에 대해 시도할 CLSID 목록을 얻으려면 이 페이지를 방문해야 합니다:
...
[Join-Object.ps1](https://github.com/ohpe/juicy-potato/blob/master/CLSID/utils/Join-Object.ps1)를 다운로드하여 PS 세션에 로드하고 [GetCLSID.ps1](https://github.com/ohpe/juicy-potato/blob/master/CLSID/GetCLSID.ps1)를 다운로드하고 실행하십시오. 해당 스크립트는 테스트할 가능한 CLSID 목록을 생성합니다.
