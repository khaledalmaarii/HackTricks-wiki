...
* **शामिल हों** [**💬**](https://emojipedia.org/speech-balloon/) **डिस्कॉर्ड समूह** या [**टेलीग्राम समूह**](https://t.me/peass) में या **मुझे** **ट्विटर** **🐦**[**@carlospolopm**](https://twitter.com/hacktricks_live)** का** **अनुसरण** **करें**।
...
{% hint style="warning" %}
**JuicyPotato** Windows Server 2019 और Windows 10 बिल्ड 1809 के बाद काम नहीं करता है। हालांकि, [**PrintSpoofer**](https://github.com/itm4n/PrintSpoofer)**,** [**RoguePotato**](https://github.com/antonioCoco/RoguePotato)**,** [**SharpEfsPotato**](https://github.com/bugch3ck/SharpEfsPotato) का उपयोग किया जा सकता है ताकि **वे ही विशेषाधिकारों का उपयोग करें और `NT AUTHORITY\SYSTEM`** स्तर का एक्सेस प्राप्त करें। _**जांच करें:**_
{% endhint %}
...
_एक चीनी रूपांतरण रॉटेनपोटेटोएनजी का, थोड़ी मिठास के साथ, अर्थात **एक और स्थानीय विशेषाधिकार उन्नति उपकरण, विंडोज सेवा खातों से NT AUTHORITY\SYSTEM तक**_
...
> सिद्धांत के लिए, देखें [रॉटेन पोटेटो - सेवा खातों से सिस्टम तक विशेषाधिकार उन्नति](https://foxglovesecurity.com/2016/09/26/rotten-potato-privilege-escalation-from-service-accounts-to-system/) और लिंक और संदर्भों की श्रृंखला का पालन करें।

हमने पाया कि, `BITS` के अलावा हम कई COM सर्वरों का दुरुपयोग कर सकते हैं। उन्हें बस यह करने की आवश्यकता है:

1. वर्तमान उपयोगकर्ता द्वारा instantiable होना, सामान्यत: एक "सेवा उपयोगकर्ता" जिसके पास अनुकरण विशेषाधिकार होते हैं
2. `IMarshal` इंटरफेस का कार्यान्वयन करना
3. उच्च स्तर पर चलाया जाना (SYSTEM, प्रशासक, ...)

कुछ परीक्षण के बाद हमने कई [रोचक CLSID](http://ohpe.it/juicy-potato/CLSID/) की एक व्यापक सूची प्राप्त और परीक्षण किया। कई विंडोज संस्करणों पर।

### ज्यूसी विवरण <a href="#juicy-details" id="juicy-details"></a>

JuicyPotato आपको निम्नलिखित करने की अनुमति देता है:

* **लक्ष्य CLSID** _आप जिस CLSID को चाहें उसे चुनें।_ [_यहाँ_](http://ohpe.it/juicy-potato/CLSID/) _आप OS द्वारा संगठित सूची पा सकते हैं।_
* **COM सुनने का पोर्ट** _मार्शल किए गए हार्डकोडेड 6666 के बजाय आपकी पसंद के COM सुनने का पोर्ट परिभाषित करें_
* **COM सुनने का आईपी पता** _किसी भी आईपी पर सर्वर बाँधें_
* **प्रक्रिया निर्माण मोड** _अनुकरण करने वाले उपयोगकर्ता की विशेषाधिकारों के आधार पर आप `CreateProcessWithToken` (जरूरत है `SeImpersonate`), `CreateProcessAsUser` (जरूरत है `SeAssignPrimaryToken`), `दोनों` में से चुन सकते हैं_
* **लॉन्च करने की प्रक्रिया** _यदि शोषण सफल होता है तो एक कार्यान्वित या स्क्रिप्ट लॉन्च करें_
* **प्रक्रिया तर्क** _लॉन्च की गई प्रक्रिया के तर्क को अनुकूलित करें_
* **RPC सर्वर पता** _एक गुप्त दृष्टिकोण के लिए आप एक बाह्य RPC सर्वर से प्रमाणित कर सकते हैं_
* **RPC सर्वर पोर्ट** _उपयोगी है अगर आप एक बाह्य सर्वर से प्रमाणित होना चाहते हैं और फ़ायरवॉल पोर्ट `135` को अवरुद्ध कर रहा है…_
* **परीक्षण मोड** _मुख्य रूप से परीक्षण के उद्देश्यों के लिए, अर्थात CLSIDs का परीक्षण। यह DCOM बनाता है और टोकन का उपयोगकर्ता प्रिंट करता है। टेस्टिंग के लिए देखें_ [_यहाँ_](http://ohpe.it/juicy-potato/Test/)
...
वास्तविक समाधान संवेदनशील खातों और एप्लिकेशनों की सुरक्षा करना है जो `* SERVICE` खातों के तहत चलते हैं। `DCOM` को रोकना निश्चित रूप से इस उत्पीड़न को रोकेगा लेकिन इसका मूल ऑपरेटिंग सिस्टम पर गंभीर प्रभाव हो सकता है।
...
अक्सर, JuicyPotato द्वारा उपयोग किया जाने वाला डिफ़ॉल्ट CLSID **काम नहीं करता** है और उसका उपयोग विफल हो जाता है। सामान्यत: एक स्थिति में कई प्रयासों की आवश्यकता होती है ताकि एक **काम करने वाला CLSID** मिल सके। किसी विशिष्ट ऑपरेटिंग सिस्टम के लिए जांचने के लिए CLSIDs की सूची प्राप्त करने के लिए आपको इस पृष्ठ पर जाना चाहिए:
...
फिर [test\_clsid.bat ](https://github.com/ohpe/juicy-potato/blob/master/Test/test\_clsid.bat) डाउनलोड करें (CLSID सूची और juicypotato executable के पथ को बदलें) और इसे निष्पादित करें। यह हर CLSID की कोशिश करना शुरू कर देगा, और **जब पोर्ट नंबर बदल जाएगा, तो इसका मतलब है कि CLSID काम कर गया है**।
