...
Você pode **verificar** o _**Manifesto**_ de um binário usando a ferramenta _**sigcheck.exe**_ da Sysinternals. E você pode **ver** o **nível de integridade** dos processos usando o _Process Explorer_ ou _Process Monitor_ (da Sysinternals).
...
[**UACME**](https://github.com/hfiref0x/UACME) que é uma **compilação** de várias explorações de bypass do UAC. Note que você precisará **compilar o UACME usando o Visual Studio ou o msbuild**. A compilação criará vários executáveis (como `Source\Akagi\outout\x64\Debug\Akagi.exe`), você precisará saber **qual você precisa**.\
