...
```
如果是 **`1`**，则 UAC 是 **激活** 的；如果是 **`0`** 或者 **不存在**，则 UAC 是 **未激活** 的。
...
10     0      14393  0
```
也可以使用 [这个](https://en.wikipedia.org/wiki/Windows\_10\_version\_history) 页面获取 Windows 版本 `1607` 的构建版本。
...
如果您查看 **UACME**，您会注意到 **大多数 UAC 绕过利用 DLL 劫持漏洞**（主要是在 _C:\Windows\System32_ 中写入恶意 dll）。[阅读此内容以了解如何找到 DLL 劫持漏洞](../windows-local-privilege-escalation/dll-hijacking.md)。
