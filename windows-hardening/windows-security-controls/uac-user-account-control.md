...
Wenn UAC aktiv ist, erhält ein Administratorbenutzer 2 Tokens: einen Standardbenutzerschlüssel, um reguläre Aktionen auf regulärem Niveau auszuführen, und einen mit den Admin-Berechtigungen.
...
Dann verwenden einige Angreifer diese Art von Binärdateien, um die **UAC zu umgehen** (von **mittlerem** Integritätsniveau auf **hohes** Niveau zu heben), um beliebigen Code auszuführen, da er von einem Prozess mit **hohem Integritätsniveau** ausgeführt wird.
...
* Wenn **`5`** (**Standard**), wird der Administrator um Bestätigung gebeten, nicht-Windows-Binärdateien mit hohen Rechten auszuführen

Dann müssen Sie den Wert von **`LocalAccountTokenFilterPolicy`** überprüfen. Wenn der Wert **`0`** ist, kann nur der Benutzer mit der RID 500 (**eingebauter Administrator**) **Admin-Aufgaben ohne UAC** ausführen, und wenn er `1` ist, können **alle Konten innerhalb der Gruppe "Administratoren"** dies tun.
...
{% hint style="info" %}
Beachten Sie, dass bei grafischem Zugriff auf das Opfer das UAC-Bypassen einfach ist, da Sie einfach auf "Ja" klicken können, wenn das UAC-Prompt erscheint.
{% endhint %}
...
Wenn das UAC bereits deaktiviert ist (`ConsentPromptBehaviorAdmin` ist **`0`**), können Sie **eine Reverse-Shell mit Administratorrechten** (hoher Integritätslevel) ausführen, indem Sie beispielsweise:
```bash
...
Wenn Sie eine Shell mit einem Benutzer haben, der zur Gruppe der Administratoren gehört, können Sie **das C$** freigegebene über SMB (Dateisystem) lokal auf einer neuen Festplatte mounten und Sie werden **Zugriff auf alles im Dateisystem haben** (sogar den Administrator-Startordner).
...
Wenn Sie Zugriff auf eine **GUI haben, können Sie die UAC-Aufforderung einfach akzeptieren**, wenn Sie sie erhalten, Sie benötigen wirklich keinen Umgehung. Daher ermöglicht Ihnen der Zugriff auf eine GUI das Umgehen der UAC.
...
Wenn es Ihnen nichts ausmacht, laut zu sein, könnten Sie immer etwas wie [**https://github.com/Chainski/ForceAdmin**](https://github.com/Chainski/ForceAdmin) ausführen, das **um die Berechtigungen zu erhöhen, bis der Benutzer sie akzeptiert**.
...
1. Finden Sie eine ausführbare Datei, die **automatisch erhöht** (überprüfen Sie, ob sie bei der Ausführung auf einem hohen Integritätsniveau ausgeführt wird).
...
Besteht darin zu beobachten, ob eine **autoerhöhte ausführbare Datei** versucht, aus der **Registrierung** den **Namen/Pfad** einer **ausführbaren Datei** oder eines **Befehls** zu **lesen**, der **ausgeführt** werden soll (dies ist interessanter, wenn die ausführbare Datei diese Informationen innerhalb des **HKCU** sucht).
